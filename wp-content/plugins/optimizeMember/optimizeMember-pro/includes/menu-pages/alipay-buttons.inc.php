<?php
/**
* Menu page for optimizeMember Pro ( AliPay Buttons page ).
*
* Copyright: © 2009-2011
...
/**/
if (!class_exists ("c_ws_plugin__optimizemember_pro_menu_page_alipay_buttons"))
	{
		/**
		* Menu page for optimizeMember Pro ( AliPay Buttons page ).
		*
		...
		* @since 110531
		*/
		class c_ws_plugin__optimizemember_pro_menu_page_alipay_buttons
			{
				public function __construct ()
					{
						...
						for ($n = 1; $n <= $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["levels"]; $n++)
							{
								...
								echo '<div class="ws-menu-page-group" title="AliPay Buttons For Level ' . $labelText . ' Access">' . "\n";
								/**/
								...
								echo '<p>Very simple. All you do is customize the form fields provided, for each Membership Level that you plan to offer. Then press (Generate Button Code). These special AliPay Buttons are customized to work with optimizeMember seamlessly. Member accounts will be activated instantly, in an automated fashion. optimizeMember makes extensive use of the AliPay IPN service. optimizeMember receives updates from AliPay behind-the-scene. <em>* Buttons are NOT saved here. This is only a Button Generator. Once you\'ve generated your Button, copy/paste it into your Membership Options Page. If you lose your Button Code, you\'ll need to come back &amp; re-generate a new one.</em></p>' . "\n";
								/**/
								...
								echo '<p>Description: <input type="text" autocomplete="off" id="ws-plugin--optimizemember-pro-level' . $n . '-desc" class="alipay-buttons-description-input" value="' . format_to_edit ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["level" . $n . "_label"]) . ' / description and pricing details here." size="73" /></p>' . "\n";
								...
								echo '<p><input type="button" value="Generate Button Code" onclick="ws_plugin__optimizemember_pro_alipayButtonGenerate(\'level' . $n . '\');" class="button-primary" /></p>' . "\n";
								echo '</form>' . "\n";
								echo '</td>' . "\n";
								/**/
								echo '</tr>' . "\n";
								...
								$ws_plugin__optimizemember_pro_temp_s = trim (c_ws_plugin__optimizemember_utilities::evl (file_get_contents (dirname (dirname (__FILE__)) . "/templates/shortcodes/alipay-checkout-button-shortcode.php")));
						...
						if (!is_multisite () || !c_ws_plugin__optimizemember_utils_conds::is_multisite_farm () || is_main_site ())
							{
								...
								echo '<td>' . "\n";
								echo '<form onsubmit="return false;">' . "\n";
								echo '<p>I want to charge: $<input type="text" autocomplete="off" id="ws-plugin--optimizemember-pro-ccap-amount" value="0.01" size="4" /> / <select id="ws-plugin--optimizemember-pro-ccap-term">' . trim (c_ws_plugin__optimizemember_utilities::evl (file_get_contents (dirname (dirname (__FILE__)) . "/templates/options/alipay-membership-ccap-terms.php"))) . '</select></p>' . "\n";
								echo '<p>Description: <input type="text" autocomplete="off" id="ws-plugin--optimizemember-pro-ccap-desc" class="alipay-buttons-description-input" value="Description and pricing details here." size="73" /></p>' . "\n";
								...
								echo '<p><input type="button" value="Generate Button Code" onclick="ws_plugin__optimizemember_pro_alipayCcapButtonGenerate();" class="button-primary" /></p>' . "\n";
								echo '</form>' . "\n";
								echo '</td>' . "\n";
								/**/
								echo '</tr>' . "\n";
								...
								echo '<form onsubmit="return false;">' . "\n";
								echo '<strong>WordPress Shortcode:</strong> ( recommended for both the WordPress Visual &amp; HTML Editors )<br />' . "\n";
								$ws_plugin__optimizemember_pro_temp_s = trim (c_ws_plugin__optimizemember_utilities::evl (file_get_contents (dirname (dirname (__FILE__)) . "/templates/shortcodes/alipay-ccaps-checkout-button-shortcode.php")));
								$ws_plugin__optimizemember_pro_temp_s = preg_replace ("/%%custom%%/", c_ws_plugin__optimizemember_utils_strings::esc_refs (esc_attr ($_SERVER["HTTP_HOST"])), $ws_plugin__optimizemember_pro_temp_s);
						...
						/**/
						echo '<p>I want to charge: $<input type="text" autocomplete="off" id="ws-plugin--optimizemember-pro-sp-amount" value="0.01" size="4" /> / <select id="ws-plugin--optimizemember-pro-sp-hours" class="alipay-buttons-subscription-select">' . trim (c_ws_plugin__optimizemember_utilities::evl (file_get_contents (dirname (dirname (__FILE__)) . "/templates/options/alipay-sp-hours.php"))) . '</select></p>' . "\n";
						echo '<p>Description: <input type="text" autocomplete="off" id="ws-plugin--optimizemember-pro-sp-desc" value="Description and pricing details here." size="67" /></p>' . "\n";
						echo '<p><input type="button" value="Generate Button Code" onclick="ws_plugin__optimizemember_pro_alipaySpButtonGenerate();" class="button-primary" /></p>' . "\n";
						echo '</form>' . "\n";
						echo '</td>' . "\n";
						/**/
						echo '</tr>' . "\n";
						...
						echo '<form onsubmit="return false;">' . "\n";
						echo '<strong>WordPress Shortcode:</strong> ( recommended for both the WordPress Visual &amp; HTML Editors )<br />' . "\n";
						$ws_plugin__optimizemember_pro_temp_s = trim (c_ws_plugin__optimizemember_utilities::evl (file_get_contents (dirname (dirname (__FILE__)) . "/templates/shortcodes/alipay-sp-checkout-button-shortcode.php")));
						$ws_plugin__optimizemember_pro_temp_s = preg_replace ("/%%custom%%/", c_ws_plugin__optimizemember_utils_strings::esc_refs (esc_attr ($_SERVER["HTTP_HOST"])), $ws_plugin__optimizemember_pro_temp_s);
						...
						echo '<li><code>success=""</code> Success Return URL <em>( optional )</em>. optimizeMember handles this automatically for you. However, if you would prefer to take control over the landing page after checkout <em>( i.e. your own custom Thank-You Page )</em>, you can. If supplied, this must be a full URL, starting with <code>http://</code>. Note, optimizeMember will NOT use this value if an existing account holder is being modified. optimizeMember handles account updates <em>( i.e. billing modification )</em> in a more dynamic way. Your Success Return URL is only applied to (new) Customers.</li>' . "\n";
						echo '</ul>' . "\n";
						echo '</td>' . "\n";
						/**/
						echo '</tr>' . "\n";
...
/**/
new c_ws_plugin__optimizemember_pro_menu_page_alipay_buttons ();
?>