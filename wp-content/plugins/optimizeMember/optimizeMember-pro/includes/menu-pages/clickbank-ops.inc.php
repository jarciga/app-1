<?php
/**
* Menu page for optimizeMember Pro (ClickBank® Options page).
*
* Copyright: © 2009-2011
...
if (!class_exists ("c_ws_plugin__optimizemember_pro_menu_page_clickbank_ops"))
	{
		/**
		* Menu page for optimizeMember Pro (ClickBank® Options page).
		*
		...
		* @since 110531
		*/
		class c_ws_plugin__optimizemember_pro_menu_page_clickbank_ops
			{
				public function __construct ()
					{
						...
						echo '<div class="ws-menu-page-section ws-plugin--optimizemember-pro-clickbank-account-details-section">' . "\n";
						echo '<h3>ClickBank® Account Details (required)</h3>' . "\n";
						echo '<p><a href="http://www.clickbank.com" target="_blank" rel="external">ClickBank®</a> is a secure online retail outlet for more than 70,000 digital product vendors and 110,000 active affiliate marketers. ClickBank® makes a sale somewhere in the world every three seconds, safely processing more than 27,000 digital transactions a day. They serve more than 200 countries, and are consistently ranked as one of the most highly-trafficked sites on the web.</p>' . "\n";
						echo '<p>optimizeMember has been integrated with ClickBank® for Direct Payments and also for Recurring Billing. In order to take advantage of this integration, you will need to have a ClickBank® Merchant Account. Once you have an account, all of the details below can be generated from inside of your ClickBank® Merchant account. If you need assistance, please check their <a href="http://www.clickbank.com/clickbank-help" target="_blank" rel="external">help section</a>.</p>' . "\n";
						...
						echo '<th>' . "\n";
						echo '<label for="ws-plugin--optimizemember-pro-clickbank-username">' . "\n";
						...
						echo '<td>' . "\n";
						echo '<input type="text" autocomplete="off" name="ws_plugin__optimizemember_pro_clickbank_username" id="ws-plugin--optimizemember-pro-clickbank-username" value="' . format_to_edit ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_clickbank_username"]) . '" /><br />' . "\n";
						...
						echo '<th>' . "\n";
						echo '<label for="ws-plugin--optimizemember-pro-clickbank-clerk-key">' . "\n";
						...
						echo '<td>' . "\n";
						echo '<input type="password" autocomplete="off" name="ws_plugin__optimizemember_pro_clickbank_clerk_key" id="ws-plugin--optimizemember-pro-clickbank-clerk-key" value="' . format_to_edit ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_clickbank_clerk_key"]) . '" /><br />' . "\n";
						echo 'This can be generated at ClickBank®. optimizeMember needs a Clerk Key with "Read Access".' . "\n";
						echo '</td>' . "\n";
						...
						echo '<th>' . "\n";
						echo '<label for="ws-plugin--optimizemember-pro-clickbank-developer-key">' . "\n";
						...
						echo '<td>' . "\n";
						echo '<input type="password" autocomplete="off" name="ws_plugin__optimizemember_pro_clickbank_developer_key" id="ws-plugin--optimizemember-pro-clickbank-developer-key" value="' . format_to_edit ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_clickbank_developer_key"]) . '" /><br />' . "\n";
						...
						if (!is_multisite () || !c_ws_plugin__optimizemember_utils_conds::is_multisite_farm () || is_main_site ())
							{
								...
								echo '<em>This enables API, IPN and Return Page logging. The log files are stored here: <code>' . esc_html (c_ws_plugin__optimizemember_utils_dirs::doc_root_path ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["logs_dir"])) . '</code></em><br />' . "\n";
								echo '</td>' . "\n";
								...
								echo '<p style="font-size:110%; margin-top:0;"><span>We HIGHLY recommend that you enable logging during your initial testing phase. Logs produce lots of useful details that can help in debugging. Logs can help you find issues in your configuration and/or problems during payment processing.</span></p>'."\n";
						...
						echo '<div class="ws-menu-page-section ws-plugin--optimizemember-pro-clickbank-ipn-section">' . "\n";
                        ...
                        echo '<p><strong>3.</strong> You\'ll need your IPN v2.1 URL, which is:<br /><code>'.esc_html(home_url("/?optimizemember_pro_clickbank_notify=2.1")).'</code> (or <code>=1</code>; same thing)</p>'."\n";
                        echo '<p>Or, you can choose to use v6. Your v6 IPN URL is:<br /><code>'.esc_html(home_url("/?optimizemember_pro_clickbank_notify=6")).'</code></p>'."\n";

                        echo '<p class="warning">Please do NOT integrate both IPN URLs. Choose one version or the other.</strong></p>'."\n";

                        echo '<p><strong>4.</strong> Now provide OptimizeMember with your Secret Key in the field below.</strong></p>'."\n";
						...
						echo '<th>' . "\n";
						echo '<label for="ws-plugin--optimizemember-pro-clickbank-secret-key">' . "\n";
						...
						echo '<td>' . "\n";
						echo '<input type="password" autocomplete="off" name="ws_plugin__optimizemember_pro_clickbank_secret_key" id="ws-plugin--optimizemember-pro-clickbank-secret-key" value="' . format_to_edit ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_clickbank_secret_key"]) . '" /><br />' . "\n";
						...
						echo '<p>Whenever you create <strong>Products</strong> at ClickBank®, you\'ll be asked to supply a Thank-You Page. This is where a Customer lands after they complete checkout. optimizeMember handles this dynamically, so you can use the same Thank-You Page for all of your ClickBank® Products. As long as you follow the instructions provided under: <em>optimizeMember -› ClickBank® Buttons</em>, optimizeMember will be able to handle Thank-You Page and IPN (Instant Payment Notifications) for you automatically. The integration from ClickBank® -› optimizeMember is seamless.</p>' . "\n";
						echo '<p>You\'ll need the URL for your <a href="' . esc_attr (site_url ("/?optimizemember_pro_clickbank_return&optimizemember_paypal_return=1&optimizemember_paypal_proxy=clickbank&optimizemember_paypal_proxy_use=x-preview")) . '" target="_blank" rel="external">Thank-You Page</a>, which is:<br /><code>' . esc_html (site_url ("/?optimizemember_pro_clickbank_return=1")) . '</code></p>' . "\n";

						echo '<div class="ws-menu-page-hr"></div>' . "\n";

						echo '<h3>Thank-You Page Template (<a href="#" onclick="jQuery(\'div#ws-plugin--optimizemember-pro-clickbank-return-page-template\').toggle(); return false;" class="ws-dotted-link">optional customizations</a>)</h3>' . "\n";
						echo '<div id="ws-plugin--optimizemember-pro-clickbank-return-page-template" style="display:none;">' . "\n";
						echo '<p>With optimizeMember Pro installed, you have the ability to customize your <a href="' . esc_attr (site_url ("/?optimizemember_pro_clickbank_return&optimizemember_paypal_return=1&optimizemember_paypal_proxy=clickbank&optimizemember_paypal_proxy_use=x-preview")) . '" target="_blank" rel="external">Thank-You Page Template</a>. Each of your Customers are returned back to your site immediately after they complete checkout at ClickBank®. Your Thank-You Page displays a message and instructions for the Customer. optimizeMember may change the message and instructions dynamically, based on what the Customer is actually doing <em>(i.e. based on the type of transaction that is taking place)</em>. So, although we do NOT recommend that you attempt to change the message and instructions presented dynamically by optimizeMember, you CAN certainly control the Header, and/or the overall appearance of optimizeMember\'s Thank-You Page Template.</p>' . "\n";
						echo '<p>The quickest/easiest way, is to simply add some HTML code in the box below. For instance, you might include an &lt;img&gt; tag with your logo. The box below, allows you to customize the Header section <em>(i.e. the top)</em> of optimizeMember\'s default Thank-You Page Template. Everything else, including the textual response and other important details that each Customer needs to know about, are already handled dynamically by optimizeMember <em>(based on the type of transaction that is taking place)</em>. All you need to do is customize the Header with your logo and anything else you feel is important. Although this Header customization is completely optional, we recommend an <a href="http://www.w3schools.com/tags/tag_img.asp" target="_blank" rel="external">&lt;img&gt; tag</a>, with a logo that is around 300px wide. After you "Save All Changes" below, you may <a href="' . esc_attr (site_url ("/?optimizemember_pro_clickbank_return&optimizemember_paypal_return=1&optimizemember_paypal_proxy=clickbank&optimizemember_paypal_proxy_use=x-preview")) . '" target="_blank" rel="external">click this link to see what your Header looks like</a>.</p>' . "\n";
						...
						echo '<th>' . "\n";
						echo '<label for="ws-plugin--optimizemember-pro-clickbank-return-template-header">' . "\n";
						...
						echo '<td>' . "\n";
						echo '<textarea name="ws_plugin__optimizemember_pro_clickbank_return_template_header" id="ws-plugin--optimizemember-pro-clickbank-return-template-header" rows="5" wrap="off" spellcheck="false">' . format_to_edit ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_clickbank_return_template_header"]) . '</textarea><br />' . "\n";
						...
						if (!is_multisite () || !c_ws_plugin__optimizemember_utils_conds::is_multisite_farm () || is_main_site ())
							echo '<p>It is also possible to build your own Thank-You Page Template, if you prefer. If you feel the need to create your own Thank-You Page Template, please make a copy of optimizeMember\'s default template: <code>' . esc_html (c_ws_plugin__optimizemember_utils_dirs::doc_root_path ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["dir"] . "/includes/templates/returns/default-template.php")) . '</code>. Place your copy of this default template, inside your active WordPress® theme directory, and name the file: <code>/clickbank-return.php</code>. optimizeMember will automatically find your Thank-You Page Template in this location, and optimizeMember will use your template, instead of the default. Further details are provided inside optimizeMember\'s default template file. Once your custom template file is in place, you may <a href="' . esc_attr (site_url ("/?optimizemember_pro_clickbank_return&optimizemember_paypal_return=1&optimizemember_paypal_proxy=clickbank&optimizemember_paypal_proxy_use=x-preview")) . '" target="_blank" rel="external">click this link to see what it looks like</a>.</p>' . "\n";

						echo '<p>It is also possible to bypass optimizeMember\'s Thank-You Page all together, if you prefer. You can take optimizeMember\'s Thank-You Page URL <em>(shown above)</em>, and add <code>&optimizemember_pro_clickbank_return_success=http://...</code> where the value can be set to a custom Thank-You Page URL that you prefer. In other words, if you use the <code>&optimizemember_pro_clickbank_return_success=http://...</code> parameter in your Thank-You Page URL, the initial redirection back to optimizeMember\'s default handler MUST still occur. However, instead of optimizeMember displaying its Thank-You Page Template to the Customer, optimizeMember will silently redirect the Customer to the URL that you specified in the <code>&optimizemember_pro_clickbank_return_success=http://...</code> parameter, allowing you to take complete control over what happens next. Click for an [ <a href="#" onclick="alert(\'Basic Example (please remember to URL encode the value):\\n' . esc_attr (site_url ("/?optimizemember_pro_clickbank_return=1&optimizemember_pro_clickbank_return_success=" . site_url ("/thank-you/"))) . '\\n\\nProper Example (with the URL having been encoded properly):\\n' . esc_attr (site_url ("/?optimizemember_pro_clickbank_return=1&optimizemember_pro_clickbank_return_success=" . rawurlencode (site_url ("/thank-you/")))) . '\\n\\n* For help on URL encoding, please see:\\nhttp://www.w3schools.com/tags/ref_urlencode.asp\'); return false;">example</a> ].</p>' . "\n";
						echo '</div>' . "\n";
						...
						echo '<li><code>%%subscr_id%%</code> = A unique Transaction/Subscription ID (which is sometimes generated by optimizeMember). [ <a href="#" onclick="alert(\'If you are selling Lifetime or Fixed-Term (non-recurring) access, using Buy Now functionality with ClickBank®; the %%subscr_id%% is actually set to the (ClickBank-generated) Receipt # for the purchase, as opposed to an (optimizeMember-generated) Subscription ID.\\n\\nJust in case you\\\'re wondering, the reason optimizeMember generates a unique Subscription ID, is because the Receipt # ClickBank® generates is NOT dynamic enough to handle everything optimizeMember makes possible. ClickBank® Receipts (by themselves) do NOT allow site owners to track multiple payments with a common Subscription ID. So optimizeMember makes up for this minor deficiency.\\n\\nFor instance, with Recurring ClickBank® Products, optimizeMember\\\'s %%subscr_id%% will remain constant throughout all future payments; making things easier to keep track of (on the back-end management side of things).\'); return false;">?</a> ]</li>' . "\n";
						...
						echo '<p>The ClickBank® IPN service will notify optimizeMember whenever a refund or chargeback occurs. For example, if you issue a refund to an unhappy Customer through ClickBank®, optimizeMember will eventually be notified, and the account for that Customer will either be demoted to a Free Subscriber, or deleted automatically (based on your configuration). ~ Otherwise, under normal circumstances, optimizeMember will not process an EOT until the User has completely used up the time they paid for.</em></p>' . "\n";
...
new c_ws_plugin__optimizemember_pro_menu_page_clickbank_ops ();
?>