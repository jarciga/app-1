<?php
/**
* Menu page for optimizeMember Pro ( Authorize.Net Forms page ).
*
* Copyright: © 2009-2011
...
/**/
if(!class_exists("c_ws_plugin__optimizemember_pro_menu_page_authnet_forms"))
	{
		/**
		* Menu page for optimizeMember Pro ( Authorize.Net Forms page ).
		*
		...
		* @since 110531
		*/
		class c_ws_plugin__optimizemember_pro_menu_page_authnet_forms
			{
				public function __construct()
					{
						...
						echo '<h2>Authorize.Net Pro Forms</h2>'."\n";
						/**/
						...
						$ws_plugin__optimizemember_pro_temp_s = trim(c_ws_plugin__optimizemember_utilities::evl(file_get_contents(dirname(dirname(__FILE__))."/templates/shortcodes/authnet-registration-form-shortcode.php")));
						...
						for($n = 1; $n <= $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["levels"]; $n++)
							{
								...
								echo '<div class="ws-menu-page-group" title="Authorize.Net Forms For Level '.$labelText.' Access">'."\n";
								/**/
								...
								echo '<p>Very simple. All you do is customize the form fields provided, for each Membership Level that you plan to offer. Then press (Generate Form Code). These special Authorize.Net Forms are customized to work with optimizeMember seamlessly. Member accounts will be activated instantly, in an automated fashion. When a Member cancels their Membership, or fails to make payments on time, optimizeMember will automatically terminate their Membership privileges. optimizeMember makes extensive use of the Authorize.Net Silent Post and ARB services. optimizeMember receives updates from Authorize.Net behind-the-scene.</p>'."\n";
								...
								echo '<form onsubmit="return false;">'."\n";
								echo '<strong>WordPress Shortcode:</strong> ( recommended for both the WordPress Visual &amp; HTML Editors )<br />'."\n";
								$ws_plugin__optimizemember_pro_temp_s = trim(c_ws_plugin__optimizemember_utilities::evl(file_get_contents(dirname(dirname(__FILE__))."/templates/shortcodes/authnet-checkout-form-shortcode.php")));
						...
						echo 'Packages: ';
										...
										echo '<input type="hidden" autocomplete="off" name="ws_plugin__optimizemember_security_meta_box_ccaps" id="ws-plugin--optimizemember-pro-modification-ccaps" value="" />'."\n";
						echo '</form>'."\n";
						echo '</td>'."\n";
						/**/
						echo '</tr>'."\n";
						...
						if(!is_multisite() || !c_ws_plugin__optimizemember_utils_conds::is_multisite_farm() || is_main_site())
							{
								...
								echo '<p>Currency: <select id="ws-plugin--optimizemember-pro-ccap-currency">'.trim(c_ws_plugin__optimizemember_utilities::evl(file_get_contents(dirname(dirname(__FILE__))."/templates/options/authnet-currencies.php"))).'</select> <input type="button" value="Generate Form Code" onclick="ws_plugin__optimizemember_pro_authnetCcapFormGenerate();" class="button-primary" /></p>'."\n";
								...
								echo '<form onsubmit="return false;">'."\n";
								echo '<strong>WordPress Shortcode:</strong> ( recommended for both the WordPress Visual &amp; HTML Editors )<br />'."\n";
								$ws_plugin__optimizemember_pro_temp_s = trim(c_ws_plugin__optimizemember_utilities::evl(file_get_contents(dirname(dirname(__FILE__))."/templates/shortcodes/authnet-ccaps-checkout-form-shortcode.php")));
								$ws_plugin__optimizemember_pro_temp_s = preg_replace("/%%custom%%/", c_ws_plugin__optimizemember_utils_strings::esc_refs(esc_attr($_SERVER["HTTP_HOST"])), $ws_plugin__optimizemember_pro_temp_s);
						...
						echo '<form onsubmit="return false;">'."\n";
						echo '<strong>WordPress Shortcode:</strong> ( recommended for both the WordPress Visual &amp; HTML Editors )<br />'."\n";
						$ws_plugin__optimizemember_pro_temp_s = trim(c_ws_plugin__optimizemember_utilities::evl(file_get_contents(dirname(dirname(__FILE__))."/templates/shortcodes/authnet-update-form-shortcode.php")));
						...
						echo '<p>According to Authorize.Net\'s policy on recurring billing, you MUST provide each and every Customer with an easy to way to cancel future charges. Generating a Cancellation Form here, and making that Form available to all Customers is our recommendation.</p>'."\n";
						echo '<p><em><strong>*Cancellation Process*</strong> Very simple. A Member clicks a link to a Post/Page that contains a Cancellation Form you\'ve generated. The Member clicks the Submit button to confirm the cancellation. optimizeMember is notified silently behind-the-scene, and will immediately cancel all future billing. optimizeMember will later terminate their account access, at the correct point in time. This works in conjunction with the optimizeMember Auto-EOT System. For further details, see: <code>optimizeMember -> Authorize.Net Options -> EOT Behavior</code>.</em></p>'."\n";
						/**/
						echo '<table class="form-table">'."\n";
						echo '<tbody>'."\n";
						echo '<tr>'."\n";
						...
						echo '<form onsubmit="return false;">'."\n";
						echo '<strong>WordPress Shortcode:</strong> ( recommended for both the WordPress Visual &amp; HTML Editors )<br />'."\n";
						$ws_plugin__optimizemember_pro_temp_s = trim(c_ws_plugin__optimizemember_utilities::evl(file_get_contents(dirname(dirname(__FILE__))."/templates/shortcodes/authnet-cancellation-form-shortcode.php")));
						...
						echo '<p>Currency: <select id="ws-plugin--optimizemember-pro-sp-currency">'.trim(c_ws_plugin__optimizemember_utilities::evl(file_get_contents(dirname(dirname(__FILE__))."/templates/options/authnet-currencies.php"))).'</select> <input type="button" value="Generate Form Code" onclick="ws_plugin__optimizemember_pro_authnetSpFormGenerate();" class="button-primary" /></p>'."\n";
						echo '</form>'."\n";
						echo '</td>'."\n";
						/**/
						echo '</tr>'."\n";
						...
						echo '<form onsubmit="return false;">'."\n";
						echo '<strong>WordPress Shortcode:</strong> ( recommended for both the WordPress Visual &amp; HTML Editors )<br />'."\n";
						$ws_plugin__optimizemember_pro_temp_s = trim(c_ws_plugin__optimizemember_utilities::evl(file_get_contents(dirname(dirname(__FILE__))."/templates/shortcodes/authnet-sp-checkout-form-shortcode.php")));
						$ws_plugin__optimizemember_pro_temp_s = preg_replace("/%%custom%%/", c_ws_plugin__optimizemember_utils_strings::esc_refs(esc_attr($_SERVER["HTTP_HOST"])), $ws_plugin__optimizemember_pro_temp_s);
						...
						if(!is_multisite() || !c_ws_plugin__optimizemember_utils_conds::is_multisite_farm() || is_main_site())
							{
								...
								echo '<p>'.c_ws_plugin__optimizemember_utils_strings::highlight_php(file_get_contents(dirname(__FILE__)."/code-samples/authnet-s2p-v-query-ok-1.x-php")).'</p>'."\n";
								echo '<p>optimizeMember will only verify a query string for up to 10 seconds. After 10 seconds, <code>optimizemember_pro_authnet_s2p_v_query_ok()</code> will always return <code>false</code>, even if the integrity of the query string is valid. This prevents a Customer from bookmarking your Return URL; thereby causing duplicate commissions; in case you\'re using it for tracking purposes.</p>'."\n";
								...
								echo '<div class="ws-menu-page-hr"></div>'."\n";
								/**/
								echo '<p><em>If it is your intention to allow Customers to bookmark your Custom Return URL, you can still do that. Just be aware that <code>optimizemember_pro_authnet_s2p_v_query_ok()</code> will return <code>false</code> after the first 10 seconds. If you want to verify after 10 seconds, you can pass a second argument to the function, like this:</em></p>'."\n";
								echo '<p>'.c_ws_plugin__optimizemember_utils_strings::highlight_php(file_get_contents(dirname(__FILE__)."/code-samples/authnet-s2p-v-query-ok-2.x-php")).'</p>'."\n";
								/**/
								echo '<div class="ws-menu-page-hr"></div>'."\n";
								/**/
						...
						echo '<li><code>rp="1"</code> Regular Period. Only valid w/ Membership Level Access'.((is_multisite() && c_ws_plugin__optimizemember_utils_conds::is_multisite_farm() && !is_main_site()) ? '' : ' and/or Independent Custom Capabilities').'. Must be &gt;= <code>1</code> ( ex: <code>1</code> Week, <code>3</code> Weeks, <code>2</code> Months, <code>1</code> Year ).</li>'."\n";
						echo '<li><code>rt="M"</code> Regular Term. Only valid w/ Membership Level Access'.((is_multisite() && c_ws_plugin__optimizemember_utils_conds::is_multisite_farm() && !is_main_site()) ? '' : ' and/or Independent Custom Capabilities').'. Possible values: <code>W</code> = Weeks, <code>M</code> = Months, <code>Y</code> = Years, <code>L</code> = Lifetime.</li>'."\n";
...
/**/
new c_ws_plugin__optimizemember_pro_menu_page_authnet_forms();
?>