<?php
/**
 * Menu page for (Stripe Forms page).
 */
if(realpath(__FILE__) === realpath($_SERVER['SCRIPT_FILENAME']))
	exit('Do not access this file directly.');

if(!class_exists('c_ws_plugin__optimizemember_pro_menu_page_stripe_forms'))
{
	/**
	 * Menu page for s2Member Pro (Stripe Forms page).
	 *
	 * @package s2Member\Menu_Pages
	 * @since 140617
	 */
	class c_ws_plugin__optimizemember_pro_menu_page_stripe_forms
	{
		public function __construct()
		{
			...
			echo '<div class="ws-menu-page-group" title="Quick-Start Guide For Stripe">'."\n";
			...
			echo '<img src="'.esc_attr($GLOBALS["WS_PLUGIN__"]["optimizemember_pro"]["c"]["dir_url"]).'/images/stripe-logo.png" class="ws-menu-page-right" style="width:250px; height:116px; background:#0D1F2F; border-radius:5px; border:0; margin-bottom:10px;" alt="." />'."\n";
			echo '<h3>Quick-Start Guide For Stripe Integration</h3>'."\n";

			echo '<p>The OptimizeMember Pro Module enables Pro Forms for <a href="https://stripe.com/us/features" target="_blank" rel="external">Stripe</a>. These Pro Forms integrate seamlessly with WordPress Shortcodes. This allows you to keep Customers on your site at all times, and it consolidates the Checkout/Registration steps into a single form that you can dress up just the way you like. If you would like to take advantage of Stripe integration, please supply your Stripe Publishable API Key and Secret Key. See <code>OptimizeMember -› Stripe Options</code>.</p>'."\n";
			...
			$ws_plugin__optimizemember_pro_temp_s = trim(c_ws_plugin__optimizemember_utilities::evl(file_get_contents(dirname(dirname(__FILE__)).'/templates/shortcodes/stripe-registration-form-shortcode.php')));
			$ws_plugin__optimizemember_pro_temp_s = preg_replace('/%%level%%/', c_ws_plugin__optimizemember_utils_strings::esc_refs(esc_attr("0")), $ws_plugin__optimizemember_pro_temp_s);
			$ws_plugin__optimizemember_pro_temp_s = preg_replace('/%%level_label%%/', c_ws_plugin__optimizemember_utils_strings::esc_refs(esc_attr($GLOBALS['WS_PLUGIN__']['optimizemember']['o']['level0_label'])), $ws_plugin__optimizemember_pro_temp_s);
			...
			echo '<input type="text" autocomplete="off" id="ws-plugin--optimizemember-pro-registration-shortcode" class="op-wp-shortcode" value="'.format_to_edit($ws_plugin__optimizemember_pro_temp_s).'" onclick="this.select ();" style="font-family:Consolas, monospace; width:99%;" />'."\n";
			echo '</form>'."\n";
			echo '</td>'."\n";
			...
			for($n = 1; $n <= $GLOBALS['WS_PLUGIN__']['optimizemember']['c']['levels']; $n++)
			{
                ...
                echo '<div class="ws-menu-page-group" title="Stripe Forms For Level ' . $labelText . ' Access">' . "\n";
				...
				echo '<p>Very simple. All you do is customize the form fields provided, for each Membership Level that you plan to offer. Then press (Generate Form Code). These special Stripe Forms are customized to work with OptimizeMember seamlessly. Member accounts will be activated instantly, in an automated fashion. When a Member cancels their Membership, or fails to make payments on time, OptimizeMember will automatically terminate their Membership privileges. OptimizeMember listens to your Stripe Webhook integration; i.e. OptimizeMember receives updates from Stripe behind-the-scene.</p>'."\n";
				...
				echo '<p>Currency: <select id="ws-plugin--optimizemember-pro-level'.$n.'-currency">'.trim(c_ws_plugin__optimizemember_utilities::evl(file_get_contents(dirname(dirname(__FILE__)).'/templates/options/stripe-currencies.php'))).'</select> <input type="button" class="button-primary" value="Generate Form Code" onclick="ws_plugin__optimizemember_pro_stripeFormGenerate(\'level'.$n.'\');" /></p>'."\n";
				...
				echo '<form onsubmit="return false;">'."\n";
				echo '<strong>WordPress Shortcode:</strong> (recommended for both the WordPress Visual &amp; HTML Editors)<br />'."\n";
				$ws_plugin__optimizemember_pro_temp_s = trim(c_ws_plugin__optimizemember_utilities::evl(file_get_contents(dirname(dirname(__FILE__)).'/templates/shortcodes/stripe-checkout-form-shortcode.php')));
				...
				$ws_plugin__optimizemember_pro_temp_s = preg_replace('/%%custom%%/', c_ws_plugin__optimizemember_utils_strings::esc_refs(esc_attr($_SERVER['HTTP_HOST'])), $ws_plugin__optimizemember_pro_temp_s);
				echo '<input type="text" autocomplete="off" id="ws-plugin--optimizemember-pro-level'.$n.'-shortcode" class="op-wp-shortcode" value="'.format_to_edit($ws_plugin__optimizemember_pro_temp_s).'" onclick="this.select ();" class="monospace" />'."\n";
				echo '</form>'."\n";
				echo '</td>'."\n";
			...
			for($n = 1; $n <= $GLOBALS['WS_PLUGIN__']['optimizemember']['c']['levels']; $n++)
			{
				...
				echo ($n < $GLOBALS['WS_PLUGIN__']['optimizemember']['c']['levels']) ? '<option value="downgrade:'.$n.'">&darr; Downgrade To Level #'.$n.'</option>'."\n" : '';
				echo '</optgroup>'."\n";
			...
			echo '<p>Currency: <select id="ws-plugin--optimizemember-pro-modification-currency">'.trim(c_ws_plugin__optimizemember_utilities::evl(file_get_contents(dirname(dirname(__FILE__)).'/templates/options/stripe-currencies.php'))).'</select> <input type="button" class="button-primary" value="Generate Form Code" onclick="ws_plugin__optimizemember_pro_stripeFormGenerate(\'modification\');" /></p>'."\n";
			...
			echo '<strong>WordPress Shortcode:</strong> (recommended for both the WordPress Visual &amp; HTML Editors)<br />'."\n";
			$ws_plugin__optimizemember_pro_temp_s = trim(c_ws_plugin__optimizemember_utilities::evl(file_get_contents(dirname(dirname(__FILE__)).'/templates/shortcodes/stripe-checkout-form-shortcode.php')));
			$ws_plugin__optimizemember_pro_temp_s = preg_replace('/%%level%%/', c_ws_plugin__optimizemember_utils_strings::esc_refs(esc_attr('1')), $ws_plugin__optimizemember_pro_temp_s);
			$ws_plugin__optimizemember_pro_temp_s = preg_replace('/%%level_label%%/', c_ws_plugin__optimizemember_utils_strings::esc_refs(esc_attr($GLOBALS['WS_PLUGIN__']['optimizemember']['o']['level1_label'])), $ws_plugin__optimizemember_pro_temp_s);
			$ws_plugin__optimizemember_pro_temp_s = preg_replace('/%%custom%%/', c_ws_plugin__optimizemember_utils_strings::esc_refs(esc_attr($_SERVER['HTTP_HOST'])), $ws_plugin__optimizemember_pro_temp_s);
			...
			echo '<input type="text" autocomplete="off" id="ws-plugin--optimizemember-pro-modification-shortcode" class="op-wp-shortcode" value="'.format_to_edit($ws_plugin__optimizemember_pro_temp_s).'" onclick="this.select ();" class="monospace" />'."\n";
			echo '</form>'."\n";
			echo '</td>'."\n";
			...
			if(!is_multisite() || !c_ws_plugin__optimizemember_utils_conds::is_multisite_farm() || is_main_site())
			{
				...
				echo '<p>Currency: <select id="ws-plugin--optimizemember-pro-ccap-currency">'.trim(c_ws_plugin__optimizemember_utilities::evl(file_get_contents(dirname(dirname(__FILE__)).'/templates/options/stripe-currencies.php'))).'</select> <input type="button" class="button-primary" value="Generate Form Code" onclick="ws_plugin__optimizemember_pro_stripeCcapFormGenerate();" /></p>'."\n";
				...
				echo '<form onsubmit="return false;">'."\n";
				echo '<strong>WordPress Shortcode:</strong> (recommended for both the WordPress Visual &amp; HTML Editors)<br />'."\n";
				$ws_plugin__optimizemember_pro_temp_s = trim(c_ws_plugin__optimizemember_utilities::evl(file_get_contents(dirname(dirname(__FILE__)).'/templates/shortcodes/stripe-ccaps-checkout-form-shortcode.php')));
				$ws_plugin__optimizemember_pro_temp_s = preg_replace('/%%custom%%/', c_ws_plugin__optimizemember_utils_strings::esc_refs(esc_attr($_SERVER['HTTP_HOST'])), $ws_plugin__optimizemember_pro_temp_s);
				echo '<input type="text" autocomplete="off" id="ws-plugin--optimizemember-pro-ccap-shortcode" class="op-wp-shortcode" value="'.format_to_edit($ws_plugin__optimizemember_pro_temp_s).'" onclick="this.select ();" class="monospace" />'."\n";
				echo '</form>'."\n";
				echo '</td>'."\n";
			...
			echo '<form onsubmit="return false;">'."\n";
			echo '<strong>WordPress Shortcode:</strong> (recommended for both the WordPress Visual &amp; HTML Editors)<br />'."\n";
			$ws_plugin__optimizemember_pro_temp_s = trim(c_ws_plugin__optimizemember_utilities::evl(file_get_contents(dirname(dirname(__FILE__)).'/templates/shortcodes/stripe-update-form-shortcode.php')));
			echo '<input type="text" autocomplete="off" id="ws-plugin--optimizemember-pro-update-shortcode" class="op-wp-shortcode" value="'.format_to_edit($ws_plugin__optimizemember_pro_temp_s).'" onclick="this.select ();" class="monospace" />'."\n";
			echo '</form>'."\n";
			echo '</td>'."\n";
			...
			echo '<p>According to Stripe\'s policy on recurring billing, you MUST provide each and every Customer with an easy to way to cancel future charges. Generating a Cancellation Form here, and making that Form available to all Customers is our recommendation. For further details and legalities, please visit the <a href="http://www.optimizemember.com/r/stripe-terms-of-service/" target="_blank" rel="external">Stripe Terms of Service</a>.</p>'."\n";
			echo '<p><em><strong>*Cancellation Process*</strong> Very simple. A Member clicks a link to a Post/Page that contains a Cancellation Form you\'ve generated. The Member clicks the Submit button to confirm the cancellation. OptimizeMember is notified silently behind-the-scene, and will immediately cancel all future billing. OptimizeMember will later terminate their account access, at the correct point in time. This works in conjunction with the OptimizeMember Auto-EOT System. For further details, see: <code>OptimizeMember -› Stripe Options -› EOT Behavior</code>.</em></p>'."\n";
			...
			echo '<form onsubmit="return false;">'."\n";
			echo '<strong>WordPress Shortcode:</strong> (recommended for both the WordPress Visual &amp; HTML Editors)<br />'."\n";
			$ws_plugin__optimizemember_pro_temp_s = trim(c_ws_plugin__optimizemember_utilities::evl(file_get_contents(dirname(dirname(__FILE__)).'/templates/shortcodes/stripe-cancellation-form-shortcode.php')));
			echo '<input type="text" autocomplete="off" id="ws-plugin--optimizemember-pro-cancellation-shortcode" class="op-wp-shortcode" value="'.format_to_edit($ws_plugin__optimizemember_pro_temp_s).'" onclick="this.select ();" class="monospace" />'."\n";
			echo '</form>'."\n";
			echo '</td>'."\n";
			...
			for($n = 1; $n <= $GLOBALS['WS_PLUGIN__']['optimizemember']['c']['levels']; $n++) echo '<option value="'.$n.'">OptimizeMember Level #'.$n.'</option>'."\n";
			echo '</select></p>'."\n";
			echo '<p>Paid Subscr. ID: <input type="text" autocomplete="off" id="ws-plugin--optimizemember-pro-reg-link-subscr-id" value="" size="50" /> <a href="#" onclick="alert(\'The Customer\\\'s Paid Subscr. ID (aka: Stripe Subscription ID or Transaction ID) must be unique. This value can be obtained from inside your Stripe account. Each paying Customer MUST be associated with a unique Paid Subscr. ID. If the Customer is NOT associated with a Paid Subscr. ID, you will need to generate a unique value for this field on your own. But keep in mind, OptimizeMember will be unable to maintain future communication with the Stripe Silent Post and ARB services if this value does not reflect a real Paid Subscr. ID, one that exists in your Stripe transaction log.\'); return false;" tabindex="-1">[?]</a></p>'."\n";
			echo '<p>Custom String Value: <input type="text" autocomplete="off" id="ws-plugin--optimizemember-pro-reg-link-custom" value="'.esc_attr($_SERVER['HTTP_HOST']).'" size="30" /> <a href="#" onclick="alert(\'A Paid Subscription is always associated with a Custom String that is passed through the custom=\\\'\\\''.c_ws_plugin__optimizemember_utils_strings::esc_js_sq(esc_attr($_SERVER['HTTP_HOST']), 3).'\\\'\\\' attribute of your Shortcode. This Custom Value, MUST always start with your domain name. However, you can also pipe delimit additional values after your domain, if you need to.\\n\\nFor example:\n'.c_ws_plugin__optimizemember_utils_strings::esc_js_sq(esc_attr($_SERVER['HTTP_HOST']), 3).'|cv1|cv2|cv3\'); return false;" tabindex="-1">[?]</a> <input type="button" class="button-primary" value="Generate Access Link" onclick="ws_plugin__optimizemember_pro_stripeRegLinkGenerate();" /> <img id="ws-plugin--optimizemember-pro-reg-link-loading" src="'.esc_attr($GLOBALS['WS_PLUGIN__']['optimizemember']['c']['dir_url']).'/images/ajax-loader.gif" alt="" style="display:none;" /></p>'."\n";
			...
			echo '<p id="ws-plugin--optimizemember-pro-reg-link" class="monospace" style="display:none;"></p>'."\n";
			echo '</form>'."\n";
			echo '</td>'."\n";
			...
			echo '<p>Very simple. All you do is customize the form fields provided, for each Post/Page that you plan to sell. Then press (Generate Form Code). These special Stripe Forms are customized to work with OptimizeMember seamlessly. You can even Package Additional Posts/Pages together into one transaction. <em><strong>Please note:</strong> forms are NOT saved here. This is only a Form Generator. Once you\'ve generated your Form, copy/paste it into any Post/Page you like. You\'ll want to provide your visitors with a link to the Post/Page where this Form is located.</em></p>'."\n";
			...
			echo '<p>Currency: <select id="ws-plugin--optimizemember-pro-sp-currency">'.trim(c_ws_plugin__optimizemember_utilities::evl(file_get_contents(dirname(dirname(__FILE__)).'/templates/options/stripe-currencies.php'))).'</select> <input type="button" class="button-primary" value="Generate Form Code" onclick="ws_plugin__optimizemember_pro_stripeSpFormGenerate();" /></p>'."\n";
			echo '</form>'."\n";
			echo '</td>'."\n";
			...
			echo '<form onsubmit="return false;">'."\n";
			echo '<strong>WordPress Shortcode:</strong> (recommended for both the WordPress Visual &amp; HTML Editors)<br />'."\n";
			$ws_plugin__optimizemember_pro_temp_s = trim(c_ws_plugin__optimizemember_utilities::evl(file_get_contents(dirname(dirname(__FILE__)).'/templates/shortcodes/stripe-sp-checkout-form-shortcode.php')));
			$ws_plugin__optimizemember_pro_temp_s = preg_replace('/%%custom%%/', c_ws_plugin__optimizemember_utils_strings::esc_refs(esc_attr($_SERVER['HTTP_HOST'])), $ws_plugin__optimizemember_pro_temp_s);
			echo '<input type="text" autocomplete="off" id="ws-plugin--optimizemember-pro-sp-shortcode" class="op-wp-shortcode" value="'.format_to_edit($ws_plugin__optimizemember_pro_temp_s).'" onclick="this.select ();" class="monospace" />'."\n";
			echo '</form>'."\n";
			echo '</td>'."\n";
			...
			echo '<p><select id="ws-plugin--optimizemember-pro-sp-link-hours">'.trim(c_ws_plugin__optimizemember_utilities::evl(file_get_contents(dirname(dirname(__FILE__)).'/templates/options/stripe-sp-hours.php'))).'</select> <input type="button" class="button-primary" value="Generate Access Link" onclick="ws_plugin__optimizemember_pro_stripeSpLinkGenerate();" /> <img id="ws-plugin--optimizemember-pro-sp-link-loading" src="'.esc_attr($GLOBALS['WS_PLUGIN__']['optimizemember']['c']['dir_url']).'/images/ajax-loader.gif" alt="" style="display:none;" /></p>'."\n";
			echo '<p id="ws-plugin--optimizemember-pro-sp-link" class="monospace" style="display:none;"></p>'."\n";
			echo '</form>'."\n";
			echo '</td>'."\n";
			...
			if(!is_multisite() || !c_ws_plugin__optimizemember_utils_conds::is_multisite_farm() || is_main_site())
			{
				...
				echo '<li><code>%%generate_password_url%%</code> = The URL of the page where password can be generated.</li>'."\n";
				...
				echo '<li><code>%%response%%</code> = Deprecated. Use <code>%%s_response%%</code>. A successful response message that *would* have been displayed to the Customer, had they NOT been redirected to your Custom Return URL upon success. This may contain some basic HTML. For instance, it might contain a link to the login page. You don\'t have to use this. You can generate your own response if you like.</li>'."\n";
				echo '<li><code>%%s_response%%</code> = A successful response message that *would* have been displayed to the Customer, had they NOT been redirected to your Custom Return URL upon success. This may contain some basic HTML. For instance, it might contain a link to the login page. You don\'t have to use this. You can generate your own response if you like. Value is encrypted. Use <a href="http://www.optimizemember.com/codex/stable/s2member/api_functions/package-functions/#src_doc_optimizemember_decrypt()" target="_blank" rel="external">optimizemember_decrypt()</a>.</li>'."\n";
				echo '</ul>'."\n";

				echo '<strong>Coupon Replacement Codes:</strong>'."\n";
				echo '<ul class="ws-menu-page-li-margins">'."\n";
				echo '<li><code>%%full_coupon_code%%</code> = A full Coupon Code — if one is accepted by your configuration of OptimizeMember. This may indicate an Affiliate Coupon Code, which will include your Affiliate Suffix Chars too (e.g. the full Coupon Code).</li>'."\n";
				echo '<li><code>%%coupon_code%%</code> = A Coupon Code — if one is accepted by your configuration of OptimizeMember. This will NOT include any Affiliate Suffix Chars. This indicates the actual Coupon Code accepted by your configuration of OptimizeMember (excluding any Affiliate ID).</li>'."\n";
				echo '<li><code>%%coupon_affiliate_id%%</code> = This is the end of an Affiliate Coupon Code <em>(i.e. the referring affiliate\'s ID)</em>. This is only applicable if an Affiliate Coupon Code is accepted by your configuration of OptimizeMember.</li>'."\n";
				echo '</ul>'."\n";
				...
				echo '<p>'.c_ws_plugin__optimizemember_utils_strings::highlight_php(file_get_contents(dirname(__FILE__).'/code-samples/stripe-s2p-v-query-ok-1.x-php')).'</p>'."\n";
				echo '<p>OptimizeMember will only verify a query string for up to 10 seconds. After 10 seconds, <code>optimizemember_pro_stripe_s2p_v_query_ok()</code> will always return <code>false</code>, even if the integrity of the query string is valid. This prevents a Customer from bookmarking your Return URL; thereby causing duplicate commissions; in case you\'re using it for tracking purposes.</p>'."\n";
				...
				echo '<p><em>If it is your intention to allow Customers to bookmark your Custom Return URL, you can still do that. Just be aware that <code>optimizemember_pro_stripe_s2p_v_query_ok()</code> will return <code>false</code> after the first 10 seconds. If you want to verify after 10 seconds, you can pass a second argument to the function, like this:</em></p>'."\n";
				echo '<p>'.c_ws_plugin__optimizemember_utils_strings::highlight_php(file_get_contents(dirname(__FILE__).'/code-samples/stripe-s2p-v-query-ok-2.x-php')).'</p>'."\n";
			...
			echo '<ul class="ws-menu-page-li-margins">'."\n";
			echo '<li><code>accept_coupons="1"</code> Accept Coupons? Possible values: <code>0</code> = do NOT accept Coupons on this particular Form; <code>1</code> = DO accept Coupon Codes on this particular Form. <em><strong>NOTE:</strong> Coupon Codes MUST be configured with OptimizeMember and NOT via the Stripe Dashboard. OptimizeMember Pro Forms currently do not support Stripe-generated coupon codes; only those configured with OptimizeMember. Please see: <code>OptimizeMember ⥱ Pro Coupon Codes</code> for configuration options.</em></li>'."\n";
			...
			echo '<li><code>cc="USD"</code> 3 character currency code. Not valid when <code>cancel="1"</code>. Please see <a href="https://support.stripe.com/questions/which-currencies-does-stripe-support" target="_blank" rel="external">this KB article at Stripe</a> for a list of supported currencies. <em><strong>NOTE:</strong> no matter what currency code that you use, you should ALWAYS give OptimizeMember the amount with decimals (i.e. the dollar amount). While it\'s true that some currency codes require a different denomoniation on the Stripe end of things; OptimizeMember handles these conversions internally. In short, always provide the dollar amount; NOT the amount converted to cents.</em></li>'."\n";
			echo (!is_multisite() || !c_ws_plugin__optimizemember_utils_conds::is_multisite_farm() || is_main_site()) ? '<li><code>ccaps="music,videos"</code> A comma-delimited list of Custom Capabilities (if applicable). Only valid w/ Membership Level Access and/or Independent Custom Capabilities.</li>'."\n" : '';
			echo '<li><code>coupon="SAVE-10"</code> Default/pre-filled Coupon Code. This is optional, and the Coupon Code (if supplied) must exist in your Coupon Code configuration. <em><strong>NOTE:</strong> Coupon Codes MUST be configured with OptimizeMember and NOT via the Stripe Dashboard. OptimizeMember Pro Forms currently do not support Stripe-generated coupon codes; only those configured with OptimizeMember. Please see: <code>OptimizeMember ⥱ Pro Coupon Codes</code> for configuration options.</em></li>'."\n";
			...
			echo '<li><code>default_country_code="US"</code> Not valid when <code>cancel="1"</code>. If you set this 2-character value, it will pre-configure the default Country that is selected in the drop-down menu for Checkout and Billing Update Forms. This MUST be an uppercase country code, following the <a href="http://www.iso.org/iso/english_country_names_and_code_elements" target="_blank" rel="external">ISO-3166-1 specification</a>. <em><strong>NOTE:</strong> Stripe Pro Forms currently display a country drop-down menu if (and only if) you configure taxes with OptimizeMember; i.e. only if OptimizeMember needs to know the customer\'s country for taxation. See also: <code>OptimizeMember ⥱ Stripe Options ⥱ Tax Configuration</code></em></li>'."\n";
			...
			echo '<li><code>rp="1"</code> Regular Period. Only valid w/ Membership Level Access'.((is_multisite() && c_ws_plugin__optimizemember_utils_conds::is_multisite_farm() && !is_main_site()) ? '' : ' and/or Independent Custom Capabilities').'. Must be &gt;= <code>1</code> (ex: <code>1</code> Day, <code>3</code> Weeks, <code>2</code> Months, <code>1</code> Year). Please note — Stripe supports a maximum of 1 year intervals in recurring billing plans. The minimum recurring interval is 1 day and the maximum is 1 year.</li>'."\n";
			echo '<li><code>rt="M"</code> Regular Term. Only valid w/ Membership Level Access'.((is_multisite() && c_ws_plugin__optimizemember_utils_conds::is_multisite_farm() && !is_main_site()) ? '' : ' and/or Independent Custom Capabilities').'. Possible values: <code>D</code> = Days, <code>W</code> = Weeks, <code>M</code> = Months, <code>Y</code> = Years, <code>L</code> = Lifetime. Please note — Stripe supports a maximum of 1 year intervals in recurring billing plans. The minimum recurring interval is 1 day and the maximum is 1 year.</li>'."\n";
			...
			echo '<div class="ws-menu-page-section ws-plugin--optimizemember-pro-form-options-section">'."\n";
			...
			echo '<pre class="code">';
			echo '<code>';
			echo '[optimizeMember-Pro-Stripe-Form]'."\n";
			echo "\t".'[optimizeMember-Pro-Stripe-Form /]'."\n";
			echo "\t".'[optimizeMember-Pro-Stripe-Form /]'."\n";
			echo '[/optimizeMember-Pro-Stripe-Form]';
			echo '</code>';
			...
			echo '<p>Given this simple example (as seen below); OptimizeMember will first take the primary default Shortcode Attributes (from the top-level parent Shortcode); and then it merges those together with Shortcode Attributes from a particular Checkout Option (i.e. a child). The one OptimizeMember merges with is based on the currently selected Checkout Option (e.g. the Checkout Option selected by your customer).</p>'."\n";
			...
			echo '<pre class="code">';
			echo '<code>';
			echo '[optimizeMember-Pro-Stripe-Form rp="1" rt="M" rr="1" accept_coupons="1"]'."\n";
			echo "\t".'[optimizeMember-Pro-Stripe-Form level="1" desc="Option 1 ($10.00)" ra="10.00" /]'."\n";
			echo "\t".'[optimizeMember-Pro-Stripe-Form level="2" desc="Option 2 ($20.00)" ra="20.00" /]'."\n";
			echo "\t".'[optimizeMember-Pro-Stripe-Form level="3" desc="Option 3 ($30.00)" ra="30.00" /]'."\n";
			echo '[/optimizeMember-Pro-Stripe-Form]';
			echo '</code>';
			echo '</pre>'."\n";
			echo '</td>'."\n";
...
new c_ws_plugin__optimizemember_pro_menu_page_stripe_forms();