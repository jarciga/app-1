<?php
/**
* Menu page for optimizeMember Pro ( ccBill Options page ).
*
* Copyright: © 2009-2011
...
/**/
if(!class_exists("c_ws_plugin__optimizemember_pro_menu_page_ccbill_ops"))
	{
		/**
		* Menu page for optimizeMember Pro ( ccBill Options page ).
		*
		...
		* @since 110531
		*/
		class c_ws_plugin__optimizemember_pro_menu_page_ccbill_ops
			{
				public function __construct()
					{
						...
						/**/
						echo '<div class="ws-menu-page-section ws-plugin--optimizemember-pro-ccbill-account-details-section">'."\n";
						echo '<h3>ccBill Account Details ( required )</h3>'."\n";
						echo '<p><a href="http://www.optimizepress.com/ccbill" target="_blank" rel="external">ccBill</a> is a great choice when you need to process transactions discreetly on an adult-oriented site. Drawing on its years of experience and proven payment processing platform, ccBill has a solution that will not only meet the requirements of your consumers, it will also help address the specific needs of your business.</p>'."\n";
						echo '<p>optimizeMember has been integrated with ccBill for Direct Payments and also for Recurring Billing. In order to take advantage of this integration, you will need to have a ccBill Client Account. Once you have an account, all of the details below can be generated from inside of your ccBill Client account, or by contacting ccBill via live chat. If you need further assistance, please check their <a href="http://www.optimizepress.com/ccbill-help" target="_blank" rel="external">help section</a>.</p>'."\n";
						echo '<p><em><strong>*Important*</strong> User Management needs to be turned <code>off</code> in your ccBill account. optimizeMember\'s integration with ccBill does NOT require ccBill to manage Usernames/Passwords. Instead, optimizeMember needs to be given exclusive permission to handle this for you. In your ccBill account, see: <code>Account Admin -> User Management</code>. Turn this <code>off</code>, and choose: <code>Do NOT collect Usernames/Passwords</code>. You will also want to remove Username/Password references in the <code>APPROVAL</code> Email Receipt configured in your ccBill account. In your ccBill account, see: <code>Account Admin -> Custom Emails</code>.</em></p>'."\n";
						/**/
						...
						/**/
						echo '<th>'."\n";
						echo '<label for="ws-plugin--optimizemember-pro-ccbill-client-id">'."\n";
						...
						/**/
						echo '<td>'."\n";
						echo '<input type="text" autocomplete="off" name="ws_plugin__optimizemember_pro_ccbill_client_id" id="ws-plugin--optimizemember-pro-ccbill-client-id" value="'.format_to_edit($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_ccbill_client_id"]).'" /><br />'."\n";
						...
						/**/
						echo '<th>'."\n";
						echo '<label for="ws-plugin--optimizemember-pro-ccbill-client-sid">'."\n";
						...
						/**/
						echo '<td>'."\n";
						echo '<input type="text" autocomplete="off" name="ws_plugin__optimizemember_pro_ccbill_client_sid" id="ws-plugin--optimizemember-pro-ccbill-client-sid" value="'.format_to_edit($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_ccbill_client_sid"]).'" /><br />'."\n";
						...
						/**/
						echo '<th>'."\n";
						echo '<label for="ws-plugin--optimizemember-pro-ccbill-form-name">'."\n";
						...
						/**/
						echo '<td>'."\n";
						echo '<input type="text" autocomplete="off" name="ws_plugin__optimizemember_pro_ccbill_form_name" id="ws-plugin--optimizemember-pro-ccbill-form-name" value="'.format_to_edit($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_ccbill_form_name"]).'" /><br />'."\n";
						echo 'Ex: <code>921cc</code>. Inside your ccBill Client account, go to <code>Account Admin -> Form Admin -> Create New Form</code>. Whenever you create the new Form, you will need to configure ccBill Pricing for this Form as "Dynamic" ( very important ). This one Dynamic Pricing Form will be used by optimizeMember for all Membership purchases. If ccBill asks you to supply a textual description, we suggest something very general like: <code>'.esc_html($_SERVER["HTTP_HOST"]).'</code>.'."\n";
						echo '</td>'."\n";
						/**/
						echo '</tr>'."\n";
						echo '<tr>'."\n";
						/**/
						echo '<th>'."\n";
						echo '<label for="ws-plugin--optimizemember-pro-ccbill-salt-key">'."\n";
						...
						/**/
						echo '<td>'."\n";
						echo '<input type="password" autocomplete="off" name="ws_plugin__optimizemember_pro_ccbill_salt_key" id="ws-plugin--optimizemember-pro-ccbill-salt-key" value="'.format_to_edit($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_ccbill_salt_key"]).'" /><br />'."\n";
						echo 'ccBill requires you to contact them via live chat for this Encryption Key. You will need to ask your ccBill support representative for a Triple DES Encryption Key. Sometimes referred to as a Dynamic Pricing Salt. optimizeMember needs this Key in order to communicate with ccBill, and to properly configure your Dynamic Pricing Form.'."\n";
						echo '</td>'."\n";
						/**/
						echo '</tr>'."\n";
						/**/
						...
						/**/
						echo '<div class="ws-menu-page-section ws-plugin--optimizemember-pro-ccbill-datalink-section">'."\n";
						...
						/**/
						echo '<th>'."\n";
						echo '<label for="ws-plugin--optimizemember-pro-ccbill-dl-user">'."\n";
						...
						/**/
						echo '<td>'."\n";
						echo '<input type="text" autocomplete="off" name="ws_plugin__optimizemember_pro_ccbill_dl_user" id="ws-plugin--optimizemember-pro-ccbill-dl-user" value="'.format_to_edit($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_ccbill_dl_user"]).'" /><br />'."\n";
						...
						/**/
						echo '<th>'."\n";
						echo '<label for="ws-plugin--optimizemember-pro-ccbill-dl-pass">'."\n";
						...
						/**/
						echo '<td>'."\n";
						echo '<input type="password" autocomplete="off" name="ws_plugin__optimizemember_pro_ccbill_dl_pass" id="ws-plugin--optimizemember-pro-ccbill-dl-pass" value="'.format_to_edit($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_ccbill_dl_pass"]).'" />'."\n";
						echo '</td>'."\n";
						/**/
						echo '</tr>'."\n";
						echo '<tr>'."\n";
						/**/
						echo '<th>'."\n";
						echo '<label for="ws-plugin--optimizemember-pro-ccbill-dl-cancellations">'."\n";
						...
						/**/
						echo '<td>'."\n";
						echo '<select name="ws_plugin__optimizemember_pro_ccbill_dl_cancellations" id="ws-plugin--optimizemember-pro-ccbill-dl-cancellations">'."\n";
						echo '<option value="0"'.(($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_ccbill_dl_cancellations"] === "0") ? ' selected="selected"' : '').'>No ( not required, or not yet approved )</option>'."\n";
						echo '<option value="1"'.(($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_ccbill_dl_cancellations"] === "1") ? ' selected="selected"' : '').'>Yes ( ccBill approved me for this )</option>'."\n";
						echo '</select><br />'."\n";
						...
						/**/
						echo '<div class="ws-menu-page-section ws-plugin--optimizemember-pro-ccbill-auto-return-section">'."\n";
						...
						echo '<p>Your <a href="'.esc_attr(site_url("/?optimizemember_pro_ccbill_return&optimizemember_paypal_return=1&optimizemember_paypal_proxy=ccbill&optimizemember_paypal_proxy_use=x-preview")).'" target="_blank" rel="external">ccBill Approval URL</a> is:<br /><code>'.esc_html(site_url("/?optimizemember_pro_ccbill_return=1")).'</code></p>'."\n";
						echo '<p>Your ccBill Denial URL is:<br /><code>'.esc_html(site_url("/?optimizemember_pro_ccbill_return=0")).'</code></p>'."\n";
						/**/
						echo '<div class="ws-menu-page-hr"></div>'."\n";
						/**/
						echo '<h3>Approval Page Template ( <a href="#" onclick="jQuery(\'div#ws-plugin--optimizemember-pro-ccbill-return-page-template\').toggle(); return false;" class="ws-dotted-link">optional customizations</a> )</h3>'."\n";
						echo '<div id="ws-plugin--optimizemember-pro-ccbill-return-page-template" style="display:none;">'."\n";
						echo '<p>With optimizeMember Pro installed, you have the ability to customize your <a href="'.esc_attr(site_url("/?optimizemember_pro_ccbill_return&optimizemember_paypal_return=1&optimizemember_paypal_proxy=ccbill&optimizemember_paypal_proxy_use=x-preview")).'" target="_blank" rel="external">Approval Page Template</a>. Each of your Customers are returned back to your site immediately after they complete checkout at ccBill. Your Approval Page displays a message and instructions for the Customer. optimizeMember may change the message and instructions dynamically, based on what the Customer is actually doing <em>( i.e. based on the type of transaction that is taking place )</em>. So, although we do NOT recommend that you attempt to change the message and instructions presented dynamically by optimizeMember, you CAN certainly control the Header, and/or the overall appearance of optimizeMember\'s Approval Page Template.</p>'."\n";
						echo '<p>The quickest/easiest way, is to simply add some HTML code in the box below. For instance, you might include an &lt;img&gt; tag with your logo. The box below, allows you to customize the Header section <em>( i.e. the top )</em> of optimizeMember\'s default Approval Page Template. Everything else, including the textual response and other important details that each Customer needs to know about, are already handled dynamically by optimizeMember <em>( based on the type of transaction that is taking place )</em>. All you need to do is customize the Header with your logo and anything else you feel is important. Although this Header customization is completely optional, we recommend an <a href="http://www.w3schools.com/tags/tag_img.asp" target="_blank" rel="external">&lt;img&gt; tag</a>, with a logo that is around 300px wide. After you "Save All Changes" below, you may <a href="'.esc_attr(site_url("/?optimizemember_pro_ccbill_return&optimizemember_paypal_return=1&optimizemember_paypal_proxy=ccbill&optimizemember_paypal_proxy_use=x-preview")).'" target="_blank" rel="external">click this link to see what your Header looks like</a>.</p>'."\n";
						/**/
						echo '<table class="form-table">'."\n";
						echo '<tbody>'."\n";
						echo '<tr>'."\n";
						/**/
						echo '<th>'."\n";
						echo '<label for="ws-plugin--optimizemember-pro-ccbill-return-template-header">'."\n";
						...
						/**/
						echo '<td>'."\n";
						echo '<textarea name="ws_plugin__optimizemember_pro_ccbill_return_template_header" id="ws-plugin--optimizemember-pro-ccbill-return-template-header" rows="5" wrap="off" spellcheck="false">'.format_to_edit($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_ccbill_return_template_header"]).'</textarea><br />'."\n";
						...
						/**/
						if(!is_multisite() || !c_ws_plugin__optimizemember_utils_conds::is_multisite_farm() || is_main_site())
							echo '<p>It is also possible to build your own Approval Page Template, if you prefer. If you feel the need to create your own Approval Page Template, please make a copy of optimizeMember\'s default template: <code>'.esc_html(c_ws_plugin__optimizemember_utils_dirs::doc_root_path($GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["dir"]."/includes/templates/returns/default-template.php")).'</code>. Place your copy of this default template, inside your active WordPress theme directory, and name the file: <code>/ccbill-return.php</code>. optimizeMember will automatically find your Approval Page Template in this location, and optimizeMember will use your template, instead of the default. Further details are provided inside optimizeMember\'s default template file. Once your custom template file is in place, you may <a href="'.esc_attr(site_url("/?optimizemember_pro_ccbill_return&optimizemember_paypal_return=1&optimizemember_paypal_proxy=ccbill&optimizemember_paypal_proxy_use=x-preview")).'" target="_blank" rel="external">click this link to see what it looks like</a>.</p>'."\n";
						/**/
						echo '<p>It is also possible to bypass optimizeMember\'s "Approval" system all together, if you prefer. This only works with the "Approval" system, and NOT with the Denial system <em>( it\'s not really needed in that case anyway )</em>. You can take your default Approval URL <em>( shown above )</em>, and add <code>&optimizemember_pro_ccbill_return_success=http://...</code> where the value can be set to a custom Approval URL that you prefer. In other words, if you use the <code>&optimizemember_pro_ccbill_return_success=http://...</code> parameter in your Approval URL, the initial redirection back to optimizeMember\'s default Approval system MUST still occur. However, instead of optimizeMember displaying its Approval Template to the Customer, optimizeMember will silently redirect the Customer to the URL that you specified in the <code>&optimizemember_pro_ccbill_return_success=http://...</code> parameter, allowing you to take complete control over what happens next. Click for an [ <a href="#" onclick="alert(\'Basic Example ( please remember to URL encode the value ):\\n'.esc_attr(site_url("/?optimizemember_pro_ccbill_return=1&optimizemember_pro_ccbill_return_success=".site_url("/thank-you/"))).'\\n\\nProper Example ( with the URL having been encoded properly ):\\n'.esc_attr(site_url("/?optimizemember_pro_ccbill_return=1&optimizemember_pro_ccbill_return_success=".rawurlencode(site_url("/thank-you/")))).'\\n\\n* For help on URL encoding, please see:\\nhttp://www.w3schools.com/tags/ref_urlencode.asp\'); return false;">example</a> ].</p>'."\n";
						echo '</div>'."\n";
						/**/
						echo '</div>'."\n";
						/**/
						...
						/**/
						echo '<div class="ws-menu-page-section ws-plugin--optimizemember-pro-ccbill-bg-post-section">'."\n";
						...
						echo '<p>Your ccBill Approval Post URL is:<br /><code>'.esc_html(site_url("/?optimizemember_pro_ccbill_notify=1")).'</code></p>'."\n";
						echo '<p>Your ccBill Denial Post URL is:<br /><code>'.esc_html(site_url("/?optimizemember_pro_ccbill_notify=0")).'</code></p>'."\n";
						echo '</div>'."\n";
						/**/
						echo '</div>'."\n";
						/**/
						...
						echo '<p><em><strong>*ccBill Email Receipts ( important change required )*</strong> In addition to this email, ccBill also sends your Customer an <code>APPROVAL</code> Receipt. Inside this default Email Receipt that ccBill sends to each of your Customers, there is a small section that includes a Username/Password; which is normally assigned by ccBill. However, optimizeMember\'s integration requires that ccBill User Management be turned <code>off</code> ( i.e. optimizeMember deals with this instead ) ... so you WILL need to remove the Username/Password section from the <code>APPROVAL</code> email inside your ccBill account. In your ccBill account, see: <code>Account Admin -> Custom Emails</code>.</em></p>'."\n";
						/**/
						echo '<p><em class="ws-menu-page-hilite">* The email configuration below is universally applied to all Payment Gateway integrations. [ <a href="#" onclick="alert(\'This configuration panel may ALSO appear under ( optimizeMember -> PayPal Options ). Feel free to configure this email here; but please remember that this configuration is applied universally ( i.e. SHARED ) among all Payment Gateways integrated with optimizeMember.\'); return false;">?</a> ]</em></p>'."\n";
						/**/
						echo '<table class="form-table">'."\n";
						echo '<tbody>'."\n";
						echo '<tr>'."\n";
...
/**/
new c_ws_plugin__optimizemember_pro_menu_page_ccbill_ops();
?>