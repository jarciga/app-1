<?php
/**
* Menu page for optimizeMember Pro (ClickBank® Buttons page).
*
* Copyright: © 2009-2011
...
*
* Your purchase of optimizeMember Pro includes free lifetime upgrades via optimizepress.com
...
if (!class_exists ("c_ws_plugin__optimizemember_pro_menu_page_clickbank_buttons"))
	{
		/**
		* Menu page for optimizeMember Pro (ClickBank® Buttons page).
		*
		...
		* @since 110531
		*/
		class c_ws_plugin__optimizemember_pro_menu_page_clickbank_buttons
			{
				public function __construct ()
					{
						...
						for ($n = 1; $n <= $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["levels"]; $n++)
							{
								...
								echo '<div class="ws-menu-page-group" title="ClickBank Buttons For Level ' . $labelText . ' Access">' . "\n";
								...
								echo '<p>Very simple. All you do is customize the form fields provided, for each Membership Level that you plan to offer. Then press (Generate Button Code). These special ClickBank® Buttons are customized to work with optimizeMember seamlessly. Member accounts will be activated instantly, in an automated fashion. When you, or a Member, cancels their Membership, or fails to make payments on time, optimizeMember will automatically terminate their Membership privileges. optimizeMember makes extensive use of the ClickBank® IPN service. optimizeMember receives updates from ClickBank® behind-the-scene. <em>* Buttons are NOT saved here. This is only a Button Generator. Once you\'ve generated your Button, copy/paste it into your Membership Options Page. If you lose your Button Code, you\'ll need to come back &amp; re-generate a new one.</em></p>' . "\n";
								...
								echo '<p>ClickBank Product Type: <select id="ws-plugin--optimizemember-pro-level' . $n . '-type">' . trim (c_ws_plugin__optimizemember_utilities::evl (file_get_contents (dirname (dirname (__FILE__)) . "/templates/options/clickbank-product-types.php"))) . '</select>' . "\n";
								echo '<p>ClickBank Product Item #: <input type="text" autocomplete="off" id="ws-plugin--optimizemember-pro-level' . $n . '-item-number" value="" size="5" /> <a href="#" onclick="alert(\'This MUST correlate with a Product that you\\\'ve configured in your ClickBank account.\'); return false;" tabindex="-1">[?]</a> &nbsp;&nbsp;&nbsp; <input type="button" value="Generate Button Code" onclick="ws_plugin__optimizemember_pro_clickbankButtonGenerate(\'level' . $n . '\');" class="button-primary" /></p>' . "\n";
								echo '<p id="ws-plugin--optimizemember-pro-level' . $n . '-term-line">Fixed-Term Access: <select id="ws-plugin--optimizemember-pro-level' . $n . '-term">' . trim (c_ws_plugin__optimizemember_utilities::evl (file_get_contents (dirname (dirname (__FILE__)) . "/templates/options/clickbank-fixed-terms.php"))) . '</select> <a href="#" onclick="alert(\'For Standard Products, a Fixed-Term allows optimizeMember to configure the Auto-EOT ( End Of Term ) Time after checkout is completed through ClickBank.\'); return false;" tabindex="-1">[?]</a></p>' . "\n";
								echo '<p id="ws-plugin--optimizemember-pro-level' . $n . '-periods-line">ClickBank Trial Period: <select id="ws-plugin--optimizemember-pro-level' . $n . '-p1">' . trim (c_ws_plugin__optimizemember_utilities::evl (file_get_contents (dirname (dirname (__FILE__)) . "/templates/options/clickbank-trial-p1s.php"))) . '</select>, then Re-Bill: <select id="ws-plugin--optimizemember-pro-level' . $n . '-p3">' . trim (c_ws_plugin__optimizemember_utilities::evl (file_get_contents (dirname (dirname (__FILE__)) . "/templates/options/clickbank-regular-p3s.php"))) . '</select> <a href="#" onclick="alert(\'This MUST correlate with a Product that you\\\'ve configured in your ClickBank account.\'); return false;" tabindex="-1">[?]</a></p>' . "\n";
								echo '<p>Description: <input type="text" autocomplete="off" id="ws-plugin--optimizemember-pro-level' . $n . '-desc" class="clickbank-buttons-subscription-select" value="' . format_to_edit ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["level" . $n . "_label"]) . ' / description and pricing details here." size="70" /> <a href="#" onclick="alert(\'This can be worded however you like. It does NOT need to be an exact match to the Product in your ClickBank account.\'); return false;" tabindex="-1">[?]</a></p>' . "\n";
								echo 'Packages: ';
								...
								$ws_plugin__optimizemember_pro_temp_s = trim (c_ws_plugin__optimizemember_utilities::evl (file_get_contents (dirname (dirname (__FILE__)) . "/templates/shortcodes/clickbank-checkout-button-shortcode.php")));
								$ws_plugin__optimizemember_pro_temp_s = preg_replace ("/%%item%%/", c_ws_plugin__optimizemember_utils_strings::esc_refs (esc_attr ("0")), $ws_plugin__optimizemember_pro_temp_s);
						...
						$ws_plugin__optimizemember_pro_temp_s = trim (c_ws_plugin__optimizemember_utilities::evl (file_get_contents (dirname (dirname (__FILE__)) . "/templates/buttons/clickbank-cancellation-button.php")));
						...
						echo '<form onsubmit="return false;">' . "\n";
						echo '<strong>WordPress® Shortcode:</strong> (recommended for both the WordPress® Visual &amp; HTML Editors)<br />' . "\n";
						$ws_plugin__optimizemember_pro_temp_s = trim (c_ws_plugin__optimizemember_utilities::evl (file_get_contents (dirname (dirname (__FILE__)) . "/templates/shortcodes/clickbank-cancellation-button-shortcode.php")));
						...
						if (!is_multisite () || !c_ws_plugin__optimizemember_utils_conds::is_multisite_farm () || is_main_site ())
							{
								...
								echo '<td>' . "\n";
								echo '<form onsubmit="return false;">' . "\n";
								echo '<p>ClickBank® Product Type: <select id="ws-plugin--optimizemember-pro-ccap-type">' . trim (c_ws_plugin__optimizemember_utilities::evl (file_get_contents (dirname (dirname (__FILE__)) . "/templates/options/clickbank-ccap-product-types.php"))) . '</select>' . "\n";
								echo '<p>ClickBank® Product Item #: <input type="text" autocomplete="off" id="ws-plugin--optimizemember-pro-ccap-item-number" value="" size="5" /> <a href="#" onclick="alert(\'This MUST correlate with a Product that you\\\'ve configured in your ClickBank® account.\'); return false;" tabindex="-1">[?]</a> &nbsp;&nbsp;&nbsp; <input type="button" value="Generate Button Code" onclick="ws_plugin__optimizemember_pro_clickbankCcapButtonGenerate();" class="button-primary" /></p>' . "\n";
								echo '<p>Fixed-Term Access: <select id="ws-plugin--optimizemember-pro-ccap-term">' . trim (c_ws_plugin__optimizemember_utilities::evl (file_get_contents (dirname (dirname (__FILE__)) . "/templates/options/clickbank-ccap-fixed-terms.php"))) . '</select> <a href="#" onclick="alert(\'Independent Custom Capabilities will exist on the Customer\\\'s account forever, or until an EOT (End Of Term) occurs on their primary Subscription with you, if one exists.\'); return false;" tabindex="-1">[?]</a></p>' . "\n";
								echo '<p>Description: <input type="text" autocomplete="off" id="ws-plugin--optimizemember-pro-ccap-desc" value="Description and pricing details here." size="70" /> <a href="#" onclick="alert(\'This can be worded however you like. It does NOT need to be an exact match to the Product in your ClickBank® account.\'); return false;" tabindex="-1">[?]</a></p>' . "\n";
								...
								echo '<form onsubmit="return false;">' . "\n";
								echo '<strong>WordPress® Shortcode:</strong> (recommended for both the WordPress® Visual &amp; HTML Editors)<br />' . "\n";
								$ws_plugin__optimizemember_pro_temp_s = trim (c_ws_plugin__optimizemember_utilities::evl (file_get_contents (dirname (dirname (__FILE__)) . "/templates/shortcodes/clickbank-ccaps-checkout-button-shortcode.php")));
								$ws_plugin__optimizemember_pro_temp_s = preg_replace ("/%%item%%/", c_ws_plugin__optimizemember_utils_strings::esc_refs (esc_attr ("0")), $ws_plugin__optimizemember_pro_temp_s);
								$ws_plugin__optimizemember_pro_temp_s = preg_replace ("/%%custom%%/", c_ws_plugin__optimizemember_utils_strings::esc_refs (esc_attr ($_SERVER["HTTP_HOST"])), $ws_plugin__optimizemember_pro_temp_s);
						...
						echo '<td>' . "\n";
						echo '<form onsubmit="return false;">' . "\n";
						echo '<p>ClickBank® Product Item #: <input type="text" autocomplete="off" id="ws-plugin--optimizemember-pro-sp-item-number" value="" size="5" /> <a href="#" onclick="alert(\'This MUST correlate with a Product that you\\\'ve configured in your ClickBank® account.\'); return false;" tabindex="-1">[?]</a></p>' . "\n";
						echo '<p><select id="ws-plugin--optimizemember-pro-sp-hours">' . trim (c_ws_plugin__optimizemember_utilities::evl (file_get_contents (dirname (dirname (__FILE__)) . "/templates/options/clickbank-sp-hours.php"))) . '</select></p>' . "\n";
						...
						echo '<p><input type="text" autocomplete="off" id="ws-plugin--optimizemember-pro-sp-desc" value="Description and pricing details here." size="67" /> <a href="#" onclick="alert(\'This can be worded however you like. It does NOT need to be an exact match to the Product in your ClickBank® account.\'); return false;" tabindex="-1">[?]</a></p>' . "\n";
						echo '<p><input type="button" value="Generate Button Code" onclick="ws_plugin__optimizemember_pro_clickbankSpButtonGenerate();" class="button-primary" /></p>' . "\n";
						echo '</form>' . "\n";
						echo '</td>' . "\n";
						...
						echo '<form onsubmit="return false;">' . "\n";
						echo '<strong>WordPress® Shortcode:</strong> (recommended for both the WordPress® Visual &amp; HTML Editors)<br />' . "\n";
						$ws_plugin__optimizemember_pro_temp_s = trim (c_ws_plugin__optimizemember_utilities::evl (file_get_contents (dirname (dirname (__FILE__)) . "/templates/shortcodes/clickbank-sp-checkout-button-shortcode.php")));
						$ws_plugin__optimizemember_pro_temp_s = preg_replace ("/%%item%%/", c_ws_plugin__optimizemember_utils_strings::esc_refs (esc_attr ("0")), $ws_plugin__optimizemember_pro_temp_s);
						$ws_plugin__optimizemember_pro_temp_s = preg_replace ("/%%custom%%/", c_ws_plugin__optimizemember_utils_strings::esc_refs (esc_attr ($_SERVER["HTTP_HOST"])), $ws_plugin__optimizemember_pro_temp_s);
						...
						echo '<li><code>cbskin=""</code> Optional ClickBank® Order Form Template, which MUST correlate with an Order Form you\'ve configured at ClickBank.com. <a href="#" onclick="alert(\'This MUST correlate with an Order Form that you\\\'ve configured in your ClickBank® account.\'); return false;" tabindex="-1">[?]</a></li>' . "\n";
						echo '<li><code>cbfid=""</code> Optional ClickBank® <a href="http://www.optimizepress.com/r/clickbank-pitchplus/" target="_blank" rel="external">PitchPlus Upsell Flow</a> ID, which MUST correlate with a PitchPlus Upsell Flow you\'ve configured at ClickBank.com. <a href="#" onclick="alert(\'This MUST correlate with a PitchPlus Upsell Flow that you\\\'ve configured in your ClickBank® account.\'); return false;" tabindex="-1">[?]</a></li>' . "\n";
						echo '<li><code>cbur=""</code> Optional ClickBank® <a href="http://www.optimizepress.com/r/clickbank-pitchplus/" target="_blank" rel="external">PitchPlus Upsell Flow</a> action (<code>a</code> accept or <code>d</code> decline), which MUST correlate with instructions provided by ClickBank.com for PitchPlus Upsell Flows. <a href="#" onclick="alert(\'This MUST correlate with instructions provided by ClickBank.com for PitchPlus Upsell Flows. Passing this parameter ALSO automatically enables the `cbf` session parameter; which is handled internally/automatically by optimizeMember® Pro for you (if `cbur` is specified).\'); return false;" tabindex="-1">[?]</a></li>' . "\n";
						echo '<li><code>cbf="auto"</code> Optional ClickBank® <a href="http://www.optimizepress.com/r/clickbank-pitchplus/" target="_blank" rel="external">PitchPlus Upsell Flow</a> session ID, which MUST correlate with instructions provided by ClickBank.com for PitchPlus Upsell Flows. optimizeMember® Pro handles this automatically when you define <code>cbur="a"</code> (or <code>cbur="d"</code>); and set this to <code>cbf="auto"</code> (recommended). <a href="#" onclick="alert(\'This MUST correlate with instructions provided by ClickBank.com for PitchPlus Upsell Flows. If you define `cbur`, and cbf=&quot;auto&quot;, this parameter is automatically set to the proper session ID for you; which is handled internally/automatically by optimizeMember® Pro.\'); return false;" tabindex="-1">[?]</a></li>' . "\n";
						...
						echo '<li><code>rp="1"</code> Regular Period. Only valid w/ Membership Level Access' . ((is_multisite () && c_ws_plugin__optimizemember_utils_conds::is_multisite_farm () && !is_main_site ()) ? '' : ' and/or Independent Custom Capabilities') . '. Must be &gt;= <code>1</code> (ex: <code>1</code> Week, <code>2</code> Months, <code>1</code> Month, <code>3</code> Days). Please note — ClickBank® Recurring Products support only the following recurring intervals: weekly, bi-weekly, monthly, quarterly. <a href="#" onclick="alert(\'With the exception of Fixed-Term Access on Standard Products, this MUST correlate with a Recurring Product that you\\\'ve configured in your ClickBank® account.\'); return false;" tabindex="-1">[?]</a></li>' . "\n";
						echo '<li><code>rt="D"</code> Regular Term. Only valid w/ Membership Level Access' . ((is_multisite () && c_ws_plugin__optimizemember_utils_conds::is_multisite_farm () && !is_main_site ()) ? '' : ' and/or Independent Custom Capabilities') . '. Possible values: <code>D</code> = Days, <code>W</code> = Weeks, <code>M</code> = Months, <code>Y</code> = Years, <code>L</code> = Lifetime. Please note — ClickBank® Recurring Products support only the following recurring intervals: weekly, bi-weekly, monthly, quarterly. <a href="#" onclick="alert(\'With the exception of Fixed-Term Access on Standard Products, this MUST correlate with a Product that you\\\'ve configured in your ClickBank® account.\'); return false;" tabindex="-1">[?]</a></li>' . "\n";
						echo '<li><code>rr="0"</code> Recurring directive. Only valid w/ Membership Level Access' . ((is_multisite () && c_ws_plugin__optimizemember_utils_conds::is_multisite_farm () && !is_main_site ()) ? '' : ' and/or Independent Custom Capabilities') . '. Possible values: <code>0</code> = "Standard" Product, charge is NOT recurring; <code>1</code> = "Recurring" Product with possible Trial Period, and Rebill charges. <a href="#" onclick="alert(\'This MUST correlate with a Product that you\\\'ve configured in your ClickBank® account.\'); return false;" tabindex="-1">[?]</a></li>' . "\n";
						...
						echo '<p>When you generate a Button Code, optimizeMember will make a Shortcode available to you. Ultimately, your Shortcode produces a clickable link that will send Customers over to ClickBank® where they\'ll complete the checkout process. You may notice that optimizeMember will attach several s2_ Variables to the link it generates. These s2_ Variables will be pre-configured by one of optimizeMember\'s Button Generators automatically; so there really is nothing more you need to do. However, many site owners like to know exactly how these s2_ Variables work. Below, is a brief overview of each possible s2_ Variable.</p>' . "\n";
						...
						echo '<li><code>&amp;s2_desc=Brief Description</code> Description optimizeMember uses for your ClickBank® Product. This can/could be different from what you have on file at ClickBank® for a particular Product.</li>' . "\n";
						...
						echo '<li><code>&amp;s2_p1=0 D</code> For Recurring Products, this tells optimizeMember how many Trial Days are in your ClickBank® Product configuration.</li>' . "\n";
						echo '<li><code>&amp;s2_p3=1 M</code> For Recurring Products, this tells optimizeMember how your ClickBank® Product is rebilled (<code><em>2 W = Bi-Weekly, 1 M = Monthly, 3 M = Quarterly, 1 Y = Yearly</em></code>).</li>' . "\n";
						echo '<li><code>&amp;s2_subscr_id=s2-' . esc_html (uniqid ()) . '</code> This is optimizeMember\'s Unique Subscr. ID for Recurring Products. For ClickBank® Recurring Products, this is stored by optimizeMember after checkout as the Paid Subscr. ID. Otherwise, for Standard Products, optimizeMember will record the ClickBank® Receipt # instead. The reason optimizeMember generates a unique Subscription ID, is because the Receipt # ClickBank® generates is NOT dynamic enough to handle everything optimizeMember makes possible. ClickBank® Receipts (by themselves) do NOT allow site owners to track multiple payments with a common Subscription ID. So optimizeMember makes up for this minor deficiency. For instance, with Recurring ClickBank® Products, optimizeMember\'s Subscription ID will remain constant throughout all future payments; making things easier to keep track of (on the back-end management side of things).</li>' . "\n";
...
new c_ws_plugin__optimizemember_pro_menu_page_clickbank_buttons ();
?>