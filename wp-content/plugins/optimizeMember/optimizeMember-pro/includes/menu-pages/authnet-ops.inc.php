<?php
/**
* Menu page for optimizeMember Pro ( Authorize.Net Options page ).
*
* Copyright: © 2009-2011
...
/**/
if (!class_exists ("c_ws_plugin__optimizemember_pro_menu_page_authnet_ops"))
	{
		/**
		* Menu page for optimizeMember Pro ( Authorize.Net Options page ).
		*
		...
		* @since 110531
		*/
		class c_ws_plugin__optimizemember_pro_menu_page_authnet_ops
			{
				public function __construct ()
					{
						...
						/**/
						echo '<th>' . "\n";
						echo '<label for="ws-plugin--optimizemember-pro-authnet-api-login-id">' . "\n";
						...
						/**/
						echo '<td>' . "\n";
						echo '<input type="text" autocomplete="off" name="ws_plugin__optimizemember_pro_authnet_api_login_id" id="ws-plugin--optimizemember-pro-authnet-api-login-id" value="' . format_to_edit ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_authnet_api_login_id"]) . '" /><br />' . "\n";
						...
						/**/
						echo '<th>' . "\n";
						echo '<label for="ws-plugin--optimizemember-pro-authnet-api-trans-key">' . "\n";
						...
						/**/
						echo '<td>' . "\n";
						echo '<input type="password" autocomplete="off" name="ws_plugin__optimizemember_pro_authnet_api_trans_key" id="ws-plugin--optimizemember-pro-authnet-api-trans-key" value="' . format_to_edit ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_authnet_api_trans_key"]) . '" /><br />' . "\n";
						echo 'You\'ll find this in your Authorize.Net Merchant account, under: <code>Account -> Settings</code>.' . "\n";
						echo '</td>' . "\n";
						/**/
						echo '</tr>' . "\n";
						...
						/**/
						echo '<th>' . "\n";
						echo '<label for="ws-plugin--optimizemember-pro-authnet-api-salt-key">' . "\n";
						...
						/**/
						echo '<td>' . "\n";
						echo '<input type="password" autocomplete="off" name="ws_plugin__optimizemember_pro_authnet_api_salt_key" id="ws-plugin--optimizemember-pro-authnet-api-salt-key" value="' . format_to_edit ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_authnet_api_salt_key"]) . '" /><br />' . "\n";
						...
						echo '<label for="ws-plugin--optimizemember-pro-authnet-sandbox">' . "\n";
						...
						/**/
						echo '<td>' . "\n";
						echo '<input type="radio" name="ws_plugin__optimizemember_pro_authnet_sandbox" id="ws-plugin--optimizemember-pro-authnet-sandbox-0" value="0"' . ((!$GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_authnet_sandbox"]) ? ' checked="checked"' : '') . ' /> <label for="ws-plugin--optimizemember-pro-authnet-sandbox-0">No</label> &nbsp;&nbsp;&nbsp; <input type="radio" name="ws_plugin__optimizemember_pro_authnet_sandbox" id="ws-plugin--optimizemember-pro-authnet-sandbox-1" value="1"' . (($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_authnet_sandbox"]) ? ' checked="checked"' : '') . ' /> <label for="ws-plugin--optimizemember-pro-authnet-sandbox-1">Yes, enable support for Sandbox testing.</label><br />' . "\n";
						echo '<em>Only enable this if you\'ve provided Developer credentials above.<br />This puts optimizeMember\'s Authorize.Net integration into Sandbox/Test mode.<br />See: <a href="http://www.optimizepress.com/authorize.net-test-accounts" target="_blank" rel="external">Authorize.Net Test Accounts</a></em>' . "\n";
						echo '</td>' . "\n";
						/**/
						echo '</tr>' . "\n";
						/**/
						...
						/**/
						echo '<div class="ws-menu-page-section ws-plugin--optimizemember-pro-authnet-ipn-section">' . "\n";
						...
						echo '<p>Your Authorize.Net Silent Post URL is:<br /><code>' . esc_html (site_url ("/?optimizemember_pro_authnet_notify=1")) . '</code></p>' . "\n";
						echo '</div>' . "\n";
						/**/
						echo '</div>' . "\n";
						/**/
						...
						echo '<p>This email is sent to new Customers after they successfully complete an Authorize.Net "Pro Form" submission on your site. The <strong>primary</strong> purpose of this email, is to provide the Customer with a receipt, and NOT to send them a <code>%%registration_url%%</code>, because optimizeMember\'s Authorize.Net Pro Form integration handles that automatically; based on scenario. You may want to customize this email further, by providing details that are specifically geared to your site.</p>' . "\n";
						/**/
						...
						/**/
						echo '<p><em class="ws-menu-page-hilite">* This tax configuration is universally applied to all Payment Gateway integrations. [ <a href="#" onclick="alert(\'This configuration panel may ALSO appear under ( optimizeMember -> PayPal Options ). Feel free to configure taxes here; but please remember that this configuration is applied universally ( i.e. SHARED ) among all Payment Gateways integrated with optimizeMember Pro Forms.\'); return false;">?</a> ]</em></p>' . "\n";
						/**/
						echo '<table class="form-table">' . "\n";
						echo '<tbody>' . "\n";
						echo '<tr>' . "\n";
						...
						echo '<p>The Authorize.Net ARB ( Automated Recurring Billing ) service will assist in notifying optimizeMember whenever a Subscription expires. For example, if a Customer cancels their own Subscription; or if you cancel a Customer\'s Subscription through Authorize.Net, optimizeMember will eventually be notified ( with a 24-48 hour delay ), and the account for that Customer will either be demoted to a Free Subscriber, or deleted automatically ( based on your configuration ). ~ Otherwise, under normal circumstances, optimizeMember will not process an EOT until the User has completely used up the time they paid for.</em></p>' . "\n";
						/**/
						...
						/**/
						echo '<td>' . "\n";
						echo '<select name="ws_plugin__optimizemember_triggers_immediate_eot" id="ws-plugin--optimizemember-triggers-immediate-eot" disabled="disabled">' . "\n";
						echo '<option value="none" selected="selected">Neither ( I\'ll review these two events manually )</option>' . "\n";
						echo '</select><br />' . "\n";
						echo 'N/A with Authorize.Net. Authorize.Net does NOT notify optimizeMember on refunds/chargebacks.' . "\n";
						echo '</td>' . "\n";
						/**/
						echo '</tr>' . "\n";
						echo '<tr>' . "\n";
...
/**/
new c_ws_plugin__optimizemember_pro_menu_page_authnet_ops ();
?>