...
/**/
if(!class_exists("c_ws_plugin__optimizemember_pro_login_widget"))
	{
		/**
		* Pro Login Widget.
		*
		...
		*/
		class c_ws_plugin__optimizemember_pro_login_widget /* << Register widget class. */
			extends WP_Widget /* See: /wp-includes/widgets.php for further details. */
			{
				...
				* @return null
				*/
				public function __construct() /* Builds the classname, id_base, description, etc. */
					{
						...
						$control_ops = array("width" => 400, "id_base" => "ws_plugin__optimizemember_pro_login_widget");
						/**/
						eval('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
						do_action("ws_plugin__optimizemember_pro_login_widget_before_construction", get_defined_vars(), $this);
						unset($__refs, $__v); /* Unset defined __refs, __v. */
						/**/
						parent::__construct($control_ops["id_base"], "optimizeMember Pro ( Login Widget )", $widget_ops, $control_ops);
						/**/
						do_action("ws_plugin__optimizemember_pro_login_widget_after_construction", get_defined_vars(), $this);
						/**/
						...
						if(!is_user_logged_in()) /* The User/Member is NOT logged in. */
							{
								...
								/**/
								if($ops_page && is_page($ops_page) && /* Seeking something specific? */ !empty($_GET["_optimizemember_seeking"]["_uri"]))
				...
				public function form($instance = FALSE)
					{
						...
						/**/
						eval('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
						do_action("ws_plugin__optimizemember_pro_login_widget_before_form", get_defined_vars(), $this);
						unset($__refs, $__v); /* Unset defined __refs, __v. */
						/*
						...
						*/
						...
						/**/
						echo '<label for="'.esc_attr($this->get_field_id("code")).'">Additional XHTML'.((!is_multisite() || !c_ws_plugin__optimizemember_utils_conds::is_multisite_farm() || is_main_site()) ? '/PHP' : '').' Code?</label><br />'."\n";
						...
						/**/
						echo (!is_multisite() || !c_ws_plugin__optimizemember_utils_conds::is_multisite_farm() || is_main_site()) ? '<div style="margin:15px 0 15px 0; height:1px; line-height:1px; background:#CCCCCC;"></div>'."\n" : '';
						echo (!is_multisite() || !c_ws_plugin__optimizemember_utils_conds::is_multisite_farm() || is_main_site()) ? '<em>Or include this widget dynamically via PHP:<br />'.c_ws_plugin__optimizemember_utils_strings::highlight_php('<?php echo optimizemember_pro_login_widget(); ?>').'<br /><small>See: <code>optimizeMember -> API Scripting -> Pro Login Widget</code></small></em>'."\n" : '';
						/**/
						do_action("ws_plugin__optimizemember_pro_login_widget_after_form", get_defined_vars(), $this);
						/**/
						echo '<br />'."\n";
						/**/
						return; /* Return for uniformity. */
				...
				*/
				public function update($instance = FALSE, $old = FALSE)
					{
						eval('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
						do_action("ws_plugin__optimizemember_pro_login_widget_before_update", get_defined_vars(), $this);
						unset($__refs, $__v); /* Unset defined __refs, __v. */
						/**/
						$instance = (array)c_ws_plugin__optimizemember_utils_strings::trim_deep(stripslashes_deep($instance));
						/**/
						return c_ws_plugin__optimizemember_pro_login_widget::configure_options_and_their_defaults($instance);
					}
				/**
				...
				*/
				public function configure_options_and_their_defaults($options = FALSE)
					{
						$default_options = apply_filters("ws_plugin__optimizemember_pro_login_widget_default_options", array("title" => _x("Membership Login", "s2member-front", "s2member"), "profile_title" => _x("My Profile Summary", "s2member-front", "s2member"), "signup_url" => "%%automatic%%", "my_account_url" => "%%automatic%%", "my_profile_url" => "%%automatic%%", "login_redirect" => "", "logout_redirect" => "%%home%%", "logged_in_code" => "", "logged_out_code" => "", "display_gravatar" => "1", "link_gravatar" => "1", "display_name" => "1"));
						/**/
						...
						/**/
						return apply_filters("ws_plugin__optimizemember_pro_login_widget_options", $options);
					}
			}
	}
?>