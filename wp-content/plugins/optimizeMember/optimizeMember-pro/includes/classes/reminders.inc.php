 ...
 * to our video tutorial library: {@link http://optimizemember.com/videos/}
 *
...
if (!class_exists('c_ws_plugin__optimizemember_pro_reminders')) {
    /**
     * Reminders.
     *
     * @since 151202 Reminders.
     */
    class c_ws_plugin__optimizemember_pro_reminders
    {
        ...
        public static function remind($vars = array())
        {
            ...
            if (!$GLOBALS['WS_PLUGIN__']['optimizemember']['o']['reg_email_from_name']
                || !$GLOBALS['WS_PLUGIN__']['optimizemember']['o']['reg_email_from_email']) {
            ...
            $scan_time            = apply_filters('ws_plugin__optimizemember_pro_eot_reminders_scan_time', strtotime('-1 day', self::$now), get_defined_vars());
            $per_process          = apply_filters('ws_plugin__optimizemember_pro_eot_reminders_per_process', $vars['per_process'], get_defined_vars());
            $message_bytes_in_log = apply_filters('ws_plugin__optimizemember_pro_eot_reminder_email_message_bytes_in_log', 100);

            $mail_from = '"'.str_replace('"', "'", $GLOBALS['WS_PLUGIN__']['optimizemember']['o']['reg_email_from_name']).'"'.
                ' <'.$GLOBALS['WS_PLUGIN__']['optimizemember']['o']['reg_email_from_email'].'>';

            $user_ids_to_exclude = '
                SELECT DISTINCT `user_id` AS `ID` FROM `'.$wpdb->usermeta.'`
                    WHERE
                        (`meta_key` = \''.$wpdb->prefix.'optimizemember_last_reminder_scan\' AND `meta_value` >= \''.esc_sql($scan_time).'\')
                        OR (`meta_key` = \''.$wpdb->prefix.'optimizemember_reminders_enable\' AND `meta_value` = \'0\')
            ';
            $additional_user_ids_to_exclude = apply_filters('ws_plugin__optimizemember_pro_eot_reminders_exclude_user_ids', array(), get_defined_vars());
            ...
            $sql = '
                SELECT DISTINCT `user_id` AS `ID` FROM `'.$wpdb->usermeta.'`
                    ...
                    : '').'
                        AND (
                              (`meta_key` = \''.$wpdb->prefix.'optimizemember_subscr_gateway\' AND `meta_value` != \'\')
                              OR (`meta_key` = \''.$wpdb->prefix.'optimizemember_auto_eot_time\' AND `meta_value` != \'\')
                              OR (`meta_key` = \''.$wpdb->prefix.'optimizemember_last_auto_eot_time\' AND `meta_value` != \'\')
                            )
            ...
            $email_configs_were_on = // Was enabled already?
                c_ws_plugin__optimizemember_email_configs::email_config_status();
            ...
            foreach ($user_ids as $_user_id) {
                ...
                update_user_option($_user->ID, 'optimizemember_last_reminder_scan', self::$now);

                $_eot = c_ws_plugin__optimizemember_utils_users::get_user_eot($_user->ID);
                ...
                $_mail_from  = apply_filters('ws_plugin__optimizemember_pro_eot_reminder_email_from', $mail_from, get_defined_vars());
                $_recipients = apply_filters('ws_plugin__optimizemember_pro_eot_reminder_email_recipients', $_recipients, get_defined_vars());
                $_subject    = apply_filters('ws_plugin__optimizemember_pro_eot_reminder_email_subject', $_subject, get_defined_vars());
                $_message    = apply_filters('ws_plugin__optimizemember_pro_eot_reminder_email_message', $_message, get_defined_vars());

                if (!$_recipients || !$_subject || !$_message || !$_mail_from) {
                    continue; // Final validation must not fail.
                }
                foreach (c_ws_plugin__optimizemember_utils_strings::parse_emails($_recipients) as $_recipient) {
                    ...
                    c_ws_plugin__optimizemember_utils_logs::log_entry('eot-reminders', $_log_entry);
                }
            }
        ...
        protected static function fill_replacement_codes($user, $eot, &$recipients, &$subject, &$message)
        {
            $ipn_signup_vars = // If available, these take precedence.
                (array) c_ws_plugin__optimizemember_utils_users::get_user_ipn_signup_vars($user->ID);
            ...
            if (!empty($ipn_signup_vars['initial_term'])) {
                $initial_cycle = c_ws_plugin__optimizemember_utils_time::period_term($ipn_signup_vars['initial_term']);
            ...
            if (!empty($ipn_signup_vars['regular_term'])) {
                if (!empty($ipn_signup_vars['recurring'])) {
                    $regular_cycle           = c_ws_plugin__optimizemember_utils_time::period_term($ipn_signup_vars['regular_term'], true);
                    $recurring_regular_cycle = $ipn_signup_vars['recurring'].' / '.c_ws_plugin__optimizemember_utils_time::period_term($ipn_signup_vars['regular_term'], true);
                } else {
                    $regular_cycle           = c_ws_plugin__optimizemember_utils_time::period_term($ipn_signup_vars['regular_term'], false);
                    $recurring_regular_cycle = __('0 / non-recurring', 'optimizemember-front', 'optimizemember');
                }
            ...
            foreach (array( // In case IPN Signup Vars are unavailable.
                ...
                $_value     = (string) get_user_option('optimizemember_'.$_key, $user->ID);
                $recipients = str_ireplace('%%'.$_key.'%%', $_value, $recipients);
                $subject    = str_ireplace('%%'.$_key.'%%', $_value, $subject);
                $message    = str_ireplace('%%'.$_key.'%%', $_value, $message);
            }
            ...
            $user_ip    = get_user_option('optimizemember_registration_ip', $user->ID);
            ...
            $user_role  = c_ws_plugin__optimizemember_user_access::user_access_role($user);
            ...
            $user_level = c_ws_plugin__optimizemember_user_access::user_access_level($user);
            ...
            $user_level_label = c_ws_plugin__optimizemember_user_access::user_access_label($user);
            ...
            $user_ccaps = implode(',', c_ws_plugin__optimizemember_user_access::user_access_ccaps($user));
            ...
            if (is_array($fields = get_user_option('optimizemember_custom_fields', $user->ID))) {
            ...
            foreach (preg_split('/\|/', get_user_option('optimizemember_custom', $user->ID)) as $_key => $_value) {
            ...
            $eot_descriptive_time = c_ws_plugin__optimizemember_utils_time::approx_time_difference(self::$now, $eot['time'], 'floor');
            ...
            foreach (apply_filters('ws_plugin__optimizemember_pro_eot_reminder_email_custom_rcs', array(), get_defined_vars()) as $_custom_rc_key => $_custom_rc_value) {
            ...
            if (!is_multisite() || !c_ws_plugin__optimizemember_utils_conds::is_multisite_farm() || is_main_site()) {
                ...
                $recipients = c_ws_plugin__optimizemember_utilities::evl($recipients, $evl_vars);
                $subject    = c_ws_plugin__optimizemember_utilities::evl($subject, $evl_vars);
                $message    = c_ws_plugin__optimizemember_utilities::evl($message, $evl_vars);
            }
        }
