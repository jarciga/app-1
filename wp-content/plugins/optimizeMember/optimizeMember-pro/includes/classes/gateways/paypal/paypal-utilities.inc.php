...
/**/
if(!class_exists("c_ws_plugin__optimizemember_pro_paypal_utilities"))
	{
		/**
		* PayPal utilities.
		*
		...
		* @since 1.5
		*/
		class c_ws_plugin__optimizemember_pro_paypal_utilities
			{
				/**
				...
				public static function payflow_get_profile($subscr_id = FALSE)
					{
						...
						/**/
						if(($profile = c_ws_plugin__optimizemember_paypal_utilities::paypal_payflow_api_response($payflow)) && empty($profile["__error"]))
							return $profile;
						/**/
						$payflow["TENDER"] = "P";
						if(($profile = c_ws_plugin__optimizemember_paypal_utilities::paypal_payflow_api_response($payflow)) && empty($profile["__error"]))
				...
				public static function payflow_cancel_profile($subscr_id = FALSE)
					{
						...
						$payflow["ORIGPROFILEID"] = $subscr_id;
						/**/
						if(($cancellation = c_ws_plugin__optimizemember_paypal_utilities::paypal_payflow_api_response($payflow)) && empty($cancellation["__error"]))
							return true;
						/**/
						$payflow["TENDER"] = "P";
						if(($cancellation = c_ws_plugin__optimizemember_paypal_utilities::paypal_payflow_api_response($payflow)) && empty($cancellation["__error"]))
				...
				public static function paypal_maestro_solo_2gbp($attr = FALSE, $card_type = FALSE)
					{
						if(is_array($attr) && is_string($card_type) && in_array($card_type, array("Maestro", "Solo")))
							if(!empty($attr["cc"]) && strcasecmp($attr["cc"], "GBP") !== 0 && is_numeric($attr["ta"]) && is_numeric($attr["ra"]))
								if(($attr["ta"] <= 0 && is_numeric($c_ta = "0")) || is_numeric($c_ta = c_ws_plugin__optimizemember_utils_cur::convert($attr["ta"], $attr["cc"], "GBP")))
									if(($attr["ra"] <= 0 && is_numeric($c_ra = "0")) || is_numeric($c_ra = c_ws_plugin__optimizemember_utils_cur::convert($attr["ra"], $attr["cc"], "GBP")))
				...
				*/
				public static function paypal_ajax_tax()
					{
						if(!empty($_POST["ws_plugin__optimizemember_pro_paypal_ajax_tax"]) && ($nonce = $_POST["ws_plugin__optimizemember_pro_paypal_ajax_tax"]) && (wp_verify_nonce($nonce, "ws-plugin--optimizemember-pro-paypal-ajax-tax") || c_ws_plugin__optimizemember_utils_encryption::decrypt($nonce) === "ws-plugin--optimizemember-pro-paypal-ajax-tax"))
									...
									So instead, s2member-pro-min.js should use c_ws_plugin__optimizemember_utils_encryption::encrypt() as an alternate form of nonce. */
							{
								...
								/**/
								if(!empty($_POST["ws_plugin__optimizemember_pro_paypal_ajax_tax_vars"]) && is_array($_p_tax_vars = c_ws_plugin__optimizemember_utils_strings::trim_deep(stripslashes_deep($_POST["ws_plugin__optimizemember_pro_paypal_ajax_tax_vars"]))))
									{
										if(is_array($attr = (!empty($_p_tax_vars["attr"])) ? unserialize(c_ws_plugin__optimizemember_utils_encryption::decrypt($_p_tax_vars["attr"])) : false))
											{
												$attr = (!empty($attr["coupon"])) ? c_ws_plugin__optimizemember_pro_paypal_utilities::paypal_apply_coupon($attr, $attr["coupon"]) : $attr;
												/**/
												...
												$a = c_ws_plugin__optimizemember_pro_paypal_utilities::paypal_cost(null, $sub_total_today, $state, $country, $zip, $currency, $desc);
