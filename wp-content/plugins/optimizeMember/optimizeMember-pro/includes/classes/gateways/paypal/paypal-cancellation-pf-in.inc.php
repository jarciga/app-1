...
/**/
if(!class_exists("c_ws_plugin__optimizemember_pro_paypal_cancellation_pf_in"))
	{
		/**
		* PayPal Cancellation Forms ( inner processing routines ).
		*
		...
		* @since 1.5
		*/
		class c_ws_plugin__optimizemember_pro_paypal_cancellation_pf_in
			{
				/**
				...
				public static function paypal_cancellation()
					{
						if(!empty($_POST["optimizemember_pro_paypal_cancellation"]["nonce"]) && ($nonce = $_POST["optimizemember_pro_paypal_cancellation"]["nonce"]) && wp_verify_nonce($nonce, "s2member-pro-paypal-cancellation"))
							{
								$GLOBALS["ws_plugin__optimizemember_pro_paypal_cancellation_response"] = array(); /* This holds the global response details. */
								$global_response = &$GLOBALS["ws_plugin__optimizemember_pro_paypal_cancellation_response"]; /* This is a shorter reference. */
								/**/
																		...
																		if(preg_match("/^(Active|ActiveProfile|Suspended|SuspendedProfile)$/i", $paypal["STATUS"])) /* Possible? */
																			{
																				...
																				/**/
																				if($post_vars["attr"]["success"] && ($custom_success_url = str_ireplace(array("%%s_response%%", /* Deprecated in v111106 ». */ "%%response%%"), array(urlencode(c_ws_plugin__optimizemember_utils_encryption::encrypt($global_response["response"])), urlencode($global_response["response"])), $post_vars["attr"]["success"])) && ($custom_success_url = trim(preg_replace("/%%(.+?)%%/i", "", $custom_success_url))))
