					...
					foreach($objs as $obj /* Run through all of the Paid Member IDs that originated their Subscription through the PayPal gateway. */)
					{
						if(($user_id = $obj->ID) && ($counter = (int)$counter + 1)) // Update counter. Only run through X records; given by $per_process.
						{
							...
							if(($subscr_id = get_user_option('optimizemember_subscr_id', $user_id)) && !get_user_option('optimizemember_auto_eot_time', $user_id))
							{
								...
								{
									$paypal['ipn_signup_vars'] = c_ws_plugin__optimizemember_utils_users::get_user_ipn_signup_vars(FALSE, $subscr_id);
									...
									else if($paypal['ipn_signup_vars'] && preg_match('/(expired|suspended|canceled|terminated|deactivated)/i', $paypal['STATUS']))
									{
										...
										$paypal['optimizemember_log'][] = 'Ignoring status (`'.$paypal['STATUS'].'`).'.
											' The user has no IPN Signup Vars recorded on-site by optimizeMember.';

									else if(!$processed) $paypal['optimizemember_log'][] = 'Ignoring status (`'.$paypal['STATUS'].'`).'.
										' It does NOT require any action on the part of optimizeMember.';
