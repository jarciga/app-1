...
if(!class_exists("c_ws_plugin__optimizemember_pro_paypal_checkout_pf_in"))
{
	/**
	 * PayPal Checkout Forms (inner processing routines).
	 *
	 ...
	 * @since 1.5
	 */
	class c_ws_plugin__optimizemember_pro_paypal_checkout_pf_in
	{
		/**
			       ...
			       && ($xco_post_vars = get_transient("s2m_".md5("optimizemember_transient_express_checkout_".$_GET["token"])))))
			{
				...
				if(!c_ws_plugin__optimizemember_pro_paypal_responses::paypal_form_attr_validation_errors($post_vars["attr"])) // Attr errors?
				{
					if(!($error = c_ws_plugin__optimizemember_pro_paypal_responses::paypal_form_submission_validation_errors("checkout", $post_vars)))
					{
						...
						if(empty($_GET["optimizemember_paypal_xco"]) && $post_vars["card_type"] === "PayPal" && ($cost_calculations["trial_total"] > 0 || $cost_calculations["total"] > 0))
						{
							...
							if(!($paypal_set_xco = array()) /* PayPal Express Checkout. */)
							{
								if($use_recurring_profile /* Use Payflow API instead? */)
								{
									...
									if(($paypal_set_xco = c_ws_plugin__optimizemember_paypal_utilities::paypal_payflow_api_response($paypal_set_xco)) && empty($paypal_set_xco["__error"]))
									{
						...
						{
							if(($old__subscr_id = get_user_option("optimizemember_subscr_id")))
								$paypal = c_ws_plugin__optimizemember_pro_paypal_utilities::payflow_get_profile($old__subscr_id);
							...
							if(!($paypal = array()) /* Recurring Profile. */)
							{
								...
								$paypal["AUTOBILLOUTSTANDINGAMT"] = apply_filters("ws_plugin__optimizemember_pro_paypal_auto_bill_op", "AddToNextBilling", get_defined_vars());

								$paypal["START"] = date("mdY", $start_time);

								$paypal["PAYPERIOD"] = c_ws_plugin__optimizemember_paypal_utilities::paypal_payflow_term($post_vars["attr"]["rt"], $post_vars["attr"]["rp"]);
								$paypal["TERM"] = ($post_vars["attr"]["rr"]) ? (($post_vars["attr"]["rrt"]) ? $post_vars["attr"]["rrt"] : "0") : "1";

								if(!empty($_GET["optimizemember_paypal_xco"]) && $_GET["optimizemember_paypal_xco"] === "optimizemember_pro_paypal_checkout_return" && !empty($_GET["token"])

								   && ($paypal_xco_details = array("TRXTYPE" => "A", "ACTION" => "G", "TENDER" => "P", "TOKEN" => $_GET["token"]))
								   && ($paypal_xco_details = c_ws_plugin__optimizemember_paypal_utilities::paypal_payflow_api_response($paypal_xco_details))
								   && empty($paypal_xco_details["__error"])

								   && ($paypal_xco_bagree = array("TRXTYPE" => "A", "ACTION" => "X", "TENDER" => "P", "TOKEN" => $paypal_xco_details["TOKEN"]))
								   && ($paypal_xco_bagree = c_ws_plugin__optimizemember_paypal_utilities::paypal_payflow_api_response($paypal_xco_bagree))
								...
								else if($_GET["optimizemember_paypal_xco"] !== "optimizemember_pro_paypal_checkout_return")
								{
									$paypal["TENDER"] = "C";
							...
							if(($cost_calculations["trial_total"] <= 0 && $cost_calculations["total"] <= 0) || (($paypal = c_ws_plugin__optimizemember_paypal_utilities::paypal_payflow_api_response($paypal)) && empty($paypal["__error"])))
							{
								...
								if($old__subscr_id && apply_filters("optimizemember_pro_cancels_old_rp_before_new_rp", TRUE, get_defined_vars()))
									c_ws_plugin__optimizemember_pro_paypal_utilities::payflow_cancel_profile($old__subscr_id, $old__baid);
						...
						else if($use_recurring_profile && !is_user_logged_in()) // Create a new account.
						{
							...
							if(!($paypal = array())) // Recurring Profile.
							{
								...
								else if(empty($_GET["optimizemember_paypal_xco"]) || $_GET["optimizemember_paypal_xco"] !== "optimizemember_pro_paypal_checkout_return")
								{
									$paypal["TENDER"] = "C";
									$paypal["ACCT"] = preg_replace("/[^0-9]/", "", $post_vars["card_number"]);
									if(preg_match("/^(?P<month>[0-9]{2})\/[0-9]{2}(?P<year_suffix>[0-9]{2})$/", $post_vars["card_expiration"], $_m))
							...
							if(($cost_calculations["trial_total"] <= 0 && $cost_calculations["total"] <= 0) || (($paypal = c_ws_plugin__optimizemember_paypal_utilities::paypal_payflow_api_response($paypal)) && empty($paypal["__error"])))
							{
								...
								if(!($create_user = array())) // Build post fields for registration configuration, and then the creation array.
								{
									...
									if(!empty($paypal_xco_bagree["BAID"])) // For registration configuration.
										$GLOBALS["ws_plugin__optimizemember_registration_vars"]["ws_plugin__optimizemember_custom_reg_field_optimizemember_subscr_baid"] = $paypal_xco_bagree["BAID"];
						...
						else if(!$use_recurring_profile && is_user_logged_in() && is_object($user = wp_get_current_user()) && ($user_id = $user->ID))
						{
							...
							if(!($paypal = array())) // Prepare a "Buy Now" transaction.
							{
								if(!empty($_GET["optimizemember_paypal_xco"]) && $_GET["optimizemember_paypal_xco"] === "optimizemember_pro_paypal_checkout_return" && !empty($_GET["token"])
								   ...
								   && ($paypal_xco_details = c_ws_plugin__optimizemember_paypal_utilities::paypal_api_response($paypal_xco_details))
								   && empty($paypal_xco_details["__error"]))
								{
							...
							if($cost_calculations["total"] <= 0 || (($paypal = c_ws_plugin__optimizemember_paypal_utilities::paypal_api_response($paypal)) && empty($paypal["__error"])))
							{
								...
								if(!$is_independent_ccaps_sale && $old__subscr_id && apply_filters("optimizemember_pro_cancels_old_rp_before_new_rp", TRUE, get_defined_vars()))
									c_ws_plugin__optimizemember_pro_paypal_utilities::payflow_cancel_profile($old__subscr_id, $old__baid);
