...
/**/
if(!class_exists("c_ws_plugin__optimizemember_pro_paypal_sp_checkout_in"))
	{
		/**
		* PayPal Specific Post/Page Forms ( inner processing routines ).
		*
		...
		* @since 1.5
		*/
		class c_ws_plugin__optimizemember_pro_paypal_sp_checkout_in
			{
				/**
				...
				*/
				public static function sp_checkout()
					{
						if(/**/(/**/!empty($_POST["optimizemember_pro_paypal_sp_checkout"]["nonce"]) && ($nonce = $_POST["optimizemember_pro_paypal_sp_checkout"]["nonce"]) && wp_verify_nonce($nonce, "s2member-pro-paypal-sp-checkout")/**/)/**/
						|| (/**/!empty($_GET["optimizemember_paypal_xco"]) && $_GET["optimizemember_paypal_xco"] === "optimizemember_pro_paypal_sp_checkout_return" /*  PayPal Express Checkout with $_GET["token"] & $_GET["PayerID"]. */
						...
						&& ($xco_post_vars = get_transient("s2m_".md5("optimizemember_transient_express_checkout_".$_GET["token"])))/**/)/**/)
							{
								$GLOBALS["ws_plugin__optimizemember_pro_paypal_sp_checkout_response"] = array(); /* This holds the global response details. */
								$global_response = &$GLOBALS["ws_plugin__optimizemember_pro_paypal_sp_checkout_response"]; /* This is a shorter reference. */
								/**/
																...
																if($_GET["optimizemember_paypal_xco"] === "optimizemember_pro_paypal_sp_checkout_return" && !empty($_GET["token"]) && ($paypal_xco_details = array("METHOD" => "GetExpressCheckoutDetails", "TOKEN" => $_GET["token"])) && ($paypal_xco_details = c_ws_plugin__optimizemember_paypal_utilities::paypal_api_response($paypal_xco_details)) && empty($paypal_xco_details["__error"]))
																	{
                                                                        ...
                                                                        $paypal["BUTTONSOURCE"] = "Creative6Media_SP";
																		/**/
																...
																if(!($ipn = array())) /* Simulated PayPal IPN. */
																	{
																		...
																		($referencing = c_ws_plugin__optimizemember_utils_users::get_user_subscr_or_wp_id()))
																			{
