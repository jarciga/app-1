...
if (!class_exists ("c_ws_plugin__optimizemember_pro_paypal_checkout_rdp_in"))
{
	/**
	 * PayPal Checkout Forms (inner processing routines).
	 *
	 * This version handles Real-Time Direct Pay *(mission critical)*, when:
	 * ``$GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_paypal_checkout_rdp"] == true``.
	 *
	 * @package optimizeMember\PayPal
	 * @since 1.5
	 */
	class c_ws_plugin__optimizemember_pro_paypal_checkout_rdp_in
	{
		/**
		...
		{
			if ((!empty($_POST["optimizemember_pro_paypal_checkout"]["nonce"]) && ($nonce = $_POST["optimizemember_pro_paypal_checkout"]["nonce"]) && wp_verify_nonce ($nonce, "optimizemember-pro-paypal-checkout"))
			    || (!empty($_GET["optimizemember_paypal_xco"]) && $_GET["optimizemember_paypal_xco"] === "optimizemember_pro_paypal_checkout_return" //  PayPal Express Checkout with $_GET["token"] & $_GET["PayerID"]?
			        ...
			        && ($xco_post_vars = get_transient ("s2m_" . md5 ("optimizemember_transient_express_checkout_" . $_GET["token"])))))
			{
				$GLOBALS["ws_plugin__optimizemember_pro_paypal_checkout_response"] = array(); // This holds the global response details.
				$global_response = &$GLOBALS["ws_plugin__optimizemember_pro_paypal_checkout_response"]; // This is a shorter reference.
				...
				$post_vars           = c_ws_plugin__optimizemember_utils_strings::trim_deep(stripslashes_deep($_POST["optimizemember_pro_paypal_checkout"]));
				...
				$post_vars["attr"]   = apply_filters("ws_plugin__optimizemember_pro_paypal_checkout_post_attr", $post_vars["attr"], get_defined_vars());
				...
				if(!empty($_GET["token"])) delete_transient("s2m_" . md5 ("optimizemember_transient_express_checkout_" . $_GET["token"]));

				if (!c_ws_plugin__optimizemember_pro_paypal_responses::paypal_form_attr_validation_errors ($post_vars["attr"])) // Attr errors?
				{
					if (!($error = c_ws_plugin__optimizemember_pro_paypal_responses::paypal_form_submission_validation_errors ("checkout", $post_vars)))
					{
						...
						$cp_2gbp_attr = c_ws_plugin__optimizemember_pro_paypal_utilities::paypal_maestro_solo_2gbp ( /* Now we use the new array of ``$cp_attr``. */$cp_attr, $post_vars["card_type"]);
						...
						if (empty($_GET["optimizemember_paypal_xco"]) && $post_vars["card_type"] === "PayPal" && ($cost_calculations["trial_total"] > 0 || $cost_calculations["total"] > 0))
						{
							...
							{
								set_transient ("s2m_" . md5 ("optimizemember_transient_express_checkout_" . $paypal_set_xco["TOKEN"]), $_POST, 10800);
						...
						else if ($use_recurring_profile && is_user_logged_in () && is_object ($user = wp_get_current_user ()) && ($user_id = $user->ID))
						{
							if (!($cur__subscr_id = get_user_option ("optimizemember_subscr_id")) || !($paypal = c_ws_plugin__optimizemember_paypal_utilities::paypal_api_response (array("METHOD" => "GetRecurringPaymentsProfileDetails", "PROFILEID" => $cur__subscr_id))) || !empty($paypal["__error"]) || !preg_match ("/^(Pending|PendingProfile)$/i", $paypal["STATUS"]))
							{
								...
								$start_time = ($post_vars["attr"]["tp"]) ? // If there's an Initial/Trial Period; start when it's over.
									c_ws_plugin__optimizemember_pro_paypal_utilities::paypal_start_time ($period1) : // After Trial is over.
									c_ws_plugin__optimizemember_pro_paypal_utilities::paypal_start_time ($period3); // Or next billing cycle.
								...
								{
									if (!empty($_GET["optimizemember_paypal_xco"]) && $_GET["optimizemember_paypal_xco"] === "optimizemember_pro_paypal_checkout_return" && !empty($_GET["token"]) && ($_paypal_xco_details = array("METHOD" => "GetExpressCheckoutDetails", "TOKEN" => $_GET["token"])) && ($_paypal_xco_details = c_ws_plugin__optimizemember_paypal_utilities::paypal_api_response ($_paypal_xco_details)) && empty($_paypal_xco_details["__error"]))
									{
								...
								if (!($paypal = array())) // Recurring Profile.
								{
									...
									if (!empty($_GET["optimizemember_paypal_xco"]) && $_GET["optimizemember_paypal_xco"] === "optimizemember_pro_paypal_checkout_return" && !empty($_GET["token"]) && ((!empty($_paypal_xco_details) && empty($_paypal_xco_details["__error"]) && ($paypal_xco_details = $_paypal_xco_details)) || (($paypal_xco_details = array("METHOD" => "GetExpressCheckoutDetails", "TOKEN" => $_GET["token"])) && ($paypal_xco_details = c_ws_plugin__optimizemember_paypal_utilities::paypal_api_response ($paypal_xco_details)) && empty($paypal_xco_details["__error"]))))
									{
								...
								if (($cost_calculations["trial_total"] <= 0 && $cost_calculations["total"] <= 0) || !$_paypal || (($_paypal = c_ws_plugin__optimizemember_paypal_utilities::paypal_api_response ($_paypal)) && empty($_paypal["__error"])))
								{
									if (($cost_calculations["trial_total"] <= 0 && $cost_calculations["total"] <= 0) || (($paypal = c_ws_plugin__optimizemember_paypal_utilities::paypal_api_response ($paypal)) && empty($paypal["__error"])))
									{
										...
										if (!($ipn = array())) // Simulated PayPal IPN.
										{
											...
											$ipn["optimizemember_paypal_proxy"] = "paypal";
											...
											$ipn["optimizemember_paypal_proxy_return_url"] = trim (c_ws_plugin__optimizemember_utils_urls::remote (home_url ("/?optimizemember_paypal_notify=1"), $ipn, array("timeout" => 20)));
										}
										if (!($paypal = array()) && ($paypal["PROFILEID"] = $old__subscr_id) && apply_filters("optimizemember_pro_cancels_old_rp_before_new_rp", TRUE, get_defined_vars()))
										{
										...
										c_ws_plugin__optimizemember_list_servers::process_list_servers_against_current_user((boolean)@$post_vars["custom_fields"]["opt_in"], TRUE, TRUE);
										...
										if ($post_vars["attr"]["success"] && substr ($ipn["optimizemember_paypal_proxy_return_url"], 0, 2) === substr ($post_vars["attr"]["success"], 0, 2) && ($custom_success_url = str_ireplace (array("%%s_response%%", /* Deprecated in v111106 ». */ "%%response%%"), array(urlencode (c_ws_plugin__optimizemember_utils_encryption::encrypt ($global_response["response"])), urlencode ($global_response["response"])), $ipn["optimizemember_paypal_proxy_return_url"])) && ($custom_success_url = trim (preg_replace ("/%%(.+?)%%/i", "", $custom_success_url))))
						...
						else if ($use_recurring_profile && !is_user_logged_in ()) // Create a new account.
						{
							    ...
							    && (!$post_vars["attr"]["tp"] || ($post_vars["attr"]["tp"] && $cost_calculations["trial_total"] > 0)))
							{
								if (!empty($_GET["optimizemember_paypal_xco"]) && $_GET["optimizemember_paypal_xco"] === "optimizemember_pro_paypal_checkout_return" && !empty($_GET["token"]) && ($_paypal_xco_details = array("METHOD" => "GetExpressCheckoutDetails", "TOKEN" => $_GET["token"])) && ($_paypal_xco_details = c_ws_plugin__optimizemember_paypal_utilities::paypal_api_response ($_paypal_xco_details)) && empty($_paypal_xco_details["__error"]))
								{
									...
									if ($post_vars["attr"]["tp"] && $cost_calculations["trial_total"] > 0)
									{
										error_log('Zvonko 1 u trial total > 0');
										$_paypal["PAYMENTREQUEST_0_CURRENCYCODE"] = $cost_calculations["cur"];
										$_paypal["PAYMENTREQUEST_0_ITEMAMT"] = $cost_calculations["trial_sub_total"];
										$_paypal["PAYMENTREQUEST_0_TAXAMT"] = $cost_calculations["trial_tax"];
										$_paypal["PAYMENTREQUEST_0_AMT"] = $cost_calculations["trial_total"];
							...
							if (($cost_calculations["trial_total"] <= 0 && $cost_calculations["total"] <= 0) || !$_paypal || (($_paypal = c_ws_plugin__optimizemember_paypal_utilities::paypal_api_response ($_paypal)) && empty($_paypal["__error"])))
							{
								if (($cost_calculations["trial_total"] <= 0 && $cost_calculations["total"] <= 0) || (($paypal = c_ws_plugin__optimizemember_paypal_utilities::paypal_api_response ($paypal)) && empty($paypal["__error"])))
								{
									...
									{
										$_POST["ws_plugin__optimizemember_custom_reg_field_user_pass1"] = @$post_vars["password1"]; // Fake this for registration configuration.
										$_POST["ws_plugin__optimizemember_custom_reg_field_first_name"] = $post_vars["first_name"]; // Fake this for registration configuration.
										$_POST["ws_plugin__optimizemember_custom_reg_field_last_name"] = $post_vars["last_name"]; // Fake this for registration configuration.
										$_POST["ws_plugin__optimizemember_custom_reg_field_opt_in"] = @$post_vars["custom_fields"]["opt_in"]; // Fake this too.
										...
										$_COOKIE["optimizemember_subscr_gateway"] = c_ws_plugin__optimizemember_utils_encryption::encrypt ("paypal"); // Fake this for registration configuration.
										$_COOKIE["optimizemember_subscr_id"] = c_ws_plugin__optimizemember_utils_encryption::encrypt ($new__subscr_id); // Fake this for registration configuration.
										$_COOKIE["optimizemember_custom"] = c_ws_plugin__optimizemember_utils_encryption::encrypt ($post_vars["attr"]["custom"]); // Fake this for registration configuration.
										$_COOKIE["optimizemember_item_number"] = c_ws_plugin__optimizemember_utils_encryption::encrypt ($post_vars["attr"]["level_ccaps_eotper"]); // Fake this too.

										$create_user["user_login"] = $post_vars["username"]; // Copy this into a separate array for `wp_create_user()`.
										$create_user["user_pass"] = c_ws_plugin__optimizemember_registrations::maybe_custom_pass($post_vars["password1"]);
						...
						else if (!$use_recurring_profile && is_user_logged_in () && is_object ($user = wp_get_current_user ()) && ($user_id = $user->ID))
						{
							if ($is_independent_ccaps_sale || !($cur__subscr_id = get_user_option ("optimizemember_subscr_id")) || !($paypal = c_ws_plugin__optimizemember_paypal_utilities::paypal_api_response (array("METHOD" => "GetRecurringPaymentsProfileDetails", "PROFILEID" => $cur__subscr_id))) || !empty($paypal["__error"]) || !preg_match ("/^(Pending|PendingProfile)$/i", $paypal["STATUS"]))
							{
								...
								{
									if (!empty($_GET["optimizemember_paypal_xco"]) && $_GET["optimizemember_paypal_xco"] === "optimizemember_pro_paypal_checkout_return" && !empty($_GET["token"]) && ($paypal_xco_details = array("METHOD" => "GetExpressCheckoutDetails", "TOKEN" => $_GET["token"])) && ($paypal_xco_details = c_ws_plugin__optimizemember_paypal_utilities::paypal_api_response ($paypal_xco_details)) && empty($paypal_xco_details["__error"]))
									{
								...
								if ($cost_calculations["total"] <= 0 || (($paypal = c_ws_plugin__optimizemember_paypal_utilities::paypal_api_response ($paypal)) && empty($paypal["__error"])))
								{
									$old__subscr_id = get_user_option ("optimizemember_subscr_id");
									$old__subscr_or_wp_id = c_ws_plugin__optimizemember_utils_users::get_user_subscr_or_wp_id ();
							...
							if ($cost_calculations["total"] <= 0 || (($paypal = c_ws_plugin__optimizemember_paypal_utilities::paypal_api_response ($paypal)) && empty($paypal["__error"])))
							{
								...
								else // Otherwise, they'll need to check their email for the auto-generated Password.
								{
									if (((is_multisite () && ($new__user_id = c_ws_plugin__optimizemember_registrations::ms_create_existing_user ($create_user["user_login"], $create_user["user_email"], $create_user["user_pass"]))) || ($new__user_id = wp_create_user ($create_user["user_login"], $create_user["user_pass"], $create_user["user_email"]))) && !is_wp_error ($new__user_id))
									{
										...
										$global_response = array("response" => _x ('<strong>Thank you.</strong> Your account has been approved.<br />&mdash; You\'ll receive an email momentarily.', "s2emember-front", "s2member"));
