...
/**/
if(!class_exists("c_ws_plugin__optimizemember_pro_paypal_update_pf_in"))
	{
		/**
		* PayPal Update Forms ( inner processing routines ).
		*
		...
		* @since 1.5
		*/
		class c_ws_plugin__optimizemember_pro_paypal_update_pf_in
			{
				/**
				...
				public static function paypal_update()
					{
						if(!empty($_POST["optimizemember_pro_paypal_update"]["nonce"]) && ($nonce = $_POST["optimizemember_pro_paypal_update"]["nonce"]) && wp_verify_nonce($nonce, "s2member-pro-paypal-update"))
							{
								$GLOBALS["ws_plugin__optimizemember_pro_paypal_update_response"] = array(); /* This holds the global response details. */
								$global_response = &$GLOBALS["ws_plugin__optimizemember_pro_paypal_update_response"]; /* This is a shorter reference. */
								/**/
