...
/**/
if(!class_exists("c_ws_plugin__optimizemember_pro_paypal_responses"))
	{
		/**
		* PayPal Pro Form responses.
		*
		...
		* @since 1.5
		*/
		class c_ws_plugin__optimizemember_pro_paypal_responses
			{
				/**
				...
				*/
				public static function paypal_cancellation_response($attr = FALSE)
					{
						$_response = $GLOBALS["ws_plugin__optimizemember_pro_paypal_cancellation_response"];
						/**/
						$_response = (!$_response) ? c_ws_plugin__optimizemember_pro_paypal_responses::paypal_form_attr_validation_errors($attr) : $_response;
						/**/
				...
				*/
				public static function paypal_update_response($attr = FALSE)
					{
						$_response = $GLOBALS["ws_plugin__optimizemember_pro_paypal_update_response"];
						/**/
						$_response = (!$_response) ? c_ws_plugin__optimizemember_pro_paypal_responses::paypal_form_attr_validation_errors($attr) : $_response;
						/**/
						if($_response && ($error = $_response["error"])) /* Error reporting. */
				...
				*/
				public static function paypal_registration_response($attr = FALSE)
					{
						$_response = $GLOBALS["ws_plugin__optimizemember_pro_paypal_registration_response"];
						/**/
						$_response = (!$_response) ? c_ws_plugin__optimizemember_pro_paypal_responses::paypal_form_attr_validation_errors($attr) : $_response;
						/**/
						if($_response && ($error = $_response["error"])) /* Error reporting. */
				...
				*/
				public static function paypal_sp_checkout_response($attr = FALSE)
					{
						$_response = $GLOBALS["ws_plugin__optimizemember_pro_paypal_sp_checkout_response"];
						/**/
						$_response = (!$_response) ? c_ws_plugin__optimizemember_pro_paypal_responses::paypal_form_attr_validation_errors($attr) : $_response;
						/**/
						if($_response && ($error = $_response["error"])) /* Error reporting. */
				...
				*/
				public static function paypal_checkout_response($attr = FALSE)
					{
						$_response = $GLOBALS["ws_plugin__optimizemember_pro_paypal_checkout_response"];
						/**/
						$_response = (!$_response) ? c_ws_plugin__optimizemember_pro_paypal_responses::paypal_form_attr_validation_errors($attr) : $_response;
						/**/
						if($_response && ($error = $_response["error"])) /* Error reporting. */
				...
				*/
				public static function paypal_form_api_validation_errors($attr = FALSE)
					{
						if(!$GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["paypal_business"])
						...
						/**/
						else if(!$GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["paypal_api_username"])
						...
						/**/
						else if(!$GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["paypal_api_password"])
						...
						/**/
						else if(!$GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["paypal_api_signature"])
						...
						if(!($response = c_ws_plugin__optimizemember_pro_paypal_responses::paypal_form_api_validation_errors($attr)) || !empty($attr["register"]))
							{
								if($attr["cancel"]) /* Special form for Cancellations. User/Member must be logged in. */
									{
										...
										/**/
										else if($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["paypal_payflow_api_username"])
											{
								...
								/**/
								else if($attr["sp"]) /* Validation routines for Specific Post/Page checkout forms. */
									{
										...
										/**/
										else if(strlen($attr["desc"]) > 100) /* Actually, this can be 127 chars; but we need plenty of room for optimizeMember's coupon info. */
								...
								/**/
								else /* Validation routines for Member Level checkout forms. This is the default functionality. */
									{
										...
										/**/
										else if($attr["rr"] !== "BN" && $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["paypal_payflow_api_username"] && !in_array($attr["rp"]."-".$attr["rt"], array("1-W", "2-W", "1-M", "3-M", "6-M", "1-Y"), TRUE))
