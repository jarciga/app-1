 ...
 * 	See: {@link http://optimizemember.com/prices/}
 *
...
if(!class_exists("c_ws_plugin__optimizemember_pro_paypal_checkout_in"))
{
	/**
	 * PayPal Checkout Forms (inner processing routines).
	 *
	 ...
	 * @since 1.5
	 */
	class c_ws_plugin__optimizemember_pro_paypal_checkout_in
	{
		/**
		...
		public static function paypal_checkout()
		{
			...
			{
				$GLOBALS["ws_plugin__optimizemember_pro_paypal_checkout_response"] =  /* This holds the global response details. */array();
				$global_response = /* This is a shorter reference. */&$GLOBALS["ws_plugin__optimizemember_pro_paypal_checkout_response"];
					...
					if(!($error = c_ws_plugin__optimizemember_pro_paypal_responses::paypal_form_submission_validation_errors("checkout", $post_vars)))
					{
						...
						if(empty($_GET["optimizemember_paypal_xco"]) && $post_vars["card_type"] === "PayPal" && ($cost_calculations["trial_total"] > 0 || $cost_calculations["total"] > 0))
						{
							...
							if /* PayPal Express Checkout. */(!($paypal_set_xco = array()))
							{
								...
								$paypal_set_xco = apply_filters('ws_plugin__optimizemember_pro_paypal_set_xco_vars', $paypal_set_xco, get_defined_vars());
							}
						...
						else if($use_recurring_profile && is_user_logged_in() && is_object($user = wp_get_current_user()) && ($user_id = $user->ID))
						{
							if(!($cur__subscr_id = get_user_option("optimizemember_subscr_id")) || !($paypal = c_ws_plugin__optimizemember_paypal_utilities::paypal_api_response(array("METHOD" => "GetRecurringPaymentsProfileDetails", "PROFILEID" => $cur__subscr_id))) || !empty($paypal["__error"]) || !preg_match("/^(Pending|PendingProfile)$/i", $paypal["STATUS"]))
							{
								...
								$start_time =  /* If there's an Initial/Trial Period; start when it's over. */($post_vars["attr"]["tp"]) ?
									/* After Trial is over. */c_ws_plugin__optimizemember_pro_paypal_utilities::paypal_start_time($period1) :
									/* Or next billing cycle. */c_ws_plugin__optimizemember_pro_paypal_utilities::paypal_start_time($period3);
						...
						else if /* Create a new account. */($use_recurring_profile && !is_user_logged_in())
						{
							...
							if(($cost_calculations["trial_total"] <= 0 && $cost_calculations["total"] <= 0) || (($paypal = c_ws_plugin__optimizemember_paypal_utilities::paypal_api_response($paypal)) && empty($paypal["__error"])))
							{
								...
								{
									$_POST["ws_plugin__optimizemember_custom_reg_field_user_pass1"] = /* Fake this for registration configuration. */ @$post_vars["password1"];
									$_POST["ws_plugin__optimizemember_custom_reg_field_first_name"] = /* Fake this for registration configuration. */ $post_vars["first_name"];
									$_POST["ws_plugin__optimizemember_custom_reg_field_last_name"] = /* Fake this for registration configuration. */ $post_vars["last_name"];
									$_POST["ws_plugin__optimizemember_custom_reg_field_opt_in"] = /* Fake this too. */ @$post_vars["custom_fields"]["opt_in"];
									...
									$_COOKIE["optimizemember_subscr_gateway"] =  /* Fake this for registration configuration. */c_ws_plugin__optimizemember_utils_encryption::encrypt("paypal");
									$_COOKIE["optimizemember_subscr_id"] = /* Fake this for registration configuration. */ c_ws_plugin__optimizemember_utils_encryption::encrypt($new__subscr_id);
									$_COOKIE["optimizemember_custom"] =  /* Fake this for registration configuration. */c_ws_plugin__optimizemember_utils_encryption::encrypt($post_vars["attr"]["custom"]);
									$_COOKIE["optimizemember_item_number"] = /* Fake this too. */ c_ws_plugin__optimizemember_utils_encryption::encrypt($post_vars["attr"]["level_ccaps_eotper"]);
