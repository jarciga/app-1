<?php
/**
* Shortcode `[optimizeMember-Pro-PayPal-Form /]` ( inner processing routines ).
*
* Copyright: © 2009-2011
...
/**/
if(!class_exists("c_ws_plugin__optimizemember_pro_paypal_form_in"))
	{
		/**
		* Shortcode `[optimizeMember-Pro-PayPal-Form /]` ( inner processing routines ).
		*
		...
		* @since 1.5
		*/
		class c_ws_plugin__optimizemember_pro_paypal_form_in
			{
				/**
						...
						if /* Cancellations. */($attr["cancel"])
							{
								...
								*/
								$hidden_inputs = '<input type="hidden" name="optimizemember_pro_paypal_cancellation[nonce]" id="s2member-pro-paypal-cancellation-nonce" value="'.esc_attr(wp_create_nonce("optimizemember-pro-paypal-cancellation")).'" />';
						...
						else if /* Free registrations. */($attr["register"])
							{
								...
								*/
								...
								Build the hidden input variables.
								*/
								$hidden_inputs = '<input type="hidden" name="optimizemember_pro_paypal_registration[nonce]" id="s2member-pro-paypal-registration-nonce" value="'.esc_attr(wp_create_nonce("optimizemember-pro-paypal-registration")).'" />';
						...
						else if /* Billing information updates. */($attr["update"])
							{
								...
								*/
								...
								Build the hidden input variables.
								*/
								$hidden_inputs = '<input type="hidden" name="optimizemember_pro_paypal_update[nonce]" id="s2member-pro-paypal-update-nonce" value="'.esc_attr(wp_create_nonce("optimizemember-pro-paypal-update")).'" />';
						...
						else if /* Specific Post/Page Access. */($attr["sp"])
							{
								...
								*/
								...
								Build the hidden input variables.
								*/
								$hidden_inputs = '<input type="hidden" name="optimizemember_pro_paypal_sp_checkout[nonce]" id="s2member-pro-paypal-sp-checkout-nonce" value="'.esc_attr(wp_create_nonce("optimizemember-pro-paypal-sp-checkout")).'" />';
						...
						else /* Signups and Modifications. */
							{
								...
								*/
								...
								Build the hidden input variables.
								*/
								$hidden_inputs = '<input type="hidden" name="optimizemember_pro_paypal_checkout[nonce]" id="s2member-pro-paypal-checkout-nonce" value="'.esc_attr(wp_create_nonce("optimizemember-pro-paypal-checkout")).'" />';
