<?php
/**
* Shortcode `[optimizeMember-Pro-ClickBank-Button /]` ( inner processing routines ).
*
* Copyright: © 2009-2011
...
if(!class_exists("c_ws_plugin__optimizemember_pro_clickbank_button_in"))
	{
		/**
		* Shortcode `[optimizeMember-Pro-ClickBank-Button /]` ( inner processing routines ).
		*
		...
		* @since 1.5
		*/
		class c_ws_plugin__optimizemember_pro_clickbank_button_in
			{
				/**
				...
				public static function sc_clickbank_button($attr = FALSE, $content = FALSE, $shortcode = FALSE)
					{
						...
						$attr = shortcode_atts(array("cbp" => "0", "cbskin" => "", "cbfid" => "", "cbur" => "", "cbf" => "auto", "click_fid_recuring" => "", "ids" => "0", "exp" => "72", "level" => "1", "ccaps" => "", "desc" => "", "custom" => $_SERVER["HTTP_HOST"], "tp" => "0", "tt" => "D", "rp" => "1", "rt" => "M", "rr" => "1", "modify" => "0", "cancel" => "0", "sp" => "0", "image" => "default", "output" => "anchor"), $attr);
						...
						if /* Modifications/Cancellations. */($attr["modify"] || $attr["cancel"])
							{
								...
								$code = preg_replace("/%%images%%/", c_ws_plugin__optimizemember_utils_strings::esc_refs(esc_attr($GLOBALS["WS_PLUGIN__"]["optimizemember_pro"]["c"]["dir_url"]."/images")), $code);
						...
						else if /* Specific Post/Page Buttons. */($attr["sp"])
							{
								...
								$code = preg_replace("/%%item%%/", c_ws_plugin__optimizemember_utils_strings::esc_refs(esc_attr($attr["cbp"])), $code);
								$code = preg_replace("/%%vendor%%/", c_ws_plugin__optimizemember_utils_strings::esc_refs(esc_attr($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_clickbank_username"])), $code);
								$code = preg_replace("/%%invoice%%/", c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($attr["sp_ids_exp"])), $code);
								$code = preg_replace("/%%desc%%/", c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($attr["desc"])), $code);
								$code = preg_replace("/%%custom%%/", c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($attr["custom"])), $code);

								$code = preg_replace("/%%cbskin%%/", c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($attr["cbskin"])), $code);
								$code = preg_replace("/%%cbfid%%/", c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($attr["cbfid"])), $code);
								$code = preg_replace("/%%cbur%%/", c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($attr["cbur"])), $code);
								$code = preg_replace("/%%cbf%%/", c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($attr["cbf"])), $code);
								$code = str_replace(array("&amp;cbskin=&amp;", "&amp;cbfid=&amp;", "&amp;cbur=&amp;", "&amp;cbf=&amp;"), "&amp;", $code);

								$code = preg_replace("/\<\?php echo OPTIMIZEMEMBER_CURRENT_USER_IP; \?\>/", c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($_SERVER["REMOTE_ADDR"])), $code);

								$code = preg_replace("/%%referencing%%/", (($referencing = c_ws_plugin__optimizemember_utils_users::get_user_subscr_or_wp_id()) ? c_ws_plugin__optimizemember_utils_strings::esc_refs("&amp;s2_referencing=".urlencode($referencing)) : ""), $code);

								if(preg_match('/ href\="(.*?)"/', $code, $m) && ($url = c_ws_plugin__optimizemember_utils_urls::n_amps($m[1])))
									$code = preg_replace('/ href\=".*?"/', ' href="'.c_ws_plugin__optimizemember_utils_strings::esc_refs(esc_attr(c_ws_plugin__optimizemember_utils_urls::add_optimizemember_sig($url))).'"', $code);
						...
						else if /* Independent Custom Capabilities. */($attr["level"] === "*")
							{
								...
								$code = preg_replace("/%%item%%/", c_ws_plugin__optimizemember_utils_strings::esc_refs(esc_attr($attr["cbp"])), $code);
								$code = preg_replace("/%%vendor%%/", c_ws_plugin__optimizemember_utils_strings::esc_refs(esc_attr($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_clickbank_username"])), $code);
								$code = preg_replace("/%%invoice%%/", c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($attr["level_ccaps_eotper"])), $code);
								$code = preg_replace("/%%desc%%/", c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($attr["desc"])), $code);
								$code = preg_replace("/%%custom%%/", c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($attr["custom"])), $code);
								...
								$code = (!$attr["rr"]) ? preg_replace("/&amp;s2_subscr_id\=s2-\<\?php echo uniqid\(\); \?\>/", "", $code) : preg_replace("/\<\?php echo uniqid\(\); \?\>/", c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode(uniqid())), $code);
						...
						else // Otherwise, we'll process this Button normally, using Membership routines.
							{
								...
								$code = preg_replace("/%%invoice%%/", c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($attr["level_ccaps_eotper"])), $code);
								$code = preg_replace("/%%desc%%/", c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($attr["desc"])), $code);
								$code = preg_replace("/%%p1%%/", (($attr["rr"]) ? c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($attr["tp"]." ".$attr["tt"])) : ""), $code);
								$code = preg_replace("/%%p3%%/", (($attr["rr"]) ? c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($attr["rp"]." ".$attr["rt"])) : ""), $code);
								$code = preg_replace("/%%custom%%/", c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($attr["custom"])), $code);
							...
							/* Recurring Feature */
							if ($attr['click_fid_recuring'] === 'Y') {
								$code = str_replace ('href', 'data-recur="click-recur" href', $code);
								$code = $code . '<p style="text-align:center;"><label for="recurring"><input type="checkbox" id="recurring" class="recurring" value="false">';
								$code = $code . 'I agree to the payment terms of this recurring product.</label></p>';
							}
