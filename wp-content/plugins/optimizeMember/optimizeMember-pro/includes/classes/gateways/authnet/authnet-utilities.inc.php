...
/**/
if(!class_exists("c_ws_plugin__optimizemember_pro_authnet_utilities"))
	{
		/**
		* Authorize.Net utilities.
		*
		...
		* @since 1.5
		*/
		class c_ws_plugin__optimizemember_pro_authnet_utilities
			{
				/**
				...
				public static function authnet_arb_response($post_vars = FALSE)
					{
						global $current_site, $current_blog; /* For Multisite support. */
						/**/
						$url = "https://".(($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_authnet_sandbox"]) ? "apitest.authorize.net" : "api.authorize.net")."/xml/v1/request.api";
						/**/
						$post_vars = (is_array($post_vars)) ? $post_vars : array(); /* Must be in array format. */
						/**/
						$post_vars["x_login"] = $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_authnet_api_login_id"];
						...
						$input_time = date("D M j, Y g:i:s a T"); /* Record input time for logging. */
						/**/
						$xml = trim(c_ws_plugin__optimizemember_utils_urls::remote($url, $xml, array_merge($req, array("timeout" => 20))));
						/**/
						$output_time = date("D M j, Y g:i:s a T"); /* Now record after output time. */
						/**/
						$response = c_ws_plugin__optimizemember_pro_authnet_utilities::_authnet_parse_arb_response($xml);
						/**/
						if(empty($response["response_code"]) || $response["response_code"] !== "I00001") /* A value of I00001 indicates success. */
							{
								if(strlen($response["response_reason_code"]) || $response["response_reason_text"])
									/* translators: Exclude `%2$s`. This is an English error returned by Authorize.Net. Please replace `%2$s` with: `Unable to process, please try again`, or something to that affect. Or, if you prefer, you could Filter ``$response["__error"]`` with `ws_plugin__optimizemember_pro_authnet_arb_response`. */
									$response["__error"] = sprintf(_x('Error #%1$s. %2$s.', "s2member-front", "s2member"), $response["response_reason_code"], rtrim($response["response_reason_text"], "."));
								/**/
								else /* Else, generate an error messsage - so something is reported back to the Customer. */
									$response["__error"] = _x("Error. Please contact Support for assistance.", "s2member-front", "s2member");
								...
								if(is_writable($logs_dir) && c_ws_plugin__optimizemember_utils_logs::archive_oversize_log_files())
									if(($log = "-------- Input vars: ( ".$input_time." ) --------\n".var_export($post_vars, true)."\n"))
										if(($log .= "-------- Output string/vars: ( ".$output_time." ) --------\n".$xml."\n".var_export($response, true)))
											file_put_contents($logs_dir."/".$log2, $logv."\n".$logm."\n".$log4."\n".$log."\n\n", FILE_APPEND);
						/**/
						return apply_filters("ws_plugin__optimizemember_pro_authnet_arb_response", c_ws_plugin__optimizemember_pro_authnet_utilities::_authnet_arb_response_filters($response), get_defined_vars());
					}
				/**
				...
				* @return array An array of response variables, parsed by optimizeMember.
				*/
				public static function _authnet_parse_arb_response($xml = FALSE)
					{
						...
						/**/
						return c_ws_plugin__optimizemember_utils_strings::trim_deep(stripslashes_deep($response));
					}
				/**
				...
				*/
				public static function authnet_postvars()
					{
						if(!empty($_REQUEST["optimizemember_pro_authnet_notify"]) && !empty($_REQUEST["x_MD5_Hash"]))
							{
								...
								foreach($postvars as $var => $value)
									if(preg_match("/^optimizemember_/", $var))
										unset($postvars[$var]);
								/**/
								$aim_digest_vars = $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_authnet_api_login_id"].$postvars["x_trans_id"].$postvars["x_amount"];
								$arb_digest_vars = $postvars["x_trans_id"].$postvars["x_amount"];
								/**/
								if(strtolower($postvars["x_MD5_Hash"]) === md5($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_authnet_api_salt_key"].$aim_digest_vars))
									return $postvars;
								/**/
								else if(strtolower($postvars["x_MD5_Hash"]) === md5($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_authnet_api_salt_key"].$arb_digest_vars))
									return $postvars;
								/**/
								else /* Nope. */
									return false;
						...
						if(is_array($array) && !empty($array["x_description"]) && preg_match("/\(\((.+?)\)\)/i", $array["x_description"], $m))
							{
								...
								/**/
								$array["s2_custom"] = (!empty($array["x_subscription_id"])) ? c_ws_plugin__optimizemember_utils_users::get_user_custom_with($array["x_subscription_id"]) : "";
								$array["s2_custom"] = ($array["s2_custom"]) ? $array["s2_custom"] : $array["s2_domain"];
								/**/
								return c_ws_plugin__optimizemember_utils_strings::trim_deep($array);
							}
						else /* False. */
							return false;
					}
				...
				public static function authnet_tax_may_apply()
					{
						if((float)$GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_default_tax"] > 0)
							return true;
						/**/
						else if($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_tax_rates"])
							return true;
						/**/
						return false;
					}
				...
				*/
				public static function authnet_ajax_tax()
					{
						if(!empty($_POST["ws_plugin__optimizemember_pro_authnet_ajax_tax"]) && ($nonce = $_POST["ws_plugin__optimizemember_pro_authnet_ajax_tax"]) && (wp_verify_nonce($nonce, "ws-plugin--optimizemember-pro-authnet-ajax-tax") || c_ws_plugin__optimizemember_utils_encryption::decrypt($nonce) === "ws-plugin--optimizemember-pro-authnet-ajax-tax"))
							/* A wp_verify_nonce() won't always work here, because s2member-pro-min.js must be cacheable. The output from wp_create_nonce() would go stale.
									So instead, s2member-pro-min.js should use ``c_ws_plugin__optimizemember_utils_encryption::encrypt()`` as an alternate form of nonce. */
							{
								...
								/**/
								if(!empty($_POST["ws_plugin__optimizemember_pro_authnet_ajax_tax_vars"]) && is_array($_p_tax_vars = c_ws_plugin__optimizemember_utils_strings::trim_deep(stripslashes_deep($_POST["ws_plugin__optimizemember_pro_authnet_ajax_tax_vars"]))))
									{
										if(is_array($attr = (!empty($_p_tax_vars["attr"])) ? unserialize(c_ws_plugin__optimizemember_utils_encryption::decrypt($_p_tax_vars["attr"])) : false))
											{
												$attr = (!empty($attr["coupon"])) ? c_ws_plugin__optimizemember_pro_authnet_utilities::authnet_apply_coupon($attr, $attr["coupon"]) : $attr;
												/**/
												...
												$a = c_ws_plugin__optimizemember_pro_authnet_utilities::authnet_cost(null, $sub_total_today, $state, $country, $zip, $currency, $desc);
														...
														if($rate = trim($rate)) /* Do NOT process empty lines. */
															{
																...
																/**/
																else if($state && preg_match("/^([A-Z]{2})\/(".preg_quote($country, "/").")$/", $location, $m) && strtoupper(c_ws_plugin__optimizemember_pro_utilities::full_state($m[1], $m[2]))."/".$m[2] === $state."/".$country)
																	$configured_rates[2] = $rate;
																/**/
							...
							if($attr["accept_coupons"] && $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_coupon_codes"])
								{
									...
									$tx = (c_ws_plugin__optimizemember_pro_authnet_utilities::authnet_tax_may_apply()) ? _x(" + tax", "s2member-front", "s2member") : "";
									$ps = _x("%", "s2member-front percentage-symbol", "s2member");
									/**/
													...
													foreach(preg_split("/[\r\n\t]+/", $_urls) as $_url /* Notify each of the URLs. */)
														...
														if(($_url = preg_replace("/%%full_coupon_code%%/i", c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($full_coupon_code)), $_url)))
															if(($_url = preg_replace("/%%coupon_code%%/i", c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($coupon_code)), $_url)))
																if(($_url = preg_replace("/%%affiliate_id%%/i", c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($affiliate_id)), $_url)))
																	if(($_url = preg_replace("/%%user_ip%%/i", c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($_SERVER["REMOTE_ADDR"])), $_url)))
																		{
																			if(($_url = trim(preg_replace("/%%(.+?)%%/i", "", $_url))) /* Cleanup any remaining Replacement Codes. */)
																				...
																				if(!($_r = 0) && ($_url = preg_replace("/^silent-php\|/i", "", $_url, 1, $_r)) && $_r && in_array /* Processing? */("affiliates-silent-post", $process))
																					c_ws_plugin__optimizemember_utils_urls::remote /* Post silently via PHP. Relies on IP tracking. */($_url, false, array("blocking" => false));
																				/**/
