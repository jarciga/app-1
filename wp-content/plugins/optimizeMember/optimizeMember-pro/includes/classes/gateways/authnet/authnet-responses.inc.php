...
/**/
if(!class_exists("c_ws_plugin__optimizemember_pro_authnet_responses"))
	{
		/**
		* Authorize.Net Pro Form responses.
		*
		...
		* @since 1.5
		*/
		class c_ws_plugin__optimizemember_pro_authnet_responses
			{
				/**
				...
				*/
				public static function authnet_cancellation_response($attr = FALSE)
					{
						$_response = $GLOBALS["ws_plugin__optimizemember_pro_authnet_cancellation_response"];
						/**/
						$_response = (!$_response) ? c_ws_plugin__optimizemember_pro_authnet_responses::authnet_form_attr_validation_errors($attr) : $_response;
						/**/
				...
				*/
				public static function authnet_update_response($attr = FALSE)
					{
						$_response = $GLOBALS["ws_plugin__optimizemember_pro_authnet_update_response"];
						/**/
						$_response = (!$_response) ? c_ws_plugin__optimizemember_pro_authnet_responses::authnet_form_attr_validation_errors($attr) : $_response;
						/**/
						if($_response && ($error = $_response["error"])) /* Error reporting. */
				...
				*/
				public static function authnet_registration_response($attr = FALSE)
					{
						$_response = $GLOBALS["ws_plugin__optimizemember_pro_authnet_registration_response"];
						/**/
						$_response = (!$_response) ? c_ws_plugin__optimizemember_pro_authnet_responses::authnet_form_attr_validation_errors($attr) : $_response;
						/**/
						if($_response && ($error = $_response["error"])) /* Error reporting. */
				...
				*/
				public static function authnet_sp_checkout_response($attr = FALSE)
					{
						$_response = $GLOBALS["ws_plugin__optimizemember_pro_authnet_sp_checkout_response"];
						/**/
						$_response = (!$_response) ? c_ws_plugin__optimizemember_pro_authnet_responses::authnet_form_attr_validation_errors($attr) : $_response;
						/**/
						if($_response && ($error = $_response["error"])) /* Error reporting. */
				...
				*/
				public static function authnet_checkout_response($attr = FALSE)
					{
						$_response = $GLOBALS["ws_plugin__optimizemember_pro_authnet_checkout_response"];
						/**/
						$_response = (!$_response) ? c_ws_plugin__optimizemember_pro_authnet_responses::authnet_form_attr_validation_errors($attr) : $_response;
						/**/
						if($_response && ($error = $_response["error"])) /* Error reporting. */
				...
				*/
				public static function authnet_form_api_validation_errors($attr = FALSE)
					{
						if(!$GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_authnet_api_login_id"])
						...
						/**/
						else if(!$GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_authnet_api_trans_key"])
						...
						/**/
						else if(!$GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_authnet_api_salt_key"])
				...
				*/
				public static function authnet_form_attr_validation_errors($attr = FALSE)
					{
						if(!($response = c_ws_plugin__optimizemember_pro_authnet_responses::authnet_form_api_validation_errors($attr)) || !empty($attr["register"]))
							{
								if($attr["cancel"]) /* Special form for Cancellations. User/Member must be logged in. */
									{
										...
										/**/
										else if(!is_object($user = wp_get_current_user()) || !($user_id = $user->ID) || !($subscr_id = get_user_option("optimizemember_subscr_id", $user_id)))
										...
										/**/
											...
											{
												if(!($authnet = c_ws_plugin__optimizemember_pro_authnet_utilities::authnet_arb_response($authnet)) || !empty($authnet["__error"]))
								...
								/**/
								else if($attr["sp"]) /* Validation routines for Specific Post/Page checkout forms. */
									{
										...
										/**/
										else if(strlen($attr["desc"]) > 100) /* Actually, this can be 255 chars; but we need plenty of room for optimizeMember's reference/coupon info. */
				...
				*/
				public static function authnet_form_submission_validation_errors($form = FALSE, $s = FALSE)
					{
						if($form === "registration" || !($response = c_ws_plugin__optimizemember_pro_authnet_responses::authnet_form_api_validation_errors()))
							{
								if($form === "cancellation") /* Special form for Cancellations. User/Member must be logged in. */
									{
										if(!is_user_logged_in())
											$response = array("response" => sprintf(_x('You must <a href="%s" rel="nofollow">log in</a> to cancel your account.', "s2member-front", "s2member"), esc_attr(wp_login_url($_SERVER["REQUEST_URI"]))), "error" => true);
										/* ----------------------------------------------------------------------------------------------------------------- */
										else if($s["attr"]["captcha"] && (!$s["recaptcha_challenge_field"] || !$s["recaptcha_response_field"] || !c_ws_plugin__optimizemember_utils_captchas::recaptcha_code_validates($s["recaptcha_challenge_field"], $s["recaptcha_response_field"])))
								...
								/**/
								else if($form === "registration") /* Validation routines for free Registration forms. */
									{
										...
										/**/
										else if($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["custom_reg_names"] && (!$s["last_name"] || !is_string($s["last_name"])))
										...
										/**/
										else if((!$s["password1"] || !is_string($s["password1"])) && $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["custom_reg_password"])
										...
										/**/
										else if($s["password2"] !== $s["password1"] && $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["custom_reg_password"])
								...
								/**/
								else if($form === "checkout") /* Validation routines for Member Level checkout forms. This is the default functionality. */
									{
										...
										/**/
											...
											$response = array("response" => $_errors->get_error_message(), "error" => true);
										/**/
										else if(!is_user_logged_in() && (!$s["password1"] || !is_string($s["password1"])) && $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["custom_reg_password"])
											$response = array("response" => _x('Missing Password. Please try again.', "s2member-front", "s2member"), "error" => true);
										/**/
										...
										/**/
											...
											$response = array("response" => _x('Invalid Password. Max length is 20 characters. Please try again.', "s2member-front", "s2member"), "error" => true);
										/**/
										else if(!is_user_logged_in() && $s["password2"] !== $s["password1"] && $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["custom_reg_password"])
											$response = array("response" => _x('Password fields do NOT match. Please try again.', "s2member-front", "s2member"), "error" => true);
										/* ----------------------------------------------------------------------------------------------------------------- */
										else if(!$s["card_type"] || !is_string($s["card_type"]))
											$response = array("response" => _x('Missing Card Type ( Billing Method ). Please try again.', "s2member-front", "s2member"), "error" => true);
