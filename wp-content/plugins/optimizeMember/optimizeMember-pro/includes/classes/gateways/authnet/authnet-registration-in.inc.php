<?php
/**
* Authorize.Net Registration Form ( inner processing routines ).
*
* Copyright: © 2009-2011
...
/**/
if (!class_exists ("c_ws_plugin__optimizemember_pro_authnet_registration_in"))
	{
		/**
		* Authorize.Net Registration Form ( inner processing routines ).
		*
		...
		* @since 1.5
		*/
		class c_ws_plugin__optimizemember_pro_authnet_registration_in
			{
				/**
				...
				*/
				public static function authnet_registration ()
					{
						if (!empty ($_POST["optimizemember_pro_authnet_registration"]["nonce"]) && ($nonce = $_POST["optimizemember_pro_authnet_registration"]["nonce"]) && wp_verify_nonce ($nonce, "s2member-pro-authnet-registration"))
							{
								$GLOBALS["ws_plugin__optimizemember_pro_authnet_registration_response"] = array (); /* This holds the global response details. */
								$global_response = &$GLOBALS["ws_plugin__optimizemember_pro_authnet_registration_response"]; /* This is a shorter reference. */
								/**/
								$post_vars = c_ws_plugin__optimizemember_utils_strings::trim_deep (stripslashes_deep ($_POST["optimizemember_pro_authnet_registration"]));
								$post_vars["attr"] = unserialize (c_ws_plugin__optimizemember_utils_encryption::decrypt ($post_vars["attr"])); /* And run a Filter. */
								$post_vars["attr"] = apply_filters ("ws_plugin__optimizemember_pro_authnet_registration_post_attr", $post_vars["attr"], get_defined_vars ());
								/**/
								...
								/**/
								if (!c_ws_plugin__optimizemember_pro_authnet_responses::authnet_form_attr_validation_errors ($post_vars["attr"])) /* Must NOT have any attr errors. */
									{
										if (!($error = c_ws_plugin__optimizemember_pro_authnet_responses::authnet_form_submission_validation_errors ("registration", $post_vars)))
											{
												if (!($create_user = array ())) /* Build post fields for registration configuration, and then the creation array. */
													{
														...
														$_EOT_ = &$GLOBALS["ws_plugin__optimizemember_registration_vars"]["ws_plugin__optimizemember_custom_reg_field_optimizemember_auto_eot_time"]; /* Quick/shorter reference to this var. */
														...
														/**/
														$GLOBALS["ws_plugin__optimizemember_registration_return_url"] = $post_vars["attr"]["success"]; /* Custom success return. */
														/**/
														...
														if (((is_multisite () && ($new__user_id = c_ws_plugin__optimizemember_registrations::ms_create_existing_user ($create_user["user_login"], $create_user["user_email"], $create_user["user_pass"]))) || ($new__user_id = wp_create_user ($create_user["user_login"], $create_user["user_pass"], $create_user["user_email"]))) && !is_wp_error ($new__user_id))
															{
																...
																/**/
																if ($post_vars["attr"]["success"] && substr ($GLOBALS["ws_plugin__optimizemember_registration_return_url"], 0, 2) === substr ($post_vars["attr"]["success"], 0, 2) && ($custom_success_url = str_ireplace (array ("%%s_response%%", /* Deprecated in v111106 ». */ "%%response%%"), array (urlencode (c_ws_plugin__optimizemember_utils_encryption::encrypt ($global_response["response"])), urlencode ($global_response["response"])), $GLOBALS["ws_plugin__optimizemember_registration_return_url"])) && ($custom_success_url = trim (preg_replace ("/%%(.+?)%%/i", "", $custom_success_url))))
