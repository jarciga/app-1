...
if(!class_exists('c_ws_plugin__optimizemember_pro_stripe_checkout_in'))
{
    /**
     * Stripe Checkout Form handler (inner processing routines).
     *
     ...
     * @since 140617
     */
    class c_ws_plugin__optimizemember_pro_stripe_checkout_in
    {
        /**
        ...
        {
            if(!empty($_POST['optimizemember_pro_stripe_checkout']['nonce'])
               && ($nonce = $_POST['optimizemember_pro_stripe_checkout']['nonce'])
               && wp_verify_nonce($nonce, 'optimizemember-pro-stripe-checkout')
            )
            {
                $GLOBALS['ws_plugin__optimizemember_pro_stripe_checkout_response'] = array(); // This holds the global response details.
                ...
                if(!c_ws_plugin__optimizemember_pro_stripe_responses::stripe_form_attr_validation_errors($post_vars['attr']))
                {
                    if(!($form_submission_validation_errors // Validate checkout input form fields.
                        = c_ws_plugin__optimizemember_pro_stripe_responses::stripe_form_submission_validation_errors('checkout', $post_vars))
                    ) // If this fails the global response is set to the error(s) returned during form field validation.
                    {
                        unset($_POST['optimizemember_pro_stripe_checkout']['source_token']); // Good one-time only.
                        unset($_POST['optimizemember_pro_stripe_checkout']['source_token_summary']); // Good one-time only.
                        ...
                        $cost_calculations = c_ws_plugin__optimizemember_pro_stripe_utilities::cost($cp_attr['ta'], $cp_attr['ra'], $post_vars['state'], $post_vars['country'], $post_vars['zip'], $cp_attr['cc'], $cp_attr['desc'], $is_bitcoin);
                        ...
                        if($use_subscription && is_user_logged_in() && is_object($user = wp_get_current_user()) && ($user_id = $user->ID))
                        {
                            ...
                            $start_time = ($post_vars['attr']['tp']) ? // If there's an Initial/Trial Period; start when it's over.
                                c_ws_plugin__optimizemember_pro_stripe_utilities::start_time($period1) : // After Trial is over.
                                c_ws_plugin__optimizemember_pro_stripe_utilities::start_time($period3); // Or next billing cycle.

                            if(!$global_response)
                                if(($post_vars['attr']['tp'] && $cost_calculations['trial_total'] > 0) || (!$post_vars['attr']['tp'] && $cost_calculations['total'] > 0))
                                {
                                    if(!is_object($stripe_customer = c_ws_plugin__optimizemember_pro_stripe_utilities::get_customer($user_id, $user->user_email, $post_vars['first_name'], $post_vars['last_name'], array(), $post_vars)))
                            ...
                            if(!$global_response)
                                if($cost_calculations['total'] > 0) // NOTE: it is optimizeMember's job to stop non-recurring subscriptions.
                                {
                                    if(!is_object($stripe_plan = c_ws_plugin__optimizemember_pro_stripe_utilities::get_plan($plan_attr)))
                                        $global_response = array('response' => $stripe_plan, 'error' => TRUE);
                                            ...
                                            && !is_object($stripe_customer = c_ws_plugin__optimizemember_pro_stripe_utilities::get_customer($user_id, $user->user_email, $post_vars['first_name'], $post_vars['last_name'], array(), $post_vars))
                                    ) $global_response = array('response' => $stripe_customer, 'error' => TRUE);

                                    else if((empty($stripe_customer_with_source) || !is_object($stripe_customer_with_source))
                                            && !is_object($stripe_customer = $stripe_customer_with_source = c_ws_plugin__optimizemember_pro_stripe_utilities::set_customer_source($stripe_customer->id, $post_vars['source_token'], $post_vars, $post_vars['attr']['reject_prepaid']))
                                    ) $global_response = array('response' => $stripe_customer, 'error' => TRUE);

                                    else if(!is_object($stripe_subscription = c_ws_plugin__optimizemember_pro_stripe_utilities::create_customer_subscription($stripe_customer->id, $stripe_plan->id, array(), $post_vars, $cost_calculations)))
                            ...
                            if(!$global_response)
                            {
                                ...
                                setcookie('optimizemember_tracking', ($optimizemember_tracking = c_ws_plugin__optimizemember_utils_encryption::encrypt($new__subscr_id)), time() + 31556926, COOKIEPATH, COOKIE_DOMAIN).
                                setcookie('optimizemember_tracking', $optimizemember_tracking, time() + 31556926, SITECOOKIEPATH, COOKIE_DOMAIN).
                                ($_COOKIE['optimizemember_tracking'] = $optimizemember_tracking);
                        ...
                        else if($use_subscription && !is_user_logged_in()) // Create a new account.
                        {
                            ...
                            if(!$global_response)
                                if(($post_vars['attr']['tp'] && $cost_calculations['trial_total'] > 0) || (!$post_vars['attr']['tp'] && $cost_calculations['total'] > 0))
                                {
                                    if(!is_object($stripe_customer = c_ws_plugin__optimizemember_pro_stripe_utilities::get_customer(0, $post_vars['email'], $post_vars['first_name'], $post_vars['last_name'], array(), $post_vars)))
                                        $global_response = array('response' => $stripe_customer, 'error' => TRUE);
                                ...
                                if($cost_calculations['total'] > 0) // NOTE: it is optimizeMember's job to stop non-recurring subscriptions.
                                {
                                    if(!is_object($stripe_plan = c_ws_plugin__optimizemember_pro_stripe_utilities::get_plan($plan_attr)))
                                        $global_response = array('response' => $stripe_plan, 'error' => TRUE);
                                            ...
                                            && !is_object($stripe_customer = c_ws_plugin__optimizemember_pro_stripe_utilities::get_customer(0, $post_vars['email'], $post_vars['first_name'], $post_vars['last_name'], array(), $post_vars))
                                    ) $global_response = array('response' => $stripe_customer, 'error' => TRUE);
                            ...
                            if(!$global_response) // No errors thus far?
                            {
                                ...
                                if($GLOBALS['WS_PLUGIN__']['optimizemember']['o']['custom_reg_fields'])
                                    foreach(json_decode($GLOBALS['WS_PLUGIN__']['optimizemember']['o']['custom_reg_fields'], TRUE) as $field)
                                    {
                                        ...
                                        if(isset($post_vars['custom_fields'][$field_var]))
                                            $GLOBALS['ws_plugin__optimizemember_registration_vars']['ws_plugin__optimizemember_custom_reg_field_'.$field_var]
                                ...
                                if(!empty($eotper)) $GLOBALS['ws_plugin__optimizemember_registration_vars']['ws_plugin__optimizemember_custom_reg_field_optimizemember_auto_eot_time']
                                    = date('Y-m-d H:i:s', c_ws_plugin__optimizemember_utils_time::auto_eot_time('', '', '', $eotper));
                                ...
                                if(((is_multisite() && ($new__user_id = c_ws_plugin__optimizemember_registrations::ms_create_existing_user($create_user['user_login'], $create_user['user_email'], $create_user['user_pass'])))
                        ...
                        else if(!$use_subscription && is_user_logged_in() && is_object($user = wp_get_current_user()) && ($user_id = $user->ID))
                        {
                            ...
                            if(!$global_response)
                            {
                                ...
                                setcookie('optimizemember_tracking', ($optimizemember_tracking = c_ws_plugin__optimizemember_utils_encryption::encrypt($new__txn_id)), time() + 31556926, COOKIEPATH, COOKIE_DOMAIN).
                                setcookie('optimizemember_tracking', $optimizemember_tracking, time() + 31556926, SITECOOKIEPATH, COOKIE_DOMAIN).
                                ($_COOKIE['optimizemember_tracking'] = $optimizemember_tracking);
                                ...
                                if($post_vars['attr']['success']
                                   && substr($ipn['optimizemember_stripe_proxy_return_url'], 0, 2) === substr($post_vars['attr']['success'], 0, 2)
                                   && ($custom_success_url = str_ireplace(array('%%s_response%%', '%%response%%'), array(urlencode(c_ws_plugin__optimizemember_utils_encryption::encrypt($global_response['response'])), urlencode($global_response['response'])), $ipn['optimizemember_stripe_proxy_return_url']))
                                   && ($custom_success_url = trim(preg_replace('/%%(.+?)%%/i', '', $custom_success_url)))
                                ) wp_redirect(c_ws_plugin__optimizemember_utils_urls::add_optimizemember_sig($custom_success_url, 's2p-v')).exit();
                            }
