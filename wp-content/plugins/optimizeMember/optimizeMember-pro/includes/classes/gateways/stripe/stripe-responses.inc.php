...
if(!class_exists('c_ws_plugin__optimizemember_pro_stripe_responses'))
{
	/**
	 * Stripe Pro Form responses.
	 *
	 ...
	 * @since 140617
	 */
	class c_ws_plugin__optimizemember_pro_stripe_responses
	{
		/**
		...
		{
			$_response = @$GLOBALS['ws_plugin__optimizemember_pro_stripe_cancellation_response'];
			$_response = (!$_response) ? c_ws_plugin__optimizemember_pro_stripe_responses::stripe_form_attr_validation_errors($attr) : $_response;
			$response  = $error = NULL; // Initialize.

			if($_response && !empty($_response['error']) && !empty($_response['response']) && ($error = $_response['error']))
			{
				$response = '<div id="optimizemember-pro-stripe-form-response" class="optimizemember-pro-stripe-form-response-error optimizemember-pro-stripe-cancellation-form-response-error">';
				$response .= $_response["response"];
				$response .= '</div>';
			}
			else if($_response && !empty($_response['response']))
			{
				$response = '<div id="optimizemember-pro-stripe-form-response" class="optimizemember-pro-stripe-form-response-info optimizemember-pro-stripe-cancellation-form-response-info">';
				$response .= $_response['response'];
				$response .= '</div>';
			}
		...
		{
			$_response = @$GLOBALS['ws_plugin__optimizemember_pro_stripe_update_response'];
			$_response = (!$_response) ? c_ws_plugin__optimizemember_pro_stripe_responses::stripe_form_attr_validation_errors($attr) : $_response;
			$response  = $error = NULL; // Initialize.

			if($_response && !empty($_response['error']) && !empty($_response['response']) && ($error = $_response['error']))
			{
				$response = '<div id="optimizemember-pro-stripe-form-response" class="optimizemember-pro-stripe-form-response-error optimizemember-pro-stripe-update-form-response-error">';
				$response .= $_response['response'];
				$response .= '</div>';
			}
			else if($_response && !empty($_response['response']))
			{
				$response = '<div id="optimizemember-pro-stripe-form-response" class="optimizemember-pro-stripe-form-response-info optimizemember-pro-stripe-update-form-response-info">';
				$response .= $_response['response'];
				$response .= '</div>';
			}
			return array('response' => $response, 'error' => $error);
		...
		{
			$_response = @$GLOBALS['ws_plugin__optimizemember_pro_stripe_registration_response'];
			$_response = (!$_response) ? c_ws_plugin__optimizemember_pro_stripe_responses::stripe_form_attr_validation_errors($attr) : $_response;
			$response  = $error = NULL; // Initialize.

			if($_response && !empty($_response['error']) && !empty($_response['response']) && ($error = $_response['error']))
			{
				$response = '<div id="optimizemember-pro-stripe-form-response" class="optimizemember-pro-stripe-form-response-error optimizemember-pro-stripe-registration-form-response-error">';
				$response .= $_response['response'];
				$response .= '</div>';
			}
			else if($_response && !empty($_response['response']))
			{
				$response = '<div id="optimizemember-pro-stripe-form-response" class="optimizemember-pro-stripe-form-response-info optimizemember-pro-stripe-registration-form-response-info">';
				$response .= $_response['response'];
				$response .= '</div>';
			}
			return array('response' => $response, 'error' => $error);
		...
		{
			$_response = @$GLOBALS['ws_plugin__optimizemember_pro_stripe_sp_checkout_response'];
			$_response = (!$_response) ? c_ws_plugin__optimizemember_pro_stripe_responses::stripe_form_attr_validation_errors($attr) : $_response;
			$response  = $error = NULL; // Initialize.

			if($_response && !empty($_response['error']) && !empty($_response['response']) && ($error = $_response['error']))
			{
				$response = '<div id="optimizemember-pro-stripe-form-response" class="optimizemember-pro-stripe-form-response-error optimizemember-pro-stripe-sp-checkout-form-response-error">';
				$response .= $_response['response'];
				$response .= '</div>';
			}
			else if($_response && !empty($_response['response']))
			{
				$response = '<div id="optimizemember-pro-stripe-form-response" class="optimizemember-pro-stripe-form-response-info optimizemember-pro-stripe-sp-checkout-form-response-info">';
				$response .= $_response['response'];
				$response .= '</div>';
			}
			return array('response' => $response, 'error' => $error);
		...
		{
			$_response = @$GLOBALS['ws_plugin__optimizemember_pro_stripe_checkout_response'];
			$_response = (!$_response) ? c_ws_plugin__optimizemember_pro_stripe_responses::stripe_form_attr_validation_errors($attr) : $_response;
			$response  = $error = NULL; // Initialize.

			if($_response && !empty($_response['error']) && !empty($_response['response']) && ($error = $_response['error']))
			{
				$response = '<div id="optimizemember-pro-stripe-form-response" class="optimizemember-pro-stripe-form-response-error optimizemember-pro-stripe-checkout-form-response-error">';
				$response .= $_response['response'];
				$response .= '</div>';
			}
			else if($_response && !empty($_response['response']))
			{
				$response = '<div id="optimizemember-pro-stripe-form-response" class="optimizemember-pro-stripe-form-response-info optimizemember-pro-stripe-checkout-form-response-info">';
				$response .= $_response['response'];
				$response .= '</div>';
			}
			return array('response' => $response, 'error' => $error);
		...
		{
			if(!$GLOBALS['WS_PLUGIN__']['optimizemember']['o']['pro_stripe_api_publishable_key'])
			...
			else if(!$GLOBALS['WS_PLUGIN__']['optimizemember']['o']['pro_stripe_api_secret_key'])
		...
		{
			if(!($response = c_ws_plugin__optimizemember_pro_stripe_responses::stripe_form_api_validation_errors($attr)) || !empty($attr['register']))
			{
				if($attr['cancel']) // Special form for Cancellations. User/Member must be logged in.
				{
					...
					else if(!is_object($user = wp_get_current_user()) || !($user_id = $user->ID) || !($subscr_cid = get_user_option('optimizemember_subscr_cid', $user_id)) || !($subscr_id = get_user_option('optimizemember_subscr_id', $user_id)))
						$response = array('response' => _x('Nothing to cancel. You\'re NOT a paid Member.', 's2member-front', 's2member'), 'error' => TRUE);

					else if(!is_object($stripe_subscription = c_ws_plugin__optimizemember_pro_stripe_utilities::get_customer_subscription($subscr_cid, $subscr_id)))
				...
				else if($attr['register']) // Free Registration does not require attr validation.
				{
					...
					else if($attr['level'] < 0 || $attr['level'] > $GLOBALS['WS_PLUGIN__']['optimizemember']['c']['levels'])
				...
				else if($attr['sp']) // Validation routines for Specific Post/Page checkout forms.
				{
					...
					else if(!preg_match($GLOBALS['WS_PLUGIN__']['optimizemember']['c']['sp_access_item_number_regex'], $attr['sp_ids_exp']))
				...
				else // Validation routines for Member Level checkout forms. This is the default functionality.
				{
					...
					else if(($attr['level'] < 1 || $attr['level'] > $GLOBALS['WS_PLUGIN__']['optimizemember']['c']['levels']) && $attr['level'] !== '*')
					...
					else if($attr['level'] !== '*' && !preg_match($GLOBALS['WS_PLUGIN__']['optimizemember']['c']['membership_item_number_w_level_regex'], $attr['level_ccaps_eotper']))
					...
					else if($attr['level'] === '*' && !preg_match($GLOBALS['WS_PLUGIN__']['optimizemember']['c']['membership_item_number_wo_level_regex'], $attr['level_ccaps_eotper']))
						$response = array('response' => _x('Invalid form configuration. Invalid "level_ccaps_eotper" attribute. Please check Shortcode Attributes.', 's2member-admin', 's2member'), 'error' => TRUE);
		...
		{
            $cp_attr             = c_ws_plugin__optimizemember_pro_stripe_utilities::apply_coupon($s['attr'], @$s['coupon'], 'attr');
			...
			if($form === 'registration' || !($response = c_ws_plugin__optimizemember_pro_stripe_responses::stripe_form_api_validation_errors()))
			{
				if($form === 'cancellation') // Special form for Cancellations. User/Member must be logged in.
				{
					if(!is_user_logged_in())
						$response = array('response' => sprintf(_x('You must <a href="%s" rel="nofollow">log in</a> to cancel your account.', 's2member-front', 's2member'), esc_attr(wp_login_url($_SERVER['REQUEST_URI']))), 'error' => TRUE);
					// -----------------------------------------------------------------------------------------------------------------
					else if($s['attr']['captcha'] && (empty($s['recaptcha_challenge_field']) || empty($s['recaptcha_response_field']) || !c_ws_plugin__optimizemember_utils_captchas::recaptcha_code_validates($s['recaptcha_challenge_field'], $s['recaptcha_response_field'])))
				...
				else if($form === 'registration') // Validation routines for free Registration forms.
				{
					...
					else if($GLOBALS['WS_PLUGIN__']['optimizemember']['o']['custom_reg_names'] && (empty($s['last_name']) || !is_string($s['last_name'])))
					...
					else if(email_exists($s['email']) && (!is_multisite() || !c_ws_plugin__optimizemember_utils_users::ms_user_login_email_can_join_blog(@$s['username'], $s['email'])))
					...
					else if(username_exists($s['username']) && (!is_multisite() || !c_ws_plugin__optimizemember_utils_users::ms_user_login_email_can_join_blog($s['username'], $s['email'])))
						$response = array('response' => _x('That Username is already in use. Please try again.', 's2member-front', 's2member'), 'error' => TRUE);

					else if(is_multisite() && !c_ws_plugin__optimizemember_utils_users::ms_user_login_email_can_join_blog($s['username'], $s['email']) && ($_response = wpmu_validate_user_signup($s['username'], $s['email'])) && is_wp_error($_errors = $_response['errors']) && $_errors->get_error_message())
						$response = array('response' => $_errors->get_error_message(), 'error' => TRUE);

					else if((empty($s['password1']) || !is_string($s['password1'])) && $GLOBALS['WS_PLUGIN__']['optimizemember']['o']['custom_reg_password'])
						$response = array('response' => _x('Missing Password. Please try again.', 's2member-front', 's2member'), 'error' => TRUE);

					else if(strlen($s['password1']) < 6 && $GLOBALS['WS_PLUGIN__']['optimizemember']['o']['custom_reg_password'])
					...
					else if(strlen($s['password1']) > 20 && $GLOBALS['WS_PLUGIN__']['optimizemember']['o']['custom_reg_password'])
					...
					else if((empty($s['password2']) || $s['password2'] !== $s['password1']) && $GLOBALS['WS_PLUGIN__']['optimizemember']['o']['custom_reg_password'])
						$response = array('response' => _x('Password fields do NOT match. Please try again.', 's2member-front', 's2member'), 'error' => TRUE);

					else if(($custom_field_validation_errors = c_ws_plugin__optimizemember_custom_reg_fields::validation_errors(isset($s['custom_fields']) ? $s['custom_fields'] : array(), c_ws_plugin__optimizemember_custom_reg_fields::custom_fields_configured_at_level($s['attr']['level'], 'registration', TRUE))))
				...
				else if($form === 'sp-checkout') // Validation routines for Specific Post/Page checkout forms.
				{
					...
					else if((empty($s['state']) || !is_string($s['state'])) && c_ws_plugin__optimizemember_pro_stripe_utilities::tax_may_apply())
						$response = array('response' => _x('Missing State/Province. Please try again.', 's2member-front', 's2member'), 'error' => TRUE);

					else if((empty($s['country']) || !is_string($s['country'])) && c_ws_plugin__optimizemember_pro_stripe_utilities::tax_may_apply())
						$response = array('response' => _x('Missing Country. Please try again.', 's2member-front', 's2member'), 'error' => TRUE);

					else if((empty($s['zip']) || !is_string($s['zip'])) && c_ws_plugin__optimizemember_pro_stripe_utilities::tax_may_apply())
				...
				else if($form === 'checkout') // Validation routines for Member Level checkout forms. This is the default functionality.
				{
					...
					else if(!is_user_logged_in() && email_exists($s['email']) && (!is_multisite() || !c_ws_plugin__optimizemember_utils_users::ms_user_login_email_can_join_blog(@$s['username'], $s['email'])))
						$response = array('response' => _x('That Email Address is already in use. Please try again.', 's2member-front', 's2member'), 'error' => TRUE);
					...
					else if(!is_user_logged_in() && username_exists($s['username']) && (!is_multisite() || !c_ws_plugin__optimizemember_utils_users::ms_user_login_email_can_join_blog($s['username'], $s['email'])))
						$response = array('response' => _x('That Username is already in use. Please try again.', 's2member-front', 's2member'), 'error' => TRUE);

					else if(!is_user_logged_in() && is_multisite() && !c_ws_plugin__optimizemember_utils_users::ms_user_login_email_can_join_blog($s['username'], $s['email']) && ($_response = wpmu_validate_user_signup($s['username'], $s['email'])) && is_wp_error($_errors = $_response['errors']) && $_errors->get_error_message())
						$response = array('response' => $_errors->get_error_message(), 'error' => TRUE);

					else if(!is_user_logged_in() && (empty($s['password1']) || !is_string($s['password1'])) && $GLOBALS['WS_PLUGIN__']['optimizemember']['o']['custom_reg_password'])
						$response = array('response' => _x('Missing Password. Please try again.', 's2member-front', 's2member'), 'error' => TRUE);

					else if(!is_user_logged_in() && strlen($s['password1']) < 6 && $GLOBALS['WS_PLUGIN__']['optimizemember']['o']['custom_reg_password'])
						$response = array('response' => _x('Invalid Password. Must be at least 6 characters. Please try again.', 's2member-front', 's2member'), 'error' => TRUE);

					else if(!is_user_logged_in() && strlen($s['password1']) > 20 && $GLOBALS['WS_PLUGIN__']['optimizemember']['o']['custom_reg_password'])
						$response = array('response' => _x('Invalid Password. Max length is 20 characters. Please try again.', 's2member-front', 's2member'), 'error' => TRUE);

					else if(!is_user_logged_in() && (empty($s['password2']) || $s['password2'] !== $s['password1']) && $GLOBALS['WS_PLUGIN__']['optimizemember']['o']['custom_reg_password'])
						$response = array('response' => _x('Password fields do NOT match. Please try again.', 's2member-front', 's2member'), 'error' => TRUE);

					else if(!is_user_logged_in() && ($custom_field_validation_errors = c_ws_plugin__optimizemember_custom_reg_fields::validation_errors(isset($s['custom_fields']) ? $s['custom_fields'] : array(), c_ws_plugin__optimizemember_custom_reg_fields::custom_fields_configured_at_level($s['attr']['level'] === '*' ? 'auto-detection' : $s['attr']['level'], 'registration', TRUE))))
						$response = array('response' => array_shift($custom_field_validation_errors), 'error' => TRUE);
					// -----------------------------------------------------------------------------------------------------------------
					else if(empty($s['source_token']) || !is_string($s['source_token']))
