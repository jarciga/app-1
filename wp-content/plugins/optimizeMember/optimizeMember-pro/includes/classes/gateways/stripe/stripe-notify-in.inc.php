...
if(!class_exists('c_ws_plugin__optimizemember_pro_stripe_notify_in'))
{
	/**
	 * Stripe Silent Post *(aka: IPN)* (inner processing routines).
	 *
	 ...
	 * @since 140617
	 */
	class c_ws_plugin__optimizemember_pro_stripe_notify_in
	{
		/**
		...
		public static function stripe_notify()
		{
			...
			{
				$stripe = array(); // Initialize array of Webhook/IPN event data and optimizeMember log details.
				...
				if(is_object($event = c_ws_plugin__optimizemember_pro_stripe_utilities::get_event()) && ($stripe['event'] = $event))
				{
					switch($event->type)
					{
						case 'invoice.payment_succeeded': // Subscription payments.
							...
							if(!empty($event->data->object)
							   ...
							   && ($stripe_invoice_total = number_format(c_ws_plugin__optimizemember_pro_stripe_utilities::cents_to_dollar_amount($stripe_invoice->total, $stripe_invoice->currency), 2, '.', ''))
							   && is_object($stripe_subscription = c_ws_plugin__optimizemember_pro_stripe_utilities::get_customer_subscription($stripe_invoice->customer, $stripe_invoice->subscription))
							   && ($ipn_signup_vars = c_ws_plugin__optimizemember_utils_users::get_user_ipn_signup_vars(0, $stripe_subscription->id))
							)
							{
								$processing = TRUE;
						...
						case 'customer.deleted': // Customer deletions.
							...
							if(!empty($event->data->object)
							   ...
							   && ($ipn_signup_vars = c_ws_plugin__optimizemember_utils_users::get_user_ipn_signup_vars(0, $stripe_customer->id))
							)
							{
								$processing = TRUE;
						...
						case 'charge.refunded': // Customer refund (partial or full).
							...
							if(!empty($event->data->object)
								...
								&& ($ipn_signup_vars = c_ws_plugin__optimizemember_utils_users::get_user_ipn_signup_vars(0, $stripe_charge->customer))
							)
							{
								$processing = TRUE;
                                ...
                                $ipn['mc_gross']    = '-'.number_format(c_ws_plugin__optimizemember_pro_stripe_utilities::cents_to_dollar_amount(abs($stripe_charge->amount), $stripe_charge->currency), 2, '.', '');
								...
								$stripe['optimizemember_log'][] = 'Stripe Webhook/IPN event type identified as: `'.$event->type.'` on: '.date('D M j, Y g:i:s a T');
								$stripe['optimizemember_log'][] = 'Webhook/IPN event `'.$event->type.'` reformulated. Piping through optimizeMember\'s core gateway processor.';
								$stripe['optimizemember_log'][] = 'Please check core IPN logs for further processing details.';
							}
							break; // Break switch handler.
						...
						case 'charge.dispute.created': // Customer dispute (chargeback).
							...
							if(!empty($event->data->object->charge)
								&& ($stripe_charge = c_ws_plugin__optimizemember_pro_stripe_utilities::get_charge($event->data->object->charge)) instanceof Stripe_Charge
					...
					if(empty($processing)) $stripe['optimizemember_log'][] = 'Ignoring this Webhook/IPN. The event does NOT require any action on the part of OptimizeMember.';
				}
		...
		public static function _maybe_end_subscription_after_payment($customer_id, $stripe_subscription)
		{
			...
			)
			{
				...
				return 'Subscription `'.$stripe_subscription->id.'` has `plan->metadata->recurring=true` `plan->metadata->recurring_times=1`.'.
			        ...
			        && strtolower($stripe_subscription->plan->interval) === 'day' // optimizeMember configures all plans with a day-based interval.
			        && strtolower($stripe_subscription->status) !== 'trialing' // Past the initial/trial period?
