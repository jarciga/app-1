...
if(!class_exists('c_ws_plugin__optimizemember_pro_stripe_registration_in'))
{
	/**
	 * Stripe Registration Form (inner processing routines).
	 *
	 ...
	 * @since 140617
	 */
	class c_ws_plugin__optimizemember_pro_stripe_registration_in
	{
		/**
		...
		{
			if(!empty($_POST['optimizemember_pro_stripe_registration']['nonce'])
			   && ($nonce = $_POST['optimizemember_pro_stripe_registration']['nonce'])
			   && wp_verify_nonce($nonce, 'optimizemember-pro-stripe-registration')
			)
			{
				$GLOBALS['ws_plugin__optimizemember_pro_stripe_registration_response'] = array(); // This holds the global response details.
				...
				if(!c_ws_plugin__optimizemember_pro_stripe_responses::stripe_form_attr_validation_errors($post_vars['attr'])) // Must NOT have any attr errors.
				{
					if(!($form_submission_validation_errors // Validate registration input form fields.
						= c_ws_plugin__optimizemember_pro_stripe_responses::stripe_form_submission_validation_errors('registration', $post_vars))
					) // If this fails the global response is set to the error(s) returned during form field validation.
					{
						...
						$GLOBALS['ws_plugin__optimizemember_registration_vars']['ws_plugin__optimizemember_custom_reg_field_optimizemember_auto_eot_time'] = ($post_vars['attr']['tp'] && $post_vars['attr']['tt'])
							? date('Y-m-d H:i:s', c_ws_plugin__optimizemember_utils_time::auto_eot_time('', '', '', $post_vars['attr']['tp'].' '.$post_vars['attr']['tt'])) : '';
						...
						)
						{
							...
							if($post_vars['attr']['success'] && substr($GLOBALS['ws_plugin__optimizemember_registration_return_url'], 0, 2) === substr($post_vars['attr']['success'], 0, 2)
							   && ($custom_success_url = str_ireplace(array('%%s_response%%', '%%response%%'), array(urlencode(c_ws_plugin__optimizemember_utils_encryption::encrypt($global_response['response'])), urlencode($global_response['response'])), $GLOBALS['ws_plugin__optimizemember_registration_return_url']))
