...
if(!class_exists('c_ws_plugin__optimizemember_pro_stripe_utilities'))
{
	/**
	 * Stripe utilities.
	 *
	 ...
	 * @since 140617
	 */
	class c_ws_plugin__optimizemember_pro_stripe_utilities
	{
        /**
         * STRIPE API version used to lock the API calls
         */
        const OPM_STRIPE_API_VERSION = '2015-02-18';
		...
		public static function get_customer($user_id = 0, $email = '', $fname = '', $lname = '', $metadata = array(), $post_vars = array())
		{
            ...
            Stripe::setApiVersion(self::OPM_STRIPE_API_VERSION);
		 ...
		 * @return string|Stripe_Customer Customer object; else error message.
		 */
		public static function set_customer_card_token($customer_id, $card_token, $post_vars = array())
		{
			...
			try // Attempt to update the customer's card/token.
			{
				...
				{
					$card = $customer->sources->data[0]; // Just one card!
        ...
        public static function set_customer_source($customer_id, $source_token, $post_vars = array(), $reject_prepaid = null)
        {
            ...
            try // Attempt to update the customer's source token.
            {
                ...
                $reject_prepaid = !empty($reject_prepaid) || $reject_prepaid === false || $reject_prepaid === '0'
                    ...
                    : (boolean)$GLOBALS['WS_PLUGIN__']['optimizemember']['o']['pro_stripe_api_reject_prepaid'];
		 ...
		 *
		 * @param array $post_vars
		 * @param array $cost_calculations
		 * @return string|Stripe_Charge Charge object; else error message.
		 */
		public static function create_customer_charge($customer_id, $amount, $currency, $description, $metadata = array(), $post_vars = array(), $cost_calculations = array())
		{
			...
			try // Attempt to charge the customer.
			{
				$charge = array(
					...
					'statement_descriptor' => $GLOBALS['WS_PLUGIN__']['optimizemember']['o']['pro_stripe_api_statement_description']
				);
		...
		public static function get_plan($shortcode_attrs, $metadata = array())
		{
			...
			$plan_id = 's2_'.md5($amount.$currency.$name.$trial_period_days.$interval_days.serialize($metadata).$GLOBALS['WS_PLUGIN__']['optimizemember']['o']['pro_stripe_api_statement_description']);
		 ...
		 * @param array $post_vars
		 * @param array $cost_calculations
		 * @return string|Stripe_Subscription Subscription object; else error message.
		 */
		...
		{
			if(empty($_REQUEST['optimizemember_pro_stripe_notify']))
		...
		{
			if(!empty($_POST['ws_plugin__optimizemember_pro_stripe_ajax_tax']) && ($nonce = $_POST['ws_plugin__optimizemember_pro_stripe_ajax_tax']) && (wp_verify_nonce($nonce, 'ws-plugin--optimizemember-pro-stripe-ajax-tax') || c_ws_plugin__optimizemember_utils_encryption::decrypt($nonce) === 'ws-plugin--optimizemember-pro-stripe-ajax-tax'))
				/* A wp_verify_nonce() won't always work here, because optimizemember-pro-min.js must be cacheable. The output from wp_create_nonce() would go stale.
						So instead, optimizemember-pro-min.js should use ``c_ws_plugin__optimizemember_utils_encryption::encrypt()`` as an alternate form of nonce. */
			{
				status_header(200); // Send a 200 OK status header.
				header('Content-Type: text/plain; charset=UTF-8'); // Content-Type text/plain with UTF-8.
				while(@ob_end_clean()) ; // Clean any existing output buffers.

				if(!empty($_POST['ws_plugin__optimizemember_pro_stripe_ajax_tax_vars']) && is_array($_p_tax_vars = c_ws_plugin__optimizemember_utils_strings::trim_deep(stripslashes_deep($_POST['ws_plugin__optimizemember_pro_stripe_ajax_tax_vars']))))
				{
					...
					{
						$attr = (!empty($attr['coupon'])) ? c_ws_plugin__optimizemember_pro_stripe_utilities::apply_coupon($attr, $attr['coupon']) : $attr;
						...
						$a = c_ws_plugin__optimizemember_pro_stripe_utilities::cost(NULL, $sub_total_today, $state, $country, $zip, $currency, $desc);
		...
		public static function cost($trial_sub_total = '', $sub_total = '', $state = '', $country = '', $zip = '', $currency = '', $desc = '')
		{
			...
			return array(
				...
				'cur'             => $currency,
				'cur_symbol'      => c_ws_plugin__optimizemember_utils_cur::symbol($currency),
