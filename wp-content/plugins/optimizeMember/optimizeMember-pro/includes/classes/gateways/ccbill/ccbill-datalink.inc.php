...
/**/
if(!class_exists("c_ws_plugin__optimizemember_pro_ccbill_datalink"))
	{
		/**
		* ccBill DataLink integration.
		*
		...
		* @since 1.5
		*/
		class c_ws_plugin__optimizemember_pro_ccbill_datalink
			{
				/**
				...
				public static function ccbill_datalink($vars = FALSE)
					{
						...
						/**/
						if /* Configd? */($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_ccbill_client_id"])
							{
										...
										if(($unprocessed_ipn_lines = trim(c_ws_plugin__optimizemember_utils_urls::remote($datalink = add_query_arg(urlencode_deep($qvrs), $datalink)))) && !preg_match("/^Error\:/i", $unprocessed_ipn_lines))
											{
												...
												$ccbill["optimizemember_log"][] = /* Record the full DataLink URL as well. */ $datalink;
												$ccbill["optimizemember_log"][] = /* Record list in log. */ $unprocessed_ipn_lines;
												/**/
										...
										else if /* In this case, there we no new IPNs. */(!preg_match("/^Error\:/i", $unprocessed_ipn_lines))
											{
												...
												$ccbill["optimizemember_log"][] = /* Record the full DataLink URL as well. */ $datalink;
												$ccbill["optimizemember_log"][] = /* Log this; just in case. */ $unprocessed_ipn_lines;
											}
										else /* Otherwise, we need to record any errors that were found in the DataLink response. */
											{
												...
												$ccbill["optimizemember_log"][] = /* Record the full DataLink URL as well. */ $datalink;
												$ccbill["optimizemember_log"][] = /* Log error msg. */ $unprocessed_ipn_lines;
											}
										/**/
								...
								/**/
								else if /* Anything? */(($unprocessed_ipn_lines = trim(get_transient("s2m_".md5("optimizemember_pro_ccbill_datalink_ipns")))))
									{
