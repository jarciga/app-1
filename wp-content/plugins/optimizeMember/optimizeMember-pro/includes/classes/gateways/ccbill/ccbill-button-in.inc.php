<?php
/**
* Shortcode `[optimizeMember-Pro-ccBill-Button /]` ( inner processing routines ).
*
* Copyright: © 2009-2011
...
/**/
if(!class_exists("c_ws_plugin__optimizemember_pro_ccbill_button_in"))
	{
		/**
		* Shortcode `[optimizeMember-Pro-ccBill-Button /]` ( inner processing routines ).
		*
		...
		* @since 1.5
		*/
		class c_ws_plugin__optimizemember_pro_ccbill_button_in
			{
				/**
				...
				public static function sc_ccbill_button($attr = FALSE, $content = FALSE, $shortcode = FALSE)
					{
						...
						if /* ccBill Modifications/Cancellations. */($attr["modify"] || $attr["cancel"])
						...
						else if /* Specific Post/Page Buttons. */($attr["sp"])
							{
								...
								/**/
								if /* Are we referencing a User/Member in the database? */($referencing = c_ws_plugin__optimizemember_utils_users::get_user_subscr_or_wp_id())
									$vars["s2_referencing"] = /* Add this into the ``$vars`` array. */ $referencing;
								/**/
								$code = preg_replace("/%%url%%/", c_ws_plugin__optimizemember_utils_strings::esc_refs(esc_attr($url = c_ws_plugin__optimizemember_pro_ccbill_utilities::ccbill_link_gen($vars))), $code);
								/**/
								$code = $_code = ($attr["image"] && $attr["image"] !== "default") ? preg_replace('/ src\="(.*?)"/', ' src="'.c_ws_plugin__optimizemember_utils_strings::esc_refs(esc_attr($attr["image"])).'"', $code) : preg_replace('/ src\="(.*?)"/', ' src="'.c_ws_plugin__optimizemember_utils_strings::esc_refs(esc_attr($default_image)).'"', $code);
								/**/
								$code = ($attr["output"] === "anchor") ? /* Buttons already in anchor format. */ $code : $code;
						...
						else if /* Independent Custom Capabilities. */($attr["level"] === "*")
							{
								...
								/**/
								if /* Are we referencing an account already in the database? */($referencing = c_ws_plugin__optimizemember_utils_users::get_user_subscr_or_wp_id())
									$vars["s2_referencing"] = /* Add this into the ``$vars`` array. */ $referencing;
								/**/
								$code = preg_replace("/%%url%%/", c_ws_plugin__optimizemember_utils_strings::esc_refs(esc_attr($url = c_ws_plugin__optimizemember_pro_ccbill_utilities::ccbill_link_gen($vars))), $code);
								/**/
