...
if(!class_exists('c_ws_plugin__optimizemember_pro_coupons'))
{
	/**
	 * Coupon Codes.
	 *
	 * @package optimizeMember\Coupons
	 * @since 150122
	 */
	class c_ws_plugin__optimizemember_pro_coupons
	{
		public $list = '';
		...
		public function __construct($args = array())
		{
			...
			$this->list_to_coupons($GLOBALS['WS_PLUGIN__']['optimizemember']['o']['pro_coupon_codes'], $args['update']);
		}
		...
		public function apply($attr, $coupon_code = '', $return = 'attr', $process = array())
		{
			...
			if($coupon_code) // Only if we do have a coupon code.
				if(($coupon = $this->valid_coupon($coupon_code, $attr)))
				{
					...
					$tx = c_ws_plugin__optimizemember_pro_taxes::may_apply() ? ' '._x('+ tax', 's2member-front', 's2member') : '';
					...
					if(strlen($_affiliate_suffix_chars = $GLOBALS['WS_PLUGIN__']['optimizemember']['o']['pro_affiliate_coupon_code_suffix_chars']))
					...
					if($coupon['flat_discount']) // If it's a flat-rate coupon.
					{
						...
						else if(!$attr['sp'] && $attr['tp'] && $coupon['directive'] === 'ta-only')
						{
							...
							$desc     = sprintf(_x('Discount: %s off. (Now: %s, then %s)', 's2member-front', 's2member'), $cs.number_format($coupon['flat_discount'], 2, '.', ''), $cs.c_ws_plugin__optimizemember_utils_time::amount_period_term($ta, $attr['tp'].' '.$attr['tt']).$tx, $cs.c_ws_plugin__optimizemember_utils_time::amount_period_term($ra, $attr['rp'].' '.$attr['rt'], $attr['rr']));
							$response = sprintf(_x('<div>Discount: <strong>%s off</strong>. (Now: <strong>%s, then %s</strong>)</div>', 's2member-front', 's2member'), $cs.number_format($coupon['flat_discount'], 2, '.', ''), $cs.c_ws_plugin__optimizemember_utils_time::amount_period_term($ta, $attr['tp'].' '.$attr['tt']).$tx, $cs.c_ws_plugin__optimizemember_utils_time::amount_period_term($ra, $attr['rp'].' '.$attr['rt'], $attr['rr']));
						}
						...
						else if(!$attr['sp'] && !$attr['tp'] && $coupon['directive'] === 'ra-only')
						{
							...
							$desc     = sprintf(_x('Discount: %s off. (Now: %s)', 's2member-front', 's2member'), $cs.number_format($coupon['flat_discount'], 2, '.', ''), $cs.c_ws_plugin__optimizemember_utils_time::amount_period_term($ra, $attr['rp'].' '.$attr['rt'], $attr['rr']).$tx);
							$response = sprintf(_x('<div>Discount: <strong>%s off</strong>. (Now: <strong>%s</strong>)</div>', 's2member-front', 's2member'), $cs.number_format($coupon['flat_discount'], 2, '.', ''), $cs.c_ws_plugin__optimizemember_utils_time::amount_period_term($ra, $attr['rp'].' '.$attr['rt'], $attr['rr']).$tx);
						}
						else if(!$attr['sp'] && !$attr['tp'] && !$coupon['directive'])
						{
							$coupon_applies = TRUE;
					...
					else if($coupon['percentage_discount']) // Else if it's a percentage.
					{
						...
						else if(!$attr['sp'] && $attr['tp'] && $coupon['directive'] === 'ta-only')
						{
							...
							$desc     = sprintf(_x('Discount: %s off. (Now: %s, then %s)', 's2member-front', 's2member'), number_format($coupon['percentage_discount'], 0).$ps, $cs.c_ws_plugin__optimizemember_utils_time::amount_period_term($ta, $attr['tp'].' '.$attr['tt']).$tx, $cs.c_ws_plugin__optimizemember_utils_time::amount_period_term($ra, $attr['rp'].' '.$attr['rt'], $attr['rr']));
							$response = sprintf(_x('<div>Discount: <strong>%s off</strong>. (Now: <strong>%s, then %s</strong>)</div>', 's2member-front', 's2member'), number_format($coupon['percentage_discount'], 0).$ps, $cs.c_ws_plugin__optimizemember_utils_time::amount_period_term($ta, $attr['tp'].' '.$attr['tt']).$tx, $cs.c_ws_plugin__optimizemember_utils_time::amount_period_term($ra, $attr['rp'].' '.$attr['rt'], $attr['rr']));
						}
						else if(!$attr['sp'] && $attr['tp'] && $coupon['directive'] === 'ra-only')
						{
							$coupon_applies = TRUE;
						...
						else if(!$attr['sp'] && !$attr['tp'] && $coupon['directive'] === 'ra-only')
						{
							...
							$desc     = sprintf(_x('Discount: %s off. (Now: %s)', 's2member-front', 's2member'), number_format($coupon['percentage_discount'], 0).$ps, $cs.c_ws_plugin__optimizemember_utils_time::amount_period_term($ra, $attr['rp'].' '.$attr['rt'], $attr['rr']).$tx);
							$response = sprintf(_x('<div>Discount: <strong>%s off</strong>. (Now: <strong>%s</strong>)</div>', 's2member-front', 's2member'), number_format($coupon['percentage_discount'], 0).$ps, $cs.c_ws_plugin__optimizemember_utils_time::amount_period_term($ra, $attr['rp'].' '.$attr['rt'], $attr['rr']).$tx);
						}
						else if(!$attr['sp'] && !$attr['tp'] && !$coupon['directive'])
						{
							$coupon_applies = TRUE;
					...
					if($coupon_applies) // Apply the coupon here; if applicable.
					{
						...
						if($affiliate_id && empty($_COOKIE['idev']) && (in_array('affiliates-silent-post', $process) || in_array('affiliates-1px-response', $process)))
							foreach(preg_split('/['."\r\n\t".']+/', $GLOBALS['WS_PLUGIN__']['optimizemember']['o']['pro_affiliate_coupon_code_tracking_urls'], NULL, PREG_SPLIT_NO_EMPTY) as $_url)
							{
											...
											if(($_url = preg_replace('/%%user_ip%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($_SERVER['REMOTE_ADDR'])), $_url)))
												if(($_url = trim(preg_replace('/%%(.+?)%%/i', '', $_url))) /* Cleanup any remaining Replacement Codes. */)
												{
													if(!($_r = 0) && ($_url = preg_replace('/^silent-php\|/i', '', $_url, 1, $_r)) && $_r && in_array('affiliates-silent-post', $process))
														c_ws_plugin__optimizemember_utils_urls::remote($_url, FALSE, array('blocking' => FALSE)); // Post silently via PHP. Relies on IP tracking.
		...
		public function generate_gifts($args)
		{
			...
			{
				$_gift_code = c_ws_plugin__optimizemember_utils_encryption::uunnci_key_20_max();
		...
		{
			if(is_admin() && !empty($_REQUEST['page']) && $_REQUEST['page'] === 'ws-plugin--optimizemember-pro-coupon-codes' && !empty($_POST['ws_plugin__optimizemember_options_save']))
			{
				$coupons                                                     = new c_ws_plugin__optimizemember_pro_coupons(array('update' => 'counters'));
				$GLOBALS['WS_PLUGIN__']['optimizemember']['o']['pro_coupon_codes'] = $coupons->list;
			}
		}
	}
}
