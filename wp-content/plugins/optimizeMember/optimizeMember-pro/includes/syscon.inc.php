<?php
/**
* System configuration for optimizeMember Pro.
*
...
*/
$GLOBALS["WS_PLUGIN__"]["optimizemember_pro"]["c"]["dir"] = dirname(dirname(__FILE__));
/*
Determine the base directory name.
*/
$GLOBALS["WS_PLUGIN__"]["optimizemember_pro"]["c"]["dir_base"] = basename(dirname(dirname(__FILE__)));
/*
Determine full URL to the directory this plugin resides in.
*/
$GLOBALS["WS_PLUGIN__"]["optimizemember_pro"]["c"]["dir_url"] = (stripos(__FILE__, WP_CONTENT_DIR) !== 0) ? /* Have to assume plugins dir? */
plugins_url($GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["dir_base"]."/".basename(dirname(dirname(__FILE__)))) : /* Otherwise, this gives it a chance to live anywhere in the content dir. */
content_url(preg_replace("/^(.*?)\/".preg_quote(basename(WP_CONTENT_DIR), "/")."/", "", str_replace(DIRECTORY_SEPARATOR, "/", dirname(dirname(__FILE__)))));
//die($GLOBALS["WS_PLUGIN__"]["optimizemember_pro"]["c"]["dir_url"]);
/*
...
*/
$GLOBALS["WS_PLUGIN__"]["optimizemember_pro"]["c"]["plugin_basename"] = basename(dirname($GLOBALS["WS_PLUGIN__"]["optimizemember_pro"]["l"]))."/".basename($GLOBALS["WS_PLUGIN__"]["optimizemember_pro"]["l"]);
/*
Turn upsells & donations off in menu pages of back-end adminstrative panels.
*/
$GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["menu_pages"] = array_merge($GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["menu_pages"], array("upsell-pro" => false));
/*
Configure checksum time for both of the syscon.inc.php files.
*/
$GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["checksum"] = $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["checksum"] + filemtime(__FILE__);
/*
...
if(defined("MEMBERSHIP_LEVELS") && is_numeric(MEMBERSHIP_LEVELS))
	if(MEMBERSHIP_LEVELS >= $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["min_levels"] && MEMBERSHIP_LEVELS <= $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["max_levels"])
		$GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["levels"] = (int)MEMBERSHIP_LEVELS;
/*
Add some new default options for the Pro Module.
*/
if(!function_exists("ws_plugin__optimizemember_pro_default_options"))
	{
		add_filter("ws_plugin__optimizemember_default_options", "ws_plugin__optimizemember_pro_default_options");
		/**
		* Configures default options array for optimizeMember Pro.
		*
		...
		* @return array Merged with default options from the optimizeMember Framework.
		* 	All Pro options should be prefixed with `pro_`.
		*/
		function ws_plugin__optimizemember_pro_default_options($default_options = array())
			{
				...
				'pro_eot_reminder_email_subject'          => array('_' => _x('Renewal Reminder (Account Expires in %%eot_descriptive_time%%)', 's2member-front', 's2member')),
                ...
                'pro_stripe_api_image'                    => str_ireplace(array('http:', 'https:'), '', $GLOBALS['WS_PLUGIN__']['optimizemember_pro']['c']['dir_url']).'/images/stripe-square.png',
...
*/
if(!function_exists("ws_plugin__optimizemember_pro_options_before_checksum"))
	{
		add_filter("ws_plugin__optimizemember_options_before_checksum", "ws_plugin__optimizemember_pro_options_before_checksum");
		/**
		* Configures options array; while handling optimizeMember Pro options.
		*
		* **WARNING:** This is a system configuration function, please DO NOT EDIT this function directly.
		* 	Instead, use the plugin options panel in WordPress to override these settings.
		*
		...
		* 	``$GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]``, by reference.
		* @return array Merged with options from the optimizeMember Framework.
		* 	All Pro options should be prefixed with `pro_`.
		*/
		function ws_plugin__optimizemember_pro_options_before_checksum(&$options = array())
			{
				$pro_default_options = ws_plugin__optimizemember_pro_default_options();
				/*
				...
				*/
				...
				/**/
				foreach($options /* $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"] */ as $key => &$value)
					{
				...
				/**/
				return $options; /* $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"] by reference. */
			}
	}
?>