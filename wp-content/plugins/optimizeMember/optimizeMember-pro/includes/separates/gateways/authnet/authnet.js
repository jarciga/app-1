						...
						if ($(submissionSection + ' input#s2member-pro-authnet-registration-names-not-required-or-not-possible').length)
							{
								...
								$(registrationSection + ' > div#s2member-pro-authnet-registration-form-first-name-div :input').attr (ariaFalseReadonly);
								/**/
								$(registrationSection + ' > div#s2member-pro-authnet-registration-form-last-name-div').hide ();
								$(registrationSection + ' > div#s2member-pro-authnet-registration-form-last-name-div :input').attr (ariaFalseReadonly);
							}
					}) ();
				/**/
						...
						if ($(submissionSection + ' input#s2member-pro-authnet-registration-password-not-required-or-not-possible').length)
							{
								...
								$(registrationSection + ' > div#s2member-pro-authnet-registration-form-password-div :input').attr (ariaFalseReadonly);
							}
					}) ();
				/**/
								...
								if (state && country && zip && thisTaxLocation && (!cTaxLocation || cTaxLocation !== thisTaxLocation) && (cTaxLocation = thisTaxLocation))
									{
										...
										cTaxTimeout = setTimeout( /* Create a new cTaxTimeout with a one second delay. */function()
											{
												cTaxReq = $.post ('<?php echo c_ws_plugin__optimizemember_utils_strings::esc_js_sq (admin_url ("/admin-ajax.php")); ?>', {'action': 'ws_plugin__optimizemember_pro_authnet_ajax_tax', 'ws_plugin__optimizemember_pro_authnet_ajax_tax': '<?php echo c_ws_plugin__optimizemember_utils_strings::esc_js_sq (c_ws_plugin__optimizemember_utils_encryption::encrypt ("ws-plugin--optimizemember-pro-authnet-ajax-tax")); ?>', 'ws_plugin__optimizemember_pro_authnet_ajax_tax_vars[attr]': attr, 'ws_plugin__optimizemember_pro_authnet_ajax_tax_vars[state]': state, 'ws_plugin__optimizemember_pro_authnet_ajax_tax_vars[country]': country, 'ws_plugin__optimizemember_pro_authnet_ajax_tax_vars[zip]': zip}, function(response, textStatus)
													{
														clearTimeout /* Clear cTaxTimeout. */(cTaxTimeout), cTaxTimeout = null;
														/**/
														try /* Try/catch here. jQuery will sometimes return a successful response in IE, whenever the connection is aborted with a null response. */
						...
						if ($(submissionSection + ' input#s2member-pro-authnet-checkout-password-not-required-or-not-possible').length)
							{
								...
								$(registrationSection + ' > div#s2member-pro-authnet-checkout-form-password-div :input').attr (ariaFalseReadonly);
							}
					}) ();
				/**/
				(handleExistingUsers = /* eventTrigger is passed by jQuery for DOM events. */ function(eventTrigger)
						...
						if /* If User/Member is already logged in. */ (OPTIMIZEMEMBER_CURRENT_USER_IS_LOGGED_IN)
							{
								...
								/**/
								$(registrationSection + ' input#s2member-pro-authnet-checkout-email').val (OPTIMIZEMEMBER_CURRENT_USER_EMAIL).attr (ariaFalseReadonly);
								$(registrationSection + ' input#s2member-pro-authnet-checkout-username').val (OPTIMIZEMEMBER_CURRENT_USER_LOGIN).attr (ariaFalseReadonly);
								/**/
								$(registrationSection + ' > div#s2member-pro-authnet-checkout-form-password-div').hide ();
								$(registrationSection + ' > div#s2member-pro-authnet-checkout-form-password-div :input').attr (ariaFalseReadonly);
								/**/
								if ($.trim ($(registrationSection + ' > div#s2member-pro-authnet-checkout-form-registration-section-title').html ()) === '<?php echo c_ws_plugin__optimizemember_utils_strings::esc_js_sq (_x ("Create Profile", "s2member-front", "s2member")); ?>')
									$(registrationSection + ' > div#s2member-pro-authnet-checkout-form-registration-section-title').html ('<?php echo c_ws_plugin__optimizemember_utils_strings::esc_js_sq (_x ("Your Profile", "s2member-front", "s2member")); ?>');
								/**/
