...
jQuery(document).ready (function($)
	{
		var $clForm, $upForm, $rgForm, $spForm, $coForm, jumpToResponses, preloadAjaxLoader, ariaTrue = {'aria-required': 'true'}, ariaFalse = {'aria-required': 'false'}, disabled = {'disabled': 'disabled'}, ariaFalseDis = {'aria-required': 'false', 'disabled': 'disabled'}, ariaFalseReadonly = {'aria-required': 'false', 'readonly': 'readonly'};
		/**/
		...
		if (($clForm = $('form#s2member-pro-paypal-cancellation-form')).length === 1)
			{
				...
				/**/
				ws_plugin__optimizemember_animateProcessing($submissionButton, 'reset'), $submissionButton.removeAttr ('disabled');
				/**/
				...
				$upForm.submit ( /* Form validation. */function()
					{
						...
						if (!$(cardType + ':checked').val ())
							{
								alert('<?php echo c_ws_plugin__optimizemember_utils_strings::esc_js_sq (_x ("Please choose a Billing Method.", "s2member-front", "s2member")); ?>');
								/**/
								return false;
							}
						/**/
						...
						if ($(submissionSection + ' input#s2member-pro-paypal-registration-names-not-required-or-not-possible').length)
							{
								...
								$(registrationSection + ' > div#s2member-pro-paypal-registration-form-first-name-div :input').attr (ariaFalseReadonly);
								/**/
								$(registrationSection + ' > div#s2member-pro-paypal-registration-form-last-name-div').hide ();
								$(registrationSection + ' > div#s2member-pro-paypal-registration-form-last-name-div :input').attr (ariaFalseReadonly);
							}
					}) ();
				/**/
						...
						if ($(submissionSection + ' input#s2member-pro-paypal-registration-password-not-required-or-not-possible').length)
							{
								...
								$(registrationSection + ' > div#s2member-pro-paypal-registration-form-password-div :input').attr (ariaFalseReadonly);
							}
					}) ();
				/**/
		...
		/**/
			...
			{
				var handleCouponIssues, handleTaxIssues, taxMayApply = true, calculateTax, cTaxDelay, cTaxTimeout, cTaxReq, cTaxLocation, handleExistingUsers, handleBillingMethod, couponSection = 'div#s2member-pro-paypal-sp-checkout-form-coupon-section', couponApplyButton = couponSection + ' input#s2member-pro-paypal-sp-checkout-coupon-apply', registrationSection = 'div#s2member-pro-paypal-sp-checkout-form-registration-section', billingMethodSection = 'div#s2member-pro-paypal-sp-checkout-form-billing-method-section', cardType = billingMethodSection + ' input[name="optimizemember_pro_paypal_sp_checkout\[card_type\]"]', billingAddressSection = 'div#s2member-pro-paypal-sp-checkout-form-billing-address-section', $ajaxTaxDiv = $(billingAddressSection + ' > div#s2member-pro-paypal-sp-checkout-form-ajax-tax-div'), captchaSection = 'div#s2member-pro-paypal-sp-checkout-form-captcha-section', submissionSection = 'div#s2member-pro-paypal-sp-checkout-form-submission-section', submissionNonceVerification = submissionSection + ' input#s2member-pro-paypal-sp-checkout-nonce', submissionButton = submissionSection + ' input#s2member-pro-paypal-sp-checkout-submit';
				/**/
				var submitButton = '<input type="submit" id="s2member-pro-paypal-sp-checkout-submit" class="s2member-pro-paypal-submit s2member-pro-paypal-sp-checkout-submit" value="<?php echo c_ws_plugin__optimizemember_utils_strings::esc_js_sq (esc_attr (_x ("Submit Form", "s2member-front", "s2member"))); ?>" tabindex="500" />';
				var submitExpressCheckoutButton = '<input type="image" src="https://www.paypal.com/<?php echo c_ws_plugin__optimizemember_utils_strings::esc_js_sq (esc_attr (_x ("en_US", "s2member-front paypal-button-lang-code", "s2member"))); ?>/i/btn/btn_xpressCheckout.gif" id="s2member-pro-paypal-sp-checkout-submit" class="s2member-pro-paypal-submit s2member-pro-paypal-sp-checkout-submit" value="<?php echo c_ws_plugin__optimizemember_utils_strings::esc_js_sq (esc_attr (_x ("Submit Form", "s2member-front", "s2member"))); ?>" tabindex="500" />';
				/**/
				ws_plugin__optimizemember_animateProcessing($(submissionButton), 'reset'), $(submissionButton).removeAttr ('disabled'), $(couponApplyButton).removeAttr ('disabled');
				/**/
								...
								if (state && country && zip && thisTaxLocation && (!cTaxLocation || cTaxLocation !== thisTaxLocation) && (cTaxLocation = thisTaxLocation))
									{
										...
										/**/
										$ajaxTaxDiv.html ('<div><img src="<?php echo $vars["i"]; ?>/ajax-loader.gif" alt="<?php echo c_ws_plugin__optimizemember_utils_strings::esc_js_sq (esc_attr (_x ("Calculating Sales Tax...", "s2member-front", "s2member"))); ?>" /> <?php echo c_ws_plugin__optimizemember_utils_strings::esc_js_sq (_x ("calculating sales tax...", "s2member-front", "s2member")); ?></div>');
										/**/
										cTaxTimeout = setTimeout( /* Create a new cTaxTimeout with a one second delay. */function()
											{
												cTaxReq = $.post ('<?php echo c_ws_plugin__optimizemember_utils_strings::esc_js_sq (admin_url ("/admin-ajax.php")); ?>', {'action': 'ws_plugin__optimizemember_pro_paypal_ajax_tax', 'ws_plugin__optimizemember_pro_paypal_ajax_tax': '<?php echo c_ws_plugin__optimizemember_utils_strings::esc_js_sq (c_ws_plugin__optimizemember_utils_encryption::encrypt ("ws-plugin--optimizemember-pro-paypal-ajax-tax")); ?>', 'ws_plugin__optimizemember_pro_paypal_ajax_tax_vars[attr]': attr, 'ws_plugin__optimizemember_pro_paypal_ajax_tax_vars[state]': state, 'ws_plugin__optimizemember_pro_paypal_ajax_tax_vars[country]': country, 'ws_plugin__optimizemember_pro_paypal_ajax_tax_vars[zip]': zip}, function(response)
													{
				...
				$spForm.submit ( /* Form validation. */function()
					{
						...
						/**/
						$(submissionButton).attr (disabled), ws_plugin__optimizemember_animateProcessing($(submissionButton)), $(couponApplyButton).attr (disabled);
						/**/
						return true;
					});
			}
								...
								if (state && country && zip && thisTaxLocation && (!cTaxLocation || cTaxLocation !== thisTaxLocation) && (cTaxLocation = thisTaxLocation))
									{
										...
										cTaxTimeout = setTimeout( /* Create a new cTaxTimeout with a one second delay. */function()
											{
												cTaxReq = $.post ('<?php echo c_ws_plugin__optimizemember_utils_strings::esc_js_sq (admin_url ("/admin-ajax.php")); ?>', {'action': 'ws_plugin__optimizemember_pro_paypal_ajax_tax', 'ws_plugin__optimizemember_pro_paypal_ajax_tax': '<?php echo c_ws_plugin__optimizemember_utils_strings::esc_js_sq (c_ws_plugin__optimizemember_utils_encryption::encrypt ("ws-plugin--optimizemember-pro-paypal-ajax-tax")); ?>', 'ws_plugin__optimizemember_pro_paypal_ajax_tax_vars[attr]': attr, 'ws_plugin__optimizemember_pro_paypal_ajax_tax_vars[state]': state, 'ws_plugin__optimizemember_pro_paypal_ajax_tax_vars[country]': country, 'ws_plugin__optimizemember_pro_paypal_ajax_tax_vars[zip]': zip}, function(response, textStatus)
													{
														clearTimeout /* Clear cTaxTimeout. */(cTaxTimeout), cTaxTimeout = null;
														/**/
														try /* Try/catch here. jQuery will sometimes return a successful response in IE, whenever the connection is aborted with a null response. */
						...
						if ($(submissionSection + ' input#s2member-pro-paypal-checkout-password-not-required-or-not-possible').length)
							{
								...
								$(registrationSection + ' > div#s2member-pro-paypal-checkout-form-password-div :input').attr (ariaFalseReadonly);
							}
					}) ();
				/**/
				(handleExistingUsers = /* eventTrigger is passed by jQuery for DOM events. */ function(eventTrigger)
						...
						if /* If User/Member is already logged in. */ (OPTIMIZEMEMBER_CURRENT_USER_IS_LOGGED_IN)
							{
								...
								/**/
								$(registrationSection + ' input#s2member-pro-paypal-checkout-email').val (OPTIMIZEMEMBER_CURRENT_USER_EMAIL).attr (ariaFalseReadonly);
								$(registrationSection + ' input#s2member-pro-paypal-checkout-username').val (OPTIMIZEMEMBER_CURRENT_USER_LOGIN).attr (ariaFalseReadonly);
								/**/
								$(registrationSection + ' > div#s2member-pro-paypal-checkout-form-password-div').hide ();
								$(registrationSection + ' > div#s2member-pro-paypal-checkout-form-password-div :input').attr (ariaFalseReadonly);
								/**/
								if ($.trim ($(registrationSection + ' > div#s2member-pro-paypal-checkout-form-registration-section-title').html ()) === '<?php echo c_ws_plugin__optimizemember_utils_strings::esc_js_sq (_x ("Create Profile", "s2member-front", "s2member")); ?>')
									$(registrationSection + ' > div#s2member-pro-paypal-checkout-form-registration-section-title').html ('<?php echo c_ws_plugin__optimizemember_utils_strings::esc_js_sq (_x ("Your Profile", "s2member-front", "s2member")); ?>');
								/**/
								$(customFieldsSection).hide (), $(customFieldsSection + ' :input').attr (ariaFalseReadonly);
							}
					}) ();
				/**/
				(handleBillingMethod = /* eventTrigger is passed by jQuery for DOM events. */ function(eventTrigger)
