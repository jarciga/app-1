...
/**/
if (!class_exists ("c_ws_plugin__optimizemember_utils_encryption"))
	{
		/**
		* Encryption utilities.
		*
		...
		* @since 3.5
		*/
		class c_ws_plugin__optimizemember_utils_encryption
			{
				/**
				...
				* @since 111106
				*
				* @param str $key Optional. Attempt to force a specific Key. Defaults to the one configured for optimizeMember. Short of that, defaults to: ``wp_salt()``.
				...
				*/
				public static function key ($key = FALSE)
					{
						$key = (!is_string ($key) || !strlen ($key)) ? $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["sec_encryption_key"] : $key;
						...
						if (function_exists ("mcrypt_encrypt") && in_array ("rijndael-256", mcrypt_list_algorithms ()) && in_array ("cbc", mcrypt_list_modes ()))
							{
								...
								/**/
								$key = /* Obtain encryption/decryption key. */ c_ws_plugin__optimizemember_utils_encryption::key ($key);
				...
				public static function xencrypt ($string = FALSE, $key = FALSE, $w_md5_cs = TRUE)
					{
						...
						/**/
						return (strlen ($e)) ? ($base64 = c_ws_plugin__optimizemember_utils_strings::base64_url_safe_encode ($e)) : "";
					}
				/**
				* XOR two-way encryption/decryption, with a base64 wrapper.
				*
