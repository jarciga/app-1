...
/**/
if(!class_exists("c_ws_plugin__optimizemember_users_list_in"))
	{
		/**
		* Users list ( inner processing routines ).
		*
		...
		* @since 3.5
		*/
		class c_ws_plugin__optimizemember_users_list_in
			{
				/**
				...
				public static function users_list_edit_cols($user = FALSE)
					{
						...
						if(is_object($user) && !empty($user->ID) && ($user_id = $user->ID) && is_object($current_user) && !empty($current_user->ID))
							{
								$role = c_ws_plugin__optimizemember_user_access::user_access_role($user); /* This User's current WordPress Role. */
								$level = c_ws_plugin__optimizemember_user_access::user_access_level($user); /* User's Access Level for optimizeMember. */
								/**/
								if(current_user_can("edit_users") && (!is_multisite() || is_super_admin() || is_user_member_of_blog($user_id)))
									{
										...
										unset($__refs, $__v); /* Unset defined __refs, __v. */
										/**/
										if($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["custom_reg_fields"]) /* Only if configured. */
											if($fields_applicable = c_ws_plugin__optimizemember_custom_reg_fields::custom_fields_configured_at_level($level, "administrative"))
												{
													...
													echo '</tr>'."\n";
													/**/
													$fields = get_user_option("optimizemember_custom_fields", $user_id); /* Existing fields. */
													/**/
													eval('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
								...
								/**/
									...
									{
										if /* Only if configured. */($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["custom_reg_fields"])
						...
						if(is_object($user) && !empty($user->ID) && ($user_id = $user->ID) && is_object($current_user) && !empty($current_user->ID))
							{
								if(current_user_can("edit_users") && (!is_multisite() || is_super_admin() || is_user_member_of_blog($user_id)))
										...
										if(!empty($_POST) && is_array($_p = c_ws_plugin__optimizemember_utils_strings::trim_deep(stripslashes_deep($_POST))))
											{
												...
												$level = /* Might be `-1`. */ c_ws_plugin__optimizemember_user_access::user_access_role_to_level($role);
												/**/
												...
												/**/
												if($role !== $old_role) /* In this case, we need to fire Hook: `ws_plugin__optimizemember_during_collective_mods`. */
														...
														foreach(json_decode($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["custom_reg_fields"], true) as $field)
															{
																...
																$field_id_class = preg_replace("/_/", "-", $field_var);
																/**/
																if /* Field being set? */(isset($_p["ws_plugin__optimizemember_profile_".$field_var]))
																	{
												...
												else delete_user_option($user_id, "optimizemember_custom_fields");
												/**/
												...
												if /* Should we process List Servers? */(!empty($_p["ws_plugin__optimizemember_profile_opt_in"]) && !empty($role) && $level >= 0)
												...
												/**/
												if /* Delete/reset IP Restrictions? */(!empty($_p["ws_plugin__optimizemember_profile_ip_restrictions"]))
								...
								else if /* Otherwise, a User can always edit their own Profile. */($current_user->ID === $user->ID)
									{
										if(!empty($_POST) && is_array($_p = c_ws_plugin__optimizemember_utils_strings::trim_deep(stripslashes_deep($_POST))))
											{
												$role = /* Role is NOT changing here. */ c_ws_plugin__optimizemember_user_access::user_access_role($user);
															...
															foreach(json_decode($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["custom_reg_fields"], true) as $field)
																{
																	...
																	else if(isset($_p["ws_plugin__optimizemember_profile_".$field_var]))
																		{
																			if((is_array($_p["ws_plugin__optimizemember_profile_".$field_var]) && !empty($_p["ws_plugin__optimizemember_profile_".$field_var])) || (is_string($_p["ws_plugin__optimizemember_profile_".$field_var]) && strlen($_p["ws_plugin__optimizemember_profile_".$field_var])))
																				$fields[$field_var] = $_p["ws_plugin__optimizemember_profile_".$field_var];
																			else unset($fields[$field_var]);
