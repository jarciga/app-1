...
if(!class_exists('c_ws_plugin__optimizemember_paypal_notify_in_web_accept_sp'))
{
	/**
	 * optimizeMember's PayPal IPN handler (inner processing routine).
	 *
	 ...
	 * @since 110720
	 */
	class c_ws_plugin__optimizemember_paypal_notify_in_web_accept_sp
	{
		/**
		 * optimizeMember's PayPal IPN handler (inner processing routine).
		 *
		...
		public static function cp($vars = array()) // Conditional phase for ``c_ws_plugin__optimizemember_paypal_notify_in::paypal_notify()``.
		{
			...
			if((!empty($paypal['txn_type']) && preg_match('/^web_accept$/i', $paypal['txn_type']))
			   && (!empty($paypal['item_number']) && preg_match($GLOBALS['WS_PLUGIN__']['optimizemember']['c']['sp_access_item_number_regex'], $paypal['item_number']))
			...
			)
			{
				...
				if(!get_transient($transient_ipn = 's2m_ipn_'.md5('optimizemember_transient_'.$_paypal_s)) && set_transient($transient_ipn, time(), 31556926 * 10))
				{
					...
					if(($sp_access_url = c_ws_plugin__optimizemember_sp_access::sp_access_link_gen($paypal['sp_ids'], $paypal['hours'])) && is_array($cv = preg_split('/\|/', $paypal['custom'])))
					{
						...
						$sbj = preg_replace('/%%sp_access_url%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($sp_access_url), $GLOBALS['WS_PLUGIN__']['optimizemember']['o'][(($_REQUEST['optimizemember_paypal_proxy'] && preg_match('/pro-emails/', $_REQUEST['optimizemember_paypal_proxy_use'])) ? 'pro_' : '').'sp_email_subject']);
						$sbj = preg_replace('/%%sp_access_exp%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(c_ws_plugin__optimizemember_utils_time::approx_time_difference(time(), strtotime('+'.$paypal['hours'].' hours'))), $sbj);

						$msg = preg_replace('/%%sp_access_url%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($sp_access_url), $GLOBALS['WS_PLUGIN__']['optimizemember']['o'][(($_REQUEST['optimizemember_paypal_proxy'] && preg_match('/pro-emails/', $_REQUEST['optimizemember_paypal_proxy_use'])) ? 'pro_' : '').'sp_email_message']);
						$msg = preg_replace('/%%sp_access_exp%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(c_ws_plugin__optimizemember_utils_time::approx_time_difference(time(), strtotime('+'.$paypal['hours'].' hours'))), $msg);

						$rec = preg_replace('/%%sp_access_url%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($sp_access_url), $GLOBALS['WS_PLUGIN__']['optimizemember']['o'][(($_REQUEST['optimizemember_paypal_proxy'] && preg_match('/pro-emails/', $_REQUEST['optimizemember_paypal_proxy_use'])) ? 'pro_' : '').'sp_email_recipients']);
						$rec = preg_replace('/%%sp_access_exp%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(c_ws_plugin__optimizemember_utils_time::approx_time_difference(time(), strtotime('+'.$paypal['hours'].' hours'))), $rec);

						if(($rec = c_ws_plugin__optimizemember_utils_strings::fill_cvs($rec, $paypal['custom'])) && ($rec = preg_replace('/%%txn_id%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['txn_id']), $rec)))
							if(($rec = preg_replace('/%%amount%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['mc_gross']), $rec))) // Full amount of the payment, before fee is subtracted.
								if(($rec = preg_replace('/%%currency%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['currency']), $rec)) && ($rec = preg_replace('/%%currency_symbol%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['currency_symbol']), $rec)))
									if(($rec = preg_replace('/%%txn_baid%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['txn_baid']), $rec)) && ($rec = preg_replace('/%%txn_cid%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['txn_cid']), $rec)))
													...
													if(($rec = preg_replace('/%%payer_email%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['payer_email']), $rec)))
														if(($rec = preg_replace('/%%user_ip%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['ip']), $rec)))
															if(($rec = preg_replace('/%%full_coupon_code%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($coupon['full_coupon_code']), $rec)) && ($rec = preg_replace('/%%coupon_code%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($coupon['coupon_code']), $rec)) && ($rec = preg_replace('/%%coupon_affiliate_id%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($coupon['affiliate_id']), $rec)))
																...
																if(($sbj = c_ws_plugin__optimizemember_utils_strings::fill_cvs($sbj, $paypal['custom'])) && ($sbj = preg_replace('/%%txn_id%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['txn_id']), $sbj)))
																	if(($sbj = preg_replace('/%%amount%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['mc_gross']), $sbj))) // Full amount of the payment, before fee is subtracted.
																		if(($sbj = preg_replace('/%%currency%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['currency']), $sbj)) && ($sbj = preg_replace('/%%currency_symbol%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['currency_symbol']), $sbj)))
																			if(($sbj = preg_replace('/%%txn_baid%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['txn_baid']), $sbj)) && ($sbj = preg_replace('/%%txn_cid%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['txn_cid']), $sbj)))
																							...
																							if(($sbj = preg_replace('/%%payer_email%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['payer_email']), $sbj)))
																								if(($sbj = preg_replace('/%%user_ip%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['ip']), $sbj)))
																									if(($sbj = preg_replace('/%%full_coupon_code%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($coupon['full_coupon_code']), $sbj)) && ($sbj = preg_replace('/%%coupon_code%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($coupon['coupon_code']), $sbj)) && ($sbj = preg_replace('/%%coupon_affiliate_id%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($coupon['affiliate_id']), $sbj)))
																										...
																										if(($msg = c_ws_plugin__optimizemember_utils_strings::fill_cvs($msg, $paypal['custom'])) && ($msg = preg_replace('/%%txn_id%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['txn_id']), $msg)))
																											if(($msg = preg_replace('/%%amount%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['mc_gross']), $msg))) // Full amount of the payment, before fee is subtracted.
																												if(($msg = preg_replace('/%%currency%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['currency']), $msg)) && ($msg = preg_replace('/%%currency_symbol%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['currency_symbol']), $msg)))
																													if(($msg = preg_replace('/%%txn_baid%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['txn_baid']), $msg)) && ($msg = preg_replace('/%%txn_cid%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['txn_cid']), $msg)))
						...
						if($processing && $GLOBALS['WS_PLUGIN__']['optimizemember']['o']['sp_sale_notification_urls'] && is_array($cv = preg_split('/\|/', $paypal['custom'])))
						{
							foreach(preg_split('/['."\r\n\t".']+/', $GLOBALS['WS_PLUGIN__']['optimizemember']['o']['sp_sale_notification_urls']) as $url)
								...
								if(($url = c_ws_plugin__optimizemember_utils_strings::fill_cvs($url, $paypal['custom'], true)) && ($url = preg_replace('/%%sp_access_url%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(rawurlencode($sp_access_url)), $url)))
									if(($url = preg_replace('/%%sp_access_exp%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode(c_ws_plugin__optimizemember_utils_time::approx_time_difference(time(), strtotime('+'.$paypal['hours'].' hours')))), $url)))
										if(($url = preg_replace('/%%currency%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($paypal['currency'])), $url)) && ($url = preg_replace('/%%currency_symbol%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($paypal['currency_symbol'])), $url)))
											if(($url = preg_replace('/%%amount%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($paypal['mc_gross'])), $url)) && ($url = preg_replace('/%%txn_id%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($paypal['txn_id'])), $url)))
												if(($url = preg_replace('/%%txn_baid%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($paypal['txn_baid'])), $url)) && ($url = preg_replace('/%%txn_cid%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($paypal['txn_cid'])), $url)))
						...
						if($processing && $GLOBALS['WS_PLUGIN__']['optimizemember']['o']['sp_sale_notification_recipients'] && is_array($cv = preg_split('/\|/', $paypal['custom'])))
						{
							...
							if(($msg = c_ws_plugin__optimizemember_utils_strings::fill_cvs($msg, $paypal['custom'])) && ($msg = preg_replace('/%%sp_access_url%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($sp_access_url), $msg)))
								if(($msg = preg_replace('/%%sp_access_exp%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(c_ws_plugin__optimizemember_utils_time::approx_time_difference(time(), strtotime('+'.$paypal['hours'].' hours'))), $msg)))
									if(($msg = preg_replace('/%%currency%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['currency']), $msg)) && ($msg = preg_replace('/%%currency_symbol%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['currency_symbol']), $msg)))
						...
						{
							if(($code = c_ws_plugin__optimizemember_utils_strings::fill_cvs($code, $paypal['custom'])) && ($code = preg_replace('/%%amount%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['mc_gross']), $code)) && ($code = preg_replace('/%%txn_id%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['txn_id']), $code)))
								if(($code = preg_replace('/%%currency%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['currency']), $code)) && ($code = preg_replace('/%%currency_symbol%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['currency_symbol']), $code)))
									if(($code = preg_replace('/%%txn_baid%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['txn_baid']), $code)) && ($code = preg_replace('/%%txn_cid%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['txn_cid']), $code)))
					...
					else $paypal['optimizemember_log'][] = 'Unable to generate Access Link for Specific Post/Page Access. Does your Leading Post/Page still exist?';
				}
			...
			else return apply_filters('c_ws_plugin__optimizemember_paypal_notify_in_web_accept_sp', FALSE, get_defined_vars());
		}
	}
}