...
if(!class_exists('c_ws_plugin__optimizemember_paypal_notify_in_wa_ccaps_wo_level'))
{
	/**
	 * optimizeMember's PayPal IPN handler (inner processing routine).
	 *
	 ...
	 * @since 110815
	 */
	class c_ws_plugin__optimizemember_paypal_notify_in_wa_ccaps_wo_level
	{
		/**
		 * optimizeMember's PayPal IPN handler (inner processing routine).
		 *
		...
		public static function cp($vars = array()) // Conditional phase for ``c_ws_plugin__optimizemember_paypal_notify_in::paypal_notify()``.
		{
			...
			if((!empty($paypal['txn_type']) && preg_match('/^web_accept$/i', $paypal['txn_type']))
			   && (!empty($paypal['item_number']) && preg_match($GLOBALS['WS_PLUGIN__']['optimizemember']['c']['membership_item_number_wo_level_regex'], $paypal['item_number']))
					...
					if(preg_match('/(referenc|associat|updat|upgrad)/i', $paypal['option_name1']) && $paypal['option_selection1']) // Must have this information for Capability additions.
					{
						if(($user_id = c_ws_plugin__optimizemember_utils_users::get_user_id_with($paypal['txn_id'], $paypal['option_selection1'])) && is_object($user = new WP_User ($user_id)) && $user->ID)
						{
							if(!$user->has_cap('administrator')) // Do NOT process this routine on Administrators.
							{
								...
								$sbj = $GLOBALS['WS_PLUGIN__']['optimizemember']['o']['ccap_email_subject']; // The same for standard and w/ Pro-Forms.
								$msg = $GLOBALS['WS_PLUGIN__']['optimizemember']['o']['ccap_email_message']; // The same for standard and w/ Pro-Forms.
								$rec = $GLOBALS['WS_PLUGIN__']['optimizemember']['o']['ccap_email_recipients']; // The same for standard and w/ Pro-Forms.

								if(($rec = c_ws_plugin__optimizemember_utils_strings::fill_cvs($rec, $paypal['custom'])) && ($rec = preg_replace('/%%(?:subscr|txn)_id%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['txn_id']), $rec)))
									if(($rec = preg_replace('/%%(?:subscr|txn)_baid%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['txn_baid']), $rec)) && ($rec = preg_replace('/%%(?:subscr|txn)_cid%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['txn_cid']), $rec)))
										if(($rec = preg_replace('/%%currency%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['currency']), $rec)) && ($rec = preg_replace('/%%currency_symbol%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['currency_symbol']), $rec)))
											if(($rec = preg_replace('/%%amount%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['mc_gross']), $rec)) && ($rec = preg_replace('/%%txn_id%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['txn_id']), $rec)))
																				...
																				if(($rec = preg_replace('/%%user_login%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($user->user_login), $rec)))
																					if(($rec = preg_replace('/%%user_ip%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($user_reg_ip), $rec)))
																						if(($rec = preg_replace('/%%user_id%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($user_id), $rec)))
																							...
																							if(($sbj = c_ws_plugin__optimizemember_utils_strings::fill_cvs($sbj, $paypal['custom'])) && ($sbj = preg_replace('/%%(?:subscr|txn)_id%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['txn_id']), $sbj)))
																								if(($sbj = preg_replace('/%%(?:subscr|txn)_baid%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['txn_baid']), $sbj)) && ($sbj = preg_replace('/%%(?:subscr|txn)_cid%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['txn_cid']), $sbj)))
																									if(($sbj = preg_replace('/%%currency%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['currency']), $sbj)) && ($sbj = preg_replace('/%%currency_symbol%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['currency_symbol']), $sbj)))
																										if(($sbj = preg_replace('/%%amount%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['mc_gross']), $sbj)) && ($sbj = preg_replace('/%%txn_id%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['txn_id']), $sbj)))
																																			...
																																			if(($sbj = preg_replace('/%%user_login%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($user->user_login), $sbj)))
																																				if(($sbj = preg_replace('/%%user_ip%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($user_reg_ip), $sbj)))
																																					if(($sbj = preg_replace('/%%user_id%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($user_id), $sbj)))
																																						...
																																						if(($msg = c_ws_plugin__optimizemember_utils_strings::fill_cvs($msg, $paypal['custom'])) && ($msg = preg_replace('/%%(?:subscr|txn)_id%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['txn_id']), $msg)))
																																							if(($msg = preg_replace('/%%(?:subscr|txn)_baid%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['txn_baid']), $msg)) && ($msg = preg_replace('/%%(?:subscr|txn)_cid%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['txn_cid']), $msg)))
																																																				...
																																																				if(($msg = preg_replace('/%%user_id%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($user_id), $msg)))
																																																				{
																																																					...
																																																					if(($rec = trim(preg_replace('/%%(.+?)%%/i', '', $rec))) && ($sbj = trim(preg_replace('/%%(.+?)%%/i', '', $sbj))) && ($msg = trim(preg_replace('/%%(.+?)%%/i', '', $msg))))
																																																					{
																																																						...
																																																						foreach(c_ws_plugin__optimizemember_utils_strings::parse_emails($rec) as $recipient  /* Go through a possible list of recipients. */)
																																																							c_ws_plugin__optimizemember_email_configs::email_config().wp_mail($recipient, apply_filters('ws_plugin__optimizemember_capabilities_email_sbj', $sbj, get_defined_vars()), apply_filters('ws_plugin__optimizemember_capabilities_email_msg', $msg, get_defined_vars()), 'From: "'.preg_replace('/"/', '"', $GLOBALS['WS_PLUGIN__']['optimizemember']['o']['reg_email_from_name']).'" <'.$GLOBALS['WS_PLUGIN__']['optimizemember']['o']['reg_email_from_email'].'>'."\r\n".'Content-Type: text/plain; charset=UTF-8').c_ws_plugin__optimizemember_email_configs::email_config_release();

																																																						$paypal['optimizemember_log'][] = 'Capability Confirmation Email sent to: '.$rec.'.';
																																																					}
																																																				}
								...
								{
									if(($url = c_ws_plugin__optimizemember_utils_strings::fill_cvs($url, $paypal['custom'], true)) && ($url = preg_replace('/%%(?:subscr|txn)_id%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($paypal['txn_id'])), $url)))
										if(($url = preg_replace('/%%(?:subscr|txn)_baid%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($paypal['txn_baid'])), $url)) && ($url = preg_replace('/%%(?:subscr|txn)_cid%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($paypal['txn_cid'])), $url)))
								...
								{
									if(($code = c_ws_plugin__optimizemember_utils_strings::fill_cvs($code, $paypal['custom'])) && ($code = preg_replace('/%%(?:subscr|txn)_id%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['txn_id']), $code)))
										if(($code = preg_replace('/%%(?:subscr|txn)_baid%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['txn_baid']), $code)) && ($code = preg_replace('/%%(?:subscr|txn)_cid%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['txn_cid']), $code)))
											if(($code = preg_replace('/%%currency%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['currency']), $code)) && ($code = preg_replace('/%%currency_symbol%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['currency_symbol']), $code)))
												if(($code = preg_replace('/%%amount%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['mc_gross']), $code)) && ($code = preg_replace('/%%txn_id%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['txn_id']), $code)))
							...
							else $paypal['optimizemember_log'][] = 'Unable to add new Capabilities. The existing User ID is associated with an Administrator. Stopping here. Otherwise, an Administrator could lose access.';
						}
						else $paypal['optimizemember_log'][] = 'Unable to add new Capabilities. Could not get the existing User ID from the DB. Please check the `on0` and `os0` variables in your Button Code.';
					}
					else $paypal['optimizemember_log'][] = 'Unable to add new Capabilities. Missing User/Member details. Please check the `on0` and `os0` variables in your Button Code.';
				}
			...
			else return apply_filters('c_ws_plugin__optimizemember_paypal_notify_in_wa_ccaps_wo_level', FALSE, get_defined_vars());
		}
	}
}