...
/**/
if(!class_exists("c_ws_plugin__optimizemember_paypal_utilities"))
	{
		/**
		* PayPal utilities.
		*
		...
		* @since 3.5
		*/
		class c_ws_plugin__optimizemember_paypal_utilities
			{
				/**
				...
				public static function paypal_postvars()
					{
						...
						do_action("ws_plugin__optimizemember_before_paypal_postvars", get_defined_vars());
						unset($__refs, $__v); /* Unset defined __refs, __v. */
						/*
						Custom conditionals can be applied by filters.
						*/
						eval('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
						if(!($postvars = apply_filters("ws_plugin__optimizemember_during_paypal_postvars_conditionals", array(), get_defined_vars())))
							{
								unset($__refs, $__v); /* Unset defined __refs, __v. */
								/**/
								if(!empty($_GET["tx"]) && empty($_GET["optimizemember_paypal_proxy"]))
									{
										$postback["tx"] = $_GET["tx"];
										$postback["cmd"] = "_notify-synch";
										$postback["at"] = $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["paypal_identity_token"];
										/**/
										$endpoint = ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["paypal_sandbox"]) ? "www.sandbox.paypal.com" : "www.paypal.com";
										/**/
										if(preg_match("/^SUCCESS/i", ($response = trim(c_ws_plugin__optimizemember_utils_urls::remote("https://".$endpoint."/cgi-bin/webscr", $postback, array("timeout" => 20))))))
											{
												...
												/**/
												return apply_filters("ws_plugin__optimizemember_paypal_postvars", $postvars, get_defined_vars());
											}
										else /* Nope. */
											return false;
									}
								else if(!empty($_REQUEST) && is_array($postvars = stripslashes_deep($_REQUEST)))
									{
										...
										foreach($postvars as $key => $value)
											if(preg_match("/^optimizemember_/", $key))
												unset($postvars[$key]);
										/**/
										$postback = $postvars; /* Copy. */
										$postback["cmd"] = "_notify-validate";
										/**/
										$postvars = c_ws_plugin__optimizemember_utils_strings::trim_deep($postvars);
										/**/
										if(!empty($postvars["charset"]) && function_exists("mb_convert_encoding"))
											{
												foreach($postvars as &$value)
										...
										$endpoint = ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["paypal_sandbox"]) ? "www.sandbox.paypal.com" : "www.paypal.com";
										/**/
										if(!empty($_REQUEST["optimizemember_paypal_proxy"]) && !empty($_REQUEST["optimizemember_paypal_proxy_verification"]) && $_REQUEST["optimizemember_paypal_proxy_verification"] === c_ws_plugin__optimizemember_paypal_utilities::paypal_proxy_key_gen())
											return apply_filters("ws_plugin__optimizemember_paypal_postvars", array_merge($postvars, array("proxy_verified" => $_REQUEST["optimizemember_paypal_proxy"])), get_defined_vars());
										/**/
										else if(empty($_POST) && !empty($_GET["optimizemember_paypal_proxy"]) && !empty($_GET["optimizemember_paypal_proxy_verification"]) && c_ws_plugin__optimizemember_utils_urls::optimizemember_sig_ok($_SERVER["REQUEST_URI"], false, false, "optimizemember_paypal_proxy_verification")) {
											return apply_filters("ws_plugin__optimizemember_paypal_postvars", array_merge($postvars, array("proxy_verified" => $_GET["optimizemember_paypal_proxy"])), get_defined_vars());
										}
										/**/
										else if(trim(strtolower(c_ws_plugin__optimizemember_utils_urls::remote("https://".$endpoint."/cgi-bin/webscr", $postback, array("timeout" => 20)))) === "verified") {
											return apply_filters("ws_plugin__optimizemember_paypal_postvars", $postvars, get_defined_vars());
										}
										/**/
										else /* Nope. */ {
				...
				public static function paypal_api_response($post_vars = FALSE)
					{
						...
						/**/
						eval('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
						do_action("ws_plugin__optimizemember_before_paypal_api_response", get_defined_vars());
						unset /* Unset defined __refs, __v. */($__refs, $__v);
						/**/
						$url = "https://".(($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["paypal_sandbox"]) ? "api-3t.sandbox.paypal.com" : "api-3t.paypal.com")."/nvp";
						/**/
						$post_vars = apply_filters("ws_plugin__optimizemember_paypal_api_post_vars", $post_vars, get_defined_vars());
						$post_vars = (is_array($post_vars)) ? $post_vars : array();
						/**/
						$post_vars["VERSION"] = /* Configure the PayPal API version. */ "71.0";
						$post_vars["USER"] = $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["paypal_api_username"];
						$post_vars["PWD"] = $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["paypal_api_password"];
						$post_vars["SIGNATURE"] = $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["paypal_api_signature"];
						/**/
						foreach($post_vars as $_key => &$_value /* We need to clean these up. */)
							$_value = c_ws_plugin__optimizemember_paypal_utilities::paypal_api_nv_cleanup($_key, $_value);
						unset($_key, $_value);
						/**/
						$input_time = /* Record input/nvp for logging. */ date("D M j, Y g:i:s a T");
						/**/
						$nvp = trim(c_ws_plugin__optimizemember_utils_urls::remote($url, $post_vars, array("timeout" => 20)));
						/**/
						...
						if(!$response["ACK"] || !preg_match("/^(Success|SuccessWithWarning)$/i", $response["ACK"]))
							{
								if(strlen($response["L_ERRORCODE0"]) || $response["L_SHORTMESSAGE0"] || $response["L_LONGMESSAGE0"])
									/* translators: Exclude `%2$s` and `%3$s`. These are English details returned by PayPal. Replace `%2$s` and `%3$s` with: `Unable to process, please try again`, or something to that affect. Or, if you prefer, you could Filter ``$response["__error"]`` with `ws_plugin__optimizemember_paypal_api_response`. */
						...
						/**/
						return apply_filters("ws_plugin__optimizemember_paypal_api_response", c_ws_plugin__optimizemember_paypal_utilities::_paypal_api_response_filters($response), get_defined_vars());
					}
				/**
				...
				*/
				public static function _paypal_api_response_filters($response = FALSE)
					{
						eval('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
						do_action("_ws_plugin__optimizemember_before_paypal_api_response_filters", get_defined_vars());
						unset($__refs, $__v); /* Unset defined __refs, __v. */
						/**/
						...
						/**/
						return $response; /* Filters already applied with: ``ws_plugin__optimizemember_paypal_api_response``. */
					}
				/**
				...
				public static function paypal_api_nv_cleanup($key = FALSE, $value = FALSE)
					{
						...
						/**/
						return apply_filters("ws_plugin__optimizemember_paypal_api_nv_cleanup", $value, get_defined_vars());
					}
				/**
				...
				*/
				public static function _paypal_payflow_api_response_filters($response = FALSE)
					{
						eval('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
						do_action("_ws_plugin__optimizemember_before_paypal_payflow_api_response_filters", get_defined_vars());
						unset($__refs, $__v); /* Unset defined __refs, __v. */
						/**/
						if(!empty($response["__error"]) && !empty($response["L_ERRORCODE0"]))
							{
						...
						/**/
						return $response; /* Filters already applied with: ``ws_plugin__optimizemember_paypal_payflow_api_response``. */
					}
				/**
				...
				public static function paypal_payflow_api_nv_cleanup($key = FALSE, $value = FALSE)
					{
						...
						|| preg_match("/^L_BILLINGAGREEMENTDESCRIPTION[0-9]+$/", $key)) && strlen($value) > 60)
							$value = substr($value, 0, 57)."...";
						/**/
						return apply_filters("ws_plugin__optimizemember_paypal_payflow_api_nv_cleanup", $value, get_defined_vars());
					}
				/**
				...
				*/
				public static function paypal_pro_term($term = FALSE)
					{
						eval('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
						do_action("ws_plugin__optimizemember_before_paypal_pro_term", get_defined_vars());
						unset($__refs, $__v); /* Unset defined __refs, __v. */
						/**/
						...
						/**/
						return apply_filters("ws_plugin__optimizemember_paypal_pro_term", $pro_term, get_defined_vars());
					}
				/**
				...
				*/
				public static function paypal_payflow_term($term = FALSE, $period = FALSE)
					{
						eval('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
						do_action("ws_plugin__optimizemember_before_paypal_payflow_term", get_defined_vars());
						unset($__refs, $__v); /* Unset defined __refs, __v. */
						/**/
						...
						/**/
							...
							$payflow_term = "SMYR";
						/**/
						return apply_filters("ws_plugin__optimizemember_paypal_payflow_term", $payflow_term, get_defined_vars());
					}
				/**
				...
				*/
				public static function paypal_std_term($term = FALSE)
					{
						eval('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
						do_action("ws_plugin__optimizemember_before_paypal_std_term", get_defined_vars());
						unset($__refs, $__v); /* Unset defined __refs, __v. */
						/**/
						...
						/**/
						return apply_filters("ws_plugin__optimizemember_paypal_std_term", $std_term, get_defined_vars());
					}
				/**
				...
				*/
				public static function paypal_pro_subscr_id($array_or_string = FALSE)
					{
						eval('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
						do_action("ws_plugin__optimizemember_before_paypal_pro_subscr_id", get_defined_vars());
						unset($__refs, $__v); /* Unset defined __refs, __v. */
						/**/
						...
						/**/
						return apply_filters("ws_plugin__optimizemember_paypal_pro_subscr_id", ((!empty($subscr_id)) ? $subscr_id : false), get_defined_vars());
					}
				/**
				...
				*/
				public static function paypal_pro_item_number($array_or_string = FALSE)
					{
						eval('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
						do_action("ws_plugin__optimizemember_before_paypal_pro_item_number", get_defined_vars());
						unset($__refs, $__v); /* Unset defined __refs, __v. */
						/**/
						...
						/**/
						if(!empty($_item_number) && preg_match($GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["membership_item_number_w_or_wo_level_regex"], $_item_number))
							$item_number = $_item_number;
						/**/
						else if(!empty($_item_number) && preg_match($GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["sp_access_item_number_regex"], $_item_number))
							$item_number = $_item_number;
						/**/
						return apply_filters("ws_plugin__optimizemember_paypal_pro_item_number", ((!empty($item_number)) ? $item_number : false), get_defined_vars());
					}
				/**
				...
				*/
				public static function paypal_pro_item_name($array_or_string = FALSE)
					{
						eval('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
						do_action("ws_plugin__optimizemember_before_paypal_pro_item_name", get_defined_vars());
						unset($__refs, $__v); /* Unset defined __refs, __v. */
						/**/
						...
						/**/
						return apply_filters("ws_plugin__optimizemember_paypal_pro_item_name", ((!empty($item_name)) ? $item_name : false), get_defined_vars());
					}
				/**
				...
				*/
				public static function paypal_pro_period1($array_or_string = FALSE, $default = "0 D")
					{
						eval('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
						do_action("ws_plugin__optimizemember_before_paypal_pro_period1", get_defined_vars());
						unset($__refs, $__v); /* Unset defined __refs, __v. */
						/**/
						if(isset($array_or_string["PROFILENAME"]) /* Payflow API alternative. */)
							$array_or_string["PROFILEREFERENCE"] = $array_or_string["PROFILENAME"];
						...
						if(!empty($_period1)) /* Were we able to get a `period1` string? */
							{
								...
								if(strlen($span) !== 1) /* To Standard format. */
									$span = c_ws_plugin__optimizemember_paypal_utilities::paypal_std_term($span);
								/**/
								...
								/**/
								return apply_filters("ws_plugin__optimizemember_paypal_pro_period1", $period1, get_defined_vars());
							}
						else /* Default. */
							return apply_filters("ws_plugin__optimizemember_paypal_pro_period1", $default, get_defined_vars());
					}
				/**
				...
				*/
				public static function paypal_pro_period3($array_or_string = FALSE, $default = "1 D")
					{
						eval('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
						do_action("ws_plugin__optimizemember_before_paypal_pro_period3", get_defined_vars());
						unset($__refs, $__v); /* Unset defined __refs, __v. */
						/**/
						if(isset($array_or_string["PROFILENAME"]) /* Payflow API alternative. */)
							$array_or_string["PROFILEREFERENCE"] = $array_or_string["PROFILENAME"];
						...
						if(!empty($_period3)) /* Were we able to get a `period3` string? */
							{
								...
								/**/
								return apply_filters("ws_plugin__optimizemember_paypal_pro_period3", $period3, get_defined_vars());
							}
						else /* Default. */
							return apply_filters("ws_plugin__optimizemember_paypal_pro_period3", $default, get_defined_vars());
					}
			}
	}
?>