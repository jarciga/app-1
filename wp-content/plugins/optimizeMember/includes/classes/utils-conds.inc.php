...
/**/
if(!class_exists("c_ws_plugin__optimizemember_utils_conds"))
	{
		/**
		* Conditional utilities.
		*
		...
		* @since 3.5
		*/
		class c_ws_plugin__optimizemember_utils_conds
			{
				/**
				* Determines whether or not optimizeMember Pro is installed.
				*
				* @package optimizeMember\Utilities
				* @since 110720
				*
				* @return bool True if optimizeMember Pro is installed, else false.
				*/
				public static function pro_is_installed()
					{
						return (defined("WS_PLUGIN__OPTIMIZEMEMBER_PRO_VERSION") && did_action("ws_plugin__optimizemember_pro_loaded"));
					}
            ...
            public static function bbp_is_installed($query_active_plugins = NULL)
            {
                ...
                $s2o = (defined('WS_PLUGIN__OPTIMIZEMEMBER_ONLY') && WS_PLUGIN__OPTIMIZEMEMBER_ONLY) ? TRUE : FALSE;
				...
				* @since 110720
				*
				* @param bool $query_active_plugins Optional. If true, this conditional will query active plugins too. Defaults to true if {@link optimizeMember\WS_PLUGIN__OPTIMIZEMEMBER_ONLY} is true, else false.
				...
				*
				* With optimizeMember, this option may also indicate a Multisite Blog Farm.
				* ``$GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["mms_registration_file"] === "wp-signup"``.
				*
				* @package optimizeMember\Utilities
				* @since 3.5
				*
				...
				*/
				public static function is_multisite_farm()
					{
						return (is_multisite() && ((is_main_site() && $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["mms_registration_file"] === "wp-signup") || (defined("MULTISITE_FARM") && MULTISITE_FARM)));
					}
				/**
				...
				*/
				public static function is_site_root($url_uri = FALSE)
					{
						if(is_array($parse = c_ws_plugin__optimizemember_utils_urls::parse_url($url_uri)))
							{
								$parse["path"] = (!empty($parse["path"])) ? ((strpos($parse["path"], "/") === 0) ? $parse["path"] : "/".$parse["path"]) : "/";
								/**/
								if(empty($parse["host"]) || strcasecmp($parse["host"], c_ws_plugin__optimizemember_utils_urls::parse_url(site_url(), PHP_URL_HOST)) === 0)
									if($parse["path"] === "/" || rtrim($parse["path"], "/") === rtrim(c_ws_plugin__optimizemember_utils_urls::parse_url(site_url(), PHP_URL_PATH), "/"))
