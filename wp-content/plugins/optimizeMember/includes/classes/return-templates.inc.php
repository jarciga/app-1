<?php
/**
* optimizeMember's Return Page template handler.
*
* Copyright: © 2009-2011
...
/**/
if (!class_exists ("c_ws_plugin__optimizemember_return_templates"))
	{
		/**
		* optimizeMember's Return Page template handler.
		*
		...
		* @since 110720
		*/
		class c_ws_plugin__optimizemember_return_templates
			{
				/**
				...
				public static function return_template ($template = FALSE, $response = FALSE, $continue_html = FALSE, $continue_link = FALSE)
					{
						...
						$code = preg_replace ("/%%doctype_html_head%%/i", c_ws_plugin__optimizemember_utils_strings::esc_refs (apply_filters ("ws_plugin__optimizemember_return_template_doctype_html_head", $doctype_html_head, get_defined_vars ())), $code);
						/**/
						$code = preg_replace ("/%%header%%/i", c_ws_plugin__optimizemember_utils_strings::esc_refs (apply_filters ("ws_plugin__optimizemember_return_template_header", sprintf (_x ('[ %s ] <strong><em>says&hellip;</em></strong>', "s2member-front", "s2member"), esc_html ($_SERVER["HTTP_HOST"])), get_defined_vars ())), $code);
						/**/
						$code = preg_replace ("/%%response%%/i", c_ws_plugin__optimizemember_utils_strings::esc_refs (apply_filters ("ws_plugin__optimizemember_return_template_response", $response, get_defined_vars ())), $code);
						$code = preg_replace ("/%%continue%%/i", c_ws_plugin__optimizemember_utils_strings::esc_refs (apply_filters ("ws_plugin__optimizemember_return_template_continue", '<a href="' . esc_attr ($continue_link) . '">' . $continue_html . '</a>', get_defined_vars ())), $code);
						$code = preg_replace ("/%%support%%/i", c_ws_plugin__optimizemember_utils_strings::esc_refs (apply_filters ("ws_plugin__optimizemember_return_template_support", sprintf (_x ('If you need assistance, please <a href="%s" target="_blank">contact support</a>.', "s2member-front", "s2member"), esc_attr ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["reg_email_support_link"])), get_defined_vars ())), $code);
						$code = preg_replace ("/%%tracking%%/i", c_ws_plugin__optimizemember_utils_strings::esc_refs (apply_filters ("ws_plugin__optimizemember_return_template_tracking", c_ws_plugin__optimizemember_tracking_codes::generate_all_tracking_codes (), get_defined_vars ())), $code);
						/**/
