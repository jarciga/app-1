<?php
/**
* Installation routines for optimizeMember.
*
* Copyright: © 2009-2011
...
/**/
if(!class_exists("c_ws_plugin__optimizemember_installation"))
	{
		/**
		* Installation routines for optimizeMember.
		*
		...
		* @since 3.5
		*/
		class c_ws_plugin__optimizemember_installation
			{
				/**
				...
				public static function activate($reactivation_reason = FALSE)
					{
						...
						/**/
						c_ws_plugin__optimizemember_roles_caps::config_roles(); /* Config Roles/Caps. */
						...
						/**/
						c_ws_plugin__optimizemember_files::write_no_gzip_into_root_htaccess /* Handle the root `.htaccess` file as well now, for GZIP exclusions. */();
						/**/
						...
						/**/
						if($GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["configured"]) /* If we are re-activating. */
							{
								$v = get_option("ws_plugin__optimizemember_activated_version"); /* Currently. */
								/**/
								...
								if(!$v || !version_compare($v, "3.2.6", ">=")) /* Needs to be upgraded? */
									/* Version 3.2.6 fixed `optimizemember_ccaps_req` being stored empty and/or w/ one empty element in the array. */
									{
								...
								if(!$v || !version_compare($v, "110912", ">=") && $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["filter_wp_query"] === array("all"))
									/* optimizeMember v110912 changed the way the "all" option for Alternative Views was handled. */
									{
								...
								/**/
								if(!is_multisite() || !c_ws_plugin__optimizemember_utils_conds::is_multisite_farm() || is_main_site()) /* No Changelog on a Multisite Blog Farm. */
				...
				public static function deactivate()
					{
						...
						if($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["run_deactivation_routines"])
							{
								c_ws_plugin__optimizemember_roles_caps::unlink_roles(); /* Unlink Roles/Caps. */
								/**/
								c_ws_plugin__optimizemember_files::remove_no_gzip_from_root_htaccess /* Remove GZIP exclusions. */();
								/**/
								if(is_dir($files_dir = $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["files_dir"]))
									{
								...
								/**/
								$wpdb->query("DELETE FROM `".$wpdb->options."` WHERE `option_name` LIKE '%".esc_sql(c_ws_plugin__optimizemember_utils_strings::like_escape("optimizemember_"))."%'");
								$wpdb->query("DELETE FROM `".$wpdb->options."` WHERE `option_name` LIKE '".esc_sql(c_ws_plugin__optimizemember_utils_strings::like_escape("_transient_s2m_"))."%'");
								$wpdb->query("DELETE FROM `".$wpdb->options."` WHERE `option_name` LIKE '".esc_sql(c_ws_plugin__optimizemember_utils_strings::like_escape("_transient_timeout_s2m_"))."%'");
								$wpdb->query("DELETE FROM `".$wpdb->postmeta."` WHERE `meta_key` LIKE '%".esc_sql(c_ws_plugin__optimizemember_utils_strings::like_escape("optimizemember_"))."%'");
								$wpdb->query("DELETE FROM `".$wpdb->usermeta."` WHERE `meta_key` LIKE '%".esc_sql(c_ws_plugin__optimizemember_utils_strings::like_escape("optimizemember_"))."%'");
								/**/
