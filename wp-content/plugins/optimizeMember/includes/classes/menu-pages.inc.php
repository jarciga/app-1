...
/**/
if(!class_exists("c_ws_plugin__optimizemember_menu_pages"))
	{
		/**
		* Administrative menu pages.
		*
		...
		* @since 3.5
		*/
		class c_ws_plugin__optimizemember_menu_pages
			{
				/**
				* Pre-display errors.
				*
				...
				public static function update_all_options($new_options = FALSE, $verified = FALSE, $update_other = TRUE, $display_notices = TRUE, $enqueue_notices = FALSE, $request_refresh = FALSE)
					{
						...
						do_action("ws_plugin__optimizemember_before_update_all_options", get_defined_vars()); /* If you use this Hook, be sure to use ``wp_verify_nonce()``. */
						...
						if($verified || (!empty($_POST["ws_plugin__optimizemember_options_save"]) && ($nonce = $_POST["ws_plugin__optimizemember_options_save"]) && wp_verify_nonce($nonce, "ws-plugin--optimizemember-options-save")))
							{
								$options = $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]; /* Acquire the full existing configuration options array here. */
								/**/
								...
								/**/
								foreach($new_options as $key => $value) /* Find all keys contained within ``$new_options`` matching `^ws_plugin__optimizemember_`. */
									if(strpos($key, "ws_plugin__optimizemember_") === 0) { /* A relevant ``$new_options`` key matching `^ws_plugin__optimizemember_`? */
										if ($key == 'ws_plugin__optimizemember_ccp') {
											if(array_shift($value) === "remove-signal"){

												$options['ccp'] = array_diff($options['ccp'], array(reset($value)));
												/* Remove from post meta */
													/*For a post*/
													$allposts = get_posts( 'meta_key=optimizemember_ccaps_req' );
													foreach( $allposts as $post ) {
    													$prev_value = get_post_meta( $post->ID, 'optimizemember_ccaps_req', true );
    													$meta_value = array_diff($prev_value, array(reset($value)));
														update_post_meta( $post->ID,'optimizemember_ccaps_req', $meta_value, $prev_value );
    												}
    												/*For a page*/
													$allpages = get_pages( 'meta_key=optimizemember_ccaps_req' );
													foreach( $allpages as $page ) {
    													$prev_value = get_post_meta( $page->ID, 'optimizemember_ccaps_req', true );
    													$meta_value = array_diff($prev_value, array(reset($value)));
														update_post_meta( $page->ID,'optimizemember_ccaps_req', $meta_value, $prev_value );
    												}
											} else {
												$value = array_filter($new_options[$key]);
												// making sure there are no strange characters in $value
										...
										/**/
										if($key === "ws_plugin__optimizemember_configured") /* optimizeMember is now configured ( according to these options )? */
								...
								/**/
								eval('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
								do_action("ws_plugin__optimizemember_during_update_all_options", get_defined_vars()); /* If you use this Hook, be sure to use ``wp_verify_nonce()``. */
								unset($__refs, $__v); /* Unset defined __refs, __v. */
								/**/
								...
								/**/
								if(empty($_GET["page"]) || $_GET["page"] !== "ws-plugin--optimizemember-mms-ops") /* Do NOT display page-conflict-warnings on the Main Multisite Configuration panel. */
									{
										/*
										 * enqueue_admin_notice notice for this is called on every GET request through 'membership_option_check'
										 * display_admin_notice is called here
										 */
										...
										if(!$options["membership_options_page"] && ($display_notices === true || in_array("page-conflict-warnings", (array)$display_notices)))
											c_ws_plugin__optimizemember_admin_notices::display_admin_notice($notice, true);
								...
								/**/
								$updated_all_options = true; /* Flag indicating this routine was processed successfully; and that all optimizeMember options have been updated successfully.*/
							}
						/**/
						eval('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
						do_action("ws_plugin__optimizemember_after_update_all_options", get_defined_vars()); /* If you use this Hook, be sure to use ``wp_verify_nonce()``. */
						unset($__refs, $__v); /* Unset defined __refs, __v. */
						/**/
				...
				*/
				public static function add_admin_options()
					{
						do_action("ws_plugin__optimizemember_before_add_admin_options", get_defined_vars());
						/**/
						add_filter("plugin_action_links", "c_ws_plugin__optimizemember_menu_pages::_add_settings_link", 10, 2);
						/**/
						if(apply_filters("ws_plugin__optimizemember_during_add_admin_options_create_menu_items", true, get_defined_vars()))
							{
								if((is_multisite() && c_ws_plugin__optimizemember_utils_conds::is_multisite_farm() && !is_main_site()) || apply_filters("ws_plugin__optimizemember_during_add_admin_options_clear_right_side", false, get_defined_vars()))
									$GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["menu_pages"] = array(); /* Clear right side. */
								/**/
								$menu = apply_filters("ws_plugin__optimizemember_during_add_admin_options_menu_slug", "ws-plugin--optimizemember-start", get_defined_vars());
								/**/
								if(apply_filters("ws_plugin__optimizemember_during_add_admin_options_add_menu_page", true, get_defined_vars()))
									add_menu_page("OptimizeMember", "OptimizeMember", "create_users", $menu, "c_ws_plugin__optimizemember_menu_pages::start_page", $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["dir_url"]."/images/brand-favicon.png", '30.384567');
								/**/

								if(apply_filters("ws_plugin__optimizemember_during_add_admin_options_add_start_page", true, get_defined_vars()))
									add_submenu_page($menu, "OptimizeMember Quick-Start Guide", "Quick-Start Guide", "create_users", "ws-plugin--optimizemember-start", "c_ws_plugin__optimizemember_menu_pages::start_page");
								/**/
								if(apply_filters("ws_plugin__optimizemember_during_add_admin_options_add_divider_1", true, get_defined_vars())) /* Divider. */
								...
								/**/
								// if(apply_filters("ws_plugin__optimizemember_during_add_admin_options_add_new_gen_ops_page", true, get_defined_vars()))
								// 	add_submenu_page($menu, "OptimizeMember NEW General Options", "NEW General Options", "create_users", "ws-plugin--optimizemember-new-gen-ops", "c_ws_plugin__optimizemember_menu_pages::opm_gen_ops_page");
								/**/
								// if(apply_filters("ws_plugin__optimizemember_during_add_admin_options_add_divider_1", true, get_defined_vars())) /* Divider. */
								// 	add_submenu_page($menu, "", '<span style="display:block; margin:1px 0 1px -5px; padding:0; height:1px; line-height:1px; background:#CCCCCC;"></span>', "create_users", "#");
								/**/
								if(apply_filters("ws_plugin__optimizemember_during_add_admin_options_add_gen_ops_page", true, get_defined_vars()))
									add_submenu_page($menu, "OptimizeMember General Options", "General Options", "create_users", "ws-plugin--optimizemember-gen-ops", "c_ws_plugin__optimizemember_menu_pages::gen_ops_page");
								/**/
								if(apply_filters("ws_plugin__optimizemember_during_add_admin_options_add_res_ops_page", true, get_defined_vars()))
									add_submenu_page($menu, "OptimizeMember Restriction Options", "Restriction Options", "create_users", "ws-plugin--optimizemember-res-ops", "c_ws_plugin__optimizemember_menu_pages::res_ops_page");
								/**/
								if(apply_filters("ws_plugin__optimizemember_during_add_admin_options_add_down_ops_page", (!is_multisite() || !c_ws_plugin__optimizemember_utils_conds::is_multisite_farm() || is_main_site()), get_defined_vars()))
									add_submenu_page($menu, "OptimizeMember Download Options", "Download Options", "create_users", "ws-plugin--optimizemember-down-ops", "c_ws_plugin__optimizemember_menu_pages::down_ops_page");
								/**/
								if(apply_filters("ws_plugin__optimizemember_during_add_admin_options_add_divider_2", true, get_defined_vars())) /* Divider. */
									add_submenu_page($menu, "", '<span style="display:block; margin:1px 0 1px -5px; padding:0; height:1px; line-height:1px; background:#CCCCCC;"></span>', "create_users", "#");
								/**/
								if(apply_filters("ws_plugin__optimizemember_during_add_admin_options_add_new_user_page", true, get_defined_vars())) /* Shortcut. */
									add_submenu_page($menu, "OptimizeMember / Add A Member", "Add A Member", "create_users", "user-new.php");
								/**/
								if(apply_filters("ws_plugin__optimizemember_during_add_admin_options_add_browse_users_page", true, get_defined_vars())) /* Shortcut. */
									add_submenu_page($menu, "OptimizeMember / Browse Members", "Browse Members", "create_users", "users.php");
								/**/
								if(apply_filters("ws_plugin__optimizemember_during_add_admin_options_add_divider_3", true, get_defined_vars())) /* Divider. */
									add_submenu_page($menu, "", '<span style="display:block; margin:1px 0 1px -5px; padding:0; height:1px; line-height:1px; background:#CCCCCC;"></span>', "create_users", "#");
								/**/
								if(apply_filters("ws_plugin__optimizemember_during_add_admin_options_add_paypal_ops_page", true, get_defined_vars()))
									add_submenu_page($menu, "OptimizeMember PayPal Options", "PayPal Options", "create_users", "ws-plugin--optimizemember-paypal-ops", "c_ws_plugin__optimizemember_menu_pages::paypal_ops_page");
								/**/
								if(apply_filters("ws_plugin__optimizemember_during_add_admin_options_add_paypal_buttons_page", true, get_defined_vars()))
									add_submenu_page($menu, "OptimizeMember PayPal Buttons", "PayPal Buttons", "create_users", "ws-plugin--optimizemember-paypal-buttons", "c_ws_plugin__optimizemember_menu_pages::paypal_buttons_page");
								/**/
								if(apply_filters("ws_plugin__optimizemember_during_add_admin_options_add_divider_4", true, get_defined_vars())) /* Divider. */
									add_submenu_page($menu, "", '<span style="display:block; margin:1px 0 1px -5px; padding:0; height:1px; line-height:1px; background:#CCCCCC;"></span>', "create_users", "#");
								/**/
								if(apply_filters("ws_plugin__optimizemember_during_add_admin_options_add_trk_ops_page", true, get_defined_vars()))
									add_submenu_page($menu, "OptimizeMember API / Tracking", "API / Tracking", "create_users", "ws-plugin--optimizemember-trk-ops", "c_ws_plugin__optimizemember_menu_pages::trk_ops_page");
								/**/
								// if(apply_filters("ws_plugin__optimizemember_during_add_admin_options_add_els_ops_page", true, get_defined_vars()))
									// add_submenu_page($menu, "OptimizeMember API / List Servers", "API / List Servers", "create_users", "ws-plugin--optimizemember-els-ops", "c_ws_plugin__optimizemember_menu_pages::els_ops_page");
								/**/
								if(apply_filters("ws_plugin__optimizemember_during_add_admin_options_add_api_ops_page", true, get_defined_vars()))
									add_submenu_page($menu, "OptimizeMember API / Notifications", "API / Notifications", "create_users", "ws-plugin--optimizemember-api-ops", "c_ws_plugin__optimizemember_menu_pages::api_ops_page");
								/**/
								if(apply_filters("ws_plugin__optimizemember_during_add_admin_options_add_scripting_page", true, get_defined_vars()))
									add_submenu_page($menu, "OptimizeMember API / Scripting", "API / Scripting", "create_users", "ws-plugin--optimizemember-scripting", "c_ws_plugin__optimizemember_menu_pages::scripting_page");
								/**/
								if(apply_filters("ws_plugin__optimizemember_during_add_admin_options_add_divider_5", true, get_defined_vars())) /* Divider. */
									add_submenu_page($menu, "", '<span style="display:block; margin:1px 0 1px -5px; padding:0; height:1px; line-height:1px; background:#CCCCCC;"></span>', "create_users", "#");
								/**/
								if(apply_filters("ws_plugin__optimizemember_during_add_admin_options_add_integrations_page", (!is_multisite() || !c_ws_plugin__optimizemember_utils_conds::is_multisite_farm() || is_main_site()), get_defined_vars()))
									add_submenu_page($menu, "OptimizeMember / Other Integrations", "Other Integrations", "create_users", "ws-plugin--optimizemember-integrations", "c_ws_plugin__optimizemember_menu_pages::integrations_page");
								/**/
								if(apply_filters("ws_plugin__optimizemember_during_add_admin_options_add_mms_ops_page", (!is_multisite() || is_main_site()), get_defined_vars()))
									add_submenu_page($menu, "OptimizeMember Multisite Configuration", "Multisite (Config)", "create_users", "ws-plugin--optimizemember-mms-ops", "c_ws_plugin__optimizemember_menu_pages::mms_ops_page");

                                if /* Divider. */(apply_filters("ws_plugin__optimizemember_during_add_admin_options_add_divider_5", true, get_defined_vars()))
                                    add_submenu_page($menu, "", '<span style="display:block; margin:1px 0 1px -5px; padding:0; height:1px; line-height:1px; background:#CCCCCC;"></span>', "create_users", "#");

                                if(apply_filters("ws_plugin__optimizemember_during_add_admin_options_add_logs_page", (!is_multisite() || !c_ws_plugin__optimizemember_utils_conds::is_multisite_farm() || is_main_site()), get_defined_vars()))
                                    add_submenu_page($menu, "optimizeMember Logs", "Log Files (Debug)", "create_users", "ws-plugin--optimizemember-logs", "c_ws_plugin__optimizemember_menu_pages::logs_page");
								/**/
								// if(apply_filters("ws_plugin__optimizemember_during_add_admin_options_add_divider_6", true, get_defined_vars())) /* Divider. */
								// 	add_submenu_page($menu, "", '<span style="display:block; margin:1px 0 1px -5px; padding:0; height:1px; line-height:1px; background:#CCCCCC;"></span>', "create_users", "#");
								/**/
								// if(apply_filters("ws_plugin__optimizemember_during_add_admin_options_add_info_page", (!is_multisite() || !c_ws_plugin__optimizemember_utils_conds::is_multisite_farm() || is_main_site()), get_defined_vars()))
									//add_submenu_page($menu, "optimizeMember Information", "optimizeMember Info", "create_users", "ws-plugin--optimizemember-info", "c_ws_plugin__optimizemember_menu_pages::info_page");
								/**/
								do_action("ws_plugin__optimizemember_during_add_admin_options_additional_pages", get_defined_vars());
							}
						/**/
						do_action("ws_plugin__optimizemember_after_add_admin_options", get_defined_vars());
						/**/
				...
				*/
				public static function add_network_admin_options()
					{
						do_action("ws_plugin__optimizemember_before_add_network_admin_options", get_defined_vars());
						/**/
						if(apply_filters("ws_plugin__optimizemember_during_add_network_admin_options_create_menu_items", true, get_defined_vars()))
							{
								if((is_multisite() && c_ws_plugin__optimizemember_utils_conds::is_multisite_farm() && !is_main_site()) || apply_filters("ws_plugin__optimizemember_during_add_network_admin_options_clear_right_side", false, get_defined_vars()))
									$GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["menu_pages"] = array(); /* Clear right side. */
								/**/
								$menu = "ws-plugin--optimizemember-mms-ops"; /* Used below for nesting additional sub-menu pages. */
								/**/
								add_menu_page("optimizeMember", "optimizeMember", "create_users", $menu, "c_ws_plugin__optimizemember_menu_pages::mms_ops_page", $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["dir_url"]."/images/brand-favicon.png");
								/**/
								add_submenu_page($menu, "optimizeMember Multisite ( Configuration )", "Multisite (Config)", "create_users", "ws-plugin--optimizemember-mms-ops", "c_ws_plugin__optimizemember_menu_pages::mms_ops_page");
								/**/
								if(apply_filters("ws_plugin__optimizemember_during_add_network_admin_options_add_info_page", (!is_multisite() || !c_ws_plugin__optimizemember_utils_conds::is_multisite_farm() || is_main_site()), get_defined_vars()))
									add_submenu_page($menu, "optimizeMember Information", "optimizeMember Info", "create_users", "ws-plugin--optimizemember-info", "c_ws_plugin__optimizemember_menu_pages::info_page");
								/**/
								do_action("ws_plugin__optimizemember_during_add_network_admin_options_additional_pages", get_defined_vars());
							}
						/**/
						do_action("ws_plugin__optimizemember_after_add_network_admin_options", get_defined_vars());
						/**/
						return; /* Return for uniformity. */
					}
				/**
				...
				* @param str $plugin_file Expects path to a plugin file. We need to test against this for optimizeMember.
				...
				*/
				public static function _add_settings_link($actions = FALSE, $plugin_file = FALSE)
					{
						eval('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
						do_action("_ws_plugin__optimizemember_before_add_settings_link", get_defined_vars());
						unset($__refs, $__v); /* Unset defined __refs, __v. */
						/**/
						if($plugin_file === $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["plugin_basename"] && is_array($actions))
							{
								$settings = '<a href="'.esc_attr(admin_url("/admin.php?page=ws-plugin--optimizemember-gen-ops")).'">Settings</a>';
								array_unshift($actions, apply_filters("ws_plugin__optimizemember_add_settings_link", $settings, get_defined_vars()));
								/**/
								eval('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
								do_action("_ws_plugin__optimizemember_during_add_settings_link", get_defined_vars());
								unset($__refs, $__v); /* Unset defined __refs, __v. */
							}
						/**/
						return apply_filters("_ws_plugin__optimizemember_add_settings_link", $actions, get_defined_vars());
					}
				/**
				...
				*/
				public static function add_admin_scripts()
					{
						do_action("ws_plugin__optimizemember_before_add_admin_scripts", get_defined_vars());
						/**/
						if(!empty($_GET["page"]) && preg_match("/ws-plugin--optimizemember-/", $_GET["page"]))
							{
								...
								wp_enqueue_script("jquery-sprintf", $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["dir_url"]."/includes/jquery/jquery.sprintf/jquery.sprintf-min.js", array("jquery"), c_ws_plugin__optimizemember_utilities::ver_checksum());
								wp_enqueue_script("jquery-json-ps", $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["dir_url"]."/includes/jquery/jquery.json-ps/jquery.json-ps-min.js", array("jquery"), c_ws_plugin__optimizemember_utilities::ver_checksum());
								wp_enqueue_script("jquery-ui-effects", $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["dir_url"]."/includes/jquery/jquery.ui-effects/jquery.ui-effects-min.js", array("jquery", "jquery-ui-core"), c_ws_plugin__optimizemember_utilities::ver_checksum());
								wp_enqueue_script("ws-plugin--optimizemember-menu-pages", site_url("/?ws_plugin__optimizemember_menu_pages_js=".urlencode(mt_rand())), array("jquery", "thickbox", "media-upload", "jquery-sprintf", "jquery-json-ps", "jquery-ui-core", "jquery-ui-effects", "password-strength-meter"), c_ws_plugin__optimizemember_utilities::ver_checksum());
								/**/
								do_action("ws_plugin__optimizemember_during_add_admin_scripts", get_defined_vars());
							}
						/**/
						do_action("ws_plugin__optimizemember_after_add_admin_scripts", get_defined_vars());
						/**/
						return; /* Return for uniformity. */
					}
				/**
				...
				*/
				public static function start_page()
					{
						do_action("ws_plugin__optimizemember_before_start_page", get_defined_vars());
						/**/
						include_once dirname(dirname(__FILE__))."/menu-pages/start.inc.php";
						/**/
						do_action("ws_plugin__optimizemember_after_start_page", get_defined_vars());
						/**/
						return; /* Return for uniformity. */
					}
				/**
            ...
            {
                do_action("ws_plugin__optimizemember_before_logs_page", get_defined_vars());

                c_ws_plugin__optimizemember_menu_pages::update_all_options();
                c_ws_plugin__optimizemember_menu_pages::archive_logs_start_fresh();
                c_ws_plugin__optimizemember_menu_pages::delete_logs_start_fresh();
                ...
                if /* Logging disabled? */(!$GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["gateway_debug_logs"])
                    c_ws_plugin__optimizemember_admin_notices::display_admin_notice('Logging is currently disabled by your configuration.');

                include_once dirname(dirname(__FILE__))."/menu-pages/logs.inc.php";

                do_action("ws_plugin__optimizemember_after_logs_page", get_defined_vars());
            ...
            public static function log_file_downloader()
            {
                ...
                if(!empty($_GET["ws_plugin__optimizemember_download_log_file"]) && is_string($log_file = $_GET["ws_plugin__optimizemember_download_log_file"]) && strpos($log_file, "..") === false && strpos(basename($log_file), ".") !== 0)
                    if(!empty($_GET["ws_plugin__optimizemember_download_log_file_v"]) && is_string($nonce = $_GET["ws_plugin__optimizemember_download_log_file_v"]) && wp_verify_nonce($nonce, "ws-plugin--optimizemember-download-log-file-v"))
                    {
                        $logs_dir = $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["logs_dir"];
            ...
            public static function logs_zip_downloader()
            {
                ...
                if(!empty($_POST["ws_plugin__optimizemember_logs_download_zip"]) && is_string($nonce = $_POST["ws_plugin__optimizemember_logs_download_zip"]) && wp_verify_nonce($nonce, "ws-plugin--optimizemember-logs-download-zip"))
                {
                    $logs_dir = $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["logs_dir"];
                    $optimizemember_logs_zip = $logs_dir."/".$_SERVER["HTTP_HOST"]."--optimizemember-logs.zip";

                    if(is_dir($logs_dir)) // Do we have a logs directory?
                    {
                        include_once ABSPATH . "wp-admin/includes/class-pclzip.php";

                        if(file_exists($optimizemember_logs_zip) && is_writable($optimizemember_logs_zip))
                            unlink($optimizemember_logs_zip);

                        $archive = new PclZip($optimizemember_logs_zip);
                        $archive->create($logs_dir, PCLZIP_OPT_REMOVE_ALL_PATH);
                    }

                    if(file_exists($optimizemember_logs_zip))
                        $optimizemember_logs_zip_size = filesize($optimizemember_logs_zip);
                    else $optimizemember_logs_zip_size = 0;
                    ...
                    header("Content-Length: ".$optimizemember_logs_zip_size);
                    header("Expires: ".gmdate("D, d M Y H:i:s", strtotime("-1 week"))." GMT");
                    header("Last-Modified: ".gmdate("D, d M Y H:i:s")." GMT");
                    header("Cache-Control: no-cache, must-revalidate, max-age=0");
                    header("Cache-Control: post-check=0, pre-check=0", false);
                    ...
                    header('Content-Disposition: attachment; filename="'.basename($optimizemember_logs_zip).'"');

                    if($optimizemember_logs_zip_size && is_resource($resource = fopen($optimizemember_logs_zip, "rb")))
                    {
                        $_bytes_to_read = $optimizemember_logs_zip_size; // Total bytes we need to read for this file.
            ...
            public static function archive_logs_start_fresh()
            {
                ...
                if(!empty($_POST["ws_plugin__optimizemember_logs_archive_start_fresh"]) && is_string($nonce = $_POST["ws_plugin__optimizemember_logs_archive_start_fresh"]) && wp_verify_nonce($nonce, "ws-plugin--optimizemember-logs-archive-start-fresh"))
                {
                    ...
                    if(!empty($error))
                        c_ws_plugin__optimizemember_admin_notices::display_admin_notice("Unknown error when attempting to archive log files. Please check directory permissions.", true);
                    else c_ws_plugin__optimizemember_admin_notices::display_admin_notice("All log files have been archived succesfully.");
                }
            }
            ...
            public static function delete_logs_start_fresh()
            {
                ...
                if(!empty($_POST["ws_plugin__optimizemember_logs_delete_start_fresh"]) && is_string($nonce = $_POST["ws_plugin__optimizemember_logs_delete_start_fresh"]) && wp_verify_nonce($nonce, "ws-plugin--optimizemember-logs-delete-start-fresh"))
                {
                    if(is_dir($logs_dir = $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["logs_dir"]))
                    ...
                    if(!empty($error))
                        c_ws_plugin__optimizemember_admin_notices::display_admin_notice("Unknown error when attempting to delete log files. Please check directory permissions.", true);
                    else c_ws_plugin__optimizemember_admin_notices::display_admin_notice("All log files have been deleted succesfully.");
                }
            }
				/**
				* Builds and handles the NEWGeneral Options page.
				*
				* @package optimizeMember\Menu_Pages
				* @since 3.5
				*
				* @return null
				*/
				public static function opm_gen_ops_page()
					{
						do_action("ws_plugin__optimizemember_before_opm_gen_ops_page", get_defined_vars());
						/**/
						c_ws_plugin__optimizemember_menu_pages::update_all_options();
						/**/
						include_once dirname(dirname(__FILE__))."/menu-pages/opm-gen-ops.inc.php";
						/**/
						do_action("ws_plugin__optimizemember_after_opm_gen_ops_page", get_defined_vars());
						/**/
						return; /* Return for uniformity. */
					}
				/**
				...
				*/
				public static function gen_ops_page()
					{
						do_action("ws_plugin__optimizemember_before_gen_ops_page", get_defined_vars());
						/**/
						c_ws_plugin__optimizemember_menu_pages::update_all_options();
						/**/
						include_once dirname(dirname(__FILE__))."/menu-pages/gen-ops.inc.php";
						/**/
						do_action("ws_plugin__optimizemember_after_gen_ops_page", get_defined_vars());
						/**/
						return; /* Return for uniformity. */
					}
				/**
				...
				*/
				public static function res_ops_page()
					{
						do_action("ws_plugin__optimizemember_before_res_ops_page", get_defined_vars());
						/**/
						c_ws_plugin__optimizemember_menu_pages::update_all_options();
						/**/
						include_once dirname(dirname(__FILE__))."/menu-pages/res-ops.inc.php";
						/**/
						do_action("ws_plugin__optimizemember_after_res_ops_page", get_defined_vars());
						/**/
						return; /* Return for uniformity. */
					}
				/**
				...
				*/
				public static function paypal_ops_page()
					{
						do_action("ws_plugin__optimizemember_before_paypal_ops_page", get_defined_vars());
						/**/
						c_ws_plugin__optimizemember_menu_pages::update_all_options();
						/**/
						$logs_dir = $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["logs_dir"];
						...
						/**/
						do_action("ws_plugin__optimizemember_after_paypal_ops_page", get_defined_vars());
						/**/
						return; /* Return for uniformity. */
					}
				/**
				...
				public static function down_ops_page()
					{
						...
						/**/
						if(!c_ws_plugin__optimizemember_files::no_gzip_rules_in_root_htaccess() /* If optimizeMember's GZIP exclusions do NOT yet exist in the root `.htaccess` file. */)
							c_ws_plugin__optimizemember_files::write_no_gzip_into_root_htaccess(). /* Handle the root `.htaccess` file now. */clearstatcache();
						/**/
				...
				*/
				public static function trk_ops_page()
					{
						do_action("ws_plugin__optimizemember_before_trk_ops_page", get_defined_vars());
						/**/
						c_ws_plugin__optimizemember_menu_pages::update_all_options();
						/**/
						include_once dirname(dirname(__FILE__))."/menu-pages/trk-ops.inc.php";
						/**/
						do_action("ws_plugin__optimizemember_after_trk_ops_page", get_defined_vars());
						/**/
						return; /* Return for uniformity. */
					}
				/**
				...
				*/
				public static function els_ops_page()
					{
						do_action("ws_plugin__optimizemember_before_els_ops_page", get_defined_vars());
						/**/
						c_ws_plugin__optimizemember_menu_pages::update_all_options();
						/**/
						include_once dirname(dirname(__FILE__))."/menu-pages/els-ops.inc.php";
						/**/
						do_action("ws_plugin__optimizemember_after_els_ops_page", get_defined_vars());
						/**/
						return; /* Return for uniformity. */
					}
				/**
				...
				*/
				public static function api_ops_page()
					{
						do_action("ws_plugin__optimizemember_before_api_ops_page", get_defined_vars());
						/**/
						c_ws_plugin__optimizemember_menu_pages::update_all_options();
						/**/
						include_once dirname(dirname(__FILE__))."/menu-pages/api-ops.inc.php";
						/**/
						do_action("ws_plugin__optimizemember_after_api_ops_page", get_defined_vars());
						/**/
						return; /* Return for uniformity. */
					}
				/**
				...
				*/
				public static function scripting_page()
					{
						do_action("ws_plugin__optimizemember_before_scripting_page", get_defined_vars());
						/**/
						include_once dirname(dirname(__FILE__))."/menu-pages/scripting.inc.php";
						/**/
						do_action("ws_plugin__optimizemember_after_scripting_page", get_defined_vars());
						/**/
						return; /* Return for uniformity. */
					}
				/**
				...
				*/
				public static function integrations_page()
					{
						do_action("ws_plugin__optimizemember_before_integrations_page", get_defined_vars());
						/**/
						include_once dirname(dirname(__FILE__))."/menu-pages/integrations.inc.php";
						/**/
						do_action("ws_plugin__optimizemember_after_integrations_page", get_defined_vars());
						/**/
						return; /* Return for uniformity. */
					}
				/**
				* Builds and handles the optimizeMember Info page.
				*
				* @package optimizeMember\Menu_Pages
				* @since 3.5
				*
				...
				*/
				public static function info_page()
					{
						do_action("ws_plugin__optimizemember_before_info_page", get_defined_vars());
						/**/
						include_once dirname(dirname(__FILE__))."/menu-pages/info.inc.php";
						/**/
						do_action("ws_plugin__optimizemember_after_info_page", get_defined_vars());
						/**/
						return; /* Return for uniformity. */
					}

				public static function update_new_options_via_ajax()
				{
					self::update_all_options();die();
				}
			}
	}
?>