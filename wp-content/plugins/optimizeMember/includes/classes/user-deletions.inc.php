...
*
* @package optimizeMember\User_Deletions
* @since 3.5
*/
if (realpath (__FILE__) === realpath ($_SERVER["SCRIPT_FILENAME"]))
	exit ("Do not access this file directly.");
/**/
if (!class_exists ("c_ws_plugin__optimizemember_user_deletions"))
	{
		/**
		* User deletion routines.
		*
		...
		* @since 3.5
		*/
		class c_ws_plugin__optimizemember_user_deletions
			{
				/**
				...
				public static function handle_ms_user_deletions ($user_id = FALSE, $blog_id = FALSE, $s2says = FALSE)
					{
						...
						do_action ("ws_plugin__optimizemember_before_handle_ms_user_deletions", get_defined_vars ());
						...
						if (is_multisite () && empty ($processed[$user_id])) /* Safeguard this routine against duplicate processing. */
							{
								if (($s2says || (is_blog_admin () && $pagenow === "users.php")) && ($processed[$user_id] = true))
									...
									{
										eval ('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
										do_action ("ws_plugin__optimizemember_during_handle_ms_user_deletions_before", get_defined_vars ());
										unset ($__refs, $__v); /* Unset defined __refs, __v. */
										/**/
										c_ws_plugin__optimizemember_user_deletions::handle_user_deletions ($user_id); /* Hand this over. */
										/**/
										eval ('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
										do_action ("ws_plugin__optimizemember_during_handle_ms_user_deletions_after", get_defined_vars ());
										unset ($__refs, $__v); /* Unset defined __refs, __v. */
									}
							}
						/**/
						eval ('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
						do_action ("ws_plugin__optimizemember_after_handle_ms_user_deletions", get_defined_vars ());
						unset ($__refs, $__v); /* Unset defined __refs, __v. */
						/**/
				...
				public static function handle_user_deletions ($user_id = FALSE)
					{
						...
						/**/
						eval ('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
						do_action ("ws_plugin__optimizemember_before_handle_user_deletions", get_defined_vars ());
						unset ($__refs, $__v); /* Unset defined __refs, __v. */
						/**/
						if (empty ($processed[$user_id]) && ($processed[$user_id] = true)) /* Safeguard this routine against duplicate processing. */
							{
								...
								(!empty ($GLOBALS["ws_plugin__optimizemember_eot_del_type"])) ? /* Is the global available for use? */
								$GLOBALS["ws_plugin__optimizemember_eot_del_type"] : "user-removal-deletion"; /* Else use default. */
								/**/
								$custom = get_user_option ("optimizemember_custom", $user_id); /* An EOT Notification is triggered, EVEN if empty. */
								$subscr_id = get_user_option ("optimizemember_subscr_id", $user_id); /* And also, EVEN if this is empty. */
								$fields = get_user_option ("optimizemember_custom_fields", $user_id); /* Used in API Notifications. */
								$user_reg_ip = get_user_option ("optimizemember_registration_ip", $user_id); /* In API Notifications. */
								/**/
								eval ('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
								do_action ("ws_plugin__optimizemember_during_handle_user_before_deletions", get_defined_vars ());
								do_action ("ws_plugin__optimizemember_during_collective_eots", $user_id, get_defined_vars (), $eot_del_type, "removal-deletion");
								unset ($__refs, $__v); /* Unset defined __refs, __v. */
								/**/
								delete_user_option ($user_id, "optimizemember_custom"); /* Remove User options ( for this Blog ). */
								...
								delete_user_option ($user_id, "optimizemember_registration_ip");
								/**/
                                ...
                                delete_user_option ($user_id, "optimizemember_capability_times");
								delete_user_option ($user_id, "optimizemember_sp_references");
								/**/
								...
								/**/
								delete_user_option ($user_id, "optimizemember_file_download_access_arc");
								delete_user_option ($user_id, "optimizemember_file_download_access_log");
								/**/
								delete_user_option ($user_id, "optimizemember_login_counter");
								delete_user_option ($user_id, "optimizemember_notes");
								/**/
								if (is_object ($user = new WP_User ($user_id)) && $user->ID && $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["eot_del_notification_urls"] && is_array ($cv = preg_split ("/\|/", $custom)))
									{
										foreach (preg_split ("/[\r\n\t]+/", $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["eot_del_notification_urls"]) as $url) /* Handle EOT Notifications on user deletion. */
											/**/
											if (($url = c_ws_plugin__optimizemember_utils_strings::fill_cvs($url, $custom, true)) && ($url = preg_replace ("/%%eot_del_type%%/i", c_ws_plugin__optimizemember_utils_strings::esc_refs (urlencode ($eot_del_type)), $url)) && ($url = preg_replace ("/%%subscr_id%%/i", c_ws_plugin__optimizemember_utils_strings::esc_refs (urlencode ($subscr_id)), $url)))
								...
								/**/
								if (is_object ($user = new WP_User ($user_id)) && $user->ID && $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["eot_del_notification_recipients"] && is_array ($cv = preg_split ("/\|/", $custom)))
									{
										...
										$msg .= "cv9: %%cv9%%";
										/**/
										if (($msg = c_ws_plugin__optimizemember_utils_strings::fill_cvs($msg, $custom)) && ($msg = preg_replace ("/%%eot_del_type%%/i", c_ws_plugin__optimizemember_utils_strings::esc_refs ($eot_del_type), $msg)) && ($msg = preg_replace ("/%%subscr_id%%/i", c_ws_plugin__optimizemember_utils_strings::esc_refs ($subscr_id), $msg)))
								...
								/**/
								eval ('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
								do_action ("ws_plugin__optimizemember_during_handle_user_deletions", get_defined_vars ());
								unset ($__refs, $__v); /* Unset defined __refs, __v. */
							}
						/**/
						eval ('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
						do_action ("ws_plugin__optimizemember_after_handle_user_deletions", get_defined_vars ());
						unset ($__refs, $__v); /* Unset defined __refs, __v. */
						/**/
						return; /* Return for uniformity. */
					}
