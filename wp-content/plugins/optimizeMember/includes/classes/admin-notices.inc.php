...
/**/
if (!class_exists ("c_ws_plugin__optimizemember_admin_notices"))
	{
		/**
		* Enqueues/displays administrative notices.
		*
		...
		* @since 3.5
		*/
		class c_ws_plugin__optimizemember_admin_notices
			{
				/**
				...
				public static function display_admin_notice ($notice = FALSE, $error = FALSE, $dismiss = FALSE)
					{
						ob_start();
						eval('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
						do_action ("ws_plugin__optimizemember_before_display_admin_notice", get_defined_vars ());
						unset ($__refs, $__v); /* Unset defined __refs, __v. */
						/**/
						if (is_string ($notice) && $notice && $error) /* Slightly different/special format for errors. */
							{
								$notice .= ($dismiss) ? ' [ <a href="' . esc_attr(add_query_arg ("ws-plugin--optimizemember-dismiss-admin-notice", urlencode (md5 ($notice)), $_SERVER["REQUEST_URI"])) . '">dismiss message</a> ]' : '';
								/**/
						...
						/**/
						do_action ("ws_plugin__optimizemember_after_display_admin_notice", get_defined_vars ());
						/**/
                        ob_end_flush();
						return; /* Return for uniformity. */
					}
