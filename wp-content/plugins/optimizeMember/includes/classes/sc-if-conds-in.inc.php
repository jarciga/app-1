<?php
/**
 * Shortcode `[opmIf /]` (inner processing routines).
 */
if(!defined('WPINC')) // MUST have WordPress.
	exit('Do not access this file directly.');

if(!class_exists('c_ws_plugin__optimizemember_sc_if_conds_in'))
{
	/**
	 * Shortcode `[s2If /]` (inner processing routines).
	 *
	 * @package optimizeMember\s2If
	 * @since 3.5
	 */
	class c_ws_plugin__optimizemember_sc_if_conds_in
	{
		/**
		...
		public static function sc_if_conditionals($attr = array(), $content = '', $shortcode = '')
		{
			...
			do_action('ws_plugin__optimizemember_before_sc_if_conditionals', get_defined_vars());
			...
			$blog_farm_safe = apply_filters('ws_plugin__optimizemember_sc_if_conditionals_blog_farm_safe',
			...
			$pro_is_installed = c_ws_plugin__optimizemember_utils_conds::pro_is_installed(); // Has pro version?

			$sc_conds_allow_arbitrary_php = @$GLOBALS['WS_PLUGIN__']['optimizemember']['o']['sc_conds_allow_arbitrary_php'];
			if(!$pro_is_installed || (is_multisite() && c_ws_plugin__optimizemember_utils_conds::is_multisite_farm() && !is_main_site()))
			...
			{
				if(($condition_succeeded = c_ws_plugin__optimizemember_sc_if_conds_in::evl($attr['php'])))
					$condition_content = isset($content_if) ? $content_if : $content;
				else $condition_content = isset($content_else) ? $content_else : '';

				if($condition_content) $condition_content = c_ws_plugin__optimizemember_utils_strings::trim_html($condition_content);

				return do_shortcode(apply_filters('ws_plugin__optimizemember_sc_if_conditionals', $condition_content, get_defined_vars()));
			}
			...
			foreach(array_keys(get_defined_vars()) as $__v) $__refs[$__v] =& $$__v;
			do_action('ws_plugin__optimizemember_before_sc_if_conditionals_after_conditional_logic', get_defined_vars());
			unset($__refs, $__v); // Allows variables to be modified by reference.
			...
			if($conditional_logic === 'AND') // This is the AND variation. This routine analyzes conditionals using AND logic (the default behavior).
			{
				foreach($attr as $attr_value) // This is the AND variation. This routine analyzes conditionals using AND logic (the default behavior).
				{
					...
					{
						if(!is_multisite() || !c_ws_plugin__optimizemember_utils_conds::is_multisite_farm() || is_main_site() || !(preg_match('/[\$\(\)]/', $attr_args) || preg_match('/new['."\r\n\t".'\s]/i', $attr_args)))
						{
							...
							{
								if(!is_multisite() || !c_ws_plugin__optimizemember_utils_conds::is_multisite_farm() || is_main_site() || in_array(strtolower($conditional), $blog_farm_safe))
								{
