<?php
/**
* Shortcode `[optimizeMember-PayPal-Button]` ( inner processing routines ).
*
* Copyright: © 2009-2011
...
/**/
if (!class_exists ("c_ws_plugin__optimizemember_sc_paypal_button_in"))
	{
		/**
		* Shortcode `[optimizeMember-PayPal-Button]` ( inner processing routines ).
		*
		...
		* @since 3.5
		*/
		class c_ws_plugin__optimizemember_sc_paypal_button_in
			{
				/**
				...
				public static function sc_paypal_button ($attr = FALSE, $content = FALSE, $shortcode = FALSE)
					{
						...
						$force_notify_url_scheme = apply_filters("ws_plugin__optimizemember_during_sc_paypal_button_force_notify_url_scheme", null, get_defined_vars ());
						$force_return_url_scheme = apply_filters("ws_plugin__optimizemember_during_sc_paypal_button_force_return_url_scheme", null, get_defined_vars ());
						...
						if /* Cancellation Buttons. */ ($attr["cancel"])
						...
						else if /* Specific Post/Page Buttons. */ ($attr["sp"])
							{
								...
								/**/
								$paypal_invoice_input_value = /* optimizeMember's Unique Code~IP combo. */ uniqid () . "~" . $_SERVER["REMOTE_ADDR"];
								/**/
								$attr["sp_ids_exp"] = /* Combined "sp:ids:expiration hours". */ "sp:" . $attr["ids"] . ":" . $attr["exp"];
								/**/
                                $success_return_url = /* optimizeMember handles this all by itself. However, it can be Filtered (see below). */ home_url ("/?optimizemember_paypal_return=1", $force_return_url_scheme);
								...
								$code = preg_replace ("/%%notify_url%%/", c_ws_plugin__optimizemember_utils_strings::esc_refs (esc_attr (site_url ("/?optimizemember_paypal_notify=1", $force_notify_url_scheme, $force_notify_url_scheme))), $code);
						...
						else if /* Independent Custom Capabilities. */ ($attr["level"] === "*")
							{
								...
								/**/
								$success_return_url = /* optimizeMember handles this all by itself. However, it can be Filtered. */ site_url ("/?optimizemember_paypal_return=1", $force_return_url_scheme);
								$success_return_url = apply_filters ("ws_plugin__optimizemember_during_sc_paypal_button_success_return_url", $success_return_url, get_defined_vars ());
								/**/
								...
								$code = preg_replace ("/%%paypal_business%%/", c_ws_plugin__optimizemember_utils_strings::esc_refs (esc_attr ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["paypal_business"])), $code);
								$code = preg_replace ("/%%cancel_return%%/", c_ws_plugin__optimizemember_utils_strings::esc_refs (esc_attr (home_url ("/"))), $code);
								$code = preg_replace ("/%%notify_url%%/", c_ws_plugin__optimizemember_utils_strings::esc_refs (esc_attr (site_url ("/?optimizemember_paypal_notify=1", $force_notify_url_scheme))), $code);
								$code = preg_replace ("/%%return%%/", c_ws_plugin__optimizemember_utils_strings::esc_refs (esc_attr ($success_return_url)), $code);
								$code = preg_replace ("/%%custom%%/", c_ws_plugin__optimizemember_utils_strings::esc_refs (esc_attr ($attr["custom"])), $code);
								/**/
								$code = preg_replace ('/ name\="lc" value\="(.*?)"/', ' name="lc" value="' . c_ws_plugin__optimizemember_utils_strings::esc_refs (esc_attr ($attr["lc"])) . '"', $code);
						...
						else /* Otherwise, we'll process this Button normally, using Membership routines. */
							{
								...
								/**/
								$success_return_url = /* optimizeMember handles this all by itself. However, it can be Filtered ( see below ). */ site_url ("/?optimizemember_paypal_return=1", $force_return_url_scheme);
								...
								$code = preg_replace ("/%%cancel_return%%/", c_ws_plugin__optimizemember_utils_strings::esc_refs (esc_attr (home_url ("/"))), $code); /* This brings them back to Front Page. */
								$code = preg_replace ("/%%notify_url%%/", c_ws_plugin__optimizemember_utils_strings::esc_refs (esc_attr (site_url ("/?optimizemember_paypal_notify=1", $force_notify_url_scheme))), $code);
								$code = preg_replace ("/%%return%%/", c_ws_plugin__optimizemember_utils_strings::esc_refs (esc_attr ($success_return_url)), $code);
								$code = preg_replace ("/%%custom%%/", c_ws_plugin__optimizemember_utils_strings::esc_refs (esc_attr ($attr["custom"])), $code);
