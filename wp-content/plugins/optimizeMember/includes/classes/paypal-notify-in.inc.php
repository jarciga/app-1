...
if(!class_exists('c_ws_plugin__optimizemember_paypal_notify_in'))
{
	...
	class c_ws_plugin__optimizemember_paypal_notify_in
	{
		...
		public static function paypal_notify()
		{
			...
			if(!empty($_GET['optimizemember_paypal_notify']) && ($GLOBALS['WS_PLUGIN__']['optimizemember']['o']['paypal_business'] || !empty($_REQUEST['optimizemember_paypal_proxy'])))
			{
				...
				if(is_array($paypal = c_ws_plugin__optimizemember_paypal_utilities::paypal_postvars()) && ($_paypal = $paypal) && ($_paypal_s = serialize($_paypal)))
				{
					...
					$paypal['subscr_gateway'] = (!empty($_REQUEST['optimizemember_paypal_proxy'])) ? esc_html(trim(stripslashes($_REQUEST['optimizemember_paypal_proxy']))) : 'paypal';

					$coupon = (!empty($_REQUEST['optimizemember_paypal_proxy_coupon']) && is_array($_REQUEST['optimizemember_paypal_proxy_coupon'])) ? stripslashes_deep($_REQUEST['optimizemember_paypal_proxy_coupon']) : array();
					...
					if(empty($paypal['custom']) && !empty($paypal['recurring_payment_id'])) // Recurring Profile ID.
						...
						$paypal['custom'] = c_ws_plugin__optimizemember_utils_users::get_user_custom_with($paypal['mp_id']);
					...
					if(!empty($paypal['custom']) && preg_match('/^'.preg_quote(preg_replace('/\:([0-9]+)$/', '', $_SERVER['HTTP_HOST']), '/').'/i', $paypal['custom']))
					{
						...
						if(!apply_filters('ws_plugin__optimizemember_during_paypal_notify_conditionals', FALSE, get_defined_vars()))
						{
							...
							else if(($_paypal_cp = c_ws_plugin__optimizemember_paypal_notify_in_billing_agreement_signup::cp(get_defined_vars())))
								$paypal = $_paypal_cp;
				...
				else // Extensive log reporting here. This is an area where many site owners find trouble. Depending on server configuration; remote HTTPS connections may fail.
				{
					if (!empty($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] === 'GET' && !empty($_SERVER['HTTP_USER_AGENT'])) {
						if (preg_match('/(msie|trident|gecko|webkit|presto|konqueror|playstation)[\/ ]([0-9\.]+)/i', $_SERVER['HTTP_USER_AGENT'])) {
							$paypal['optimizemember_indicator'] = 'This PayPal IPN Handler by OptimizeMember is active & listening.';
							}
					}
					...
					//$paypal['optimizemember_log'][] = 'Please see this KB article: `http://www.optimizemember.com/kb/server-scanner/`. We suggest that you run the optimizeMember Server Scanner.';
				...
				*/
				if(!empty($_REQUEST['optimizemember_paypal_proxy']))
					$paypal['optimizemember_paypal_proxy'] = esc_html(trim(stripslashes((string)$_REQUEST['optimizemember_paypal_proxy'])));
				/*
				Add IPN proxy use vars (when available) to the ``$paypal`` array.
				*/
				if(!empty($_REQUEST['optimizemember_paypal_proxy_use']))
					$paypal['optimizemember_paypal_proxy_use'] = esc_html(trim(stripslashes((string)$_REQUEST['optimizemember_paypal_proxy_use'])));
				/*
				Add IPN proxy coupon vars (when available) to the ``$paypal`` array.
				*/
				if(!empty($_REQUEST['optimizemember_paypal_proxy_coupon']))
					$paypal['optimizemember_paypal_proxy_coupon'] = stripslashes_deep((array)$_REQUEST['optimizemember_paypal_proxy_coupon']);
				/*
				Also add IPN proxy self-verification (when available) to the ``$paypal`` array.
				*/
				if(!empty($_REQUEST['optimizemember_paypal_proxy_verification']))
					$paypal['optimizemember_paypal_proxy_verification'] = esc_html(trim(stripslashes((string)$_REQUEST['optimizemember_paypal_proxy_verification'])));
				/*
				...
				*/
				...
				if (!empty($paypal['optimizemember_paypal_proxy_return_url'])) {
					exit($paypal['optimizemember_paypal_proxy_return_url']);
				} elseif (!empty($paypal['optimizemember_indicator'])) {
					exit($paypal['optimizemember_indicator']);
				} else {
