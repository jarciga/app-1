<?php
/**
* Shortcode `[opmFile /]` ( inner processing routines ).
*
* Copyright: © 2009-2011
...
/**/
if(!class_exists("c_ws_plugin__optimizemember_sc_files_in"))
	{
		/**
		* Shortcode `[opmFile /]` ( inner processing routines ).
		*
		...
		* @since 110926
		*/
		class c_ws_plugin__optimizemember_sc_files_in
			{
				/**
				...
				public static function sc_get_file($attr = FALSE, $content = FALSE, $shortcode = FALSE)
					{
						...
						do_action("ws_plugin__optimizemember_before_sc_get_file", get_defined_vars());
						...
						/**/
						eval('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
						do_action("ws_plugin__optimizemember_before_sc_get_file_after_shortcode_atts", get_defined_vars());
						unset /* Unset defined __refs, __v. */($__refs, $__v);
						/**/
						...
						if /* Looking for a File Download URL? */(!empty($config) && isset($config["file_download"]))
							{
								$_get = c_ws_plugin__optimizemember_files::create_file_download_url($config, $get_streamer_array);
								/**/
						...
						return apply_filters("ws_plugin__optimizemember_sc_get_file", ((isset($get)) ? $get : null), get_defined_vars());
					}
				/**
