...
/**/
if (!class_exists ("c_ws_plugin__optimizemember_login_redirects"))
	{
		/**
		* Login redirections.
		*
		...
		* @since 3.5
		*/
		class c_ws_plugin__optimizemember_login_redirects
			{
				/**
				...
				public static function login_redirect ($username = '', $user = NULL)
					{
                        ...
                        if(is_string($username) && $username && is_object($user) && !empty($user->ID) && ($user_id = $user->ID))
							{
								if (!get_user_option ("optimizemember_registration_ip", $user_id)) /* Have we got this yet? */
								...
								/**/
								if ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["custom_reg_password"]) /* Nag em? */
								...
								if (($redirect = apply_filters ("ws_plugin__optimizemember_login_redirect", (($user->has_cap ("manage_options")) ? false : true), get_defined_vars ())))
									{
										$obey_redirect_to = apply_filters ("ws_plugin__optimizemember_obey_login_redirect_to", /* By default, we obey this. */ true, get_defined_vars ());
										/**/
										if (!$obey_redirect_to || empty ($_REQUEST["redirect_to"]) || !is_string ($_REQUEST["redirect_to"]) || $_REQUEST["redirect_to"] === admin_url () || preg_match ("/^\/?wp-admin\/?$/", $_REQUEST["redirect_to"]))
											{
												...
												/**/
												else if ($redirection_url = c_ws_plugin__optimizemember_login_redirects::login_redirection_url ($user))
													wp_redirect ($redirection_url); /* Special Redirection URL configured with optimizeMember. */
												/**/
												else /* Else we use the Login Welcome Page configured for optimizeMember. */
				...
				*/
				public static function login_redirection_url ($user = FALSE, $root_returns_false = FALSE)
					{
						eval ('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
						do_action ("ws_plugin__optimizemember_before_login_redirection_url", get_defined_vars ());
						unset ($__refs, $__v); /* Unset defined __refs, __v. */
						/**/
						$url = $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["login_redirection_override"];
						$url = c_ws_plugin__optimizemember_login_redirects::fill_login_redirect_rc_vars ($url, $user, $root_returns_false);
						/**/
						return apply_filters ("ws_plugin__optimizemember_login_redirection_url", $url, get_defined_vars ());
					}
				/**
				...
				*/
				public static function login_redirection_uri ($user = FALSE, $root_returns_false = FALSE)
					{
						eval ('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
						do_action ("ws_plugin__optimizemember_before_login_redirection_uri", get_defined_vars ());
						unset ($__refs, $__v); /* Unset defined __refs, __v. */
						/**/
						if (($url = c_ws_plugin__optimizemember_login_redirects::login_redirection_url ($user, $root_returns_false)))
							$uri = c_ws_plugin__optimizemember_utils_urls::parse_uri ($url);
						/**/
						return apply_filters ("ws_plugin__optimizemember_login_redirection_uri", ((!empty ($uri)) ? $uri : false), get_defined_vars ());
					}
				/**
				...
				public static function fill_login_redirect_rc_vars ($url = FALSE, $user = FALSE, $root_returns_false = FALSE)
					{
						...
						/**/
						$url = preg_replace ("/%%current_user_login%%/i", c_ws_plugin__optimizemember_utils_strings::esc_refs ($user_login), $url);
						$url = preg_replace ("/%%current_user_id%%/i", c_ws_plugin__optimizemember_utils_strings::esc_refs ($user_id), $url);
						$url = preg_replace ("/%%current_user_level%%/i", c_ws_plugin__optimizemember_utils_strings::esc_refs ($user_level), $url);
						$url = preg_replace ("/%%current_user_role%%/i", c_ws_plugin__optimizemember_utils_strings::esc_refs ($user_role), $url);
						$url = preg_replace ("/%%current_user_ccaps%%/i", c_ws_plugin__optimizemember_utils_strings::esc_refs ($user_ccaps), $url);
						$url = preg_replace ("/%%current_user_logins%%/i", c_ws_plugin__optimizemember_utils_strings::esc_refs ($user_logins), $url);
						/**/
						if ( /* Only if optimizeMember's fault » */$url !== $orig_url && (!($parse = c_ws_plugin__optimizemember_utils_urls::parse_url ($url, -1, false)) || (!empty ($parse["path"]) && strpos ($parse["path"], "//") !== false)))
							$url = site_url ("/"); /* Defaults to Home Page. We don't return invalid URLs produced by empty Replacement Codes ( i.e. with `//` ). */
						/**/
						if ($root_returns_false /* Used by optimizeMember's security gate. */ && c_ws_plugin__optimizemember_utils_conds::is_site_root ($url))
