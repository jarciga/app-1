		...
		public static function cp($vars = array() /* Conditional phase for ``c_ws_plugin__optimizemember_paypal_notify_in::paypal_notify()``. */)
		{
			...
			)
			{
				...
				if(!get_transient($transient_ipn = 's2m_ipn_'.md5('optimizemember_transient_'.$_paypal_s)) && set_transient($transient_ipn, time(), 31556926 * 10))
				{
					...
					$paypal['currency_symbol'] = c_ws_plugin__optimizemember_utils_cur::symbol($paypal['currency']);

					if(!empty($coupon['coupon_code']) && c_ws_plugin__optimizemember_utils_conds::pro_is_installed())
						{
							$coupon_class = new c_ws_plugin__optimizemember_pro_coupons();
							$coupon_class->update_uses($coupon['coupon_code']);
						}
					$ipn_signup_vars = $paypal; // Create array of IPN signup vars w/o optimizemember_log.
					unset($ipn_signup_vars['optimizemember_log']);
					/*
					...
					if(preg_match('/(referenc|associat|updat|upgrad)/i', $paypal['option_name1']) && $paypal['option_selection1'])
					{
						...
						if(($user_id = c_ws_plugin__optimizemember_utils_users::get_user_id_with($paypal['subscr_id'], $paypal['option_selection1'])) && is_object($user = new WP_User ($user_id)) && $user->ID)
						{
							if(!$user->has_cap('administrator')) // Do NOT process this routine on Administrators.
							{
								...
								update_user_option($user_id, 'optimizemember_subscr_baid', $paypal['subscr_baid']);
								update_user_option($user_id, 'optimizemember_subscr_cid', $paypal['subscr_cid']);
								...
								else delete_user_option($user_id, 'optimizemember_auto_eot_time');
								...
								if(!empty($coupon['full_coupon_code']) && c_ws_plugin__optimizemember_utils_conds::pro_is_installed())
								{
									$user_coupons = is_array($user_coupons = get_user_option('optimizemember_coupon_codes', $user_id)) ? $user_coupons : array();
									$user_coupons = array_unique(array_merge($user_coupons, (array)$coupon['full_coupon_code']));
									update_user_option($user_id, 'optimizemember_coupon_codes', $user_coupons);
								...
								c_ws_plugin__optimizemember_user_notes::clear_user_note_lines($user_id, '/^Paid Subscr\. ID @ time of demotion\:/');

								$paypal['optimizemember_log'][] = 'optimizeMember Level/Capabilities updated w/ advanced update routines.';

								$sbj = $GLOBALS['WS_PLUGIN__']['optimizemember']['o']['modification_email_subject']; // The same for standard and w/ Pro-Forms.
								$msg = $GLOBALS['WS_PLUGIN__']['optimizemember']['o']['modification_email_message']; // The same for standard and w/ Pro-Forms.
								$rec = $GLOBALS['WS_PLUGIN__']['optimizemember']['o']['modification_email_recipients']; // The same for standard and w/ Pro-Forms.

								if(($rec = c_ws_plugin__optimizemember_utils_strings::fill_cvs($rec, $paypal['custom'])) && ($rec = preg_replace('/%%subscr_id%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['subscr_id']), $rec)))
									if(($rec = preg_replace('/%%subscr_baid%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['subscr_baid']), $rec)) && ($rec = preg_replace('/%%subscr_cid%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['subscr_cid']), $rec)))
										if(($rec = preg_replace('/%%currency%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['currency']), $rec)) && ($rec = preg_replace('/%%currency_symbol%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['currency_symbol']), $rec)))
																...
																if(($rec = preg_replace('/%%first_name%%/i', c_ws_plugin__optimizemember_utils_strings::esc_dq(c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['first_name'])), $rec)) && ($rec = preg_replace('/%%last_name%%/i', c_ws_plugin__optimizemember_utils_strings::esc_dq(c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['last_name'])), $rec)))
																	if(($rec = preg_replace('/%%full_name%%/i', c_ws_plugin__optimizemember_utils_strings::esc_dq(c_ws_plugin__optimizemember_utils_strings::esc_refs(trim($paypal['first_name'].' '.$paypal['last_name']))), $rec))) // **NOTE** c_ws_plugin__optimizemember_utils_strings::esc_dq() is applied here. (ex. 'N\'ame' <email>).
																		if(($rec = preg_replace('/%%payer_email%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['payer_email']), $rec)))
																			...
																			if(($rec = preg_replace('/%%full_coupon_code%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($coupon['full_coupon_code']), $rec)) && ($rec = preg_replace('/%%coupon_code%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($coupon['coupon_code']), $rec)) && ($rec = preg_replace('/%%coupon_affiliate_id%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($coupon['affiliate_id']), $rec)))

																				if(($rec = preg_replace('/%%user_first_name%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($user->first_name), $rec)) && ($rec = preg_replace('/%%user_last_name%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($user->last_name), $rec)))
																					if(($rec = preg_replace('/%%user_full_name%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(trim($user->first_name.' '.$user->last_name)), $rec)))
																						if(($rec = preg_replace('/%%user_email%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($user->user_email), $rec)))
																							if(($rec = preg_replace('/%%user_login%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($user->user_login), $rec)))
																								if(($rec = preg_replace('/%%user_ip%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($user_reg_ip), $rec)))
																									if(($rec = preg_replace('/%%user_id%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($user_id), $rec)))

																										if(($sbj = c_ws_plugin__optimizemember_utils_strings::fill_cvs($sbj, $paypal['custom'])) && ($sbj = preg_replace('/%%subscr_id%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['subscr_id']), $sbj)))
																											if(($sbj = preg_replace('/%%subscr_baid%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['subscr_baid']), $sbj)) && ($sbj = preg_replace('/%%subscr_cid%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['subscr_cid']), $sbj)))
																												if(($sbj = preg_replace('/%%currency%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['currency']), $sbj)) && ($sbj = preg_replace('/%%currency_symbol%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['currency_symbol']), $sbj)))
																																		...
																																		if(($sbj = preg_replace('/%%first_name%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['first_name']), $sbj)) && ($sbj = preg_replace('/%%last_name%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['last_name']), $sbj)))
																																			if(($sbj = preg_replace('/%%full_name%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(trim($paypal['first_name'].' '.$paypal['last_name'])), $sbj)))
																																				if(($sbj = preg_replace('/%%payer_email%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['payer_email']), $sbj)))
																																					...
																																					if(($sbj = preg_replace('/%%full_coupon_code%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($coupon['full_coupon_code']), $sbj)) && ($sbj = preg_replace('/%%coupon_code%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($coupon['coupon_code']), $sbj)) && ($sbj = preg_replace('/%%coupon_affiliate_id%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($coupon['affiliate_id']), $sbj)))

																																						if(($sbj = preg_replace('/%%user_first_name%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($user->first_name), $sbj)) && ($sbj = preg_replace('/%%user_last_name%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($user->last_name), $sbj)))
																																							if(($sbj = preg_replace('/%%user_full_name%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(trim($user->first_name.' '.$user->last_name)), $sbj)))
																																								if(($sbj = preg_replace('/%%user_email%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($user->user_email), $sbj)))
																																									if(($sbj = preg_replace('/%%user_login%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($user->user_login), $sbj)))
																																										if(($sbj = preg_replace('/%%user_ip%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($user_reg_ip), $sbj)))
																																											if(($sbj = preg_replace('/%%user_id%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($user_id), $sbj)))

																																												if(($msg = c_ws_plugin__optimizemember_utils_strings::fill_cvs($msg, $paypal['custom'])) && ($msg = preg_replace('/%%subscr_id%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['subscr_id']), $msg)))
																																													if(($msg = preg_replace('/%%subscr_baid%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['subscr_baid']), $msg)) && ($msg = preg_replace('/%%subscr_cid%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['subscr_cid']), $msg)))
																																														if(($msg = preg_replace('/%%currency%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['currency']), $msg)) && ($msg = preg_replace('/%%currency_symbol%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['currency_symbol']), $msg)))
																																																				...
																																																				if(($msg = preg_replace('/%%first_name%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['first_name']), $msg)) && ($msg = preg_replace('/%%last_name%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['last_name']), $msg)))
																																																					if(($msg = preg_replace('/%%full_name%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(trim($paypal['first_name'].' '.$paypal['last_name'])), $msg)))
																																																						if(($msg = preg_replace('/%%payer_email%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['payer_email']), $msg)))
																																																							...
																																																							if(($msg = preg_replace('/%%full_coupon_code%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($coupon['full_coupon_code']), $msg)) && ($msg = preg_replace('/%%coupon_code%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($coupon['coupon_code']), $msg)) && ($msg = preg_replace('/%%coupon_affiliate_id%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($coupon['affiliate_id']), $msg)))
																																																											...
																																																											if(($msg = preg_replace('/%%user_login%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($user->user_login), $msg)))
																																																												if(($msg = preg_replace('/%%user_ip%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($user_reg_ip), $msg)))
																																																													if(($msg = preg_replace('/%%user_id%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($user_id), $msg)))
																																																													{
																																																														...
																																																														{
																																																															$rec = preg_replace('/%%'.preg_quote($var, '/').'%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(maybe_serialize($val)), $rec);
																																																															$sbj = preg_replace('/%%'.preg_quote($var, '/').'%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(maybe_serialize($val)), $sbj);
																																																															$msg = preg_replace('/%%'.preg_quote($var, '/').'%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(maybe_serialize($val)), $msg);
																																																														}
																																																														if(($rec = trim(preg_replace('/%%(.+?)%%/i', '', $rec))) && ($sbj = trim(preg_replace('/%%(.+?)%%/i', '', $sbj))) && ($msg = trim(preg_replace('/%%(.+?)%%/i', '', $msg))))
																																																														{
																																																															...
																																																															{
																																																																$sbj = c_ws_plugin__optimizemember_utilities::evl($sbj, get_defined_vars());
																																																																$msg = c_ws_plugin__optimizemember_utilities::evl($msg, get_defined_vars());
																																																															}
																																																															foreach(c_ws_plugin__optimizemember_utils_strings::parse_emails($rec) as $recipient /* Go through a possible list of recipients. */)
																																																																c_ws_plugin__optimizemember_email_configs::email_config().wp_mail($recipient, apply_filters('ws_plugin__optimizemember_modification_email_sbj', $sbj, get_defined_vars()), apply_filters('ws_plugin__optimizemember_modification_email_msg', $msg, get_defined_vars()), 'From: "'.preg_replace('/"/', "'", $GLOBALS['WS_PLUGIN__']['optimizemember']['o']['reg_email_from_name']).'" <'.$GLOBALS['WS_PLUGIN__']['optimizemember']['o']['reg_email_from_email'].'>'."\r\n".'Content-Type: text/plain; charset=UTF-8').c_ws_plugin__optimizemember_email_configs::email_config_release();

																																																															$paypal['optimizemember_log'][] = 'Modification Confirmation Email sent to: '.$rec.'.';
																																																														}
																																																													}
								if($processing && $GLOBALS['WS_PLUGIN__']['optimizemember']['o']['modification_notification_urls'] && is_array($cv = preg_split('/\|/', $paypal['custom'])))
								{
									foreach(preg_split('/['."\r\n\t".']+/', $GLOBALS['WS_PLUGIN__']['optimizemember']['o']['modification_notification_urls']) as $url)
										...
										if(($url = c_ws_plugin__optimizemember_utils_strings::fill_cvs($url, $paypal['custom'], true)) && ($url = preg_replace('/%%subscr_id%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($paypal['subscr_id'])), $url)))
											if(($url = preg_replace('/%%subscr_baid%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($paypal['subscr_baid'])), $url)) && ($url = preg_replace('/%%subscr_cid%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($paypal['subscr_cid'])), $url)))
												if(($url = preg_replace('/%%currency%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($paypal['currency'])), $url)) && ($url = preg_replace('/%%currency_symbol%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($paypal['currency_symbol'])), $url)))
																...
																if(($url = preg_replace('/%%first_name%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($paypal['first_name'])), $url)) && ($url = preg_replace('/%%last_name%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($paypal['last_name'])), $url)))
																	if(($url = preg_replace('/%%full_name%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode(trim($paypal['first_name'].' '.$paypal['last_name']))), $url)))
																		if(($url = preg_replace('/%%payer_email%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($paypal['payer_email'])), $url)))
																			...
																			if(($url = preg_replace('/%%full_coupon_code%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($coupon['full_coupon_code'])), $url)) && ($url = preg_replace('/%%coupon_code%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($coupon['coupon_code'])), $url)) && ($url = preg_replace('/%%coupon_affiliate_id%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($coupon['affiliate_id'])), $url)))
								...
								{
									if(($code = c_ws_plugin__optimizemember_utils_strings::fill_cvs($code, $paypal['custom'])) && ($code = preg_replace('/%%subscr_id%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['subscr_id']), $code)))
										if(($code = preg_replace('/%%subscr_baid%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['subscr_baid']), $code)) && ($code = preg_replace('/%%subscr_cid%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['subscr_cid']), $code)))
											if(($code = preg_replace('/%%currency%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['currency']), $code)) && ($code = preg_replace('/%%currency_symbol%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['currency_symbol']), $code)))
															...
															if(($code = preg_replace('/%%first_name%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['first_name']), $code)) && ($code = preg_replace('/%%last_name%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['last_name']), $code)))
																if(($code = preg_replace('/%%full_name%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(trim($paypal['first_name'].' '.$paypal['last_name'])), $code)))
																	if(($code = preg_replace('/%%payer_email%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['payer_email']), $code)))
																		...
																		if(($code = preg_replace('/%%full_coupon_code%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($coupon['full_coupon_code']), $code)) && ($code = preg_replace('/%%coupon_code%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($coupon['coupon_code']), $code)) && ($code = preg_replace('/%%coupon_affiliate_id%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($coupon['affiliate_id']), $code)))
							...
							else $paypal['optimizemember_log'][] = 'Unable to modify Subscription. The existing User ID is associated with an Administrator. Stopping here. Otherwise, an Administrator could lose access.';
						}
						else $paypal['optimizemember_log'][] = 'Unable to modify Subscription. Could not get the existing User ID from the DB. Please check the `on0` and `os0` variables in your Button Code.';
					...
					*/
					else // Else this is a normal Subscription signup, we are not updating anything.
					{
						...
						if(($registration_url = c_ws_plugin__optimizemember_register_access::register_link_gen($paypal['subscr_gateway'], $paypal['subscr_id'], $paypal['custom'], $paypal['item_number'])) && is_array($cv = preg_split('/\|/', $paypal['custom'])))
						{
							...
							$sbj = preg_replace('/%%registration_url%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($registration_url), $GLOBALS['WS_PLUGIN__']['optimizemember']['o'][(($_REQUEST['optimizemember_paypal_proxy'] && preg_match('/pro-emails/', $_REQUEST['optimizemember_paypal_proxy_use'])) ? 'pro_' : '').'signup_email_subject']);
							$msg = preg_replace('/%%registration_url%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($registration_url), $GLOBALS['WS_PLUGIN__']['optimizemember']['o'][(($_REQUEST['optimizemember_paypal_proxy'] && preg_match('/pro-emails/', $_REQUEST['optimizemember_paypal_proxy_use'])) ? 'pro_' : '').'signup_email_message']);
							$rec = preg_replace('/%%registration_url%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($registration_url), $GLOBALS['WS_PLUGIN__']['optimizemember']['o'][(($_REQUEST['optimizemember_paypal_proxy'] && preg_match('/pro-emails/', $_REQUEST['optimizemember_paypal_proxy_use'])) ? 'pro_' : '').'signup_email_recipients']);
						...
						else $paypal['optimizemember_log'][] = 'Unable to generate Registration URL for Membership Access. Possible data corruption within the IPN response.';

						foreach(array_keys(get_defined_vars()) as $__v) $__refs[$__v] =& $$__v;
						do_action('ws_plugin__optimizemember_during_paypal_notify_after_subscr_signup_wo_update_vars', get_defined_vars());
						unset($__refs, $__v);
					}
					if($processing && $_REQUEST['optimizemember_paypal_proxy'] && ($url = $_REQUEST['optimizemember_paypal_proxy_return_url']) && is_array($cv = preg_split('/\|/', $paypal['custom']))) // A Proxy is requesting a Return URL?
					{
						if((!empty($user_id) && !empty($user) && is_object($user) && $user->ID) || (($user_id = c_ws_plugin__optimizemember_utils_users::get_user_id_with($paypal['subscr_id'], $paypal['option_selection1'])) && is_object($user = new WP_User ($user_id)) && $user->ID))
						{
					...
					if($processing // Process a payment now? Special cases for web_accept and/or Proxy requests with `subscr-signup-as-subscr-payment`.
					   && (preg_match('/^web_accept$/i', $paypal['txn_type']) || ($_REQUEST['optimizemember_paypal_proxy'] && preg_match('/subscr-signup-as-subscr-payment/', $_REQUEST['optimizemember_paypal_proxy_use']) && $paypal['txn_id'] && $paypal['mc_gross'] > 0))
					   && ((!empty($user_id) && !empty($user) && is_object($user) && $user->ID) || (($user_id = c_ws_plugin__optimizemember_utils_users::get_user_id_with($paypal['subscr_id'], $paypal['option_selection1'])) && is_object($user = new WP_User ($user_id)) && $user->ID))
					)
					{
						...
						if(empty($processed_coupons) && !empty($coupon['full_coupon_code']) && c_ws_plugin__optimizemember_utils_conds::pro_is_installed())
							{
								$user_coupons = is_array($user_coupons = get_user_option('optimizemember_coupon_codes', $user_id)) ? $user_coupons : array();
								$user_coupons = array_unique(array_merge($user_coupons, (array)$coupon['full_coupon_code']));
								update_user_option($user_id, 'optimizemember_coupon_codes', $user_coupons);
