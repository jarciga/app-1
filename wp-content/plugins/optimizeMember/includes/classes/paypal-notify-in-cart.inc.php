...
if (!class_exists ("c_ws_plugin__optimizemember_paypal_notify_in_cart"))
	{
		/**
		* optimizeMember's PayPal IPN handler (inner processing routine).
		*
		...
		* @since 110720
		*/
		class c_ws_plugin__optimizemember_paypal_notify_in_cart
			{
				/**
				* optimizeMember's PayPal IPN handler (inner processing routine).
				*
						...
						if (!empty($paypal["txn_type"]) && preg_match ("/^cart$/i", $paypal["txn_type"]))
							{
								...
								do_action("ws_plugin__optimizemember_during_paypal_notify_before_cart", get_defined_vars ());
								unset($__refs, $__v);
								...
								if (!get_transient ($transient_ipn = "s2m_ipn_" . md5 ("optimizemember_transient_" . $_paypal_s)) && set_transient ($transient_ipn, time (), 31556926 * 10))
									{
										$paypal["optimizemember_log"][] = "optimizeMember `txn_type` identified as ( `cart` ).";

										$processing = $during = true; // Yes, we ARE processing this.

										$paypal["optimizemember_log"][] = "The `txn_type` does not require any action on the part of optimizeMember.";
										$paypal["optimizemember_log"][] = "optimizeMember Pro handles Cart events on-site, with an IPN proxy.";

										foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;
										do_action("ws_plugin__optimizemember_during_paypal_notify_during_cart", get_defined_vars ());
										unset($__refs, $__v);
									}
								...
								foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;
								do_action("ws_plugin__optimizemember_during_paypal_notify_after_cart", get_defined_vars ());
								unset($__refs, $__v);

								return apply_filters("c_ws_plugin__optimizemember_paypal_notify_in_cart", $paypal, get_defined_vars ());
							}
						else return apply_filters("c_ws_plugin__optimizemember_paypal_notify_in_cart", false, get_defined_vars ());
					}
			}
	}