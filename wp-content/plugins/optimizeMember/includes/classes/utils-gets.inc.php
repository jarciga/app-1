...
/**/
if (!class_exists ("c_ws_plugin__optimizemember_utils_gets"))
	{
		/**
		* Get utilities.
		*
		...
		* @since 3.5
		*/
		class c_ws_plugin__optimizemember_utils_gets
			{
				/**
				...
				public static function get_all_category_ids ()
					{
                        ...
                        if(is_array($category_ids)) // Use a WP function for this.
							$category_ids = c_ws_plugin__optimizemember_utils_arrays::force_integers ($category_ids);
						/**/
             ...
             * @since 3.5
             *
             * @param bool|int|string $parent_category A numeric Category ID.
             ...
             * @since 111101
             *
             * @param bool|string $tags Tag slugs/names/IDs; comma-delimited.
             ...
             * @since 3.5
             *
             * @param bool|string $post_type Optional. If provided, return all Post IDs of a spefcific Post Type.
				...
				public static function get_all_post_ids ($post_type = FALSE)
					{
						...
						if (is_array ($post_ids = $wpdb->get_col ("SELECT `ID` FROM `" . $wpdb->posts . "` WHERE `post_status` = 'publish' AND " . (($post_type) ? "`post_type` = '" . esc_sql ((string)$post_type) . "'" : "`post_type` NOT IN('page','attachment','nav_menu_item','revision')"))))
							$post_ids = c_ws_plugin__optimizemember_utils_arrays::force_integers ($post_ids);
						/**/
				...
				public static function get_all_page_ids ()
					{
						...
						if (is_array ($page_ids = $wpdb->get_col ("SELECT `ID` FROM `" . $wpdb->posts . "` WHERE `post_status` = 'publish' AND `post_type` = 'page'")))
							$page_ids = c_ws_plugin__optimizemember_utils_arrays::force_integers ($page_ids);
						/**/
				...
				public static function get_all_singular_ids_with_ccaps ()
					{
						global $wpdb; /* Need this global DB object reference here. */
						/**/
						if (is_array ($singular_ids = $wpdb->get_col ("SELECT `post_id` FROM `" . $wpdb->postmeta . "` WHERE `meta_key` = 'optimizemember_ccaps_req' AND `meta_value` != ''")))
             ...
             * @since 111101
             *
             * @param bool|object $user Optional. A `WP_User` object. If this is a valid `WP_User` object, test against this ``$user``, else all are unavailable.
				...
				public static function get_unavailable_singular_ids_with_ccaps ($user = FALSE)
					{
						global $wpdb; /* Need this global DB object reference here. */
						/**/
						if (is_array ($results = $wpdb->get_results ("SELECT `post_id`, `meta_value` FROM `" . $wpdb->postmeta . "` WHERE `meta_key` = 'optimizemember_ccaps_req' AND `meta_value` != ''")))
									...
									/**/
									else if (is_array ($ccaps = /* Make sure we unserialize. */ @unserialize ($r->meta_value)))
										{
											foreach ($ccaps as $ccap) /* Test for Custom Capability Restrictions now. */
												if (strlen ($ccap) && !$user->has_cap ("access_optimizemember_ccap_" . $ccap))
													{
			...
			/**
				* Retrieves a unique array of all Singular IDs in the database that has dripped content set
				*
				* @package optimizeMember\Utilities
				* @since 111101
				*
				* @return array Unique array of all Singular IDs *( as integers )* that are dripped content
				*/
				public static function get_all_singular_ids_with_dripped_content ()
					{
						global $wpdb; /* Need this global DB object reference here. */
						/**/
						if (is_array ($singular_ids = $wpdb->get_col ("SELECT `post_id` FROM `" . $wpdb->postmeta . "` WHERE `meta_key` = 'optimizemember_drip_days' AND `meta_value` != ''")))
							$singular_ids = c_ws_plugin__optimizemember_utils_arrays::force_integers ($singular_ids);
						/**/
						return (!empty ($singular_ids) && is_array ($singular_ids)) ? array_unique ($singular_ids) : array ();
					}
            ...
            /**
             * Retrieves a unique array of unavailable Singular IDs that has dripped content set
             *
             * Only returns Singular IDs that require Custom Capabilities;
             *    and ONLY those which are NOT satisfied by ``$user``.
             *
             ...
             * @since 111101
             *
             * @param bool|WP_User $user Optional. A `WP_User` object. If this is a valid `WP_User` object, test against this ``$user``, else all are unavailable.
             * @return array Unique array of all Singular IDs *( as integers )* NOT available to ``$user``, due to drp content rules
             */
				public static function get_unavailable_singular_ids_with_dripped_content ($user = FALSE)
					{
						global $wpdb; /* Need this global DB object reference here. */
						/**/
						if (is_array ($results = $wpdb->get_results ("SELECT `post_id`, `meta_value` FROM `" . $wpdb->postmeta . "` WHERE `meta_key` = 'optimizemember_drip_days' AND `meta_value` != ''"))) {
							foreach ($results as $r) /* Now we need to check drip days and registration level date against ``$user``. If ``$user`` is a valid `WP_User` object, else all are unavailable. */
								{
									if (!is_object ($user) || empty ($user->ID)) /* No ``$user`` object? Maybe not logged-in?. */
										$singular_ids[] = (int)$r->post_id; /* It's NOT available. There is no ``$user``. */
									/**/
									else if ($drips = $r->meta_value) {
                                        if(!$user->has_cap('administrator')) { // do not process this for admins
                                            $time = optimizemember_paid_registration_time('level' . OPTIMIZEMEMBER_CURRENT_USER_ACCESS_LEVEL);
                                            if ($time && $time >= ($_days_ago = strtotime("-$drips days"))) {
                                                $singular_ids[] = (int)$r->post_id; /* It's NOT available. */
				...
				*/
				public static function get_all_singular_ids_with_sp ($exclude_conflicts = FALSE)
					{
						if (is_array (($singular_ids = ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["specific_ids"] && is_array ($singular_ids = preg_split ("/[\r\n\t\s;,]+/", $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["specific_ids"]))) ? $singular_ids : array ())))
							$singular_ids = c_ws_plugin__optimizemember_utils_arrays::force_integers ($singular_ids);
						/**/
						if (!empty ($singular_ids) && is_array ($singular_ids) && $exclude_conflicts /* Return ONLY those which are NOT in conflict with other Restrictions? */)
							{
								$x_ids = array ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["login_welcome_page"], $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["membership_options_page"], $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["file_download_limit_exceeded_page"]);
								/**/
								$x_ids = array_merge ($x_ids, c_ws_plugin__optimizemember_utils_gets::get_all_singular_ids_with_ccaps ());
								/**/
								for ($n = 0; $n <= $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["levels"]; $n++)
									{
										if ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["level" . $n . "_catgs"] === "all")
											{
												$catgs = c_ws_plugin__optimizemember_utils_gets::get_all_category_ids ();
												$x_ids = array_merge ($x_ids, c_ws_plugin__optimizemember_utils_gets::get_singular_ids_in_terms ($catgs));
										...
										foreach (($catgs = preg_split ("/[\r\n\t\s;,]+/", $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["level" . $n . "_catgs"])) as $catg)
											$catgs = array_merge ($catgs, c_ws_plugin__optimizemember_utils_gets::get_all_child_category_ids ($catg));
										/**/
										$x_ids = /* Exclude the full list. */ array_merge ($x_ids, c_ws_plugin__optimizemember_utils_gets::get_singular_ids_in_terms ($catgs));
										unset /* Just a little housekeeping here. */ ($catgs, $catg);
									}
								/**/
								for ($n = 0; $n <= $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["levels"]; $n++)
									{
										if ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["level" . $n . "_ptags"] === "all")
											{
												$tags = c_ws_plugin__optimizemember_utils_gets::get_all_tag_ids ();
												$x_ids = array_merge ($x_ids, c_ws_plugin__optimizemember_utils_gets::get_singular_ids_in_terms ($tags));
												continue; /* Continue. The `all` specification is absolute. There's nothing more. */
											}
										/**/
										$tags = c_ws_plugin__optimizemember_utils_gets::get_tags_converted_to_ids ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["level" . $n . "_ptags"]);
										/**/
										$x_ids = /* Exclude the full list. */ array_merge ($x_ids, c_ws_plugin__optimizemember_utils_gets::get_singular_ids_in_terms ($tags));
										unset /* Just a little housekeeping here. */ ($tags);
									}
								/**/
								for ($n = 0; $n <= $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["levels"]; $n++)
									{
										if ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["level" . $n . "_posts"] === "all")
											{
												$x_ids = array_merge ($x_ids, c_ws_plugin__optimizemember_utils_gets::get_all_post_ids ());
												continue; /* Continue. The `all` specification is absolute. There's nothing more. */
											}
										/**/
										foreach (($posts = preg_split ("/[\r\n\t\s;,]+/", $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["level" . $n . "_posts"])) as $p)
											if (strpos ($p, "all-") === 0 && preg_match ("/^all-(.+)$/", $p, $m) /* Protecting `all-` of a specific Post Type? */)
												if (is_array ($p_of_type = c_ws_plugin__optimizemember_utils_gets::get_all_post_ids ($m[1])) && !empty ($p_of_type))
								...
								for ($n = 0; $n <= $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["levels"]; $n++)
									{
										if ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["level" . $n . "_pages"] === "all")
											{
												$x_ids = array_merge ($x_ids, c_ws_plugin__optimizemember_utils_gets::get_all_page_ids ());
												continue; /* Continue. The `all` specification is absolute. There's nothing more. */
											}
										/**/
								...
								/**/
								$x_ids = array_unique (c_ws_plugin__optimizemember_utils_arrays::force_integers ($x_ids));
				...
				public static function get_unavailable_singular_ids_with_sp ($exclude_conflicts = FALSE)
					{
						if ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["specific_ids"] && is_array ($_singular_ids = preg_split ("/[\r\n\t\s;,]+/", $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["specific_ids"])))
							foreach ($_singular_ids as $_singular_id) /* Now check access to this Singular, against the current Visitor, via read-only ``c_ws_plugin__optimizemember_sp_access::sp_access()``. */
             ...
             * @since 110912
             *
             * @param array|bool $terms Required. An array of term IDs.
