...
if(!class_exists('c_ws_plugin__optimizemember_paypal_notify_in_subscr_or_rp_payment_w_level'))
{
	/**
	 * optimizeMember's PayPal IPN handler (inner processing routine).
	 *
	 ...
	 * @since 110720
	 */
	class c_ws_plugin__optimizemember_paypal_notify_in_subscr_or_rp_payment_w_level
	{
		/**
		 * optimizeMember's PayPal IPN handler (inner processing routine).
		 *
		...
		public static function cp($vars = array()) // Conditional phase for ``c_ws_plugin__optimizemember_paypal_notify_in::paypal_notify()``.
		{
			...
			)
			{
				...
				if(!get_transient($transient_ipn = 's2m_ipn_'.md5('optimizemember_transient_'.$_paypal_s)) && set_transient($transient_ipn, time(), 31556926 * 10))
				{
					...
					if(($user_id = c_ws_plugin__optimizemember_utils_users::get_user_id_with($paypal['subscr_id'])) && is_object($user = new WP_User ($user_id)) && $user->ID)
					{
						...
						update_user_option($user_id, 'optimizemember_last_payment_time', time()); // Also update last payment time.

						$paypal['optimizemember_log'][] = 'Updated Payment Times for this Member.'; // Flag this action in the log.
			...
			else return apply_filters('c_ws_plugin__optimizemember_paypal_notify_in_subscr_or_rp_payment_w_level', FALSE, get_defined_vars());
		}
	}
}