...
/**/
if (!class_exists ("c_ws_plugin__optimizemember_no_cache"))
	{
		/**
		* No-cache routines.
		*
		...
		* @since 3.5
		*/
		class c_ws_plugin__optimizemember_no_cache
			{
				/**
				...
				public static function no_cache_constants ($no_cache = FALSE)
					{
						...
						if(!$once && (empty($_GET['qcAC']) || !filter_var($_GET['qcAC'], FILTER_VALIDATE_BOOLEAN))
							&& (
								...
								|| c_ws_plugin__optimizemember_systematics::is_s2_systematic_use_page()
							)
						)
						{
								/**
								...
								*/
								...
								/**/
								c_ws_plugin__optimizemember_no_cache::$headers = /* Headers required too. */ true;
								/**/
				...
				* Disallow browser caching when/if no-cache Constants are set by {@link optimizeMember\No_Cache\c_ws_plugin__optimizemember_no_cache::no_cache_constants()},
				* 	via static variable boolean value for: ``c_ws_plugin__optimizemember_no_cache::$headers``.
				*
				...
				public static function no_cache_headers ($no_cache = FALSE)
					{
						...
						/**/
						eval ('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
						do_action ("ws_plugin__optimizemember_before_no_cache_headers", get_defined_vars ());
						unset ($__refs, $__v); /* Unset defined __refs, __v. */
						/**/
						$using_selective_behavior = /* Off by default. */ apply_filters ("ws_plugin__optimizemember_no_cache_headers_selective", false, get_defined_vars ());
						/**/
                        if(!$once && !headers_sent() // Only once, and only if possible.
                            ...
                            && ($no_cache || !$using_selective_behavior || c_ws_plugin__optimizemember_no_cache::$headers)
                        )
							if ( /* Give Filters a chance. */!apply_filters ("ws_plugin__optimizemember_disable_no_cache_headers", false, get_defined_vars ()))
								{
									...
									/**/
									do_action ("ws_plugin__optimizemember_during_no_cache_headers", get_defined_vars ());
								}
						/**/
						do_action ("ws_plugin__optimizemember_after_no_cache_headers", get_defined_vars ());
						/**/
						return true; /* Always return true. */
					}
			}
