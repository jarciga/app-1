...
if(!class_exists('c_ws_plugin__optimizemember_paypal_notify_in_subscr_or_rp_eots_w_level'))
{
	/**
	 * optimizeMember's PayPal IPN handler (inner processing routine).
	 *
	 ...
	 * @since 110720
	 */
	class c_ws_plugin__optimizemember_paypal_notify_in_subscr_or_rp_eots_w_level
	{
		/**
		 * optimizeMember's PayPal IPN handler (inner processing routine).
		 *
		...
		public static function cp($vars = array()) // Conditional phase for ``c_ws_plugin__optimizemember_paypal_notify_in::paypal_notify()``.
		{
			...
			if(((!empty($paypal['txn_type']) && preg_match('/^(subscr_eot|recurring_payment_expired|recurring_payment_suspended_due_to_max_failed_payment)$/i', $paypal['txn_type']) && ($recurring = TRUE))
			   ...
			   && (!empty($paypal['subscr_id']) || ($paypal['subscr_id'] = c_ws_plugin__optimizemember_paypal_utilities::paypal_pro_subscr_id($paypal)) || (!empty($paypal['parent_txn_id']) && ($paypal['subscr_id'] = $paypal['parent_txn_id']))) // Other MUST haves.
			   && (!empty($paypal['period1']) || ($paypal['period1'] = c_ws_plugin__optimizemember_paypal_utilities::paypal_pro_period1($paypal, FALSE)) || empty($recurring) || ($paypal['period1'] = c_ws_plugin__optimizemember_utils_users::get_user_ipn_signup_var('period1', FALSE, $paypal['subscr_id'])) || ($paypal['period1'] = '0 D'))
			   && (!empty($paypal['period3']) || ($paypal['period3'] = c_ws_plugin__optimizemember_paypal_utilities::paypal_pro_period3($paypal, FALSE)) || empty($recurring) || ($paypal['period3'] = c_ws_plugin__optimizemember_utils_users::get_user_ipn_signup_var('period3', FALSE, $paypal['subscr_id'])) || ($paypal['period3'] = '1 D'))
			   && ((!empty($paypal['item_number']) || ($paypal['item_number'] = c_ws_plugin__optimizemember_paypal_utilities::paypal_pro_item_number($paypal)) || ($paypal['item_number'] = c_ws_plugin__optimizemember_utils_users::get_user_ipn_signup_var('item_number', FALSE, $paypal['subscr_id'])) || ($paypal['item_number'] = '1')) && preg_match($GLOBALS['WS_PLUGIN__']['optimizemember']['c']['membership_item_number_w_level_regex'], $paypal['item_number']))
			   && (!empty($paypal['item_name']) || ($paypal['item_name'] = c_ws_plugin__optimizemember_paypal_utilities::paypal_pro_item_name($paypal)) || ($paypal['item_name'] = c_ws_plugin__optimizemember_utils_users::get_user_ipn_signup_var('item_name', FALSE, $paypal['subscr_id'])) || ($paypal['item_name'] = $_SERVER['HTTP_HOST']))
			   && (!empty($paypal['payer_email']) || ($paypal['payer_email'] = c_ws_plugin__optimizemember_utils_users::get_user_ipn_signup_var('payer_email', FALSE, $paypal['subscr_id'])) || ($paypal['payer_email'] = c_ws_plugin__optimizemember_utils_users::get_user_email_with($paypal['subscr_id'])))
			...
			)
			{
				...
				if(!get_transient($transient_ipn = 's2m_ipn_'.md5('optimizemember_transient_'.$_paypal_s)) && set_transient($transient_ipn, time(), 31556926 * 10))
				{
					...
					$is_partial_refund     = // Partial refund detection. All refunds processed against Subscriptions are considered partials. Full refunds occur only against Buy Now transactions.
						(!$is_refund || (!empty($paypal['mc_gross']) && ($original_txn_type = c_ws_plugin__optimizemember_utils_users::get_user_ipn_signup_var('txn_type', FALSE, $paypal['subscr_id'])) === 'web_accept'
						                 && ($original_mc_gross = c_ws_plugin__optimizemember_utils_users::get_user_ipn_signup_var('mc_gross', FALSE, $paypal['subscr_id'])) <= abs($paypal['mc_gross']))) ? FALSE : TRUE;
					...
					else $paypal['optimizemember_log'][] = 'optimizeMember `txn_type` identified as '.($identified_as = '( `subscr_eot|recurring_payment_expired|recurring_payment_suspended_due_to_max_failed_payment` ) - or - `recurring_payment_profile_cancel` w/ `initial_payment_status` ( `failed` )').'.';

					if(empty($_REQUEST['optimizemember_paypal_proxy'])) // Only on true PayPal IPNs; e.g., we can bypass this on proxied IPNs.
					{
						$paypal['optimizemember_log'][] = 'Sleeping for 15 seconds. Waiting for a possible ( `subscr_signup|subscr_modify|recurring_payment_profile_created` ).';
					...
					{
						$fields      = get_user_option('optimizemember_custom_fields', $user_id); // These will be needed below.
						$user_reg_ip = get_user_option('optimizemember_registration_ip', $user_id); // Needed below.
						$user_reg_ip = $paypal['ip'] = ($user_reg_ip) ? $user_reg_ip : $paypal['ip'];

						if((!$is_refund_or_reversal && !$is_delayed_eot && !get_user_option('optimizemember_auto_eot_time', $user_id))
						   || ($is_refund_or_reversal && $is_partial_refund && $GLOBALS['WS_PLUGIN__']['optimizemember']['o']['triggers_immediate_eot'] === 'refunds,partial_refunds,reversals')
						   || ($is_refund_or_reversal && !$is_partial_refund && $GLOBALS['WS_PLUGIN__']['optimizemember']['o']['triggers_immediate_eot'] === 'refunds,reversals')
						   || ($is_refund && !$is_partial_refund && $GLOBALS['WS_PLUGIN__']['optimizemember']['o']['triggers_immediate_eot'] === 'refunds')
						   || ($is_reversal && $GLOBALS['WS_PLUGIN__']['optimizemember']['o']['triggers_immediate_eot'] === 'reversals')
						)
						{
							if(!$user->has_cap('administrator')) // Do NOT process this routine on Administrators.
							{
								if($GLOBALS['WS_PLUGIN__']['optimizemember']['o']['auto_eot_system_enabled']) // EOT enabled?
								{
									if($GLOBALS['WS_PLUGIN__']['optimizemember']['o']['membership_eot_behavior'] === 'demote')
									{
										...
										if(apply_filters('ws_plugin__optimizemember_remove_ccaps_during_eot_events', (bool)$GLOBALS['WS_PLUGIN__']['optimizemember']['o']['eots_remove_ccaps'] || $is_refund_or_reversal, get_defined_vars()))
										...
										delete_user_option($user_id, 'optimizemember_subscr_baid');
										delete_user_option($user_id, 'optimizemember_subscr_cid');
										...
										c_ws_plugin__optimizemember_user_notes::append_user_notes($user_id, 'Paid Subscr. ID @ time of demotion: '.$paypal['subscr_gateway'].' ⥱ '.$paypal['subscr_id']);
										...
										{
											foreach(preg_split('/['."\r\n\t".']+/', $GLOBALS['WS_PLUGIN__']['optimizemember']['o']['eot_del_notification_urls']) as $url) // Handle EOT Notifications.

												if(($url = c_ws_plugin__optimizemember_utils_strings::fill_cvs($url, $paypal['custom'], true)) && ($url = preg_replace('/%%eot_del_type%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($eot_del_type)), $url)) && ($url = preg_replace('/%%subscr_id%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($paypal['subscr_id'])), $url)))
										...
										if($processing && $GLOBALS['WS_PLUGIN__']['optimizemember']['o']['eot_del_notification_recipients'] && is_array($cv = preg_split('/\|/', $paypal['custom'])))
										{
											...
											if (($msg = c_ws_plugin__optimizemember_utils_strings::fill_cvs($msg, $paypal['custom'])) && ($msg = preg_replace('/%%eot_del_type%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($eot_del_type), $msg)) && ($msg = preg_replace('/%%subscr_id%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['subscr_id']), $msg)))
									...
									else if($GLOBALS['WS_PLUGIN__']['optimizemember']['o']['membership_eot_behavior'] === 'delete')
									{
										$processing = $during = TRUE; // Yes, we ARE processing this.

										$eot_del_type = $GLOBALS['ws_plugin__optimizemember_eot_del_type'] = // Configure EOT/Del type.
										...
										if(is_multisite()) // Multisite does NOT actually delete; ONLY removes.
										{
											...
											// This will automatically trigger `eot_del_notification_urls` as well.
											wp_delete_user($user_id); // `c_ws_plugin__optimizemember_user_deletions::handle_user_deletions()`
								...
								else // Otherwise, treat this as if it were a cancellation. EOTs are currently disabled.
								{
									...
									$paypal['optimizemember_log'][] = 'Auto-EOT is currently disabled. Skipping EOT (demote|delete), for now.';
							...
							else $paypal['optimizemember_log'][] = 'Unable to (demote|delete) Member. The existing User ID is associated with an Administrator. Stopping here. Otherwise, an Administrator could lose access.';
						}
						else if($is_delayed_eot && !get_user_option('optimizemember_auto_eot_time', $user_id))
						{
							if(!$user->has_cap('administrator')) // Do NOT process this routine on Administrators.
							{
								$processing = $during = TRUE; // Yes, we ARE processing this.
								...
								update_user_option($user_id, 'optimizemember_auto_eot_time', $auto_eot_time); // optimizeMember will follow-up on this later.
							...
							else $paypal['optimizemember_log'][] = 'Ignoring Delayed EOT. The existing User ID is associated with an Administrator. Stopping here. Otherwise, an Administrator could lose access.';
						}
							...
							$paypal['optimizemember_log'][] = 'Skipping (demote|delete) Member. Your configuration dictates that optimizeMember should NOT take any immediate action on an EOT associated with a Partial Refund. An optimizeMember API Notification will still be processed however.';

						else if($is_refund && !$is_partial_refund)
							$paypal['optimizemember_log'][] = 'Skipping (demote|delete) Member. Your configuration dictates that optimizeMember should NOT take any immediate action on an EOT associated with a Full Refund. An optimizeMember API Notification will still be processed however.';
					...
					else $paypal['optimizemember_log'][] = 'Unable to (demote|delete) Member. Could not get the existing User ID from the DB. It\'s possible that it was ALREADY processed through another IPN, removed manually by a Site Administrator, or by optimizeMember\'s Auto-EOT Sys.';
					/*
					...
					*/
					...
					{
						$fields      = ($user_id) ? get_user_option('optimizemember_custom_fields', $user_id) : array(); // These will be needed below.
						$user_reg_ip = ($user_id) ? get_user_option('optimizemember_registration_ip', $user_id) : ''; // Needed below.
						...
						if($GLOBALS['WS_PLUGIN__']['optimizemember']['o']['ref_rev_notification_urls'] && is_array($cv = preg_split('/\|/', $paypal['custom'])))
						{
							foreach(preg_split('/['."\r\n\t".']+/', $GLOBALS['WS_PLUGIN__']['optimizemember']['o']['ref_rev_notification_urls']) as $url)
								...
								if(($url = c_ws_plugin__optimizemember_utils_strings::fill_cvs($url, $paypal['custom'], true)) && ($url = preg_replace('/%%subscr_id%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($paypal['subscr_id'])), $url)) && ($url = preg_replace('/%%parent_txn_id%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($paypal['parent_txn_id'])), $url)))
									if(($url = preg_replace('/%%subscr_baid%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($paypal['subscr_baid'])), $url)) && ($url = preg_replace('/%%subscr_cid%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($paypal['subscr_cid'])), $url)))
										if(($url = preg_replace('/%%item_number%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($paypal['item_number'])), $url)) && ($url = preg_replace('/%%item_name%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($paypal['item_name'])), $url)))
											if(($url = preg_replace('/%%currency%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($paypal['currency'])), $url)) && ($url = preg_replace('/%%currency_symbol%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($paypal['currency_symbol'])), $url)))
												if(($url = preg_replace('/%%-amount%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($paypal['mc_gross'])), $url)) && ($url = preg_replace('/%%-fee%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs(urlencode($paypal['mc_fee'])), $url)))
						...
						if($GLOBALS['WS_PLUGIN__']['optimizemember']['o']['ref_rev_notification_recipients'] && is_array($cv = preg_split('/\|/', $paypal['custom'])))
						{
							...
							if(($msg = c_ws_plugin__optimizemember_utils_strings::fill_cvs($msg, $paypal['custom'])) && ($msg = preg_replace('/%%subscr_id%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['subscr_id']), $msg)) && ($msg = preg_replace('/%%parent_txn_id%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['parent_txn_id']), $msg)))
								if(($msg = preg_replace('/%%subscr_baid%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['subscr_baid']), $msg)) && ($msg = preg_replace('/%%subscr_cid%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['subscr_cid']), $msg)))
									if(($msg = preg_replace('/%%item_number%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['item_number']), $msg)) && ($msg = preg_replace('/%%item_name%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['item_name']), $msg)))
										if(($msg = preg_replace('/%%currency%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['currency']), $msg)) && ($msg = preg_replace('/%%currency_symbol%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['currency_symbol']), $msg)))
											if(($msg = preg_replace('/%%-amount%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['mc_gross']), $msg)) && ($msg = preg_replace('/%%-fee%%/i', c_ws_plugin__optimizemember_utils_strings::esc_refs($paypal['mc_fee']), $msg)))
			...
			else return apply_filters('c_ws_plugin__optimizemember_paypal_notify_in_subscr_or_rp_eots_w_level', FALSE, get_defined_vars());
		}
	}
}