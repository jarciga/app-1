<?php
/**
* optimizeMember's Auto-EOT System *( EOT = End Of Term )*.
*
* Copyright: © 2009-2011
...
*
* @package optimizeMember\Auto_EOT_System
* @since 3.5
*/
if (realpath (__FILE__) === realpath ($_SERVER["SCRIPT_FILENAME"]))
	exit ("Do not access this file directly.");
/**/
if (!class_exists ("c_ws_plugin__optimizemember_auto_eots"))
	{
		/**
		* optimizeMember's Auto-EOT System *( EOT = End Of Term )*.
		*
		...
		* @since 3.5
		*/
		class c_ws_plugin__optimizemember_auto_eots
			{
				/**
				* Adds a scheduled task for optimizeMember's Auto-EOT System.
				*
				* @package optimizeMember\Auto_EOT_System
				* @since 3.5
				*
				...
				*/
				public static function add_auto_eot_system ()
					{
						do_action ("ws_plugin__optimizemember_before_add_auto_eot_system", get_defined_vars ());
						/**/
						if (!c_ws_plugin__optimizemember_auto_eots::delete_auto_eot_system ())
							{
								return apply_filters ("ws_plugin__optimizemember_add_auto_eot_system", false, get_defined_vars ());
							}
						else if (function_exists ("wp_cron")) /* Otherwise, we can schedule? */
							{
								wp_schedule_event (time (), "every10m", "ws_plugin__optimizemember_auto_eot_system__schedule");
								/**/
								return apply_filters ("ws_plugin__optimizemember_add_auto_eot_system", true, get_defined_vars ());
							}
				...
				/**
				* Deletes all scheduled tasks for optimizeMember's Auto-EOT System.
				*
				* @package optimizeMember\Auto_EOT_System
				* @since 3.5
				*
				...
				*/
				public static function delete_auto_eot_system ()
					{
						do_action ("ws_plugin__optimizemember_before_delete_auto_eot_system", get_defined_vars ());
						/**/
						if (function_exists ("wp_cron")) /* Is `wp_cron()` even available? */
							{
								wp_clear_scheduled_hook ("ws_plugin__optimizemember_auto_eot_system__schedule"); /* Since v3.0.3. */
								/**/
								return apply_filters ("ws_plugin__optimizemember_delete_auto_eot_system", true, get_defined_vars ());
							}
						else /* Otherwise, it would appear that WP-Cron is not available. */
							{
								return apply_filters ("ws_plugin__optimizemember_delete_auto_eot_system", false, get_defined_vars ());
							}
					}
				/**
				...
				* To increase, use: ``add_filter ("ws_plugin__optimizemember_auto_eot_system_per_process");``.
				*
				* This function makes an important Hook available: `ws_plugin__optimizemember_after_auto_eot_system`.
				* This Hook is used by some of optimizeMember Pro's Gateway integrations; allowing CRON processing
				...
				* 	Can also be Filtered with `ws_plugin__optimizemember_auto_eot_system_per_process`.
				* @return null
				*/
				public static function auto_eot_system ($per_process = 3)
					{
						...
						do_action ("ws_plugin__optimizemember_before_auto_eot_system", get_defined_vars ());
						unset ($__refs, $__v); /* Unset defined __refs, __v. */
						/**/
						if ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["auto_eot_system_enabled"]) /* Enabled? */
							{
								$per_process = apply_filters ("ws_plugin__optimizemember_auto_eot_system_per_process", $per_process, get_defined_vars ());
								/**/
								if (is_array ($eots = $wpdb->get_results ("SELECT `user_id` AS `ID` FROM `" . $wpdb->usermeta . "` WHERE `meta_key` = '" . $wpdb->prefix . "optimizemember_auto_eot_time' AND `meta_value` != '' AND `meta_value` <= '" . esc_sql(strtotime ("now")) . "' LIMIT " . $per_process)))
									{
										foreach ($eots as $eot) /* Go through the array of EOTS. We need to (demote|delete) each of them. */
											{
												if (($user_id = $eot->ID) && is_object ($user = new WP_User ($user_id)) && $user->ID)
													{
														delete_user_option ($user_id, "optimizemember_auto_eot_time"); /* Always delete. */
														/**/
																...
																if ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["membership_eot_behavior"] === "demote")
																	{
																		...
																		/**/
																		eval ('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
																		do_action ("ws_plugin__optimizemember_during_auto_eot_system_during_before_demote", get_defined_vars ());
																		...
																		delete_user_option($user_id, 'optimizemember_authnet_payment_failures');

																		update_user_option($user_id, 'optimizemember_last_auto_eot_time', $auto_eot_time);
																		/**/
																		delete_user_option ($user_id, "optimizemember_file_download_access_log");
																		/**/
																		c_ws_plugin__optimizemember_user_notes::append_user_notes ($user_id, "Demoted by optimizeMember: " . date ("D M j, Y g:i a T"));
																		/**/
																		if ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["eot_del_notification_urls"] && is_array ($cv = preg_split ("/\|/", $custom)))
																			{
																				foreach (preg_split ("/[\r\n\t]+/", $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["eot_del_notification_urls"]) as $url) /* Handle EOT Notifications. */
																					/**/
																					if (($url = c_ws_plugin__optimizemember_utils_strings::fill_cvs($url, $custom, true)) && ($url = preg_replace ("/%%eot_del_type%%/i", c_ws_plugin__optimizemember_utils_strings::esc_refs (urlencode ("auto-eot-cancellation-expiration-demotion")), $url)) && ($url = preg_replace ("/%%subscr_id%%/i", c_ws_plugin__optimizemember_utils_strings::esc_refs (urlencode ($subscr_id)), $url)))
																		...
																		/**/
																		if ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["eot_del_notification_recipients"] && is_array ($cv = preg_split ("/\|/", $custom)))
																			{
																				...
																				$msg .= "cv9: %%cv9%%";
																				/**/
																				if(($msg = c_ws_plugin__optimizemember_utils_strings::fill_cvs($msg, $custom)) && ($msg = preg_replace ("/%%eot_del_type%%/i", c_ws_plugin__optimizemember_utils_strings::esc_refs ("auto-eot-cancellation-expiration-demotion"), $msg)) && ($msg = preg_replace ("/%%subscr_id%%/i", c_ws_plugin__optimizemember_utils_strings::esc_refs ($subscr_id), $msg)))
																		...
																		/**/
																		eval ('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
																		do_action ("ws_plugin__optimizemember_during_auto_eot_system_during_demote", get_defined_vars ());
																		unset ($__refs, $__v); /* Unset defined __refs, __v. */
																	}
																else if ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["membership_eot_behavior"] === "delete")
																	{
																		$eot_del_type = $GLOBALS["ws_plugin__optimizemember_eot_del_type"] = "auto-eot-cancellation-expiration-deletion";
																		/**/
																		eval ('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
																		do_action ("ws_plugin__optimizemember_during_auto_eot_system_during_before_delete", get_defined_vars ());
																		...
																		/**/
																		eval ('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
																		do_action ("ws_plugin__optimizemember_during_auto_eot_system_during_delete", get_defined_vars ());
																		unset ($__refs, $__v); /* Unset defined __refs, __v. */
																	}
																/**/
																eval ('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
																do_action ("ws_plugin__optimizemember_during_auto_eot_system", get_defined_vars ());
																unset ($__refs, $__v); /* Unset defined __refs, __v. */
															}
													}
											}
						...
						/**/
						c_ws_plugin__optimizemember_utils_logs::cleanup_expired_s2m_transients (); /* Cleanup. */
						/**/
						eval ('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
						do_action ("ws_plugin__optimizemember_after_auto_eot_system", get_defined_vars ());
						unset ($__refs, $__v); /* Unset defined __refs, __v. */
						/**/
