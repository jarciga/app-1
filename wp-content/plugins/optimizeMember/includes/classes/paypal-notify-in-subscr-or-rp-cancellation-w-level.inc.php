...
if (!class_exists ("c_ws_plugin__optimizemember_paypal_notify_in_subscr_or_rp_cancellation_w_level"))
	{
		/**
		* optimizeMember's PayPal IPN handler (inner processing routine).
		*
		...
		* @since 110720
		*/
		class c_ws_plugin__optimizemember_paypal_notify_in_subscr_or_rp_cancellation_w_level
			{
				/**
				* optimizeMember's PayPal IPN handler (inner processing routine).
				*
				...
				public static function cp ($vars = array()) // Conditional phase for ``c_ws_plugin__optimizemember_paypal_notify_in::paypal_notify()``.
					{
						...
						&& (!empty($paypal["period1"]) || ($paypal["period1"] = c_ws_plugin__optimizemember_paypal_utilities::paypal_pro_period1 ($paypal)) || ($paypal["period1"] = "0 D"))
						&& (!empty($paypal["period3"]) || ($paypal["period3"] = c_ws_plugin__optimizemember_paypal_utilities::paypal_pro_period3 ($paypal)))
						...
						&& (!empty($paypal["subscr_cid"]) || ($paypal["subscr_cid"] = $paypal["subscr_id"])))
							{
								...
								if (!get_transient ($transient_ipn = "s2m_ipn_" . md5 ("optimizemember_transient_" . $_paypal_s)) && set_transient ($transient_ipn, time (), 31556926 * 10))
									{
										$paypal["optimizemember_log"][] = "optimizeMember `txn_type` identified as ( `subscr_cancel|recurring_payment_profile_cancel|mp_cancel` ).";
										...
										if (($user_id = c_ws_plugin__optimizemember_utils_users::get_user_id_with ($paypal["subscr_id"])) && is_object ($user = new WP_User ($user_id)) && $user->ID)
											{
												if (!$user->has_cap ("administrator")) // Do NOT process this routine on Administrators.
													{
														...
														if (!get_user_option ("optimizemember_auto_eot_time", $user_id)) // Respect existing.
															{
																$processing = $during = true; // Yes, we ARE processing this.

																$auto_eot_time = c_ws_plugin__optimizemember_utils_time::auto_eot_time ($user_id, $paypal["period1"], $paypal["period3"]);

																update_user_option ($user_id, "optimizemember_auto_eot_time", $auto_eot_time); // optimizeMember follows-up later.
														...
														else $paypal["optimizemember_log"][] = "Ignoring Cancellation. An Auto-EOT Time is already set for this Member. An optimizeMember API Notification will still be processed however.";

														if ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["cancellation_notification_urls"] && is_array($cv = preg_split ("/\|/", $paypal["custom"])))
															{
																foreach (preg_split ("/[\r\n\t]+/", $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["cancellation_notification_urls"]) as $url) // Handle Cancellation Notifications.
												...
												else $paypal["optimizemember_log"][] = "Ignoring Cancellation. The existing User ID is associated with an Administrator. Stopping here. Otherwise, an Administrator could lose access.";
											}
										else $paypal["optimizemember_log"][] = "Unable to handle Cancellation. Could not get the existing User ID from the DB.";
									}
						...
						else return apply_filters("c_ws_plugin__optimizemember_paypal_notify_in_subscr_or_rp_cancellation_w_level", false, get_defined_vars ());
					}
			}
	}