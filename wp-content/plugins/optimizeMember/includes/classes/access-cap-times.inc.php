...
if(!class_exists('c_ws_plugin__optimizemember_access_cap_times'))
{
	/**
	 * Access CAP Times.
	 *
	 * @package optimizeMember\CCAPS
	 * @since 140514
	 */
	class c_ws_plugin__optimizemember_access_cap_times
	{
		/**
		...
		public static function log_access_cap_times($meta_id, $object_id, $meta_key, $meta_value)
		{
			...
			foreach($caps as &$_caps_prev_now)
			{
				...
				foreach($_caps_prev_now as $_cap => $_enabled)
					if(strpos($_cap, 'access_optimizemember_') === 0)
			...
			$ac_times = get_user_option('optimizemember_access_cap_times', $user_id);
			...
			update_user_option($user_id, 'optimizemember_access_cap_times', $ac_times);
		}
		...
		public static function get_access_cap_times($user_id, $access_caps = array())
		{
			...
			if(($user_id = (integer)$user_id))
			{
				...
				/* ------- Begin back compat. with `optimizemember_paid_registration_times`. */

				// $update_ac_times = empty($ac_times) ? FALSE : TRUE;
				$ac_times_min = !empty($ac_times) ? min(array_keys($ac_times)) : 0;
				if(($r_time = c_ws_plugin__optimizemember_registration_times::registration_time($user_id)) && (empty($ac_times_min) || $r_time < $ac_times_min))
					$ac_times[number_format(($r_time += .0001), 4, '.', '')] = 'level0';

				if(is_array($pr_times = get_user_option('optimizemember_paid_registration_times', $user_id)))
				{
					$role_objects = $GLOBALS['wp_roles']->role_objects;
					foreach($pr_times as $_level => $_time)
						if(isset($role_objects['optimizemember_'.$_level]) && (empty($ac_times_min) || $_time < $ac_times_min))
							foreach(array_keys($role_objects['optimizemember_'.$_level]->capabilities) as $_cap)
								if(strpos($_cap, 'access_optimizemember_') === 0)
									$ac_times[number_format(($_time += .0001), 4, '.', '')] = substr($_cap, 16);
					unset($_level, $_time, $_cap);
				}
				/* ------- End back compat. with `optimizemember_paid_registration_times`. */
			...
			return apply_filters('ws_plugin__optimizemember_get_access_cap_times', $ac_times, get_defined_vars());
		}
	}
}