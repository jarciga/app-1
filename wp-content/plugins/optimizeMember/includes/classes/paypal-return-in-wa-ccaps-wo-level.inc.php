...
if(!class_exists('c_ws_plugin__optimizemember_paypal_return_in_wa_ccaps_wo_level'))
{
	/**
	 * optimizeMember's PayPal Auto-Return/PDT handler (inner processing routine).
	 *
	 ...
	 * @since 110720
	 */
	class c_ws_plugin__optimizemember_paypal_return_in_wa_ccaps_wo_level
	{
		/**
		 * optimizeMember's PayPal Auto-Return/PDT handler (inner processing routine).
		 *
		...
		public static function cp($vars = array() /* Conditional phase for ``c_ws_plugin__optimizemember_paypal_notify_in::paypal_notify()``. */)
		{
					...
					if(preg_match('/(referenc|associat|updat|upgrad)/i', $paypal['option_name1']) && $paypal['option_selection1'] /* Must have this information for Capability additions. */)
					{
						if(($user_id = c_ws_plugin__optimizemember_utils_users::get_user_id_with($paypal['txn_id'], $paypal['option_selection1'])) && is_object($user = new WP_User ($user_id)) && $user->ID)
						{
							if(!$user->has_cap('administrator') /* Do NOT process this routine on Administrators. */)
							{
								...
								setcookie('optimizemember_tracking', ($optimizemember_tracking = c_ws_plugin__optimizemember_utils_encryption::encrypt($paypal['txn_id'])), time() + 31556926, COOKIEPATH, COOKIE_DOMAIN).setcookie('optimizemember_tracking', $optimizemember_tracking, time() + 31556926, SITECOOKIEPATH, COOKIE_DOMAIN).($_COOKIE['optimizemember_tracking'] = $optimizemember_tracking);
			...
			else return apply_filters('c_ws_plugin__optimizemember_paypal_return_in_wa_ccaps_wo_level', FALSE, get_defined_vars());
		}
	}
}