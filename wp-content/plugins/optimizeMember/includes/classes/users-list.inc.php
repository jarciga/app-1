...
/**/
if (!class_exists ("c_ws_plugin__optimizemember_users_list"))
	{
		/**
		* Users list.
		*
		...
		* @since 3.5
		*/
		class c_ws_plugin__optimizemember_users_list
			{
				/**
				...
				*/
				public static function users_list_edit_cols ($user = FALSE)
					{
						return c_ws_plugin__optimizemember_users_list_in::users_list_edit_cols ($user);
					}
				/**
				...
				*/
				public static function users_list_update_cols ($user_id = FALSE)
					{
						return c_ws_plugin__optimizemember_users_list_in::users_list_update_cols ($user_id);
					}
				/**
				...
				public static function users_list_query (&$query = FALSE)
					{
						...
						do_action ("ws_plugin__optimizemember_before_users_list_search", get_defined_vars ());
						unset ($__refs, $__v); /* Unset defined __refs, __v. */
						/**/
						if (isset ($query->query_vars) && !is_network_admin ()) /* NOT in Network admin panels. */
							if (is_array ($qv = $query->query_vars) && ($s = trim ($qv["search"], "* \t\n\r\0\x0B")) && ($s = "%".esc_sql(c_ws_plugin__optimizemember_utils_strings::like_escape($s))."%"))
								{
									$query->query_from = " FROM `" . $wpdb->users . "` INNER JOIN `" . $wpdb->usermeta . "` ON `" . $wpdb->users . "`.`ID` = `" . $wpdb->usermeta . "`.`user_id`";
									/**/
									$query->query_where = " WHERE '1' = '1' AND (" . apply_filters ("ws_plugin__optimizemember_before_users_list_search_where_or_before", "", get_defined_vars ());
									$query->query_where .= " (`" . $wpdb->usermeta . "`.`meta_key` = '" . $wpdb->prefix . "optimizemember_subscr_id' AND `" . $wpdb->usermeta . "`.`meta_value` LIKE '" . $s . "')";
									$query->query_where .= " OR (`" . $wpdb->usermeta . "`.`meta_key` = '" . $wpdb->prefix . "optimizemember_custom' AND `" . $wpdb->usermeta . "`.`meta_value` LIKE '" . $s . "')";
									$query->query_where .= " OR (`" . $wpdb->usermeta . "`.`meta_key` = '" . $wpdb->prefix . "optimizemember_custom_fields' AND `" . $wpdb->usermeta . "`.`meta_value` LIKE '" . $s . "')";
									...
									$query->query_where .= apply_filters ("ws_plugin__optimizemember_before_users_list_search_where_or_after", "", get_defined_vars ()) . ")"; /* Leaving room for additional searches here. */
									...
									(($qv["role"]) ? " AND `meta_value` LIKE '%" . esc_sql (c_ws_plugin__optimizemember_utils_strings::like_escape ($qv["role"])) . "%'" : "") . ")";
									/**/
									$query->query_from = apply_filters ("ws_plugin__optimizemember_before_users_list_search_from", $query->query_from, get_defined_vars ());
									$query->query_where = apply_filters ("ws_plugin__optimizemember_before_users_list_search_where", $query->query_where, get_defined_vars ());
								}
						/**/
						eval ('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
						do_action ("ws_plugin__optimizemember_after_users_list_search", get_defined_vars ());
						unset ($__refs, $__v); /* Unset defined __refs, __v. */
						/**/
				...
				*/
				public static function users_list_cols ($cols = FALSE)
					{
						eval ('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
						do_action ("ws_plugin__optimizemember_before_users_list_cols", get_defined_vars ());
						unset ($__refs, $__v); /* Unset defined __refs, __v. */
						/**/
						$cols["optimizemember_registration_time"] = "Registration Date";
						/**/
						if (apply_filters ("ws_plugin__optimizemember_users_list_cols_display_paid_registration_times", false))
							$cols["optimizemember_paid_registration_times"] = "Paid Registr. Date";
						/**/
						$cols["optimizemember_subscr_id"] = "Paid Subscr. ID";
						/**/
						if (!is_multisite () || !c_ws_plugin__optimizemember_utils_conds::is_multisite_farm () || is_main_site ())
							$cols["optimizemember_ccaps"] = "Custom Capabilities";
						/**/
							...
							foreach (json_decode ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["custom_reg_fields"], true) as $field)
								{
									...
									$cols["optimizemember_custom_field_" . $field_var] = $field_title;
								}
						/**/
						$cols["optimizemember_login_counter"] = "# Of Logins";
						/**/
						eval ('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
						do_action ("ws_plugin__optimizemember_during_users_list_cols", get_defined_vars ());
						unset ($__refs, $__v); /* Unset defined __refs, __v. */
						/**/
						return apply_filters ("ws_plugin__optimizemember_users_list_cols", $cols, get_defined_vars ());
					}
				/**
				...
				public static function users_list_display_cols ($val = FALSE, $col = FALSE, $user_id = FALSE)
					{
						...
						/**/
						eval ('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
						do_action ("ws_plugin__optimizemember_before_users_list_display_cols", get_defined_vars ());
						unset ($__refs, $__v); /* Unset defined __refs, __v. */
						/**/
						$user = (is_object ($user) && $user_id === $last_user_id) ? $user : new WP_User ($user_id);
						/**/
						if ($col === "optimizemember_registration_time")
						...
						/**/
						else if ($col === "optimizemember_paid_registration_times")
							{
								$val = ""; /* Initialize $val before we begin. */
								if (is_array ($v = get_user_option ("optimizemember_paid_registration_times", $user_id)))
						...
						/**/
						else if ($col === "optimizemember_subscr_id")
							$val = ($v = get_user_option ("optimizemember_subscr_id", $user_id)) ? esc_html ($v) : "—";
						/**/
						else if ($col === "optimizemember_ccaps") /* Custom Capabilities. */
							{
						...
						/**/
						else if (preg_match ("/^optimizemember_custom_field_/", $col))
							{
								if (!$last_fields_id || $last_fields_id !== $user_id)
									$fields = get_user_option ("optimizemember_custom_fields", $user_id);
								/**/
								$field_var = preg_replace ("/^optimizemember_custom_field_/", "", $col);
								/**/
						...
						/**/
						else if ($col === "optimizemember_login_counter")
							$val = ($v = get_user_option ("optimizemember_login_counter", $user_id)) ? esc_html ($v) : "—";
						/**/
						$last_user_id = $user_id; /* Record this for internal optimizations. */
						/**/
						return apply_filters ("ws_plugin__optimizemember_users_list_display_cols", ((strlen ($val)) ? $val : "—"), get_defined_vars ());
					}
			}
	}
?>