...
if(!class_exists('c_ws_plugin__optimizemember_utils_time'))
{
	/**
	 * Time utilities.
	 *
	 ...
	 * @since 3.5
	 */
	class c_ws_plugin__optimizemember_utils_time
	{
		/**
		 ...
		 *
		 * Used by optimizeMember's built-in Auto-EOT System, and also by its IPN routines.
		 ...
		 *   But only when ``$GLOBALS['WS_PLUGIN__']['optimizemember']['o']['eot_time_ext_behavior'] === 'extend'``.
		 *
		...
		{
			$eot_grace_time = (integer)$GLOBALS['WS_PLUGIN__']['optimizemember']['o']['eot_grace_time'];
			$eot_grace_time = (integer)apply_filters('ws_plugin__optimizemember_eot_grace_time', $eot_grace_time);
			...
			if($user_id && ($user = new WP_User ($user_id)) && $user->ID)
			{
				...
				$last_payment_time         = get_user_option('optimizemember_last_payment_time', $user_id);
				$last_payment_time         = (int)$lpt ? (int)$lpt : (int)$last_payment_time;
				$last_paid_access_cap_time = 0; // Initialize the last access cap time.
				if(($access_cap_times = c_ws_plugin__optimizemember_access_cap_times::get_access_cap_times($user_id)))
			...
			if($ext && $GLOBALS['WS_PLUGIN__']['optimizemember']['o']['eot_time_ext_behavior'] === 'extend')
