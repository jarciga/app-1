...
if(!class_exists('c_ws_plugin__optimizemember_utils_logs'))
{
    ...
    class c_ws_plugin__optimizemember_utils_logs
    {
        ...
        public static function log_entry($slug, $data)
        {
            ...
            c_ws_plugin__optimizemember_utils_logs::archive_oversize_log_files();

            file_put_contents($logs_dir.'/'.$log2,
                'LOG ENTRY: '.$logt."\n".$logv."\n".$logm."\n".$log4."\n".
                c_ws_plugin__optimizemember_utils_logs::conceal_private_info(print_r($data, TRUE))."\n\n",
        ...
        public static function http_api_debug($response = NULL, $state = NULL, $class = NULL, $args = NULL, $url = NULL)
        {
            ...
            if(!empty($args['optimizemember']) || strpos($url, 'optimizemember') !== FALSE)
        ...
        public static function cleanup_expired_s2m_transients($stagger = TRUE)
        {
            ...
            if(($expired_s2m_transients = $wpdb->get_results("SELECT * FROM `".$wpdb->options."` WHERE `option_name` LIKE '".esc_sql(c_ws_plugin__optimizemember_utils_strings::like_escape('_transient_timeout_s2m_'))."%' AND `option_value` < '".esc_sql(time())."' LIMIT 5")))
        ...
        ( // Array keys are regex patterns matching their associated log file names.
            '/gateway\-core\-ipn/'   => array('short' => 'Core IPN and post-processing handler.', 'long' => 'This log file records all communication between OptimizeMember and the IPN/Webhook/Callback services associated with various payment gateways. All transactions pass through OptimizeMember\'s core processor, and they will be logged in this file; including transactions processed via OptimizeMember Pro-Forms—for all Payment Gateway integrations.'),
            '/gateway\-core\-rtn/'   => array('short' => 'Core PDT/Auto-Return communication.', 'long' => 'This log file records all communication between OptimizeMember and the PDT/Auto-Return/Thank-You services associated with various payment gateways (i.e., routines that help OptimizeMember process Thank-You pages). This log file is not used in OptimizeMember Pro-Form integrations however.'),

            '/stripe\-api/'          => array('short' => 'Stripe API communication.', 'long' => 'This log file records all communication between OptimizeMember and Stripe APIs. See also: gateway-core-ipn.log (OptimizeMember\'s core processor).'),
            '/stripe\-ipn/'          => array('short' => 'Stripe Webhook/IPN communication.', 'long' => 'This log file records the Webhook/IPN data that Stripe sends to OptimizeMember. See also: gateway-core-ipn.log (OptimizeMember\'s core processor).'),

            '/paypal\-api/'          => array('short' => 'PayPal API communication.', 'long' => 'This log file records all communication between OptimizeMember and PayPal APIs. Such as PayPal Button Encryption and PayPal Pro API calls that process transactions. This log file may be used (in some scenarios), even if you\'re running a PayPal Payments Pro (Payflow Edition) account. See also: gateway-core-ipn.log (OptimizeMember\'s core processor).'),
            '/paypal\-payflow\-api/' => array('short' => 'PayPal Pro (PayFlow Edition) API communication.', 'long' => 'This log file records all communication between OptimizeMember and the PayPal Payments Pro (PayFlow Edition) APIs. This log file is only used if you operate a PayPal Payments Pro (PayFlow Edition) account; i.e., only if you integrate OptimizeMember Pro with Payflow for Recurring Billing. See also: gateway-core-ipn.log (OptimizeMember\'s core processor).'),

            '/authnet\-api/'         => array('short' => 'Authorize.Net API communication.', 'long' => 'This log file records all communication between OptimizeMember and Authorize.Net APIs (for both AIM and ARB integrations). See also: gateway-core-ipn.log (OptimizeMember\'s core processor).'),
            ...
            '/authnet\-ipn/'         => array('short' => 'Authorize.Net Silent Post/IPN communication.', 'long' => 'This log file records the Silent Post/IPN data Authorize.Net sends to OptimizeMember with details regarding new transactions. See also: gateway-core-ipn.log (OptimizeMember\'s core processor).'),
            ...
            '/getresponse\-api/'     => array('short' => 'GetResponse API communication.', 'long' => 'This log file records all of OptimizeMember\'s communication with GetResponse APIs.'),
