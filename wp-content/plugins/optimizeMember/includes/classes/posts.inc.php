<?php
/**
* optimizeMember's Post protection routines *( for current Post )*.
*
* Copyright: © 2009-2011
...
/**/
if (!class_exists ("c_ws_plugin__optimizemember_posts"))
	{
		/**
		* optimizeMember's Post protection routines *( for current Post )*.
		*
		...
		* @since 3.5
		*/
		class c_ws_plugin__optimizemember_posts
			{
				/**
								...
								if (!c_ws_plugin__optimizemember_systematics::is_wp_systematic_use_page ()) /* Do NOT touch WordPress Systematics. This excludes all WordPress Systematics. */
									{
										...
										if ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["login_redirection_override"] && ($login_redirection_uri = c_ws_plugin__optimizemember_login_redirects::login_redirection_uri ($user, "root-returns-false")) && preg_match ("/^" . preg_quote ($login_redirection_uri, "/") . "$/", $_SERVER["REQUEST_URI"]) && c_ws_plugin__optimizemember_no_cache::no_cache_constants (true) && (!$user || !$user->has_cap ("access_optimizemember_level0")))
											c_ws_plugin__optimizemember_mo_page::wp_redirect_w_mop_vars /* Configure MOP Vars here. */ ("post", $post_id, "level", 0, $_SERVER["REQUEST_URI"], "sys") . exit ();
										/**/
										else if (!c_ws_plugin__optimizemember_systematics::is_systematic_use_page ()) /* Do NOT protect Systematics. However, there is 1 exception above. */
											{
												for ($n = $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["levels"]; $n >= 0; $n--) /* Post Level restrictions ( including Custom Post Types ). Go through each Level. */
													{
														if ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["level" . $n . "_posts"] === "all" && c_ws_plugin__optimizemember_no_cache::no_cache_constants (true) && (!$user || !$user->has_cap ("access_optimizemember_level" . $n)))
															c_ws_plugin__optimizemember_mo_page::wp_redirect_w_mop_vars /* Configure MOP Vars here. */ ("post", $post_id, "level", $n, $_SERVER["REQUEST_URI"]) . exit ();
														/**/
														else if (strpos ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["level" . $n . "_posts"], "all-") !== false && in_array ("all-" . $post->post_type . "s", preg_split ("/[\r\n\t\s;,]+/", $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["level" . $n . "_posts"])) && c_ws_plugin__optimizemember_no_cache::no_cache_constants (true) && (!$user || !$user->has_cap ("access_optimizemember_level" . $n)))
															c_ws_plugin__optimizemember_mo_page::wp_redirect_w_mop_vars /* Configure MOP Vars here. */ ("post", $post_id, "level", $n, $_SERVER["REQUEST_URI"]) . exit ();
														/**/
														else if ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["level" . $n . "_posts"] && in_array ($post_id, preg_split ("/[\r\n\t\s;,]+/", $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["level" . $n . "_posts"])) && c_ws_plugin__optimizemember_no_cache::no_cache_constants (true) && (!$user || !$user->has_cap ("access_optimizemember_level" . $n)))
															c_ws_plugin__optimizemember_mo_page::wp_redirect_w_mop_vars /* Configure MOP Vars here. */ ("post", $post_id, "level", $n, $_SERVER["REQUEST_URI"]) . exit ();
													}
												/**/
												for ($n = $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["levels"]; $n >= 0; $n--) /* Category Level restrictions. Go through each Level. */
													{
												...
												if (has_tag ()) /* Here we take a look to see if this Post has any Tags. If so, we need to run the full set of routines against Tags also. */
													{
														for ($n = $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["levels"]; $n >= 0; $n--) /* Tag Level restrictions. Go through each Level. */
															{
																if ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["level" . $n . "_ptags"] === "all" && c_ws_plugin__optimizemember_no_cache::no_cache_constants (true) && (!$user || !$user->has_cap ("access_optimizemember_level" . $n)))
																	c_ws_plugin__optimizemember_mo_page::wp_redirect_w_mop_vars /* Configure MOP Vars here. */ ("post", $post_id, "level", $n, $_SERVER["REQUEST_URI"], "ptag") . exit ();
																/**/
														...
														if ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["level" . $n . "_ruris"]) /* URIs configured at this Level? */
															...
															foreach (preg_split ("/[\r\n\t]+/", c_ws_plugin__optimizemember_ruris::fill_ruri_level_access_rc_vars ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["level" . $n . "_ruris"], $user)) as $str)
																if ($str && preg_match ("/" . preg_quote ($str, "/") . "/", $_SERVER["REQUEST_URI"]) && c_ws_plugin__optimizemember_no_cache::no_cache_constants (true) && (!$user || !$user->has_cap ("access_optimizemember_level" . $n)))
																	c_ws_plugin__optimizemember_mo_page::wp_redirect_w_mop_vars /* Configure MOP Vars here. */ ("post", $post_id, "level", $n, $_SERVER["REQUEST_URI"], "ruri") . exit ();
													}
												/**/
												if (is_array ($ccaps_req = get_post_meta ($post_id, "optimizemember_ccaps_req", true)) && !empty ($ccaps_req) && c_ws_plugin__optimizemember_no_cache::no_cache_constants (true))
													{
														foreach ($ccaps_req as $ccap) /* The ``$user`` MUST satisfy ALL Custom Capability requirements. Stored as an array of Custom Capabilities. */
															if (strlen ($ccap) && (!$user || !$user->has_cap ("access_optimizemember_ccap_" . $ccap)) /* Does this ``$user``, have this Custom Capability? */)
																c_ws_plugin__optimizemember_mo_page::wp_redirect_w_mop_vars /* Configure MOP Vars here. */ ("post", $post_id, "ccap", $ccap, $_SERVER["REQUEST_URI"], "ccap") . exit ();
													}
												/**/
													/**/
												$dripDays = get_post_meta($post_id, "optimizemember_drip_days", true);
												if (!empty ($dripDays) && c_ws_plugin__optimizemember_no_cache::no_cache_constants (true))
													{
														$dripUrl = get_post_meta($post_id, "optimizemember_drip_redirect_url", true);
														...
														if ($time && $time >= ($_days_ago = strtotime("-$dripDays days")) && !current_user_can('manage_options')) {
															if (!empty($dripUrl)) {
																...
																c_ws_plugin__optimizemember_mo_page::wp_redirect_w_mop_vars /* Configure MOP Vars here. */ ("page", $post_id, "drip_days", $dripDays, $_SERVER["REQUEST_URI"], "drip_days") . exit ();
															}
														} 
													}
												/**/
												if ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["specific_ids"] && in_array ($post_id, preg_split ("/[\r\n\t\s;,]+/", $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["specific_ids"])) && c_ws_plugin__optimizemember_no_cache::no_cache_constants (true) && !c_ws_plugin__optimizemember_sp_access::sp_access ($post_id))
													c_ws_plugin__optimizemember_mo_page::wp_redirect_w_mop_vars /* Configure MOP Vars here. */ ("post", $post_id, "sp", $post_id, $_SERVER["REQUEST_URI"], "sp") . exit ();
											}
										/**/
