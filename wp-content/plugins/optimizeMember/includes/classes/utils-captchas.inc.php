...
if (!class_exists ('c_ws_plugin__optimizemember_utils_captchas'))
{
	/**
	 * Captcha utilities.
	 *
	 ...
	 * @since 3.5
	 */
	class c_ws_plugin__optimizemember_utils_captchas
	{
		/**
		...
		{
			return apply_filters('ws_plugin__optimizemember_recaptcha_version', '1', get_defined_vars());
		}

		/**
		 * Public/private keys to use for reCAPTCHA™.
		 *
		 * @package OptimzieMember\Utilities
		 * @since 111203
		 *
		...
		{
			// NOTE: Version 2 keys are only possible w/ optimizeMember Pro filters.

			$public  = $GLOBALS['WS_PLUGIN__']['optimizemember']['c']['recaptcha']['public_key'];
			$private = $GLOBALS['WS_PLUGIN__']['optimizemember']['c']['recaptcha']['private_key'];

			return apply_filters('ws_plugin__optimizemember_recaptcha_keys', array('public' => $public, 'private' => $private), get_defined_vars ());
		}
		...
		public static function recaptcha_post_vars($post_vars = array())
		{
			...
			if(self::recaptcha_version() === '2')
			{
				...
				return apply_filters('ws_plugin__optimizemember_recaptcha_post_vars', $post_vars, get_defined_vars());
			}
		...
		public static function recaptcha_code_validates($challenge = '', $response = '')
		{
			...
			if(self::recaptcha_version() === '2') // New API verifier.
			{
				...
				$api_response  = c_ws_plugin__optimizemember_utils_urls::remote('https://www.google.com/recaptcha/api/siteverify', $api_post_vars);
			...
			else // Old API call; note that this is NOT over SSL for some reason.
			{
				...
				$api_response  = c_ws_plugin__optimizemember_utils_urls::remote ('http://www.google.com/recaptcha/api/verify', $api_post_vars);
