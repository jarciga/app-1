...
*
* @package optimizeMember\User_Securities
* @since 3.5
*/
if (realpath (__FILE__) === realpath ($_SERVER["SCRIPT_FILENAME"]))
	exit ("Do not access this file directly.");
/**/
if (!class_exists ("c_ws_plugin__optimizemember_user_securities"))
	{
		/**
		* User securities.
		*
		...
		* @since 3.5
		*/
		class c_ws_plugin__optimizemember_user_securities
			{
				/**
				...
				*/
				public static function initialize () /* Initializes the Filter for `user_has_cap`. */
					{
						add_filter ("user_has_cap", "c_ws_plugin__optimizemember_user_securities::user_capabilities", 10, 3);
					}
				/**
				...
				public static function user_capabilities ($capabilities = FALSE, $caps_map = FALSE, $args = FALSE)
					{
						...
						do_action ("ws_plugin__optimizemember_before_user_capabilities", get_defined_vars ());
						unset ($__refs, $__v); /* Unset defined __refs, __v. */

						/**/
						if(!empty($capabilities['access_optimizemember_ccap_all_ccaps']) && !empty($args[0]) && preg_match('/^access_optimizemember_ccap_/i', $args[0]) && apply_filters('ws_plugin__optimizemember_all_ccaps_enable', TRUE, get_defined_vars()))
							$capabilities = array_merge((array)$capabilities, array($args[0] => 1));

						elseif (!is_multisite () && !empty ($capabilities["administrator"]) && !empty ($args[0]) && preg_match ("/^access_optimizemember_ccap_/i", $args[0]) && apply_filters ("ws_plugin__optimizemember_admins_have_all_ccaps", true, get_defined_vars ()))
							$capabilities = array_merge ((array)$capabilities, array ($args[0] => 1));
						/**/
						else if (is_multisite () && c_ws_plugin__optimizemember_utils_conds::is_multisite_farm () && (is_super_admin () || !empty ($capabilities["administrator"])) && !empty ($args[0]) && ($args[0] === "edit_user" || $args[0] === "edit_users"))
						...
						/**/
						return apply_filters ("ws_plugin__optimizemember_user_capabilities", $capabilities, get_defined_vars ());
					}
				/**
				...
				public static function ms_allow_edits ($allow = FALSE)
					{
						...
						/**/
						eval ('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
						do_action ("ws_plugin__optimizemember_before_ms_allow_edits", get_defined_vars ());
						unset ($__refs, $__v); /* Unset defined __refs, __v. */
						/**/
						...
						/**/
						return apply_filters ("ws_plugin__optimizemember_ms_allow_edits", $allow, get_defined_vars ());
					}
				/**
				...
				* Any other restrictions you need to impose must be done through custom programming, using optimizeMember's Conditionals.
				* 	See `optimizeMember -> API Scripting`.
				*
				* @package optimizeMember\User_Securities
				* @since 3.5
				*
				...
				*/
				public static function hide_password_fields ($show = TRUE, $user = FALSE)
					{
						eval ('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
						do_action ("ws_plugin__optimizemember_before_hide_password_fields", get_defined_vars ());
						unset ($__refs, $__v); /* Unset defined __refs, __v. */
						/**/
						if ($show && is_multisite () && c_ws_plugin__optimizemember_utils_conds::is_multisite_farm ())
						...
						/**/
						return apply_filters ("ws_plugin__optimizemember_hide_password_fields", $show, get_defined_vars ());
					}
