...
/**/
if(!class_exists("c_ws_plugin__optimizemember_utils_strings"))
	{
		/**
		* String utilities.
		*
		...
		* @since 3.5
		*/
		class c_ws_plugin__optimizemember_utils_strings
			{
				/**
				...
				*
				* @deprecated Starting with optimizeMember v120103, please use:
				* ``c_ws_plugin__optimizemember_utils_strings::esc_refs()``.
				*/
				...
				*/
				public static function trim($value = FALSE, $chars = FALSE, $extra_chars = FALSE)
					{
						return c_ws_plugin__optimizemember_utils_strings::trim_deep($value, $chars, $extra_chars);
					}
				/**
				* Trims deeply; or use {@link optimizeMember\Utilities\c_ws_plugin__optimizemember_utils_strings::trim()}.
				*
				* @package optimizeMember\Utilities
				* @since 3.5
				*
				* @see optimizeMember\Utilities\c_ws_plugin__optimizemember_utils_strings::trim()
				* @see http://php.net/manual/en/function.trim.php
				*
				* @param str|array $value Either a string, an array, or a multi-dimensional array, filled with integer and/or string values.
				* @param str|bool $chars Optional. Defaults to false, indicating the default trim chars ` \t\n\r\0\x0B`. Or, set to a specific string of chars.
				...
				For further details, see: <http://php.net/manual/en/function.array-map.php>. */
							{
								foreach($value as &$r) /* Reference. */
									$r = c_ws_plugin__optimizemember_utils_strings::trim_deep($r, $chars);
				...
				*/
				public static function trim_dq_deep($value = FALSE)
					{
						return c_ws_plugin__optimizemember_utils_strings::trim_deep($value, false, '"');
					}
				/**
				...
				*/
				public static function trim_qts_deep($value = FALSE)
					{
						$qts = implode("|", array_keys /* Keys are regex patterns. */(c_ws_plugin__optimizemember_utils_strings::$quote_entities_w_variations));
						/**/
						return is_array($value) ? array_map("c_ws_plugin__optimizemember_utils_strings::trim_qts_deep", $value) : preg_replace("/^(?:".$qts.")+|(?:".$qts.")+$/", "", (string)$value);
					}
				...
				*
				* This is useful when preparing an input array for ``c_ws_plugin__optimizemember_utils_arrays::in_regex_array()``.
				*
				* @package optimizeMember\Utilities
				* @since 3.5
				*
				...
				*/
				...
				For further details, see: <http://php.net/manual/en/function.array-map.php>. */
							{
								foreach($value as &$r) /* Reference. */
									$r = c_ws_plugin__optimizemember_utils_strings::wrap_deep($r, $beg, $end, $wrap_e);
								return $value; /* Return modified array. */
							}
				...
				*/
				...
				For further details, see: <http://php.net/manual/en/function.array-map.php>. */
							{
								foreach($value as &$r) /* Reference. */
									$r = c_ws_plugin__optimizemember_utils_strings::preg_quote_deep($r, $delimiter);
								return $value; /* Return modified array. */
							}
			 ...
			 */
				...
				For further details, see: <http://php.net/manual/en/function.array-map.php>. */
							{
								...
								foreach /* Loop through array. */($value as $v)
									$emails = array_merge($emails, c_ws_plugin__optimizemember_utils_strings::parse_emails($v));
								return $emails; /* Return array. */
							}
						$delimiter = /* Supports semicolons or commas. */ (strpos((string)$value, ";") !== false) ? ";" : ",";
						foreach(c_ws_plugin__optimizemember_utils_strings::trim_deep(preg_split("/".preg_quote($delimiter, "/")."+/", (string)$value)) as $section)
							{
				...
				*
				* Note, this function is backward compatible with routines supplied by optimizeMember in the past;
				...
				*/
				public static function rsa_sha1_sign($string = FALSE, $key = FALSE)
					{
						$key = /* Fixes key wrappers. */ c_ws_plugin__optimizemember_utils_strings::_rsa_sha1_key_fix_wrappers((string)$key);
						/**/
						$signature = /* Command line. */ c_ws_plugin__optimizemember_utils_strings::_rsa_sha1_shell_sign((string)$string, (string)$key);
						/**/
						if(empty($signature) && stripos(PHP_OS, "win") === 0 && file_exists(($openssl = "c:\openssl-win32\bin\openssl.exe")))
							$signature = c_ws_plugin__optimizemember_utils_strings::_rsa_sha1_shell_sign((string)$string, (string)$key, /* Specific location. */ $openssl);
						/**/
						...
						if(empty($signature)) /* Now, if we're still empty, trigger an error here. */
							trigger_error("optimizeMember was unable to generate an RSA-SHA1 signature.".
				...
				*/
				public static function _rsa_sha1_shell_sign($string = FALSE, $key = FALSE, $openssl = FALSE)
					{
						if(function_exists("shell_exec") && ($esa = "escapeshellarg") && ($openssl = (($openssl && is_string($openssl)) ? $openssl : "openssl")) && ($temp_dir = c_ws_plugin__optimizemember_utils_dirs::get_temp_dir()))
							{
