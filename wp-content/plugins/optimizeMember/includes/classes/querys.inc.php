...
if(!class_exists('c_ws_plugin__optimizemember_querys'))
{
    /**
     * Query protection routines.
     *
     ...
     * @since 3.5
     */
    class c_ws_plugin__optimizemember_querys
    {
        /**
        ...
        public static function query_level_access(&$wp_query = NULL, $force = FALSE)
        {
            ...
            if(is_object($wp_query) && !$wp_query->get('___s2_is_bbp_has_replies'))
                ...
                c_ws_plugin__optimizemember_querys::_query_level_access_sys($wp_query); // Systematics.
            ...
            {
                if((!is_admin() || c_ws_plugin__optimizemember_querys::_is_admin_ajax_search($wp_query)) && !$wp_query->get('___s2_is_bbp_has_replies') /* See: <http://bit.ly/1obLpv4> */)
                {
                    $suppressing_filters = $wp_query->get('suppress_filters'); // Filter suppression on?

                    if((!$suppressing_filters && $force) // Forcing this routine bypasses all of these other conditionals. Works with API function ``attach_optimizemember_query_filters()``.
                        ...
                        || (($suppressing_filters !== 'n/a') && (in_array('all', $o) || in_array('nav-menus', $o)) && in_array('wp_get_nav_menu_items', ($callers = (isset($callers) ? $callers : c_ws_plugin__optimizemember_utilities::callers()))))
                    )
                    {
                        ...
                        {
                            $bbpress_restrictions_enable = apply_filters('ws_plugin__optimizemember_bbpress_restrictions_enable', TRUE);
                            $bbpress_installed           = c_ws_plugin__optimizemember_utils_conds::bbp_is_installed(); // bbPress is installed?
                            ...
                            for($n = $GLOBALS['WS_PLUGIN__']['optimizemember']['c']['levels']; $n >= 0; $n--) // Category Level Restrictions.
                            {
                                ...
                                else if($GLOBALS['WS_PLUGIN__']['optimizemember']['o']['level'.$n.'_catgs'] && (!$user || !current_user_can('access_optimizemember_level'.$n)))
                                {
                            ...
                            for($n = $GLOBALS['WS_PLUGIN__']['optimizemember']['c']['levels']; $n >= 0; $n--) // Tag Level Restrictions.
                            {
                                ...
                                else if($GLOBALS['WS_PLUGIN__']['optimizemember']['o']['level'.$n.'_ptags'] && (!$user || !current_user_can('access_optimizemember_level'.$n)))
                                {
                            ...
                            for($n = $GLOBALS['WS_PLUGIN__']['optimizemember']['c']['levels']; $n >= 0; $n--) // Post Level Restrictions.
                            {
                                ...
                                else if($GLOBALS['WS_PLUGIN__']['optimizemember']['o']['level'.$n.'_posts'] && (!$user || !current_user_can('access_optimizemember_level'.$n)))
                                {
                                    foreach(($_posts = preg_split('/['."\r\n\t".'\s;,]+/', $GLOBALS['WS_PLUGIN__']['optimizemember']['o']['level'.$n.'_posts'])) as $_p)
                                    {
                                        if(strpos($_p, 'all-') === 0 && preg_match('/^all-(.+?)$/', $_p, $_m)) // Protecting `all-` of a specific Post Type?
                                            if((is_array($_p_of_type = c_ws_plugin__optimizemember_utils_gets::get_all_post_ids($_m[1])) || (substr($_m[1], -1) === 's' && is_array($_p_of_type = c_ws_plugin__optimizemember_utils_gets::get_all_post_ids(substr($_m[1], 0, -1))))) && !empty($_p_of_type))
                                                $_posts = array_merge($_posts, $_p_of_type); // Merge all Posts of this Post Type.
                                    }
                                    if($bbpress_restrictions_enable && $bbpress_installed)
                                        $_posts = array_merge($_posts, c_ws_plugin__optimizemember_utils_gets::get_all_child_post_ids($_posts, $bbpress_topic_post_type));
                                    $_posts = array_unique(c_ws_plugin__optimizemember_utils_arrays::force_integers($_posts)); // Force integers.
                            ...
                            for($n = $GLOBALS['WS_PLUGIN__']['optimizemember']['c']['levels']; $n >= 0; $n--) // Page Level Restrictions.
                            {
                                ...
                                else if($GLOBALS['WS_PLUGIN__']['optimizemember']['o']['level'.$n.'_pages'] && (!$user || !current_user_can('access_optimizemember_level'.$n)))
                                {
        ...
        public static function _query_level_access_sys(&$wp_query = NULL)
        {
            ...
            foreach(array_keys(get_defined_vars()) as $__v) $__refs[$__v] =& $$__v;
            do_action('_ws_plugin__optimizemember_before_query_level_access_sys', get_defined_vars());
            unset($__refs, $__v); // Housekeeping.

            if(is_object($wpdb) && is_object($wp_query) && !($suppressing_filters = $wp_query->get('suppress_filters')))
                if((!is_admin() && ($wp_query->is_search() || $wp_query->is_feed())) || c_ws_plugin__optimizemember_querys::_is_admin_ajax_search($wp_query))
                {
                    $s = $systematics = array($GLOBALS['WS_PLUGIN__']['optimizemember']['o']['login_welcome_page'], $GLOBALS['WS_PLUGIN__']['optimizemember']['o']['membership_options_page'], $GLOBALS['WS_PLUGIN__']['optimizemember']['o']['file_download_limit_exceeded_page']);
                    $s = $systematics = c_ws_plugin__optimizemember_utils_arrays::force_integers($s); // Force integer values here.

                    $wp_query->set('post__not_in', array_unique(array_merge(c_ws_plugin__optimizemember_utils_arrays::force_integers((array)$wp_query->get('post__not_in')), $s)));
                    $wp_query->set('post__in', array_unique(array_diff(c_ws_plugin__optimizemember_utils_arrays::force_integers((array)$wp_query->get('post__in')), $s)));

                    foreach(array_keys(get_defined_vars()) as $__v) $__refs[$__v] =& $$__v;
                    do_action('_ws_plugin__optimizemember_during_query_level_access_sys', get_defined_vars());
                    unset($__refs, $__v); // Housekeeping.
                }
            foreach(array_keys(get_defined_vars()) as $__v) $__refs[$__v] =& $$__v;
            do_action('_ws_plugin__optimizemember_after_query_level_access_sys', get_defined_vars());
            unset($__refs, $__v); // Housekeeping.
        }
        ...
        {
            global $wpdb; // Global DB object reference.
            $wp_query = &c_ws_plugin__optimizemember_querys::$current_wp_query;
            ...
            if(is_array($items) && !empty($items) && is_object($wpdb) && is_object($wp_query) && $wp_query->get('suppress_filters') !== 'n/a')
            {
                ...
                $x_post_ids = c_ws_plugin__optimizemember_utils_arrays::force_integers($x_post_ids);

                $x_taxonomy_ids = array_merge((array)$wp_query->get('category__not_in'), (array)$wp_query->get('tag__not_in'));
                $x_taxonomy_ids = c_ws_plugin__optimizemember_utils_arrays::force_integers($x_taxonomy_ids);

                // removing dripped content also
                $x_dripped_ids = c_ws_plugin__optimizemember_utils_gets::get_unavailable_singular_ids_with_dripped_content(wp_get_current_user());
                $x_dripped_ids = c_ws_plugin__optimizemember_utils_arrays::force_integers($x_dripped_ids);
                ...
                foreach($items as $key => $item) // Filter through all navigational menu ``$items``.
                    if(isset($item->ID, $item->object_id, $item->type) && (int)$item->ID !== (int)$item->object_id && in_array($item->type, array('post_type', 'category'), TRUE))
                        if(($item->type === 'post_type' && in_array($item->object_id, $x_post_ids)) || ($item->type === 'category' && in_array($item->object_id, $x_taxonomy_ids)
                                || $item->type === 'post_type' && in_array($item->object_id, $x_dripped_ids)))
                        {
            ...
            return apply_filters('_ws_plugin__optimizemember_query_level_access_navs', $items, get_defined_vars());
        }
        ...
        public static function _query_level_access_coms($cwhere = '', &$wp_query = NULL)
        {
            ...
            if(is_string($cwhere) && is_object($wpdb) && is_object($wp_query) && !$wp_query->get('suppress_filters'))
            {
                ...
                $x_terms     = array_unique(c_ws_plugin__optimizemember_utils_arrays::force_integers($x_terms)); // Force integer values.
                $x_singulars = c_ws_plugin__optimizemember_utils_gets::get_singular_ids_in_terms($x_terms); // Singulars.
        ...
        public static function _is_admin_ajax_search(&$wp_query = NULL)
        {
            ...
            if(is_object($wpdb) && is_object($wp_query) && is_admin() && $wp_query->is_search())
                if(defined('DOING_AJAX') && DOING_AJAX && !empty($_REQUEST['action']) && (did_action('wp_ajax_nopriv_'.$_REQUEST['action']) || did_action('wp_ajax_'.$_REQUEST['action'])))
                    return apply_filters('_ws_plugin__optimizemember_is_admin_ajax_search', TRUE, get_defined_vars());

            return apply_filters('_ws_plugin__optimizemember_is_admin_ajax_search', FALSE, get_defined_vars());
        }
        ...
        {
            if(!$GLOBALS['WS_PLUGIN__']['optimizemember']['o']['filter_wp_query']
                || (!in_array('all', $GLOBALS['WS_PLUGIN__']['optimizemember']['o']['filter_wp_query'])
                    && !in_array('pages', $GLOBALS['WS_PLUGIN__']['optimizemember']['o']['filter_wp_query']))
            ) return $excludes; // Not applicable.

            $systematics   = array(); // Initialize.
            $systematics[] = $GLOBALS['WS_PLUGIN__']['optimizemember']['o']['file_download_limit_exceeded_page'];
            if(!is_user_logged_in()) $systematics[] = $GLOBALS['WS_PLUGIN__']['optimizemember']['o']['login_welcome_page'];
            $systematics = c_ws_plugin__optimizemember_utils_arrays::force_integers($systematics); // Force integer values here.
            $excludes    = array_merge($excludes, $systematics);

            for($n = $GLOBALS['WS_PLUGIN__']['optimizemember']['c']['levels']; $n >= 0; $n--) // Here we need to exclude any Page not available to the current user.
            {
                if($GLOBALS['WS_PLUGIN__']['optimizemember']['o']['level'.$n.'_pages'] === 'all' && !current_user_can('access_optimizemember_level'.$n))
                    $excludes = array_merge($excludes, c_ws_plugin__optimizemember_utils_arrays::force_integers(c_ws_plugin__optimizemember_utils_gets::get_all_page_ids()));
                else if($GLOBALS['WS_PLUGIN__']['optimizemember']['o']['level'.$n.'_pages'] && !current_user_can('access_optimizemember_level'.$n))
                    $excludes = array_merge($excludes, c_ws_plugin__optimizemember_utils_arrays::force_integers(preg_split('/['."\r\n\t".'\s;,]+/', $GLOBALS['WS_PLUGIN__']['optimizemember']['o']['level'.$n.'_pages'])));
            }
            return apply_filters('_ws_plugin__optimizemember_query_level_access_list_pages', $excludes, get_defined_vars());
        }
