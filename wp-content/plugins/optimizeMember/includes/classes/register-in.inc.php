...
/**/
if (!class_exists ("c_ws_plugin__optimizemember_register_in"))
	{
		/**
		* Registration Links ( inner processing routines ).
		*
		...
		* @since 3.5
		*/
		class c_ws_plugin__optimizemember_register_in
			{
				/**
				...
				public static function register ()
					{
						...
						/**/
						if (!empty ($_GET["optimizemember_register"])) /* If they're attempting to access the registration system. */
							{
								eval('while (@ob_end_clean ());'); /* First we end/clean any output buffers that may exist already. */
								/**/
								$msg_503  = _x ('<strong>Your Link Expired:</strong><br />', "s2member-front", "s2member");
								$msg_503 .= _x ('Most probably you already completed registration. Please <a href="' . wp_login_url() . '"><strong>login</strong></a> to the page<br /><br />', "s2member-front", "s2member");
								$msg_503 .= _x ('Please contact Support if you need assistance.', "s2member-front", "s2member");
								/**/
										...
										if (count ($register) === 6 && $register[0] === "subscr_gateway_subscr_id_custom_item_number_time" /* Does the checksum value match up here? */)
											{
												if (is_numeric ($register[5]) && $register[5] <= strtotime ("now") && $register[5] >= strtotime ("-" . apply_filters ("ws_plugin__optimizemember_register_link_exp_time", "2 days", get_defined_vars ())))
													{
														$_COOKIE["optimizemember_subscr_gateway"] = /* For ``reg_cookies_ok ()``. */ c_ws_plugin__optimizemember_utils_encryption::encrypt ($register[1]);
														$_COOKIE["optimizemember_subscr_id"] = /* For ``reg_cookies_ok ()``. */ c_ws_plugin__optimizemember_utils_encryption::encrypt ($register[2]);
														$_COOKIE["optimizemember_custom"] = /* For ``reg_cookies_ok ()``. */ c_ws_plugin__optimizemember_utils_encryption::encrypt ($register[3]);
														$_COOKIE["optimizemember_item_number"] = /* For ``reg_cookies_ok ()``. */ c_ws_plugin__optimizemember_utils_encryption::encrypt ($register[4]);
														/**/
														if (($reg_cookies = c_ws_plugin__optimizemember_register_access::reg_cookies_ok ()) && extract ($reg_cookies) /* Needed? */)
															{
																...
																else if (($location = c_ws_plugin__optimizemember_utils_urls::wp_register_url ()))
																	{
																		echo '<script type="text/javascript">' . "\n";
																		echo "window.location = '" . c_ws_plugin__optimizemember_utils_strings::esc_js_sq ($location) . "';";
																		echo '</script>' . "\n";
