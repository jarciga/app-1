...
if(!class_exists('c_ws_plugin__optimizemember_paypal_return_in_subscr_or_wa_w_level'))
{
	/**
	 * optimizeMember's PayPal Auto-Return/PDT handler (inner processing routine).
	 *
	 ...
	 * @since 110720
	 */
	class c_ws_plugin__optimizemember_paypal_return_in_subscr_or_wa_w_level
	{
		/**
		 * optimizeMember's PayPal Auto-Return/PDT handler (inner processing routine).
		 *
		...
		public static function cp($vars = array()) // Conditional phase for ``c_ws_plugin__optimizemember_paypal_notify_in::paypal_notify()``.
		{
			...
			)
			{
				...
				if(!get_transient($transient_rtn = 's2m_rtn_'.md5('optimizemember_transient_'.$_paypal_s)) && set_transient($transient_rtn, time(), 31556926 * 10))
				{
					...
					if((preg_match('/^subscr_payment$/i', $paypal['txn_type']) && !empty($_GET['optimizemember_paypal_return_tra']))
					   && (($tra = c_ws_plugin__optimizemember_utils_encryption::decrypt(trim(stripslashes($_GET['optimizemember_paypal_return_tra'])))) && is_array($tra = maybe_unserialize($tra)))
					...
					)
					{
						...
						unset($ipn_signup_vars['optimizemember_log']); // Create array of wouldbe IPN signup vars w/o optimizemember_log.
					}
					...
					if(preg_match('/(referenc|associat|updat|upgrad)/i', $paypal['option_name1']) && $paypal['option_selection1'])
					{
						...
						if(($user_id = c_ws_plugin__optimizemember_utils_users::get_user_id_with($paypal['subscr_id'], $paypal['option_selection1'])) && is_object($user = new WP_User($user_id)) && $user->ID)
						{
							if(!$user->has_cap('administrator') /* Do NOT process this routine on Administrators. */)
							{
								...
								{
									$paypal['optimizemember_log'][] = 'Redirecting Customer to the Login Page (after displaying a quick thank-you message). They need to log back in.';
						...
						else // Unable to modify Subscription. Could not get the existing User ID from the DB.
						{
							...
							echo c_ws_plugin__optimizemember_return_templates::return_template($paypal['subscr_gateway'],
							                                                             _x('<strong>ERROR:</strong> Unable to modify Subscription.<br />Please contact Support for assistance.<br /><br />Could not get the existing User ID from the DB.', 's2member-front', 'optimizemember'),
							                                                             _x('Back To Home Page', 's2member-front', 's2member'), home_url('/'));
						}
						foreach(array_keys(get_defined_vars()) as $__v) $__refs[$__v] =& $$__v;
					...
					*/
					else // Else this is a normal Subscription signup, we are not updating an existing Subscription.
					{
						...
						if(is_multisite() && c_ws_plugin__optimizemember_utils_conds::is_multisite_farm() && is_main_site())
						{
							...
							{
								$paypal['optimizemember_log'][] = 'Redirecting Customer to Signup Page (after displaying a quick thank-you message). They need to Signup/Register now.';
								...
								echo c_ws_plugin__optimizemember_return_templates::return_template($paypal['subscr_gateway'],
								                                                             ...
								                                                             _x('Please Register Now (Click Here)', 's2member-front', 's2member'), c_ws_plugin__optimizemember_utils_urls::wp_signup_url());
							}
						}
						else // Otherwise, this is NOT a Multisite install. Or it is, but the Super Administrator is NOT selling Blog creation.
						{
							...
							{
								$paypal['optimizemember_log'][] = 'Redirecting Customer to Registration Page (after displaying a quick thank-you message). They need to Register now.';

								if ($GLOBALS['WS_PLUGIN__']['optimizemember']['o']['email_reg_link'] === 'yes'){
									$paypal['optimizemember_log'][] = 'Sending email to customer with registration link';
									$approvedAccountThankYouMessage = _x('<strong>Thank you! Your account has been approved.<br />The next step is to Register a Username for immediate access.</strong>', 's2member-front', 's2member');
									$approvedAccountRegisterMessage = _x('Register Now', 's2member-front', 's2member');
									$registerEmailSubject = _x('Register a Username for immediate access', 's2member-front', 's2member');
									$registerEmailMessage = $approvedAccountThankYouMessage . '<br><br>' . '<a href="' . c_ws_plugin__optimizemember_utils_urls::wp_signup_url() . '">' . $approvedAccountRegisterMessage . '</a>';
									wp_mail($paypal['payer_email'], $registerEmailSubject, $registerEmailMessage, 'Content-Type: text/html; charset=UTF-8');
								}

								echo c_ws_plugin__optimizemember_return_templates::return_template($paypal['subscr_gateway'],
								                                                             _x('<strong>Thank you! Your account has been approved.<br />The next step is to Register a Username for immediate access.</strong>', 's2member-front', 's2member'),
								                                                             _x('Please Register Now (Click Here)', 's2member-front', 's2member'), c_ws_plugin__optimizemember_utils_urls::wp_register_url());
							}
						}
						foreach(array_keys(get_defined_vars()) as $__v) $__refs[$__v] =& $$__v;
				...
				else // Page Expired. Duplicate Return-Data.
				{
					...
					$paypal['optimizemember_log'][] = 'optimizeMember `txn_type` identified as ( `web_accept|subscr_signup|subscr_payment` ).';
					$paypal['optimizemember_log'][] = 'Page Expired. Instructing customer to check their email for further details about how to obtain access to what they purchased.';

					echo c_ws_plugin__optimizemember_return_templates::return_template($paypal['subscr_gateway'],
					                                                             '<strong>'._x('You have already completed registration.', 's2member-front', 's2member').'</strong>',
					                                                             _x('Login', 's2member-front', 's2member'), wp_login_url() );
				}
				foreach(array_keys(get_defined_vars()) as $__v) $__refs[$__v] =& $$__v;
			...
			else return apply_filters('c_ws_plugin__optimizemember_paypal_return_in_subscr_or_wa_w_level', FALSE, get_defined_vars());
		}
	}
}