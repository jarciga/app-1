<?php
/**
* File Download routines for optimizeMember.
*
* Copyright: © 2009-2011
...
/**/
if(!class_exists("c_ws_plugin__optimizemember_files"))
	{
		/**
		* File Download routines for optimizeMember.
		*
		...
		* @since 3.5
		*/
		class c_ws_plugin__optimizemember_files
			{
				/**
				...
				*/
				public static function check_file_download_access($create_file_download_url = FALSE) /* Calls inner routine. */
					{
						if(is_array($create_file_download_url) || !empty($_GET["optimizemember_file_download"])) /* Call inner routine? */
							{
				...
				*/
				public static function amazon_s3_auto_configure_acls() /* Calls inner routine. */
					{
						return c_ws_plugin__optimizemember_files_in::amazon_s3_auto_configure_acls();
					}
				/**
				...
				*
				* @deprecated Deprecated in v111029. This function is no longer used by optimizeMember.
				*/
				public static function max_download_period /* No longer used by optimizeMember. */()
					{
						do_action("ws_plugin__optimizemember_before_max_download_period", get_defined_vars());
						/**/
						for($n = 0, $max = 0; $n <= $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["levels"]; $n++)
							if(!empty($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["level".$n."_file_downloads_allowed"]))
								if(!empty($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["level".$n."_file_downloads_allowed_days"]))
									if(($days = $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["level".$n."_file_downloads_allowed_days"]))
										$max = ($max < $days) ? $days : $max;
						/**/
						return apply_filters("ws_plugin__optimizemember_max_download_period", (($max > 365) ? 365 : $max), get_defined_vars());
					}
				/**
				...
				*/
				public static function min_level_4_downloads() /* Multipurpose. False if no access. */
					{
						do_action("ws_plugin__optimizemember_before_min_level_4_downloads", get_defined_vars());
						/**/
						for($n = 0, $min = false; $n <= $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["levels"]; $n++)
							if(!empty($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["level".$n."_file_downloads_allowed"]))
								if(!empty($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["level".$n."_file_downloads_allowed_days"]))
									if(($min = $n) >= 0)
										break;
						/**/
						return apply_filters("ws_plugin__optimizemember_min_level_4_downloads", ((is_int($min)) ? $min : false), get_defined_vars());
					}
				/**
				...
				public static function file_download_key($file = FALSE, $directive = FALSE)
					{
						...
						/**/
						else if(c_ws_plugin__optimizemember_no_cache::no_cache_constants(true))
				...
				* 	relative to the `/optimizemember-files/` directory, or relative to the root of your Amazon S3 Bucket *( when applicable )*.
				...
				*/
				public static function user_downloads($user = FALSE, $not_counting_this_particular_file = FALSE, $user_log = FALSE, $user_arc = FALSE)
					{
						eval('foreach(array_keys(get_defined_vars())as$__v)$__refs[$__v]=&$$__v;');
						do_action("ws_plugin__optimizemember_before_user_downloads", get_defined_vars());
						unset($__refs, $__v); /* Unset defined __refs, __v. */
						/**/
						...
						if((is_object($user) || is_object($user = (is_user_logged_in()) ? wp_get_current_user() : false)) && !empty($user->ID) && ($user_id = $user->ID))
							{
								for($n = 0; $n <= $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["levels"]; $n++)
									{
										if($user->has_cap("access_optimizemember_level".$n)) /* Do they have access? */
											{
												if(!empty($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["level".$n."_file_downloads_allowed"]) && !empty($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["level".$n."_file_downloads_allowed_days"]))
													{
														$allowed = $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["level".$n."_file_downloads_allowed"];
														$allowed_days = $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["level".$n."_file_downloads_allowed_days"];
													}
												if($user->has_cap("optimizemember_level".$n)) /* We can stop now, if this is their Role. */
													break; /* Break now. */
											}
									}
								$log = (is_array($user_log)) ? $user_log : ((is_array($log = get_user_option("optimizemember_file_download_access_log", $user_id)) && $log !== array(false)) ? $log : array());
								$arc = (is_array($user_arc)) ? $user_arc : ((is_array($arc = get_user_option("optimizemember_file_download_access_arc", $user_id)) && $arc !== array(false)) ? $arc : array());
								/**/
						...
						/**/
						return apply_filters("ws_plugin__optimizemember_user_downloads", array("allowed" => $allowed, "allowed_days" => $allowed_days, "currently" => $currently, "log" => $log, "archive" => $arc), get_defined_vars());
					}
				/**
				...
				public static function total_downloads_of($file = FALSE, $user_id = FALSE, $check_archives_too = TRUE)
					{
						...
						if($file && is_string($file)) /* Was ``$file`` passed in properly? */
							{
								if(is_array($results = $wpdb->get_results("SELECT `meta_key`, `meta_value` FROM `".$wpdb->usermeta."` WHERE ".((is_numeric($user_id)) ? "`user_id` = '".esc_sql($user_id)."' AND " : "")."(`meta_key` = '".$wpdb->prefix."optimizemember_file_download_access_log'".(($check_archives_too) ? " OR `meta_key` = '".$wpdb->prefix."optimizemember_file_download_access_arc'" : "").") AND `meta_value` REGEXP '.*\"file\";s:[0-9]+:\"".esc_sql($file)."\".*'")))
									{
				...
				*/
				public static function no_gzip_rules_in_root_htaccess()
					{
						$start_line = /* Beginning line for this entry. */ "# BEGIN optimizeMember GZIP exclusions";
						$end_line = /* Identifying end line for this entry. */ "# END optimizeMember GZIP exclusions";
				...
				public static function write_no_gzip_into_root_htaccess()
					{
						...
						if(c_ws_plugin__optimizemember_files::remove_no_gzip_from_root_htaccess() /* Must first be able to remove any existing entry. */)
							{
								$start_line = /* Beginning line for this entry. */ "# BEGIN optimizeMember GZIP exclusions";
								$end_line = /* Identifying end line for this entry. */ "# END optimizeMember GZIP exclusions";
