<?php
/**
* System configuration for the optimizeMember plugin.
*
...
*/
$GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["dir"] = dirname(dirname(__FILE__));
/*
Determine the base directory name.
*/
$GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["dir_base"] = basename(dirname(dirname(__FILE__)));
/*
Determine the full URL to the directory this plugin resides in.
*/
$GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["dir_url"] = (stripos(__FILE__, WP_CONTENT_DIR) !== 0) ? /* Have to assume plugins dir? */
...
/*
Determine full URL to the optimizeMember-only file that loads WordPress with only optimizeMember active.
*/
$GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["s2o_url"] = $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["dir_url"]."/".preg_replace("/\.php$/", "-o.php", basename($GLOBALS["WS_PLUGIN__"]["optimizemember"]["l"]));
/*
...
*/
$GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["plugin_basename"] = basename(dirname($GLOBALS["WS_PLUGIN__"]["optimizemember"]["l"]))."/".basename($GLOBALS["WS_PLUGIN__"]["optimizemember"]["l"]);
/*
Configure the number of Membership Levels being used with optimizeMember. This is now possible. All areas of optimizeMember are now capable of adapting to this.
*/
$GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["levels"] = 4; /* Hard coded in at 4 Levels. This can only be extended when/if optimizeMember Pro is installed. */
$GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["min_levels"] = 1; /* A lower limit to protect the integrity of the optimizeMember software application. */
$GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["max_levels"] = apply_filters("ws_plugin__optimizemember_max_levels", 100); /* Filterable. */
/*
Configure regular expression matches for Membership Access Item Numbers ( including those with only Custom Capabilities ).
*/
$GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["membership_item_number_w_level_regex"] = "/^([1-9][0-9]*)(?:(?:\:((?:-all\+|\+-all|-all|\+)?[a-z_0-9,\+]*)?)?(?:\:([0-9]+ [A-Z])?)?)?$/";
$GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["membership_item_number_wo_level_regex"] = "/^(\*)(?:(?:\:((?:-all\+|\+-all|-all|\+)?[a-z_0-9,\+]*)?)?(?:\:([0-9]+ [A-Z])?)?)?$/";
$GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["membership_item_number_w_or_wo_level_regex"] = "/^([1-9][0-9]*|\*)(?:(?:\:((?:-all\+|\+-all|-all|\+)?[a-z_0-9,\+]*)?)?(?:\:([0-9]+ [A-Z])?)?)?$/";
/*
Configure regular expression match for Specific Post/Page Access Item Numbers ( all elements required here ).
*/
$GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["sp_access_item_number_regex"] = "/^(sp)(?:(?:\:([1-9][0-9,]*))(?:\:([1-9][0-9]*)))$/";
/*
Configure multibyte detection order when charset is unknown ( used by calls to `mb_convert_encoding()` ).
*/
$GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["mb_detection_order"] = "UTF-8, ISO-8859-1, WINDOWS-1252, ASCII, JIS, EUC-JP, SJIS";
/*
...
*/
$GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["streaming_file_extns"] = array_unique(array("avi", "wav", "mpa", "mpeg", "mpv", "mps", "m1v", "m2v", "mp4"/**/, "mp3", "m3u"/**/, "mp4", "flv", "f4v", "3gp", "3g2", "aac", "m4a"/**/, "webm"/**/, "ogg", "ogv", "pls", "m3u", "ogm", "m4u"/**/, "mov", "qtl", "mp4"/**/, "asf", "wmv", "wvx", "wma", "wax"/**/, "ra", "rm", "ram"));
/*
Configure directory and .htaccess for files protected by optimizeMember.
*/
$GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["files_dir"] = apply_filters("ws_plugin__optimizemember_files_dir", dirname(dirname(__FILE__))."-files".((stripos(PHP_OS, "win") === 0 && stripos($_SERVER["SERVER_SOFTWARE"], "apache") === false) ? "/app_data" : ""));
$GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["files_no_gzip_htaccess"] = dirname(__FILE__)."/templates/cfg-files/optimizemember-files-no-gzip.php";
$GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["files_dir_htaccess"] = dirname(__FILE__)."/templates/cfg-files/optimizemember-files.php";
/*
Configure the directory for logs protected by optimizeMember.
*/
$GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["logs_dir"] = apply_filters("ws_plugin__optimizemember_logs_dir", dirname(dirname(__FILE__))."-logs".((stripos(PHP_OS, "win") === 0 && stripos($_SERVER["SERVER_SOFTWARE"], "apache") === false) ? "/app_data" : ""));
$GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["logs_dir_htaccess"] = dirname(__FILE__)."/templates/cfg-files/optimizemember-logs.php";
/*
Configure the global reCaptcha ( www.websharks-inc.net / or any domain ). These public/private keys work on any installation.
*/
$GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["recaptcha"] = array("public_key" => "6LeCANsSAAAAAIIrlB3FrXe42mr0OSSZpT0pkpFK", "private_key" => "6LeCANsSAAAAAGBXMIKAirv6G4PmaGa-ORxdD-oZ", "lang" => _x("en", "s2member-front recaptcha-lang-code", "s2member"));
/*
Configure the right menu options panel for optimizeMember.
*/
$GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["menu_pages"] = array("updates" => true, "upsell-pro" => true, "installation" => false, "tools" => false, "kb" => true, "videos" => true, "support" => true, "donations" => true);
/*
Check if optimizeMember has been configured *should be set after the first config via options panel*.
*/
$GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["configured"] = get_option("ws_plugin__optimizemember_configured");
/*
This is a special option cache that holds some additional information autoloaded into WordPress.
*/
$GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["cache"] = get_option("ws_plugin__optimizemember_cache");
/*
Configure checksum time for the syscon.inc.php file.
*/
$GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["checksum"] = filemtime(__FILE__);
/*
Configure an array of pluggable functions handled by optimizeMember.
*/
$GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["pluggables"] = array();

/**
 * Setting default membership levels to 10
 * wp-config value overrides this one
 */
if (!defined("MEMBERSHIP_LEVELS")) {
	define("MEMBERSHIP_LEVELS", 10);
}
/*
Configure & validate all of the optimizeMember options; and set their defaults.
*/
if(!function_exists("ws_plugin__optimizemember_configure_options_and_their_defaults"))
	{
		/**
		* Configures an options array for the optimizeMember plugin.
		*
		...
		* @return array This merged array of options: ``$GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]``
		*/
		function ws_plugin__optimizemember_configure_options_and_their_defaults($options = FALSE)
			{
				global $current_site, $current_blog;
				/**/
				$default_options["ccp"] = array();
				/**/
				...
				$default_options["email_reg_link"] = "no";
				...
				for($n = 0, $v = 0; $n <= $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["levels"]; $n++, $v = $v + 5)$default_options["mms_registration_blogs_level".$n] = (string)$v;
				/**/
				...
				$default_options["login_reg_background_image"] = $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["dir_url"]."/images/bg.png";
				...
				/**/
				$default_options["login_reg_logo_src"] = $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["dir_url"]."/images/logo.png";
				...
				/**/
				for($n = 0; $n <= $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["levels"]; $n++)$default_options["level".$n."_mailchimp_list_ids"] = "";
				/**/
				for($n = 0; $n <= $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["levels"]; $n++)$default_options["level".$n."_aweber_list_ids"] = "";
				/**/
				for($n = 0; $n <= $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["levels"]; $n++)$default_options["level".$n."_provider"] = "";
				for($n = 0; $n <= $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["levels"]; $n++)$default_options["level".$n."_list"] = "";
				/**/
				...
				/**/
				for($n = 0, $l = array(_x("Free Subscriber", "s2member-front", "s2member"), _x("Bronze Member", "s2member-front", "s2member"), _x("Silver Member", "s2member-front", "s2member"), _x("Gold Member", "s2member-front", "s2member"), _x("Platinum Member", "s2member-front", "s2member")); $n <= $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["levels"]; $n++)$default_options["level".$n."_label"] = (!empty($l[$n])) ? $l[$n] : sprintf(_x("Level %s Member", "s2member-front", "s2member"), $n);
				/**/
				$default_options["apply_label_translations"] = "0";
				/**/
				for($n = 0; $n <= $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["levels"]; $n++)$default_options["level".$n."_file_downloads_allowed"] = "";
				/**/
				for($n = 0; $n <= $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["levels"]; $n++)$default_options["level".$n."_file_downloads_allowed_days"] = "";
				/**/
				...
				/**/
				for($n = 0; $n <= $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["levels"]; $n++)$default_options["level".$n."_ruris"] = "";
				/**/
				for($n = 0; $n <= $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["levels"]; $n++)$default_options["level".$n."_catgs"] = "";
				/**/
				for($n = 0; $n <= $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["levels"]; $n++)$default_options["level".$n."_ptags"] = "";
				/**/
				for($n = 0; $n <= $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["levels"]; $n++)$default_options["level".$n."_posts"] = "";
				/**/
				for($n = 0; $n <= $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["levels"]; $n++)$default_options["level".$n."_pages"] = "";
				/**/
				...
				/**/
				$default_options = apply_filters("ws_plugin__optimizemember_default_options", $default_options);
				/**/
				...
				*/
				$GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"] = array_merge($default_options, (($options !== false) ? (array)$options : (array)get_option("ws_plugin__optimizemember_options")));
				/**/
				/* Back compatibility for `filter_wp_query`. Changed in v110912 to array. */
				if(is_string($_ov = &$GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["filter_wp_query"]))
				...
				if($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["login_reg_logo_src"] === $default_options["login_reg_logo_src"])
					$GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["login_reg_logo_src_width"] = $default_options["login_reg_logo_src_width"];
				/**/
				/* Backward compatibility for PayPal API Credentials. Starting with v3.5+, this info is stored by the free version of optimizeMember. */
				if(empty($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["paypal_api_username"]) && !empty($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_paypal_api_username"]))
					$GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["paypal_api_username"] = $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_paypal_api_username"];
				/**/
				if(empty($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["paypal_api_password"]) && !empty($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_paypal_api_password"]))
					$GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["paypal_api_password"] = $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_paypal_api_password"];
				/**/
				if(empty($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["paypal_api_signature"]) && !empty($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_paypal_api_signature"]))
					$GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["paypal_api_signature"] = $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["pro_paypal_api_signature"];
				/*
				...
				*/
				$checksum = md5(($checksum_prefix = $GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["checksum"].serialize($default_options)).serialize(array_merge($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"], array("options_checksum" => 0))));
				/*
				...
				*/
				if($options !== false || ($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["options_checksum"] !== $checksum && $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"] !== $default_options))
					{
						foreach($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"] as $key => &$value)
							{
								if(!isset($default_options[$key]) && !preg_match("/^pro_/", $key))
									unset($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"][$key]);
								/**/
								...
								/**/
									...
									$value = $default_options[$key];
								/**/
								else if($key === "email-reg-link" && (!is_string($value) || !preg_match("/^(?:yes|no|0)$/", $value)))
									$value = $default_options[$key];
								/**/
								else if($key === "custom_reg_opt_in_label" /*&& (!is_string($value) || !strlen($value))*/) // we are ignoring database entry as no form is available
									$value = $default_options[$key];
								/**/
						...
						/**/
						if($options !== false && is_string($options["sec_encryption_key"]) && strlen($options["sec_encryption_key"]) && !in_array($options["sec_encryption_key"], $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["sec_encryption_key_history"]))
							{
								array_unshift($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["sec_encryption_key_history"], $options["sec_encryption_key"]);
								$GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["sec_encryption_key_history"] = array_slice($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["sec_encryption_key_history"], 0, 10);
							}
						/**/
						$GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"] = apply_filters_ref_array("ws_plugin__optimizemember_options_before_checksum", array(&$GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]));
						/**/
						$GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]["options_checksum"] = md5($checksum_prefix.serialize(array_merge($GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"], array("options_checksum" => 0))));
					}
				/**/
				return apply_filters_ref_array("ws_plugin__optimizemember_options", array(&$GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"]));
			}
	}

	if (!function_exists('ws_plugin__optimizemember_getMembershipLabel')) {
		/**
		 * Returns membership level label
		 * @param int $n Level
		 */
		function ws_plugin__optimizemember_getMembershipLabel($n)
		{
			$key = "level".$n."_label";
			return $GLOBALS["WS_PLUGIN__"]["optimizemember"]["o"][$key];
		}
	}
?>