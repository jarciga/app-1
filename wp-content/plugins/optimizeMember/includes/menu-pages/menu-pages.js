...
*
* This file is included with all WordPress themes/plugins by optimizePress, Inc.
*
* Copyright: © 2009-2011
...
*
* @package optimizePress\Menu Pages
* @since x.xx
*/
jQuery(document).ready (function($)
    {
        ...
        $('#ws-plugin--optimizemember-new-options-form').submit (function(e)
            {
                e.preventDefault();
                jQuery.post(ajaxurl, jQuery(this).serialize(),
                    function(resp){
                        if(typeof resp.error != 'undefined'){
                            ...
                            jQuery('#msg, .save_info').html(resp).show().delay(2000).fadeOut();
                        }
                    }
                );
                return false;
        ...
        $('.ws_plugin__optimizemember_level_provider').change(function(e) {
            var provider = jQuery(this).val(),
                level = jQuery(this).attr('data-level'),
                $list = jQuery('#ws_plugin__optimizemember_level' + level + '_list'),
                $label = jQuery('#ws_plugin__optimizemember_level' + level + '_list_label');

            // Label
            if (provider === 'infusionsoft') {
                $label.html('Follow up sequence:');
            } else if (provider === 'arpreach') {
                $label.html('Autoresponder:');
            } else {
                $label.html('List:');
            }

            // Select/input
            if (provider === 'arpreach' && $list.is('select')) {
                $list.parent().append('<input type="text" data-level="' + level + '" name="ws_plugin__optimizemember_level' + level + '_list" id="ws_plugin__optimizemember_level' + level + '_list" class="ws_plugin__optimizemember_level_list"/>');
                $list.remove();
                $list = jQuery('#ws_plugin__optimizemember_level' + level + '_list');
            } else if ($list.not('select')) {
                $list.parent().append('<select type="text" data-level="' + level + '" name="ws_plugin__optimizemember_level' + level + '_list" id="ws_plugin__optimizemember_level' + level + '_list" class="ws_plugin__optimizemember_level_list"></select>');
                $list.remove();
                $list = jQuery('#ws_plugin__optimizemember_level' + level + '_list');
            }

            // Reset lists
            if (typeof OptimizePress.lists == 'undefined') {
                OptimizePress.lists = {};
            }
            if (provider == 0) {
                $list.empty();
                $list.append('<option value="0">Select provider first</option>');
            } else if (typeof OptimizePress.lists[provider] != 'undefined') {
                $list.empty();
                $list.append(OptimizePress.lists[provider]);
            } else if (provider !== 'arpreach') {
                $list.after('<img class="op-bsw-waiting" src="images/wpspin_light.gif" alt="loading" style="position:relative;display:block;top:4px">');
                jQuery.post(OptimizePress.ajaxurl,
                    {
                    ...
                    function(response) {
                        var markup = [];
                        jQuery.each(response.lists, function (id, item) {
                            markup.push('<option value="' + id + '">' + item.name + '</option>');
                        });
                        OptimizePress.lists[provider] = markup;
                        $list.empty().append(markup);
                        $list.next().remove();
                    },
                    'json'
                );
