/**
* Core JavaScript routines for optimizeMember menu pages.
*
* Copyright: © 2009-2011
...
jQuery(document).ready(function($)
{
    ...
    ws_plugin__optimizemember_generateSecurityKey = /* Generates a unique Security Key. */ function(obj)
    {
        if(!obj) obj = '#ws-plugin--optimizemember-sec-encryption-key';
    ...
    if(location.href.match /* Any & all OptimizeMember pages. */(/page\=ws-plugin--optimizemember/))
    {
        $('input.ws-plugin--optimizemember-update-roles-button, input.ws-plugin--optimizemember-reset-roles-button').click(function()
        {
            var $this = /* Save $(this) into $this. */ $(this);
            $this.val /* Indicate loading status (please wait). */('one moment please ...');

            var levels = '<?php echo (int)$GLOBALS["WS_PLUGIN__"]["optimizemember"]["c"]["levels"]; ?>';
            var resetUpdate = ($this.hasClass('ws-plugin--optimizemember-reset-roles-button')) ? 'Reset' : 'Update';

            $.post(ajaxurl, {action: 'ws_plugin__optimizemember_update_roles_via_ajax', ws_plugin__optimizemember_update_roles_via_ajax: '<?php echo c_ws_plugin__optimizemember_utils_strings::esc_js_sq (wp_create_nonce ("ws-plugin--optimizemember-update-roles-via-ajax")); ?>'}, function(response)
            {
                if(response === '1')
                    alert('OptimizeMember\'s Roles/Capabilities '+((resetUpdate.toLowerCase() === 'reset') ? 'have been successfully reset' : 'updated successfully')+'.\nYour installation of OptimizeMember has Membership Levels 0-'+levels+'.'), $this.val(resetUpdate+' Roles/Capabilities');

                else if(response === 'l' /* Locked? */)
                    alert('Sorry, your request failed.\nOptimizeMember\'s Roles/Capabilities are locked by Filter:\nws_plugin__optimizemember_lock_roles_caps'), $this.val(resetUpdate+' Roles/Capabilities');
    ...
    if(location.href.match(/page\=ws-plugin--optimizemember-logs/))
    {
        $('input#ws-plugin--optimizemember-gateway-debug-logs-extensive-1').click(function()
        {
            var $this = $(this), thisChecked = (this.checked) ? true : false;
            if(thisChecked) $('input#ws-plugin--optimizemember-gateway-debug-logs-1').attr('checked', 'checked');
        });
        var $toggles = $('a.ws-plugin--optimizemember-log-file-viewport-toggle'); $toggles.click(function()
    {
        $('textarea#ws-plugin--optimizemember-log-file-viewer').each(function()
        {
    ...
    if(location.href.match(/page\=ws-plugin--optimizemember-mms-ops/))
    {
        $('select#ws-plugin--optimizemember-mms-registration-file').change(function()
        {
            if /* Expand/collapse relevant options; based on file selection. */($(this).val() === 'wp-signup')
            {
                var gv = $('select#ws-plugin--optimizemember-mms-registration-grants').val(), l0v = $('input#ws-plugin--optimizemember-mms-registration-blogs-level0').val();
                $('div#ws-plugin--optimizemember-mms-registration-support-package-details-wrapper').show(), $('div.ws-plugin--optimizemember-mms-registration-wp-login, table.ws-plugin--optimizemember-mms-registration-wp-login').hide(), $('div.ws-plugin--optimizemember-mms-registration-wp-signup, table.ws-plugin--optimizemember-mms-registration-wp-signup').show();
                $('div.ws-plugin--optimizemember-mms-registration-wp-signup-blogs-level0, table.ws-plugin--optimizemember-mms-registration-wp-signup-blogs-level0')[((gv === 'all') ? 'show' : 'hide')]();
                $('input#ws-plugin--optimizemember-mms-registration-blogs-level0').val(((gv === 'all') ? ((l0v > 0) ? l0v : '1') : '0'));
            }
            else if /* Expand/collapse relevant options. */($(this).val() === 'wp-login')
            {
                var gv = $('select#ws-plugin--optimizemember-mms-registration-grants').val(), l0v = $('input#ws-plugin--optimizemember-mms-registration-blogs-level0').val();
                $('div#ws-plugin--optimizemember-mms-registration-support-package-details-wrapper').hide(), $('div.ws-plugin--optimizemember-mms-registration-wp-login, table.ws-plugin--optimizemember-mms-registration-wp-login').show(), $('div.ws-plugin--optimizemember-mms-registration-wp-signup, table.ws-plugin--optimizemember-mms-registration-wp-signup').hide();
                $('div.ws-plugin--optimizemember-mms-registration-wp-signup-blogs-level0, table.ws-plugin--optimizemember-mms-registration-wp-signup-blogs-level0').hide();
                $('input#ws-plugin--optimizemember-mms-registration-blogs-level0').val('0');
            }
        }).trigger /* Fire on ready too. */('change');

        $('select#ws-plugin--optimizemember-mms-registration-grants').change(function()
        {
            $('select#ws-plugin--optimizemember-mms-registration-file').trigger('change');
        });
    }
    if(location.href.match(/page\=ws-plugin--optimizemember-gen-ops/))
    {
        ws_plugin__optimizemember_enableSecurityKey = /* Allow Security Key editing?? */ function()
        {
            if(confirm('Edit Key? Are you sure?\nThis could break your installation!\n\n*Note* If you\'ve been testing OptimizeMember, feel free to change this Key before you go live. Just don\'t go live, and then change it. You\'ll have unhappy Customers. Data corruption WILL occur! For your safety, OptimizeMember keeps a history of the last 10 Keys that you\'ve used. If you get yourself into a real situation, OptimizeMember will let you revert back to a previous Key.'))
                $('input#ws-plugin--optimizemember-sec-encryption-key').removeAttr('disabled');

            return false;
        };
        ws_plugin__optimizemember_securityKeyHistory = /* Displays history of Keys. */ function()
        {
            $('div#ws-plugin--optimizemember-sec-encryption-key-history').toggle();

            return false;
        };
        $('select#ws-plugin--optimizemember-new-user-emails-enabled').change(function()
        {
            var $pluggable = $('input#ws-plugin--optimizemember-pluggables-wp-new-user-notification'), $this = $(this), $newUserEmails = $('div#ws-plugin--optimizemember-new-user-emails');
        ...
        $('select#ws-plugin--optimizemember-login-reg-design-enabled').change(function()
        {
            var $this = $(this), $loginRegDesign = $('div#ws-plugin--optimizemember-login-reg-design');
        ...
        $('select#ws-plugin--optimizemember-custom-reg-password').change(function()
        {
            var $this = $(this), $newUserNotification = $('div#ws-plugin--optimizemember-new-user-email-details');
        ...
        if($('input#ws-plugin--optimizemember-custom-reg-fields').length && $('div#ws-plugin--optimizemember-custom-reg-field-configuration').length)
        {
            ( /* Wrap these routines inside a function to keep variables within relative scope. */function()
            {
                ...
                var $fields = $('input#ws-plugin--optimizemember-custom-reg-fields');
                var $configuration = $('div#ws-plugin--optimizemember-custom-reg-field-configuration');
                ...
                tools = '<div id="ws-plugin--optimizemember-custom-reg-field-configuration-tools"></div>', table = '<table id="ws-plugin--optimizemember-custom-reg-field-configuration-table"></table>';

                $configuration.html /* Add tools div & table div to configuration div (i.e., div#ws-plugin--optimizemember-custom-reg-field-configuration). */(tools+table);

                $tools = $('div#ws-plugin--optimizemember-custom-reg-field-configuration-tools'), $table = $('table#ws-plugin--optimizemember-custom-reg-field-configuration-table');

                ws_plugin__optimizemember_customRegFieldSectionChange = function(select)
                {
                    var section = /* Current selection (no|yes, selected by site owner). */ $(select).val();

                    var sectitle_trs = 'tr.ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-sectitle';

                    (section === 'yes') ? $(sectitle_trs).css('display', '') : $(sectitle_trs).css('display', 'none');
                };
                ws_plugin__optimizemember_customRegFieldTypeChange = /* Handle change events here. */ function(select)
                {
                    var type = /* Current selection (type of Field, selected by site owner). */ $(select).val();

                    var deflt_trs = 'tr.ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-deflt', options_trs = 'tr.ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-options', expected_trs = 'tr.ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-expected';
                ...
                ws_plugin__optimizemember_customRegFieldDelete = function(index)
                {
                ...
                ws_plugin__optimizemember_customRegFieldMoveUp = function(index)
                {
                ...
                ws_plugin__optimizemember_customRegFieldMoveDown = function(index)
                {
                    if(typeof fields[index] === 'object' && typeof fields[index+1] === 'object')
                    {
                ...
                ws_plugin__optimizemember_customRegFieldCreate = function()
                {
                    var $table = $('table#ws-plugin--optimizemember-custom-reg-field-configuration-tools-form'), field = {};
                    ...
                    if /* If it can be validated. */((field = validateField(field)))
                    {
                        ...
                        {
                            var row = 'tr.ws-plugin--optimizemember-custom-reg-field-configuration-table-row-'+(fields.length-1);
                            alert('Field created successfully.\n* Remember to "Save All Changes".');
                            //$(row).effect('highlight');
                        }, 500);
                    }
                };
                ws_plugin__optimizemember_customRegFieldUpdate = function(index)
                {
                    var $table = $('table#ws-plugin--optimizemember-custom-reg-field-configuration-tools-form'), field = {};
                    ...
                    if /* If it validates. */((field = validateField(field, index)))
                    {
                        ...
                        setTimeout( /* A momentary delay here for usability. */function()
                        {
                            var row = 'tr.ws-plugin--optimizemember-custom-reg-field-configuration-table-row-'+index;
                            alert('Field updated successfully.\n* Remember to "Save All Changes".');
                            //$(row).effect('highlight');
                        }, 500);
                    }
                };
                ws_plugin__optimizemember_customRegFieldAdd = /* Add new field links. */ function()
                {
                    buildTools /* No need to reset scroll position. */(true);
                };
                ws_plugin__optimizemember_customRegFieldEdit = /* Edit links. */ function(index)
                {
                    buildTools(false, index), scrollReset();
                };
                ws_plugin__optimizemember_customRegFieldCancel = /* Cancel form. */ function()
                {
                ...
                {
                    scrollTo(0, $('div.ws-plugin--optimizemember-custom-reg-fields-section').offset()['top']-100);
                };
                var buildTools = /* This builds tools into the configuration. */ function(adding, index)
                {
                    ...
                    html += /* Click to add a new Custom Registration/Profile Field. */ '<a href="#" onclick="ws_plugin__optimizemember_customRegFieldAdd(); return false;">Add New Field</a>';

                    tb_remove /* Remove an existing thickbox. */(), $('div#ws-plugin--optimizemember-custom-reg-field-configuration-thickbox-tools-form').remove();

                    if /* Do we need to display the adding/editing form at all? */(displayForm)
                    {
                        form += '<div id="ws-plugin--optimizemember-custom-reg-field-configuration-thickbox-tools-form">';

                        form += '<table id="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form">';
                        form += '<tbody>';

                        form += '<tr class="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-section">';
                        form += '<td colspan="2">';
                        form += '<label for="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-section">Starts A New Section?</label>';
                        form += '</td>';
                        form += '</tr>';

                        form += '<tr class="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-section">';
                        form += '<td colspan="2">';
                        form += '<select property="section" id="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-section" onchange="ws_plugin__optimizemember_customRegFieldSectionChange(this);">';
                        ...
                        form += '<tr class="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-spacer ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-sectitle ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-section"'+((field.section === 'yes') ? '' : ' style="display:none;"')+'><td colspan="2"><hr /></td></tr>';

                        form += '<tr class="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-sectitle ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-section"'+((field.section === 'yes') ? '' : ' style="display:none;"')+'>';
                        form += '<td colspan="2">';
                        form += 'Title for this new section? (optional)<br />';
                        form += '<input type="text" property="sectitle" autocomplete="off" id="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-sectitle" value="'+esc_attr(field.sectitle)+'" /><br />';
                        form += '<small>If empty, a simple divider will be used by default.</small>';
                        form += '</td>';
                        form += '</tr>';

                        form += '<tr class="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-spacer ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-type"><td colspan="2">&nbsp;</td></tr>';

                        form += '<tr class="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-type">';
                        form += '<td colspan="2">';
                        form += '<label for="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-type">Form Field Type: *</label>';
                        form += '</td>';
                        form += '</tr>';

                        form += '<tr class="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-type">';
                        form += '<td colspan="2">';
                        form += '<select property="type" id="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-type" onchange="ws_plugin__optimizemember_customRegFieldTypeChange(this);">';
                        ...
                        form += '<tr class="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-spacer ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-label"><td colspan="2">&nbsp;</td></tr>';

                        form += '<tr class="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-label">';
                        form += '<td colspan="2">';
                        form += '<label for="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-label">Field Label/Desc: *</label>';
                        form += '</td>';
                        form += '</tr>';

                        form += '<tr class="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-label">';
                        form += '<td colspan="2">';
                        form += '<input type="text" property="label" autocomplete="off" id="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-label" value="'+esc_attr(field.label)+'" /><br />';
                        form += '<small>Examples: <code>Choose Country</code>, <code>Street Address</code></small>';
                        form += '</td>';
                        form += '</tr>';

                        form += '<tr class="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-spacer ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-id"><td colspan="2">&nbsp;</td></tr>';

                        form += '<tr class="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-id">';
                        form += '<td colspan="2">';
                        form += '<label for="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-id">Unique Field ID: *</label></label>';
                        form += '</td>';
                        form += '</tr>';

                        form += '<tr class="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-id">';
                        form += '<td colspan="2">';
                        form += '<input type="text" property="id" maxlength="25" autocomplete="off" id="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-id" value="'+esc_attr(field.id)+'" /><br />';
                        ...
                        form += '<tr class="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-spacer ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-required"><td colspan="2">&nbsp;</td></tr>';

                        form += '<tr class="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-required">';
                        form += '<td colspan="2">';
                        form += '<label for="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-required">Field Required: *</label>';
                        form += '</td>';
                        form += '</tr>';

                        form += '<tr class="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-required">';
                        form += '<td colspan="2">';
                        form += '<select property="required" id="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-required">';
                        ...
                        form += '<tr class="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-spacer ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-deflt"'+((field.type.match(/^(text|textarea)$/)) ? '' : ' style="display:none;"')+'><td colspan="2">&nbsp;</td></tr>';

                        form += '<tr class="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-deflt"'+((field.type.match(/^(text|textarea)$/)) ? '' : ' style="display:none;"')+'>';
                        form += '<td colspan="2">';
                        form += '<label for="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-deflt">Default Text Value: (optional)</label>';
                        form += '</td>';
                        form += '</tr>';

                        form += '<tr class="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-deflt"'+((field.type.match(/^(text|textarea)$/)) ? '' : ' style="display:none;"')+'>';
                        form += '<td colspan="2">';
                        form += '<textarea property="deflt" wrap="off" spellcheck="false" id="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-deflt" rows="1">'+esc_html(field.deflt)+'</textarea><br />';
                        form += '<small>Default value before user input is received.</small>';
                        form += '</td>';
                        form += '</tr>';

                        form += '<tr class="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-spacer ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-options"'+((field.type.match(/^(select|selects|checkboxes|radios)$/)) ? '' : ' style="display:none;"')+'><td colspan="2">&nbsp;</td></tr>';

                        form += '<tr class="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-options"'+((field.type.match(/^(select|selects|checkboxes|radios)$/)) ? '' : ' style="display:none;"')+'>';
                        form += '<td colspan="2">';
                        form += '<label for="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-options">Option Configuration File: * (one option per line)</label><br />';
                        ...
                        form += '<tr class="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-options"'+((field.type.match(/^(select|selects|checkboxes|radios)$/)) ? '' : ' style="display:none;"')+'>';
                        form += '<td colspan="2">';
                        form += '<textarea property="options" wrap="off" spellcheck="false" id="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-options" rows="3">'+esc_html(field.options)+'</textarea><br />';
                        ...
                        form += '<tr class="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-spacer ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-expected"'+((field.type.match(/^(text|textarea)$/)) ? '' : ' style="display:none;"')+'><td colspan="2">&nbsp;</td></tr>';

                        form += '<tr class="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-expected"'+((field.type.match(/^(text|textarea)$/)) ? '' : ' style="display:none;"')+'>';
                        form += '<td colspan="2">';
                        form += '<label for="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-expected">Expected Format: *</label>';
                        form += '</td>';
                        form += '</tr>';

                        form += '<tr class="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-expected"'+((field.type.match(/^(text|textarea)$/)) ? '' : ' style="display:none;"')+'>';
                        form += '<td colspan="2">';
                        form += '<select property="expected" id="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-expected">';
                        ...
                        form += '<tr class="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-spacer ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-levels"><td colspan="2">&nbsp;</td></tr>';

                        form += '<tr class="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-levels">';
                        form += '<td colspan="2">';
                        form += '<label for="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-levels">Applicable Membership Levels: *</label>';
                        form += '</td>';
                        form += '</tr>';

                        form += '<tr class="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-levels">';
                        form += '<td colspan="2">';
                        form += '<input type="text" property="levels" autocomplete="off" id="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-levels" value="'+esc_attr(field.levels)+'" /><br />';
                        ...
                        form += '<tr class="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-spacer ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-editable"><td colspan="2">&nbsp;</td></tr>';

                        form += '<tr class="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-editable">';
                        form += '<td colspan="2">';
                        form += '<label for="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-editable">Allow Profile Edits: *</label>';
                        form += '</td>';
                        form += '</tr>';

                        form += '<tr class="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-editable">';
                        form += '<td colspan="2">';
                        form += '<select property="editable" id="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-editable">';
                        ...
                        form += '<tr class="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-spacer ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-classes"><td colspan="2">&nbsp;</td></tr>';

                        form += '<tr class="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-classes">';
                        form += '<td colspan="2">';
                        form += '<label for="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-classes">CSS Classes: (optional)</label>';
                        form += '</td>';
                        form += '</tr>';

                        form += '<tr class="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-classes">';
                        form += '<td colspan="2">';
                        form += '<input type="text" property="classes" autocomplete="off" id="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-classes" value="'+esc_attr(field.classes)+'" /><br />';
                        form += '<small>Example: <code>my-style-1 my-style-2</code></small>';
                        form += '</td>';
                        form += '</tr>';

                        form += '<tr class="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-spacer ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-styles"><td colspan="2">&nbsp;</td></tr>';

                        form += '<tr class="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-styles">';
                        form += '<td colspan="2">';
                        form += '<label for="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-styles">CSS Styles: (optional)</label>';
                        form += '</td>';
                        form += '</tr>';

                        form += '<tr class="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-styles">';
                        form += '<td colspan="2">';
                        form += '<input type="text" property="styles" autocomplete="off" id="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-styles" value="'+esc_attr(field.styles)+'" /><br />';
                        form += '<small>Example: <code>color:#000000; background:#FFFFFF;</code></small>';
                        form += '</td>';
                        form += '</tr>';

                        form += '<tr class="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-spacer ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-attrs"><td colspan="2">&nbsp;</td></tr>';

                        form += '<tr class="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-attrs">';
                        form += '<td colspan="2">';
                        form += '<label for="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-attrs">Other Attributes: (optional)</label>';
                        form += '</td>';
                        form += '</tr>';

                        form += '<tr class="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-attrs">';
                        form += '<td colspan="2">';
                        form += '<input type="text" property="attrs" autocomplete="off" id="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-attrs" value="'+esc_attr(field.attrs)+'" /><br />';
                        form += '<small>Example: <code>onkeyup="" onblur=""</code></small>';
                        form += '</td>';
                        form += '</tr>';

                        form += '<tr class="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-spacer ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-buttons"><td colspan="2">&nbsp;</td></tr>';

                        form += '<tr class="ws-plugin--optimizemember-custom-reg-field-configuration-tools-form-buttons">';
                        form += '<td align="left">';
                        form += '<input type="button" value="Cancel" onclick="ws_plugin__optimizemember_customRegFieldCancel();" />';
                        form += '</td>';
                        form += '<td align="right">';
                        form += '<input type="button" value="'+((editing) ? 'Update This Field' : 'Create Registration Field')+'" onclick="'+((editing) ? 'ws_plugin__optimizemember_customRegFieldUpdate('+index+');' : 'ws_plugin__optimizemember_customRegFieldCreate();')+'" />';
                        form += '</td>';
                        form += '</tr>';
                        ...
                        tb_show(((editing) ? 'Editing Registration/Profile Field' : 'New Custom Registration/Profile Field'), '#TB_inline?inlineId=ws-plugin--optimizemember-custom-reg-field-configuration-thickbox-tools-form');

                        $('table#ws-plugin--optimizemember-custom-reg-field-configuration-tools-form').show();
                    }
                    $tools.html(html);
                };
                var buildTable = /* This builds the table of existing fields. */ function()
                {
                    ...
                    if /* Any fields? */(fields.length > 0)
                    {
                        for(i = 0; i < fields.length; i++)
                        {
                            html += '<tr class="'+esc_attr((eo = (eo === 'o') ? 'e' : 'o'))+((fields[i].section === 'yes') ? ' s' : '')+' ws-plugin--optimizemember-custom-reg-field-configuration-table-row-'+i+'">'; // Odd/even + row identifier.
                            html += '<td nowrap="nowrap"><a class="ws-plugin--optimizemember-custom-reg-field-configuration-move-up" href="#" onclick="ws_plugin__optimizemember_customRegFieldMoveUp('+i+'); return false;"></a><a class="ws-plugin--optimizemember-custom-reg-field-configuration-move-down" href="#" onclick="ws_plugin__optimizemember_customRegFieldMoveDown('+i+'); return false;"></a></td>';
                            ...
                            html += '<td nowrap="nowrap"><a class="ws-plugin--optimizemember-custom-reg-field-configuration-edit" href="#" onclick="ws_plugin__optimizemember_customRegFieldEdit('+i+'); return false;"></a><a class="ws-plugin--optimizemember-custom-reg-field-configuration-delete" href="#" onclick="ws_plugin__optimizemember_customRegFieldDelete('+i+'); return false;"></a></td>';
                            html += '</tr>';
                        }
                    }
    ...
    if(location.href.match(/page\=ws-plugin--optimizemember-res-ops/))
    {
        $('input#ws-plugin--optimizemember-brute-force-restrictions-reset-button').click(function()
        {
            var $this = /* Save $(this) into $this. */ $(this);
            $this.val /* Indicate loading status (please wait). */('one moment please ...');

            $.post(ajaxurl, {action: 'ws_plugin__optimizemember_delete_reset_all_ip_restrictions_via_ajax', ws_plugin__optimizemember_delete_reset_all_ip_restrictions_via_ajax: '<?php echo c_ws_plugin__optimizemember_utils_strings::esc_js_sq (wp_create_nonce ("ws-plugin--optimizemember-delete-reset-all-ip-restrictions-via-ajax")); ?>'}, function(response)
            {
                alert('OptimizeMember\'s Brute Force Restriction Logs have all been reset.'), $this.val('Reset Brute Force Logs');
            });

            return false;
        });
        $('input#ws-plugin--optimizemember-ip-restrictions-reset-button').click(function()
        {
            var $this = /* Save $(this) into $this. */ $(this);
            $this.val /* Indicate loading status (please wait). */('one moment please ...');
            ...
            $.post(ajaxurl, {action: 'ws_plugin__optimizemember_delete_reset_all_ip_restrictions_via_ajax', ws_plugin__optimizemember_delete_reset_all_ip_restrictions_via_ajax: '<?php echo c_ws_plugin__optimizemember_utils_strings::esc_js_sq (wp_create_nonce ("ws-plugin--optimizemember-delete-reset-all-ip-restrictions-via-ajax")); ?>'}, function(response)
            {
                alert('OptimizeMember\'s IP Restriction Logs have all been reset.'), $this.val('Reset IP Restriction Logs');
            });
            return false;
        });
        $('div.ws-plugin--optimizemember-query-level-access-section input[type="checkbox"][name="ws_plugin__optimizemember_filter_wp_query\[\]"]').change(function()
        {
            var thisChange = /* Record value associated with change event. Allows for intutitive unchecking. */ $(this).val();

            $('div.ws-plugin--optimizemember-query-level-access-section input[type="checkbox"][name="ws_plugin__optimizemember_filter_wp_query\[\]"]').each(function()
            {
    ...
    if(location.href.match(/page\=ws-plugin--optimizemember-down-ops/))
    {
        var updateCloudFrontPrivateKey = /* Attaches to events below. */ function()
        {
            var $hiddenPrivateKey = $('input#ws-plugin--optimizemember-amazon-cf-files-private-key');
            var $visiblePrivateKeyEntry = $('textarea#ws-plugin--optimizemember-amazon-cf-files-private-key-entry');
        ...
        $('form#ws-plugin--optimizemember-options-form').submit(updateCloudFrontPrivateKey);
        $('textarea#ws-plugin--optimizemember-amazon-cf-files-private-key-entry').change(updateCloudFrontPrivateKey).trigger('change');

        var updateCloudFrontDistroCfgs = /* Attaches to events below. */ function()
        {
            var $hiddenPrivateKey = $('input#ws-plugin--optimizemember-amazon-cf-files-private-key');
            var $visiblePrivateKeyId = $('input#ws-plugin--optimizemember-amazon-cf-files-private-key-id');

            var $autoConfigDistros = $('input#ws-plugin--optimizemember-amazon-cf-files-auto-configure-distros');
            var $autoConfigDistrosStatus = $('input#ws-plugin--optimizemember-amazon-cf-files-distros-auto-config-status');
            ...
            {
                alert('OptimizeMember will need to delete and re-configure your Amazon CloudFront distributions if you change this. When you\'re done editing, click (Save All Changes) below.');
            ...
            {
                alert('OptimizeMember will need to auto-configure your Amazon CloudFront distributions for you. When you\'re done editing, click (Save All Changes) below.');
                $autoConfigDistros.attr /* Forcibly check. */('checked', 'checked');
            }
        };
        $('input#ws-plugin--optimizemember-amazon-cf-files-private-key-id').change(updateCloudFrontDistroCfgs);
        $('textarea#ws-plugin--optimizemember-amazon-cf-files-private-key-entry').change(updateCloudFrontDistroCfgs);

        $('input#ws-plugin--optimizemember-amazon-cf-files-auto-configure-distros-w-cnames').change(function()
        {
            var $this = $(this), thisChecked = (this.checked) ? true : false;
            var $autoConfigDistros = $('input#ws-plugin--optimizemember-amazon-cf-files-auto-configure-distros');
            var $autoConfigDistroCnames = $('div#ws-plugin--optimizemember-amazon-cf-files-auto-configure-distro-cnames');
    ...
    if(location.href.match(/page\=ws-plugin--optimizemember-paypal-ops/))
    {
    ...
    if(location.href.match(/page\=ws-plugin--optimizemember-paypal-buttons/))
    {
        $('div.ws-menu-page select[id]').filter( /* Filter all select elements with an id. */function()
            {
                return this.id.match(/^ws-plugin--optimizemember-(level[1-9][0-9]*|modification)-term$/);
            })
            .change(function()
            {
                var button = this.id.replace(/^ws-plugin--optimizemember-(.+?)-term$/g, '$1');
                var trialDisabled = ($(this).val().split('-')[2].replace(/[^0-1BN]/g, '') === 'BN') ? 1 : 0;

                $('p#ws-plugin--optimizemember-'+button+'-trial-line').css('display', (trialDisabled ? 'none' : ''));
                $('span#ws-plugin--optimizemember-'+button+'-trial-then').css('display', (trialDisabled ? 'none' : ''));
                $('span#ws-plugin--optimizemember-'+button+'-20p-rule').css('display', (trialDisabled ? 'none' : ''));

                (trialDisabled) ? $('input#ws-plugin--optimizemember-'+button+'-trial-period').val(0) : null;
                (trialDisabled) ? $('input#ws-plugin--optimizemember-'+button+'-trial-amount').val('0.00') : null;
            });
        $('div.ws-menu-page input[id]').filter( /* Filter all input elements with an id. */function()
            {
                return this.id.match(/^ws-plugin--optimizemember-(level[1-9][0-9]*|modification|ccap)-ccaps$/);
            })
            .keyup(function()
            {
        ...
        ws_plugin__optimizemember_paypalButtonGenerate = /* Handles PayPal Button Generation. */ function(button)
        {
            var shortCodeTemplate = '[optimizeMember-PayPal-Button %%attrs%% image="default" output="button" /]', shortCodeTemplateAttrs = '', labels = {};

            eval("<?php echo c_ws_plugin__optimizemember_utils_strings::esc_dq($labels); ?>");

            var shortCode = $('input#ws-plugin--optimizemember-'+button+'-shortcode');
            var code = $('textarea#ws-plugin--optimizemember-'+button+'-button');
            var modLevel = $('select#ws-plugin--optimizemember-modification-level');
            ...
            var desc = $.trim($('input#ws-plugin--optimizemember-'+button+'-desc').val().replace(/"/g, ""));

            var trialAmount = $('input#ws-plugin--optimizemember-'+button+'-trial-amount').val().replace(/[^0-9\.]/g, '');
            var trialPeriod = $('input#ws-plugin--optimizemember-'+button+'-trial-period').val().replace(/[^0-9]/g, '');
            var trialTerm = $('select#ws-plugin--optimizemember-'+button+'-trial-term').val().replace(/[^A-Z]/g, '');

            var regAmount = $('input#ws-plugin--optimizemember-'+button+'-amount').val().replace(/[^0-9\.]/g, '');
            var regPeriod = $('select#ws-plugin--optimizemember-'+button+'-term').val().split('-')[0].replace(/[^0-9]/g, '');
            var regTerm = $('select#ws-plugin--optimizemember-'+button+'-term').val().split('-')[1].replace(/[^A-Z]/g, '');
            var regRecur = $('select#ws-plugin--optimizemember-'+button+'-term').val().split('-')[2].replace(/[^0-1BN]/g, '');
            ...
            var pageStyle = $.trim($('input#ws-plugin--optimizemember-'+button+'-page-style').val().replace(/"/g, ''));
            var currencyCode = $('select#ws-plugin--optimizemember-'+button+'-currency').val().replace(/[^A-Z]/g, '');

            var cCaps = $.trim($.trim($('input#ws-plugin--optimizemember-'+button+'-ccaps').val()).replace(/^(-all|-al|-a|-)[;,]*/gi, '').replace(/[ \-]/g, '_').replace(/[^a-z_0-9,]/gi, '').toLowerCase());
            cCaps = ($.trim($('input#ws-plugin--optimizemember-'+button+'-ccaps').val()).match(/^(-all|-al|-a|-)[;,]*/i)) ? ((cCaps) ? '-all,' : '-all')+cCaps.toLowerCase() : cCaps.toLowerCase();
            ...
            shortCodeTemplateAttrs += 'level="'+esc_attr(level)+'" ccaps="'+esc_attr(cCaps)+'" desc="'+esc_attr(desc)+'" ps="'+esc_attr(pageStyle)+'" lc="'+esc_attr(localeCode)+'" cc="'+esc_attr(currencyCode)+'" dg="'+esc_attr(digital)+'" ns="'+esc_attr(noShipping)+'" custom="<?php echo c_ws_plugin__optimizemember_utils_strings::esc_js_sq (esc_attr ($_SERVER["HTTP_HOST"])); ?>"';
            ...
            code.html(code.val().replace(/ name\="custom" value\="(.*?)"/, ' name="custom" value="<?php echo c_ws_plugin__optimizemember_utils_strings::esc_js_sq (esc_attr ($_SERVER["HTTP_HOST"])); ?>"'));
            ...
            $('div#ws-plugin--optimizemember-'+button+'-button-prev').html(code.val().replace(/\<form/, '<form target="_blank"').replace(/\<\?php echo S2MEMBER_VALUE_FOR_PP_INV\(\); \?\>/g, Math.round(new Date().getTime())+'~<?php echo c_ws_plugin__optimizemember_utils_strings::esc_js_sq (esc_attr ($_SERVER["REMOTE_ADDR"])); ?>').replace(/\<\?php echo S2MEMBER_CURRENT_USER_VALUE_FOR_PP_(ON0|OS0|ON1|OS1); \?\>/g, ''));
        ...
        ws_plugin__optimizemember_paypalCcapButtonGenerate = /* Handles PayPal Button Generation for Independent Capabilities. */ function()
        {
            var shortCodeTemplate = '[optimizeMember-PayPal-Button %%attrs%% image="default" output="button" /]', shortCodeTemplateAttrs = '';

            var shortCode = $('input#ws-plugin--optimizemember-ccap-shortcode');
            var code = $('textarea#ws-plugin--optimizemember-ccap-button');

            var desc = $.trim($('input#ws-plugin--optimizemember-ccap-desc').val().replace(/"/g, ''));

            var regAmount = $('input#ws-plugin--optimizemember-ccap-amount').val().replace(/[^0-9\.]/g, '');
            var regPeriod = $('select#ws-plugin--optimizemember-ccap-term').val().split('-')[0].replace(/[^0-9]/g, '');
            var regTerm = $('select#ws-plugin--optimizemember-ccap-term').val().split('-')[1].replace(/[^A-Z]/g, '');
            var regRecur = $('select#ws-plugin--optimizemember-ccap-term').val().split('-')[2].replace(/[^0-1BN]/g, '');

            var /* NOT yet configurable. */ localeCode = '', digital = '0', noShipping = '1';
            var pageStyle = $.trim($('input#ws-plugin--optimizemember-ccap-page-style').val().replace(/"/g, ''));
            var currencyCode = $('select#ws-plugin--optimizemember-ccap-currency').val().replace(/[^A-Z]/g, '');

            var cCaps = $.trim($.trim($('input#ws-plugin--optimizemember-ccap-ccaps').val()).replace(/^(-all|-al|-a|-)[;,]*/gi, '').replace(/[ \-]/g, '_').replace(/[^a-z_0-9,]/gi, '').toLowerCase());
            cCaps = ($.trim($('input#ws-plugin--optimizemember-ccap-ccaps').val()).match(/^(-all|-al|-a|-)[;,]*/i)) ? ((cCaps) ? '-all,' : '-all')+cCaps.toLowerCase() : cCaps.toLowerCase();
            ...
            shortCodeTemplateAttrs += ' custom="<?php echo c_ws_plugin__optimizemember_utils_strings::esc_js_sq (esc_attr ($_SERVER["HTTP_HOST"])); ?>" ra="'+esc_attr(regAmount)+'" rp="'+esc_attr(regPeriod)+'" rt="'+esc_attr(regTerm)+'" rr="'+esc_attr(regRecur)+'"';
            shortCode.val(shortCodeTemplate.replace(/%%attrs%%/, shortCodeTemplateAttrs));
            ...
            $('div#ws-plugin--optimizemember-ccap-button-prev').html(code.val().replace(/\<form/, '<form target="_blank"').replace(/\<\?php echo S2MEMBER_VALUE_FOR_PP_INV\(\); \?\>/g, Math.round(new Date().getTime())+'~<?php echo c_ws_plugin__optimizemember_utils_strings::esc_js_sq (esc_attr ($_SERVER["REMOTE_ADDR"])); ?>').replace(/\<\?php echo S2MEMBER_CURRENT_USER_VALUE_FOR_PP_(ON0|OS0|ON1|OS1); \?\>/g, ''));
        ...
        ws_plugin__optimizemember_paypalSpButtonGenerate = /* Handles PayPal Button Generation for Specific Post/Page Access. */ function()
        {
            var shortCodeTemplate = '[optimizeMember-PayPal-Button %%attrs%% image="default" output="button" /]', shortCodeTemplateAttrs = '';

            var shortCode = $('input#ws-plugin--optimizemember-sp-shortcode');
            var code = $('textarea#ws-plugin--optimizemember-sp-button');

            var leading = $('select#ws-plugin--optimizemember-sp-leading-id').val().replace(/[^0-9]/g, '');
            var additionals = $('select#ws-plugin--optimizemember-sp-additional-ids').val() || [];
            var hours = $('select#ws-plugin--optimizemember-sp-hours').val().replace(/[^0-9]/g, '');

            var regAmount = $('input#ws-plugin--optimizemember-sp-amount').val().replace(/[^0-9\.]/g, '');
            var desc = $.trim($('input#ws-plugin--optimizemember-sp-desc').val().replace(/"/g, ''));

            var /* NOT yet configurable. */ localeCode = '', digital = '0', noShipping = '1';
            var pageStyle = $.trim($('input#ws-plugin--optimizemember-sp-page-style').val().replace(/"/g, ''));
            var currencyCode = $('select#ws-plugin--optimizemember-sp-currency').val().replace(/[^A-Z]/g, '');
            ...
            $('div#ws-plugin--optimizemember-sp-button-prev').html(code.val().replace(/\<form/, '<form target="_blank"').replace(/\<\?php echo S2MEMBER_VALUE_FOR_PP_INV\(\); \?\>/g, Math.round(new Date().getTime())+'~<?php echo c_ws_plugin__optimizemember_utils_strings::esc_js_sq (esc_attr ($_SERVER["REMOTE_ADDR"])); ?>').replace(/\<\?php echo S2MEMBER_CURRENT_USER_VALUE_FOR_PP_(ON0|OS0|ON1|OS1); \?\>/g, ''));
        ...
        ws_plugin__optimizemember_paypalRegLinkGenerate = /* Handles PayPal Link Generation. */ function()
        {
            var level = $('select#ws-plugin--optimizemember-reg-link-level').val().replace(/[^0-9]/g, '');
            var subscrID = $.trim($('input#ws-plugin--optimizemember-reg-link-subscr-id').val());
            var custom = $.trim($('input#ws-plugin--optimizemember-reg-link-custom').val());
            var cCaps = $.trim($.trim($('input#ws-plugin--optimizemember-reg-link-ccaps').val()).replace(/[ \-]/g, '_').replace(/[^a-z_0-9,]/gi, '').toLowerCase());
            var fixedTerm = $.trim($('input#ws-plugin--optimizemember-reg-link-fixed-term').val().replace(/[^A-Z 0-9]/gi, '').toUpperCase());
            var $link = $('p#ws-plugin--optimizemember-reg-link'), $loading = $('img#ws-plugin--optimizemember-reg-link-loading');
            ...
            $link.hide(), $loading.show(), $.post(ajaxurl, {action: 'ws_plugin__optimizemember_reg_access_link_via_ajax', ws_plugin__optimizemember_reg_access_link_via_ajax: '<?php echo c_ws_plugin__optimizemember_utils_strings::esc_js_sq (wp_create_nonce ("ws-plugin--optimizemember-reg-access-link-via-ajax")); ?>', optimizemember_reg_access_link_subscr_gateway: 'paypal', optimizemember_reg_access_link_subscr_id: subscrID, optimizemember_reg_access_link_custom: custom, optimizemember_reg_access_link_item_number: levelCcapsPer}, function(response)
            {
                $link.show().html('<a href="'+esc_attr(response)+'" target="_blank" rel="external">'+esc_html(response)+'</a>'), $loading.hide();
            });
            return false;
        };
        ws_plugin__optimizemember_paypalSpLinkGenerate = /* Handles PayPal Link Generation. */ function()
        {
            var leading = $('select#ws-plugin--optimizemember-sp-link-leading-id').val().replace(/[^0-9]/g, '');
            var additionals = $('select#ws-plugin--optimizemember-sp-link-additional-ids').val() || [];
            var hours = $('select#ws-plugin--optimizemember-sp-link-hours').val().replace(/[^0-9]/g, '');
            var $link = $('p#ws-plugin--optimizemember-sp-link'), $loading = $('img#ws-plugin--optimizemember-sp-link-loading');
    ...
    if(location.href.match(/page\=ws-plugin--optimizemember-els-ops/))
    {
        $('select#ws-plugin--optimizemember-custom-reg-opt-in').change(function()
        {
            var $this = $(this), val = $this.val();
            var $rows = $('tr.ws-plugin--optimizemember-custom-reg-opt-in-label-row');
            var $prevImg = $('img.ws-plugin--optimizemember-custom-reg-opt-in-label-prev-img');
        ...
        $('div.ws-plugin--optimizemember-opt-out-section input[type="checkbox"][name="ws_plugin__optimizemember_custom_reg_auto_opt_outs\[\]"]').change(function()
        {
            var /* Record value associated with change event. Also initialize checkedIndexes array. */ thisChange = $(this).val(), checkedIndexes = [];

            $('div.ws-plugin--optimizemember-opt-out-section input[type="checkbox"][name="ws_plugin__optimizemember_custom_reg_auto_opt_outs\[\]"]').each(function()
                {
                    var $this = $(this), val = $this.val(), checkboxes = 'input[type="checkbox"]';
            ...
            $('select#ws-plugin--optimizemember-custom-reg-auto-opt-out-transitions').removeAttr('disabled');
            if($.inArray(3, checkedIndexes) === -1 && $.inArray(4, checkedIndexes) === -1 && $.inArray(5, checkedIndexes) === -1 && $.inArray(6, checkedIndexes) === -1)
                $('select#ws-plugin--optimizemember-custom-reg-auto-opt-out-transitions').attr('disabled', 'disabled');
