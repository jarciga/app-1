...
/**/
if(!class_exists("c_ws_plugin__optimizemember_menu_pages_updates"))
	{
		/**
		* Newsletter/Updates for Menu Pages.
		*
		...
		* @since 111205
		*/
		class c_ws_plugin__optimizemember_menu_pages_updates
			{
						...
						if(is_object($user = wp_get_current_user()) && ($user_id = $user->ID))
							{
								...
								/**/
								if(!is_ssl() && /* Don't require remote connections when running on `localhost`. */ !c_ws_plugin__optimizemember_utils_conds::is_localhost())
									{
...
/**/
new c_ws_plugin__optimizemember_menu_pages_updates();
?>